...
exports.RetentionPolicySchema = exports.AuditQuerySchema = exports.CreateAuditLogSchema = exports.ArchiveStatus = exports.RetentionPolicy = exports.ComplianceStandard = exports.AuditSeverity = exports.AuditEventType = void 0;
...
exports.logProcessEvent = logProcessEvent;
exports.logActivityEvent = logActivityEvent;
exports.logUserAction = logUserAction;
exports.logStateChange = logStateChange;
exports.logVariableChange = logVariableChange;
...
exports.getProcessAuditTrail = getProcessAuditTrail;
...
exports.deleteExpiredAuditLogs = deleteExpiredAuditLogs;
exports.verifyAuditLogChain = verifyAuditLogChain;
exports.detectAuditLogTampering = detectAuditLogTampering;
...
(function (AuditEventType) {
    AuditEventType["PROCESS_START"] = "process_start";
    AuditEventType["PROCESS_COMPLETE"] = "process_complete";
    AuditEventType["PROCESS_FAIL"] = "process_fail";
    AuditEventType["PROCESS_CANCEL"] = "process_cancel";
    AuditEventType["PROCESS_SUSPEND"] = "process_suspend";
    AuditEventType["PROCESS_RESUME"] = "process_resume";
    AuditEventType["ACTIVITY_START"] = "activity_start";
    AuditEventType["ACTIVITY_COMPLETE"] = "activity_complete";
    AuditEventType["USER_CLAIM"] = "user_claim";
    AuditEventType["USER_UNCLAIM"] = "user_unclaim";
    AuditEventType["USER_DELEGATE"] = "user_delegate";
    AuditEventType["VARIABLE_CREATE"] = "variable_create";
    AuditEventType["VARIABLE_UPDATE"] = "variable_update";
    AuditEventType["VARIABLE_DELETE"] = "variable_delete";
    AuditEventType["DECISION_EVALUATE"] = "decision_evaluate";
    AuditEventType["ERROR_BOUNDARY"] = "error_boundary";
    AuditEventType["COMPENSATION"] = "compensation";
    AuditEventType["ESCALATION"] = "escalation";
    AuditEventType["MESSAGE_SEND"] = "message_send";
    AuditEventType["MESSAGE_RECEIVE"] = "message_receive";
    AuditEventType["TIMER_TRIGGER"] = "timer_trigger";
    AuditEventType["SIGNAL_TRIGGER"] = "signal_trigger";
...
(function (ArchiveStatus) {
    ...
    ArchiveStatus["COMPRESSED"] = "compressed";
    ArchiveStatus["COLD_STORAGE"] = "cold_storage";
...
exports.CreateAuditLogSchema = zod_1.z.object({
    ...
    beforeState: zod_1.z.record(zod_1.z.any()).optional(),
    afterState: zod_1.z.record(zod_1.z.any()).optional(),
...
exports.AuditQuerySchema = zod_1.z.object({
    eventTypes: zod_1.z.array(zod_1.z.nativeEnum(AuditEventType)).optional(),
    severities: zod_1.z.array(zod_1.z.nativeEnum(AuditSeverity)).optional(),
    startDate: zod_1.z.date().optional(),
    endDate: zod_1.z.date().optional(),
    userIds: zod_1.z.array(zod_1.z.string()).optional(),
    processInstanceIds: zod_1.z.array(zod_1.z.string().uuid()).optional(),
    entityType: zod_1.z.string().optional(),
    successOnly: zod_1.z.boolean().optional(),
    errorsOnly: zod_1.z.boolean().optional(),
    limit: zod_1.z.number().int().positive().max(1000).default(100),
    offset: zod_1.z.number().int().nonnegative().default(0),
    sortBy: zod_1.z.string().default('timestamp'),
...
exports.RetentionPolicySchema = zod_1.z.object({
    ...
    type: zod_1.z.nativeEnum(RetentionPolicy),
    retentionDays: zod_1.z.number().int().positive(),
    appliesTo: zod_1.z.array(zod_1.z.nativeEnum(AuditEventType)).optional(),
    autoArchive: zod_1.z.boolean().default(true),
...
function createAuditLogModel(sequelize) {
    ...
    AuditLogModel.init({
        ...
        eventType: {
            type: sequelize_1.DataTypes.ENUM(...Object.values(AuditEventType)),
            allowNull: false,
        },
        severity: {
            type: sequelize_1.DataTypes.ENUM(...Object.values(AuditSeverity)),
            allowNull: false,
 ...
 */
function createComplianceReportModel(sequelize) {
    class ComplianceReportModel extends sequelize_1.Model {
    }
    ComplianceReportModel.init({
        ...
        standard: {
            type: sequelize_1.DataTypes.ENUM(...Object.values(ComplianceStandard)),
            allowNull: false,
        },
        startDate: {
            type: sequelize_1.DataTypes.DATE,
 ...
 */
function createRetentionPolicyModel(sequelize) {
    class RetentionPolicyModel extends sequelize_1.Model {
    }
    RetentionPolicyModel.init({
        ...
        type: {
            type: sequelize_1.DataTypes.ENUM(...Object.values(RetentionPolicy)),
            allowNull: false,
        },
        retentionDays: {
            type: sequelize_1.DataTypes.INTEGER,
 ...
 */
function computeAuditLogHash(logEntry) {
 ...
 */
async function getLastAuditLogHash(model, transaction) {
    const lastLog = await model.findOne({
        order: [['timestamp', 'DESC']],
        limit: 1,
        transaction,
    });
    return lastLog ? lastLog.get('hash') : null;
}
// ============================================================================
...
async function createAuditLog(params) {
    const { model, eventType, severity = AuditSeverity.INFO, userId, processInstanceId, activityId, entityType, entityId, action, beforeState, afterState, metadata = {}, ipAddress, userAgent, duration, success = true, errorMessage, stackTrace, retentionDays = 365, transaction, } = params;
    // Validate input
    const validationResult = exports.CreateAuditLogSchema.safeParse({
        eventType,
        severity,
        userId,
        processInstanceId,
    ...
    if (!validationResult.success) {
        throw new common_1.BadRequestException(`Audit log validation failed: ${validationResult.error.message}`);
    }
 ...
 * ```
 */
async function logActivityEvent(params) {
 ...
 */
async function logStateChange(params) {
    const { model, eventType, entityType, entityId, action, beforeState, afterState, userId, processInstanceId, transaction, } = params;
    return createAuditLog({
        model,
        eventType,
        severity: AuditSeverity.INFO,
 ...
 * ```
 */
async function logVariableChange(params) {
    const { model, variableId, variableName, oldValue, newValue, action, userId, processInstanceId, transaction, } = params;
...
async function logDataAccess(params) {
    const { model, userId, dataType, dataId, action, processInstanceId, ipAddress, metadata, transaction, } = params;
    return createAuditLog({
        model,
...
async function logError(params) {
    const { model, eventType, error, userId, processInstanceId, activityId, entityType, entityId, context, transaction, } = params;
    return createAuditLog({
        model,
        eventType,
 ...
 * ```
 */
async function logCriticalError(params) {
...
async function queryAuditLogs(params) {
    ...
    // Validate query
    const validationResult = exports.AuditQuerySchema.safeParse(query);
    if (!validationResult.success) {
        throw new common_1.BadRequestException(`Query validation failed: ${validationResult.error.message}`);
    }
    const whereClause = {};
    if (query.eventTypes && query.eventTypes.length > 0) {
        whereClause.eventType = { [sequelize_1.Op.in]: query.eventTypes };
    }
    if (query.severities && query.severities.length > 0) {
        whereClause.severity = { [sequelize_1.Op.in]: query.severities };
    }
    if (query.startDate || query.endDate) {
        whereClause.timestamp = {};
        if (query.startDate)
            whereClause.timestamp[sequelize_1.Op.gte] = query.startDate;
        if (query.endDate)
            whereClause.timestamp[sequelize_1.Op.lte] = query.endDate;
    }
    if (query.userIds && query.userIds.length > 0) {
        whereClause.userId = { [sequelize_1.Op.in]: query.userIds };
    }
    if (query.processInstanceIds && query.processInstanceIds.length > 0) {
        whereClause.processInstanceId = { [sequelize_1.Op.in]: query.processInstanceIds };
    }
    if (query.entityType) {
 ...
 * ```
 */
async function getProcessAuditTrail(params) {
    const { model, processInstanceId, eventTypes, limit = 1000, transaction } = params;
    const whereClause = { processInstanceId };
    if (eventTypes && eventTypes.length > 0) {
        whereClause.eventType = { [sequelize_1.Op.in]: eventTypes };
    }
...
async function generateComplianceReport(params) {
    ...
    const securityEvents = logs.filter((log) => log.get('severity') === AuditSeverity.SECURITY).length;
    const failedActions = logs.filter((log) => !log.get('success')).length;
    const dataAccessEvents = logs.filter((log) => log.get('eventType') === AuditEventType.DATA_ACCESS).length;
    // Count events by type
    const eventTypeCounts = {};
    logs.forEach((log) => {
        const eventType = log.get('eventType');
    ...
    const failedAccessAttempts = logs.filter((log) => log.get('eventType') === AuditEventType.PERMISSION_CHECK && !log.get('success'));
 ...
 * ```
 */
async function exportComplianceReport(params) {
    const { reportModel, reportId, transaction } = params;
    const report = await reportModel.findByPk(reportId, { transaction });
    if (!report) {
        throw new common_1.NotFoundException(`Compliance report with ID '${reportId}' not found`);
    }
    return report.toJSON();
}
// ============================================================================
 ...
 * ```
 */
async function createRetentionPolicy(params) {
    const { model, name, type, retentionDays, appliesTo, autoArchive = true, transaction, } = params;
    const validationResult = exports.RetentionPolicySchema.safeParse({
        name,
        type,
        retentionDays,
        appliesTo,
    ...
    if (!validationResult.success) {
        throw new common_1.BadRequestException(`Retention policy validation failed: ${validationResult.error.message}`);
    }
 ...
 */
async function archiveAuditLogs(params) {
    const { auditModel, policyModel, archiveBeforeDate = new Date(Date.now() - 365 * 24 * 60 * 60 * 1000), compress = true, transaction, } = params;
    const [updated] = await auditModel.update({
        archiveStatus: compress ? ArchiveStatus.COMPRESSED : ArchiveStatus.ARCHIVED,
        archivedAt: new Date(),
    }, {
        where: {
            timestamp: { [sequelize_1.Op.lte]: archiveBeforeDate },
            archiveStatus: ArchiveStatus.ACTIVE,
        },
        transaction,
    });
 ...
 * ```
 */
async function deleteExpiredAuditLogs(params) {
    const { auditModel, dryRun = false, transaction } = params;
    const whereClause = {
        expiresAt: { [sequelize_1.Op.lte]: new Date() },
        archiveStatus: { [sequelize_1.Op.in]: [ArchiveStatus.ARCHIVED, ArchiveStatus.COMPRESSED] },
    };
    if (dryRun) {
 ...
 */
async function verifyAuditLogChain(params) {
 ...
 * ```
 */
async function detectAuditLogTampering(params) {
    const { model, logId, transaction } = params;
    const log = await model.findByPk(logId, { transaction });
    if (!log) {
        throw new common_1.NotFoundException(`Audit log with ID '${logId}' not found`);
    }
    const logData = log.toJSON();
    // Recompute hash
    const computedHash = computeAuditLogHash(logData);
    ...
    if (logData.previousHash) {
        const previousLog = await model.findOne({
            where: {
                timestamp: { [sequelize_1.Op.lt]: logData.timestamp },
            },
            order: [['timestamp', 'DESC']],
            limit: 1,
            transaction,
 ...
 * ```
 */
async function getAuditLogStatistics(params) {
    const { model, startDate, endDate, transaction } = params;
    const whereClause = {};
    if (startDate || endDate) {
        whereClause.timestamp = {};
...
//# sourceMappingURL=workflow-audit-logging.js.map