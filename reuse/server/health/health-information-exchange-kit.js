...
exports.parseHL7Message = parseHL7Message;
exports.parseADTMessage = parseADTMessage;
exports.parseORUMessage = parseORUMessage;
exports.parseORMMessage = parseORMMessage;
exports.buildHL7Message = buildHL7Message;
exports.validateHL7Message = validateHL7Message;
exports.extractPatientFromHL7 = extractPatientFromHL7;
exports.convertHL7ToFHIR = convertHL7ToFHIR;
exports.convertPIDToFHIRPatient = convertPIDToFHIRPatient;
exports.convertOBXToFHIRObservation = convertOBXToFHIRObservation;
exports.convertFHIRPatientToPID = convertFHIRPatientToPID;
exports.createFHIRBundle = createFHIRBundle;
exports.extractResourcesFromBundle = extractResourcesFromBundle;
exports.validateFHIRResource = validateFHIRResource;
exports.generateCCD = generateCCD;
exports.parseCCDA = parseCCDA;
exports.convertCCDToFHIRComposition = convertCCDToFHIRComposition;
exports.validateCCD = validateCCD;
exports.createDirectMessage = createDirectMessage;
exports.signDirectMessage = signDirectMessage;
exports.encryptDirectMessage = encryptDirectMessage;
exports.validateDirectAddress = validateDirectAddress;
exports.recordHIEConsent = recordHIEConsent;
exports.checkHIEConsent = checkHIEConsent;
exports.revokeHIEConsent = revokeHIEConsent;
exports.generateConsentForm = generateConsentForm;
exports.checkBreakGlassAccess = checkBreakGlassAccess;
...
exports.getEnterpriseEMRN = getEnterpriseEMRN;
exports.mergeMPIEntries = mergeMPIEntries;
exports.queryProviderDirectory = queryProviderDirectory;
exports.searchProvidersBySpecialty = searchProvidersBySpecialty;
exports.validateNPI = validateNPI;
exports.createDocumentMetadata = createDocumentMetadata;
exports.queryDocumentRegistry = queryDocumentRegistry;
exports.retrieveDocument = retrieveDocument;
exports.submitDocument = submitDocument;
exports.recordHIEAudit = recordHIEAudit;
exports.generateDisclosureReport = generateDisclosureReport;
 ...
 * ```
 */
function parseADTMessage(adtMessage) {
 ...
 * ```
 */
function parseORUMessage(oruMessage) {
 ...
 * ```
 */
function parseORMMessage(ormMessage) {
 ...
 * ```
 */
function buildHL7Message(message) {
    const segments = [];
    // MSH segment
 ...
 * ```
 */
function validateHL7Message(message) {
    const errors = [];
    const warnings = [];
 ...
 * ```
 */
function extractPatientFromHL7(message) {
 ...
 * ```
 */
function convertPIDToFHIRPatient(pid) {
    return {
 ...
 * ```
 */
function convertOBXToFHIRObservation(obx, patientId) {
    return {
 ...
 * ```
 */
function convertFHIRPatientToPID(fhirPatient) {
    const name = fhirPatient.name?.[0] || {};
    const address = fhirPatient.address?.[0];
    const homePhone = fhirPatient.telecom?.find((t) => t.system === 'phone' && t.use === 'home');
    const workPhone = fhirPatient.telecom?.find((t) => t.system === 'phone' && t.use === 'work');
    return {
        patientId: fhirPatient.id,
        patientIdentifierList: (fhirPatient.identifier || []).map((id) => ({
            id: id.value,
 ...
 * ```
 */
function createFHIRBundle(resources, bundleType = 'transaction') {
    return {
        resourceType: 'Bundle',
        type: bundleType,
        timestamp: new Date().toISOString(),
 ...
 * ```
 */
function extractResourcesFromBundle(bundle, resourceType) {
    return bundle.entry
 ...
 * ```
 */
function convertCCDToFHIRComposition(ccd) {
    return {
 ...
 * ```
 */
function createDirectMessage(message) {
    return {
 ...
 * ```
 */
function signDirectMessage(message, privateKey, certificate) {
 ...
 * ```
 */
function encryptDirectMessage(message, recipientCertificate) {
 ...
 * ```
 */
function validateDirectAddress(address) {
 ...
 * ```
 */
function recordHIEConsent(consent) {
    return {
        ...consent,
 ...
 * ```
 */
function checkHIEConsent(patientId, consents, organizationId) {
    const activeConsents = consents.filter(c => c.patientId === patientId &&
        c.status === 'active' &&
    ...
    if (organizationId) {
        const orgConsent = activeConsents.find(c => c.authorizedOrganizations?.includes(organizationId));
        if (orgConsent) {
            return true;
        }
    }
    // Check for general opt-in
    const optIn = activeConsents.find(c => c.consentType === 'opt-in' &&
 ...
 * ```
 */
function revokeHIEConsent(consentId, revokedBy, reason) {
    return {
        id: consentId,
        status: 'revoked',
 ...
 * ```
 */
function generateConsentForm(patientId, scope) {
    return `
<!DOCTYPE html>
<html>
<head>
 ...
 * ```
 */
function checkBreakGlassAccess(userId, patientId, justification) {
 ...
 * ```
 */
function performPatientMatching(criteria, mpiEntries) {
    const results = [];
    for (const entry of mpiEntries) {
        ...
        if (criteria.firstName && criteria.lastName) {
            const firstNameMatch = soundexMatch(criteria.firstName, entry.demographics.firstName);
            const lastNameMatch = soundexMatch(criteria.lastName, entry.demographics.lastName);
        ...
        if (criteria.address && entry.addresses && entry.addresses.length > 0) {
            const addressMatch = entry.addresses.some(addr => addr.zip === criteria.address?.zip &&
 ...
 * ```
 */
function linkPatientRecords(primaryMpiId, secondaryMpiId, linkType, linkScore) {
    return {
        mpiId: primaryMpiId,
        linkages: [{
 ...
 * ```
 */
function getEnterpriseEMRN(mpiId, mpiEntries) {
 ...
 * ```
 */
function mergeMPIEntries(survivorMpiId, duplicateMpiIds) {
    return {
        mpiId: survivorMpiId,
 ...
 * ```
 */
function queryProviderDirectory(npi) {
 ...
 * ```
 */
function searchProvidersBySpecialty(specialty, zipCode, radiusMiles = 10) {
 ...
 * ```
 */
function createDocumentMetadata(metadata) {
    return {
 ...
 * ```
 */
function queryDocumentRegistry(query, registry) {
 ...
 * ```
 */
async function retrieveDocument(documentId, repositoryId) {
 ...
 * ```
 */
async function submitDocument(metadata, content) {
    // Calculate hash
 ...
 * ```
 */
function generateDisclosureReport(patientId, auditLog, fromDate, toDate) {
 ...
 * ```
 */
function validateHIPAACompliance(transaction, consents) {
    const violations = [];
    const warnings = [];
    ...
    if (transaction.purpose !== 'emergency') {
        const hasConsent = checkHIEConsent(transaction.patientId, consents, transaction.organizationId);
        if (!hasConsent) {
 ...
 */
function parsePV1Segment(fields) {
 ...
 */
function parseHL7DateTime(hl7DateTime) {
    if (!hl7DateTime)
        return undefined;
 ...
 */
function convertPV1ToFHIREncounter(pv1, patientId) {
    return {
 ...
 */
function generateCCDAllergiesSection(allergies) {
 ...
 */
function generateCCDMedicationsSection(medications) {
 ...
 */
function generateCCDProblemsSection(problems) {
 ...
 */
function generateCCDProceduresSection(procedures) {
 ...
 */
function generateCCDVitalSignsSection(vitals) {
 ...
 */
function generateCCDImmunizationsSection(immunizations) {
 ...
 */
function generateCCDResultsSection(results) {
 ...
 */
function extractXMLValue(xml, element, attribute, attrValue) {
...
//# sourceMappingURL=health-information-exchange-kit.js.map