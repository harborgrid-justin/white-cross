...
export interface MailQuotaConfig {
  ...
  // Exchange Server quota thresholds (bytes)
  issueWarningQuota: number; // Warning threshold (e.g., 1.8GB)
  prohibitSendQuota: number; // Prevents sending (e.g., 2GB)
  prohibitSendReceiveQuota: number; // Prevents sending/receiving (e.g., 2.3GB)
...
export interface StorageCleanupConfig {
  ...
  maxTrashSize?: number;
  maxSentItemsSize?: number;
  targetQuotaPercent?: number; // Clean until quota usage is below this
...
export interface RetentionPolicyConfig {
  ...
  // HIPAA compliance
  minimumRetentionDays: number; // HIPAA minimum (e.g., 2555 days / 7 years)
...
export interface StorageAnalyticsReport {
  ...
  quotaStatistics: {
    ...
    usersNearQuota: number; // Within 10% of limit
    usersBlocked: number;
  };
 ...
 */
export const MailQuotaConfigAttributes: ModelAttributes = {
  id: {
    type: DataTypes.UUID,
  ...
  quota_type: {
    type: DataTypes.ENUM('user', 'organization', 'mailbox', 'shared'),
    allowNull: false,
    defaultValue: 'user',
    field: 'quota_type',
  },
  ...
  status: {
    type: DataTypes.ENUM('normal', 'warning', 'send-blocked', 'send-receive-blocked'),
    allowNull: false,
    defaultValue: 'normal',
  },
  last_warning_date: {
 ...
 */
export const MailQuotaConfigOptions: ModelOptions = {
 ...
 */
export const RetentionPolicyAttributes: ModelAttributes = {
  id: {
    type: DataTypes.UUID,
    defaultValue: DataTypes.UUIDV4,
    primaryKey: true,
 ...
 */
export const RetentionPolicyOptions: ModelOptions = {
 ...
 */
export const QuotaWarningNotificationAttributes: ModelAttributes = {
  id: {
    type: DataTypes.UUID,
    defaultValue: DataTypes.UUIDV4,
    primaryKey: true,
  ...
  warning_type: {
    type: DataTypes.ENUM('approaching', 'warning-threshold', 'send-blocked', 'send-receive-blocked'),
    allowNull: false,
    field: 'warning_type',
  },
  warning_level: {
 ...
 */
export const QuotaWarningNotificationOptions: ModelOptions = {
 ...
 */
export function defineMailQuotaConfigModel(sequelize: Sequelize): typeof Model {
  class MailQuotaConfig extends Model {}

  MailQuotaConfig.init(MailQuotaConfigAttributes, {
    ...MailQuotaConfigOptions,
    sequelize,
  });
 ...
 */
export function defineRetentionPolicyModel(sequelize: Sequelize): typeof Model {
  class RetentionPolicy extends Model {}

  RetentionPolicy.init(RetentionPolicyAttributes, {
 ...
 */
export function defineQuotaWarningNotificationModel(sequelize: Sequelize): typeof Model {
  class QuotaWarningNotification extends Model {}

  QuotaWarningNotification.init(QuotaWarningNotificationAttributes, {
    ...QuotaWarningNotificationOptions,
    sequelize,
  });
...
export async function createQuotaConfig(
  config: Partial<MailQuotaConfig>,
  sequelize: Sequelize,
  transaction?: Transaction
): Promise<MailQuotaConfig> {
  const MailQuotaConfig = defineMailQuotaConfigModel(sequelize);
  ...
  return quotaConfig.toJSON() as MailQuotaConfig;
}
 ...
 */
export async function getQuotaConfigByUserId(
  userId: string,
  sequelize: Sequelize
): Promise<MailQuotaConfig | null> {
  const MailQuotaConfig = defineMailQuotaConfigModel(sequelize);
  ...
  return quotaConfig ? (quotaConfig.toJSON() as MailQuotaConfig) : null;
}
...
export async function updateQuotaConfig(
  quotaConfigId: string,
  updates: Partial<MailQuotaConfig>,
  sequelize: Sequelize,
  transaction?: Transaction
): Promise<MailQuotaConfig | null> {
  const MailQuotaConfig = defineMailQuotaConfigModel(sequelize);
  ...
  return updated ? (updated.toJSON() as MailQuotaConfig) : null;
}
...
export async function canSendEmail(
  userId: string,
  sequelize: Sequelize
): Promise<{ canSend: boolean; reason?: string; quotaStatus: string }> {
...
export async function canReceiveEmail(
  userId: string,
  sequelize: Sequelize
): Promise<{ canReceive: boolean; reason?: string; quotaStatus: string }> {
  const quota = await getQuotaConfigByUserId(userId, sequelize);
 ...
 */
export async function enforceQuotaLimits(
  userId: string,
  sequelize: Sequelize,
  transaction?: Transaction
): Promise<QuotaEnforcementAction> {
  const quota = await getQuotaConfigByUserId(userId, sequelize);
  ...
  let actionType: QuotaEnforcementAction['actionType'] = 'warning-sent';
  ...
  const action: QuotaEnforcementAction = {
 ...
 */
export async function calculateMailboxSize(
  userId: string,
  sequelize: Sequelize
): Promise<number> {
...
export async function calculateFolderSize(
  folderId: string,
  sequelize: Sequelize
): Promise<{ size: number; messageCount: number }> {
  const result = await sequelize.query(
    `
    SELECT
      COALESCE(SUM(m.size), 0) + COALESCE(SUM(a.size), 0) as total_size,
 ...
 */
export async function calculateMailboxStorageStats(
  userId: string,
  sequelize: Sequelize
): Promise<MailboxStorageStats> {
  // Get quota config
  ...
  return {
    ...
    folderStats: folderStats.map((f: any) => ({
    ...
    storageByMonth: storageByMonth.map((s: any) => ({
      year: s.year,
      month: s.month,
      size: s.size,
      messageCount: s.message_count,
    })),
    quotaConfig: quotaConfig || ({} as MailQuotaConfig),
    quotaUsagePercent,
    quotaStatus,
 ...
 */
export async function calculateAttachmentStorage(
  userId: string,
  sequelize: Sequelize
): Promise<{ totalSize: number; count: number; averageSize: number }> {
  const result = await sequelize.query(
    `
    SELECT
...
export async function updateQuotaUsage(
  ...
  return await updateQuotaConfig(
    quota.id,
    {
      currentSize: newSize,
  ...
  try {
    ...
    if (config.deleteTrashOlderThanDays > 0) {
      ...
      const whereClause: WhereOptions = {
        is_deleted: true,
        deleted_at: { [Op.lt]: trashCutoffDate },
      };
      ...
      spaceFreedBytes += deletedMessages.reduce((sum: number, msg: any) => sum + (msg.size || 0), 0);
    }
    ...
    if (config.archiveSentOlderThanDays > 0) {
      ...
      if (sentMessages.length > 0 && !config.dryRun) {
        await sequelize.query(
          ...
          {
            replacements: {
              messageIds: sentMessages.map((m: any) => m.id),
            },
            transaction,
          }
        );
    ...
    if (config.deduplicateAttachments) {
      const dedupeResult = await deduplicateAttachmentStorage(
        config.userId,
        sequelize,
        transaction
      );
 ...
 */
export async function deduplicateAttachmentStorage(
  ...
  const duplicateGroups = duplicates.map((dup: any) => ({
...
export async function compressAttachments(
  ...
  const errors: CompressionResult['errors'] = [];
...
export async function createQuotaWarning(
  userId: string,
  quotaConfig: MailQuotaConfig,
  warningType: QuotaWarningNotification['warningType'],
  sequelize: Sequelize,
  transaction?: Transaction
): Promise<QuotaWarningNotification> {
  const QuotaWarningNotification = defineQuotaWarningNotificationModel(sequelize);

  const usagePercent = (quotaConfig.currentSize / quotaConfig.prohibitSendReceiveQuota) * 100;

  let warningLevel: QuotaWarningNotification['warningLevel'] = 'info';
  let message = '';
  let actionRequired = '';
  ...
  return notification.toJSON() as QuotaWarningNotification;
}
...
export async function checkAndSendQuotaWarnings(
  ...
  const warnings: QuotaWarningNotification[] = [];
...
export async function getQuotaWarnings(
  ...
  return warnings.map((w) => w.toJSON() as QuotaWarningNotification);
}
...
export async function acknowledgeQuotaWarning(
  ...
  sequelize: Sequelize
): Promise<QuotaWarningNotification | null> {
  const QuotaWarningNotification = defineQuotaWarningNotificationModel(sequelize);
  ...
  return updated ? (updated.toJSON() as QuotaWarningNotification) : null;
}
...
export async function createRetentionPolicy(
  policy: Partial<RetentionPolicyConfig>,
  sequelize: Sequelize,
  transaction?: Transaction
  ...
  return retentionPolicy.toJSON() as RetentionPolicyConfig;
}
...
export async function applyRetentionPolicy(
  ...
  sequelize: Sequelize,
  dryRun: boolean
): Promise<{ messagesDeleted: number; messagesArchived: number }> {
  const RetentionPolicy = defineRetentionPolicyModel(sequelize);
  ...
  const policyData = policy.toJSON() as RetentionPolicyConfig;
...
export async function archiveMessages(
  policyConfig: Partial<ArchivePolicyConfig>,
  sequelize: Sequelize
): Promise<{ messagesArchived: number; spaceArchived: number }> {
  ...
  const spaceArchived = archivedMessages.reduce((sum: number, msg: any) => sum + (msg.size || 0), 0);
 ...
 */
export async function generateStorageAnalyticsReport(
  ...
  const recommendations: StorageAnalyticsReport['recommendations'] = [];
  ...
  return {
    ...
    topUsersByStorage: topUsers.map((u: any) => ({
      userId: u.user_id,
 ...
 */
export async function getStorageGrowthTrend(
  userId: string | undefined,
  days: number,
  sequelize: Sequelize
): Promise<Array<{ date: Date; totalStorage: number; growthRate: number }>> {
  ...
  const trend = results.map((r: any, index: number) => {
    const growthRate =
      index > 0
 ...
 */
export function calculateQuotaPercentage(currentSize: number, quotaSize: number): number {
 ...
 */
export function determineQuotaStatus(
  currentSize: number,
  config: MailQuotaConfig
): MailQuotaConfig['status'] {
 ...
 */
export function ApiQuotaStatus(): MethodDecorator {
 ...
 */
export function ApiStorageAnalytics(): MethodDecorator {
  const { ApiOperation, ApiResponse } = require('@nestjs/swagger');
 ...
 */
export function createQuotaEnforcementCronJob(
  sequelize: Sequelize,
  ...
  return async () => {
    try {
      ...
      const quotaConfigs = await sequelize.query(
        `SELECT user_id FROM mail_quota_configs WHERE user_id IS NOT NULL`,
        { type: 'SELECT' }
      );
 ...
 */
export function createStorageCleanupCronJob(
  sequelize: Sequelize,
  ...
  return async () => {
    try {
      ...
      console.log(
        `[Storage Cleanup] Completed. Freed ${formatBytes(result.spaceFreedBytes)}, deleted ${result.messagesDeleted} messages, archived ${result.messagesArchived} messages`
      );
    } catch (error) {
