 ...
 * ```
 */
export const findVirtualMachineById = async (
  sequelize: Sequelize,
  vmId: string,
  ...
  const options: FindOptions = {
    ...
    attributes: {
      include: [
        [literal('(SELECT COUNT(*) FROM snapshots WHERE snapshots.vm_id = VirtualMachine.id)'), 'snapshotCount'],
        [literal('CASE WHEN power_state = \'running\' THEN uptime_seconds ELSE 0 END'), 'currentUptime'],
      ],
    },
  };
 ...
 * ```
 */
export const findVirtualMachinesByFilters = async (
  sequelize: Sequelize,
  filters: VmQueryFilters,
  pagination?: CursorPaginationOptions,
): Promise<{ rows: any[]; cursor: string | null }> => {
  const VirtualMachine = sequelize.models.VirtualMachine;
  ...
  if (filters.powerState?.length) {
    whereClause.powerState = { [Op.in]: filters.powerState };
  }
  ...
  if (filters.minCpu || filters.maxCpu) {
    ...
    if (filters.minCpu) whereClause.cpuCores[Op.gte] = filters.minCpu;
    if (filters.maxCpu) whereClause.cpuCores[Op.lte] = filters.maxCpu;
  }

  if (filters.minMemory || filters.maxMemory) {
    whereClause.memoryGb = {};
    if (filters.minMemory) whereClause.memoryGb[Op.gte] = filters.minMemory;
    if (filters.maxMemory) whereClause.memoryGb[Op.lte] = filters.maxMemory;
  }

  if (filters.tags?.length) {
    whereClause.tags = {
      [Op.contains]: filters.tags, // JSONB contains for PostgreSQL
    };
  }

  if (filters.createdAfter || filters.createdBefore) {
    whereClause.createdAt = {};
    if (filters.createdAfter) whereClause.createdAt[Op.gte] = filters.createdAfter;
    if (filters.createdBefore) whereClause.createdAt[Op.lte] = filters.createdBefore;
  }

  if (filters.searchTerm) {
    whereClause[Op.or] = [
      { name: { [Op.iLike]: `%${filters.searchTerm}%` } },
      { hostname: { [Op.iLike]: `%${filters.searchTerm}%` } },
      { guestOs: { [Op.iLike]: `%${filters.searchTerm}%` } },
    ];
  }
  ...
  if (pagination?.cursor) {
    ...
    whereClause.id = { [Op.gt]: cursorData.id };
  }
 ...
 * ```
 */
export const findHostsWithCapacity = async (
  sequelize: Sequelize,
  requiredCpu: number,
  ...
  return await Host.findAll({
    ...
    attributes: [
      ...
      [literal('Host.cpu_cores - COALESCE((SELECT SUM(cpu_cores) FROM virtual_machines WHERE virtual_machines.host_id = Host.id AND power_state = \'running\'), 0)'), 'availableCpu'],
      [literal('Host.memory_gb - COALESCE((SELECT SUM(memory_gb) FROM virtual_machines WHERE virtual_machines.host_id = Host.id AND power_state = \'running\'), 0)'), 'availableMemory'],
    ],
    having: {
      [Op.and]: [
        literal(`Host.cpu_cores - COALESCE((SELECT SUM(cpu_cores) FROM virtual_machines WHERE virtual_machines.host_id = Host.id AND power_state = 'running'), 0) >= ${requiredCpu}`),
        literal(`Host.memory_gb - COALESCE((SELECT SUM(memory_gb) FROM virtual_machines WHERE virtual_machines.host_id = Host.id AND power_state = 'running'), 0) >= ${requiredMemory}`),
      ],
    },
    include: [
      {
    ...
    order: [
      [literal('availableCpu'), 'DESC'],
      [literal('availableMemory'), 'DESC'],
    ],
  });
};
 ...
 * ```
 */
export const findDatastoresWithSpace = async (
  sequelize: Sequelize,
  ...
  const whereClause: WhereOptions = {
    status: 'active',
    freeSpaceGb: { [Op.gte]: requiredSpaceGb },
  };
  ...
  return await Datastore.findAll({
    ...
    attributes: [
      ...
      'capacityGb',
      'freeSpaceGb',
      [literal('(free_space_gb / capacity_gb * 100)'), 'freeSpacePercent'],
      [
        literal(`(
          SELECT COUNT(*)
          FROM virtual_machines
 ...
 * ```
 */
export const executeComplexFilter = async (
  sequelize: Sequelize,
  modelName: string,
 ...
 * ```
 */
export const searchVirtualResourcesFullText = async (
  sequelize: Sequelize,
  searchTerm: string,
  ...
  for (const type of types) {
    ...
    const searchResults = await Model.findAll({
      where: {
        [Op.or]: [
          literal(`to_tsvector('english', name) @@ plainto_tsquery('english', '${searchTerm}')`),
          literal(`to_tsvector('english', COALESCE(description, '')) @@ plainto_tsquery('english', '${searchTerm}')`),
        ],
      },
      attributes: {
        include: [
          [literal(`ts_rank(to_tsvector('english', name || ' ' || COALESCE(description, '')), plainto_tsquery('english', '${searchTerm}'))`), 'relevance'],
          [literal(`'${type}'`), 'resourceType'],
        ],
      },
      limit: 100,
    });
 ...
 * ```
 */
export const findVmsByTagHierarchy = async (
  sequelize: Sequelize,
  tagQuery: object,
): Promise<any[]> => {
  const VirtualMachine = sequelize.models.VirtualMachine;
  ...
  return await VirtualMachine.findAll({
    where: {
      tags: {
        [Op.contains]: tagQuery, // PostgreSQL JSONB contains
      },
    },
    attributes: {
      include: [[literal('tags::text'), 'tagsJson']],
    },
  });
};
 ...
 * ```
 */
export const aggregateResourcesByGroup = async (
  sequelize: Sequelize,
  modelName: string,
  ...
  const attributes: any[] = [...options.groupBy];

  options.metrics.forEach((metric) => {
    const operation = metric.operation.toLowerCase();
    attributes.push([fn(operation, col(metric.field)), metric.alias]);
  });
 ...
 * ```
 */
export const getClusterResourceUtilization = async (
  sequelize: Sequelize,
  clusterId: string,
): Promise<any> => {
 ...
 * ```
 */
export const getDatastoreUtilizationTrend = async (
  sequelize: Sequelize,
  datastoreId: string,
  days: number = 30,
): Promise<any[]> => {
 ...
 * ```
 */
export const sortVmsByMultipleCriteria = async (
  sequelize: Sequelize,
  sortCriteria: Array<{ field: string; direction: 'ASC' | 'DESC'; nulls?: 'FIRST' | 'LAST' }>,
  filters?: WhereOptions,
): Promise<any[]> => {
  const VirtualMachine = sequelize.models.VirtualMachine;

  const order: Order = sortCriteria.map((criterion) => {
    if (criterion.nulls) {
      return literal(`${criterion.field} ${criterion.direction} NULLS ${criterion.nulls}`);
    }
 ...
 * ```
 */
export const rankHostsByPerformance = async (
  sequelize: Sequelize,
  clusterId?: string,
): Promise<any[]> => {
  let whereClause = '';
 ...
 * ```
 */
export const paginateWithOffsetLimit = async (
  sequelize: Sequelize,
  modelName: string,
  page: number,
  pageSize: number,
  options?: FindOptions,
): Promise<{ rows: any[]; count: number; pages: number }> => {
  const Model = sequelize.models[modelName];
...
export const paginateWithCursor = async (
  ...
  filters?: WhereOptions,
): Promise<{ rows: any[]; nextCursor: string | null; hasMore: boolean }> => {
  const Model = sequelize.models[modelName];
 ...
 * ```
 */
export const paginateWithKeyset = async (
  sequelize: Sequelize,
  modelName: string,
  keyField: string,
  lastKey: any,
  ...
  if (lastKey !== null && lastKey !== undefined) {
    whereClause[keyField] = { [Op.gt]: lastKey };
  }
 ...
 * ```
 */
export const findVmsWithEagerLoading = async (
  sequelize: Sequelize,
  filters?: WhereOptions,
): Promise<any[]> => {
  const VirtualMachine = sequelize.models.VirtualMachine;
 ...
 * ```
 */
export const batchLoadVirtualMachines = async (
  sequelize: Sequelize,
  vmIds: string[],
  ...
  vms.forEach((vm: any) => vmMap.set(vm.id, vm));
 ...
 * ```
 */
export const findWithQueryOptimization = async (
  sequelize: Sequelize,
  modelName: string,
 ...
 * ```
 */
export const getVmCountByStatus = async (
  sequelize: Sequelize,
  clusterId?: string,
): Promise<any[]> => {
  const VirtualMachine = sequelize.models.VirtualMachine;
 ...
 * ```
 */
export const findAndLockVm = async (
  sequelize: Sequelize,
  vmId: string,
 ...
 * ```
 */
export const bulkUpdateVmStatus = async (
  sequelize: Sequelize,
  vmIds: string[],
  newStatus: string,
  transaction?: Transaction,
  ...
  const [affectedCount] = await VirtualMachine.update(
 ...
 * ```
 */
export const findVmsWithSubquery = async (sequelize: Sequelize, clusterId: string): Promise<any[]> => {
  const VirtualMachine = sequelize.models.VirtualMachine;
 ...
 * ```
 */
export const getResourceHierarchyWithCte = async (sequelize: Sequelize): Promise<any[]> => {
  return await sequelize.query(
    `
 ...
 * ```
 */
export const getVmMigrationCandidates = async (
  sequelize: Sequelize,
  sourceHostId: string,
): Promise<any[]> => {
  return await sequelize.query(
 ...
 * ```
 */
export const getStorageMigrationPlan = async (
  sequelize: Sequelize,
  sourceDatastoreId: string,
  targetFreeSpacePercent: number,
): Promise<any[]> => {
  return await sequelize.query(
    `
    WITH vm_storage AS (
 ...
 * ```
 */
export const findSnapshotsWithChain = async (
  sequelize: Sequelize,
  vmId: string,
): Promise<any[]> => {
  return await sequelize.query(
 ...
 * ```
 */
export const findVmsByResourcePool = async (
  sequelize: Sequelize,
  clusterId: string,
): Promise<any[]> => {
  return await sequelize.query(
 ...
 * ```
 */
export const findVmsWithAntiAffinityRules = async (
  sequelize: Sequelize,
  clusterId: string,
): Promise<any[]> => {
  return await sequelize.query(
 ...
 * ```
 */
export const getHostMaintenanceModeImpact = async (
  sequelize: Sequelize,
  hostId: string,
): Promise<any> => {
 ...
 * ```
 */
export const findOrphanedVirtualDisks = async (
  sequelize: Sequelize,
  datastoreId?: string,
): Promise<any[]> => {
 ...
 * ```
 */
export const getVmCostAllocation = async (
  sequelize: Sequelize,
  startDate: Date,
  endDate: Date,
): Promise<any[]> => {
 ...
 * ```
 */
export const findVmsNeedingRightSizing = async (
  sequelize: Sequelize,
  days: number = 30,
): Promise<any[]> => {
  return await sequelize.query(
 ...
 * ```
 */
export const getVmLifecycleEvents = async (
  sequelize: Sequelize,
  vmId: string,
  limit: number = 100,
): Promise<any[]> => {
  ...
  return await Event.findAll({
    ...
    attributes: [
      ...
      'createdAt',
      [
        literal(`LAG(event_type) OVER (PARTITION BY vm_id ORDER BY created_at)`),
        'previousEvent',
      ],
      [
        literal(`LEAD(event_type) OVER (PARTITION BY vm_id ORDER BY created_at)`),
        'nextEvent',
      ],
    ],
    order: [['createdAt', 'DESC']],
 ...
 * ```
 */
export const findVmsWithComplianceIssues = async (
  sequelize: Sequelize,
  policyTypes?: string[],
): Promise<any[]> => {
  let policyFilter = '';
 ...
 * ```
 */
export const getClusterDrsRecommendations = async (
  sequelize: Sequelize,
  clusterId: string,
): Promise<any[]> => {
  return await sequelize.query(
 ...
 * ```
 */
export const findVmsByBackupStatus = async (
  sequelize: Sequelize,
 ...
 * ```
 */
export const getVmDependencyGraph = async (
  sequelize: Sequelize,
  vmId: string,
): Promise<any> => {
  return await sequelize.query(
 ...
 * ```
 */
export const findVmsInMultipleDatastores = async (
  sequelize: Sequelize,
): Promise<any[]> => {
  return await sequelize.query(
    `
 ...
 * ```
 */
export const getHostFailoverCapacity = async (
  sequelize: Sequelize,
  clusterId: string,
  failoverHosts: number = 1,
): Promise<any> => {
  const [result] = await sequelize.query(
    `
 ...
 * ```
 */
export const findZombieVms = async (
  sequelize: Sequelize,
 ...
 * ```
 */
export const getVlanUtilization = async (
  sequelize: Sequelize,
): Promise<any[]> => {
  return await sequelize.query(
    `
 ...
 * ```
 */
export const findVmsWithHighChangeRate = async (
  sequelize: Sequelize,
  days: number = 7,
): Promise<any[]> => {
  return await sequelize.query(
...
export default {
  ...
  findVirtualMachinesByFilters,
  findHostsWithCapacity,
  findDatastoresWithSpace,

  // Complex filtering
  executeComplexFilter,
  searchVirtualResourcesFullText,
  findVmsByTagHierarchy,

  // Aggregation
  aggregateResourcesByGroup,
  getClusterResourceUtilization,
  getDatastoreUtilizationTrend,

  // Sorting
  sortVmsByMultipleCriteria,
  rankHostsByPerformance,
  ...
  // N+1 prevention
  findVmsWithEagerLoading,
  batchLoadVirtualMachines,

  // Performance optimization
  findWithQueryOptimization,
  getVmCountByStatus,
  ...
  // Subqueries & CTEs
  findVmsWithSubquery,
  getResourceHierarchyWithCte,

  // Raw SQL complex queries
  getVmMigrationCandidates,
  getStorageMigrationPlan,
  ...
  getVmLifecycleEvents,

  // Resource pools & allocation
  findVmsByResourcePool,
  getVmCostAllocation,

  // Performance & metrics
  getVmPerformanceMetrics,
  findVmsNeedingRightSizing,
  findVmsWithHighChangeRate,

  // Compliance & policies
  findVmsWithAntiAffinityRules,
  findVmsWithComplianceIssues,
  findVmsByBackupStatus,

  // Maintenance & operations
  getHostMaintenanceModeImpact,
  getClusterDrsRecommendations,
  getHostFailoverCapacity,

  // Cleanup & optimization
  findOrphanedVirtualDisks,
  findZombieVms,
  findVmsInMultipleDatastores,

  // Dependencies
  getVmDependencyGraph,
};
