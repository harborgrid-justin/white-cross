...
exports.validateIQN = exports.validateWWN = exports.generateWWID = exports.purgeExpiredSnapshots = exports.restoreLUNFromSnapshot = exports.createLUNSnapshot = exports.reclaimUnusedCapacity = exports.predictCapacityExhaustion = exports.generateCapacityReport = exports.identifyHotLUNs = exports.getAggregatedPerformanceMetrics = exports.recordLUNPerformanceMetrics = exports.findOrphanedLUNs = exports.queryLUNsAdvanced = exports.findLUNsByIdentifier = exports.discoverLUNs = exports.deactivateZone = exports.activateZone = exports.validateZoneConfig = exports.createZone = exports.balanceIOAcrossPaths = exports.performPathFailover = exports.monitorPathHealth = exports.registerLUNPaths = exports.configureMultipathing = exports.updateLUNAccessMode = exports.getLUNMappings = exports.unmaskLUNFromHosts = exports.maskLUNToHosts = exports.cloneLUN = exports.deleteLUN = exports.expandLUN = exports.provisionLUN = void 0;
/**
 * File: /reuse/san/san-lun-operations-kit.ts
 ...
 * ```
 */
const provisionLUN = async (LUNModel, options, transaction) => {
    const { name, size, storagePoolId, thinProvisioned = true, compression = false, deduplication = false, tier = 'capacity', autoExpand = false, maxSize, blockSize = 4, } = options;
    // Generate WWID (World Wide Identifier)
    const wwid = (0, exports.generateWWID)();
    ...
    const lunNumber = lastLUN ? lastLUN.lunNumber + 1 : 0;
...
exports.provisionLUN = provisionLUN;
/**
 ...
 * ```
 */
const deleteLUN = async (LUNModel, LUNMappingModel, lunId, force = false, transaction) => {
    const lun = await LUNModel.findByPk(lunId, { transaction });
    if (!lun) {
...
const cloneLUN = async (LUNModel, sourceLunId, cloneName, thinClone = true, transaction) => {
    ...
    return clonedLUN.toJSON();
};
exports.cloneLUN = cloneLUN;
// ============================================================================
 ...
 * ```
 */
const maskLUNToHosts = async (LUNMappingModel, options, transaction) => {
    ...
    for (const hostId of hostIds) {
        ...
        if (existing) {
            ...
            const hostLunId = lastMapping ? lastMapping.hostLunId + 1 : 0;
            ...
            mappings.push(mapping.toJSON());
        }
    }
    return mappings;
};
exports.maskLUNToHosts = maskLUNToHosts;
/**
 ...
 * ```
 */
const unmaskLUNFromHosts = async (LUNMappingModel, lunId, hostIds, transaction) => {
...
exports.unmaskLUNFromHosts = unmaskLUNFromHosts;
/**
 ...
 * ```
 */
const getLUNMappings = async (LUNMappingModel, lunId, transaction) => {
...
exports.getLUNMappings = getLUNMappings;
/**
 ...
 * ```
 */
const updateLUNAccessMode = async (LUNMappingModel, lunId, hostId, accessMode, transaction) => {
...
exports.updateLUNAccessMode = updateLUNAccessMode;
// ============================================================================
 ...
 * ```
 */
const configureMultipathing = async (MultipathConfigModel, config, transaction) => {
    ...
    if (existing) {
        await existing.update(config, { transaction });
        return existing.toJSON();
    }
    const created = await MultipathConfigModel.create(config, { transaction });
    return created.toJSON();
};
exports.configureMultipathing = configureMultipathing;
/**
 ...
 * ```
 */
const registerLUNPaths = async (LUNPathModel, lunId, hostId, discoveredPaths, transaction) => {
    const paths = [];
...
exports.registerLUNPaths = registerLUNPaths;
/**
 ...
 * ```
 */
const monitorPathHealth = async (LUNPathModel, lunId, hostId, transaction) => {
    ...
    for (const path of paths) {
        ...
        else if (pathData.pathState === 'active-unoptimized' ||
...
exports.monitorPathHealth = monitorPathHealth;
/**
 ...
 * ```
 */
const performPathFailover = async (LUNPathModel, lunId, hostId, failedPathId, options, transaction) => {
    ...
    const standbyPath = await LUNPathModel.findOne({
        where: {
            lunId,
            hostId,
            pathState: { [sequelize_1.Op.in]: ['standby', 'active-unoptimized'] },
        },
        order: [
            ['pathPriority', 'DESC'],
            ['errorCount', 'ASC'],
    ...
    return {
        success: true,
        newActivePath: standbyPath.toJSON(),
    };
};
 ...
 * ```
 */
const balanceIOAcrossPaths = async (LUNPathModel, lunId, hostId, policy, transaction) => {
    const activePaths = await LUNPathModel.findAll({
        where: {
            lunId,
            hostId,
            pathState: { [sequelize_1.Op.in]: ['active-optimized', 'active-unoptimized'] },
        },
        transaction,
    });
    const paths = [];
    for (let i = 0; i < activePaths.length; i++) {
        ...
        switch (policy) {
            ...
            case 'least-queue-depth':
                ...
                priority = 100 - (path.ioCount % 100);
                break;
            case 'service-time':
                // Prioritize paths with fewer errors
                priority = 100 - (path.errorCount % 100);
                break;
        }
...
exports.balanceIOAcrossPaths = balanceIOAcrossPaths;
// ============================================================================
// ZONING MANAGEMENT
// ============================================================================
/**
 ...
 * ```
 */
const createZone = async (ZoneConfigModel, zoneConfig, transaction) => {
    const zone = await ZoneConfigModel.create(zoneConfig, { transaction });
    return zone.toJSON();
};
 ...
 * ```
 */
const validateZoneConfig = async (ZoneConfigModel, zoneName, transaction) => {
    ...
    const initiators = zoneData.members.filter((m) => m.type === 'initiator');
    const targets = zoneData.members.filter((m) => m.type === 'target');
 ...
 * ```
 */
const activateZone = async (ZoneConfigModel, zoneId, transaction) => {
...
exports.activateZone = activateZone;
/**
 ...
 * ```
 */
const deactivateZone = async (ZoneConfigModel, zoneId, transaction) => {
    const zone = await ZoneConfigModel.findByPk(zoneId, { transaction });
    if (!zone) {
        throw new Error(`Zone ${zoneId} not found`);
    }
...
exports.deactivateZone = deactivateZone;
// ============================================================================
 ...
 * ```
 */
const discoverLUNs = async (LUNModel, options = {}, transaction) => {
    const { storagePoolIds, minSize, maxSize, tier, status, includeMetrics = false, includeSnapshots = false, includeMappings = false, } = options;
    const where = {};
    if (storagePoolIds && storagePoolIds.length > 0) {
        where.storagePoolId = { [sequelize_1.Op.in]: storagePoolIds };
    }
    if (minSize !== undefined) {
        where.size = { ...(where.size || {}), [sequelize_1.Op.gte]: minSize };
    }
    if (maxSize !== undefined) {
        where.size = { ...(where.size || {}), [sequelize_1.Op.lte]: maxSize };
    }
    if (tier && tier.length > 0) {
        where.tier = { [sequelize_1.Op.in]: tier };
    }
    ...
    return luns.map((lun) => lun.toJSON());
};
exports.discoverLUNs = discoverLUNs;
/**
 ...
 * ```
 */
const findLUNsByIdentifier = async (LUNMappingModel, identifier, transaction) => {
    const normalizedId = identifier.replace(/[:-]/g, '').toLowerCase();
    const mappings = await LUNMappingModel.findAll({
        where: {
            [sequelize_1.Op.or]: [
                { wwn: { [sequelize_1.Op.like]: `%${normalizedId}%` } },
                { iqn: { [sequelize_1.Op.like]: `%${normalizedId}%` } },
            ],
        },
        transaction,
    });
...
exports.findLUNsByIdentifier = findLUNsByIdentifier;
/**
 ...
 * ```
 */
const queryLUNsAdvanced = async (LUNModel, sequelize, filters, transaction) => {
    const where = {};
    if (filters.allocatedPercent) {
        const allocatedExpr = sequelize.literal('(allocated * 100.0 / NULLIF(size, 0))');
        if (filters.allocatedPercent.min !== undefined) {
            where[sequelize_1.Op.and] = [
                ...(where[sequelize_1.Op.and] || []),
                sequelize.where(allocatedExpr, sequelize_1.Op.gte, filters.allocatedPercent.min),
            ];
        }
        if (filters.allocatedPercent.max !== undefined) {
            where[sequelize_1.Op.and] = [
                ...(where[sequelize_1.Op.and] || []),
                sequelize.where(allocatedExpr, sequelize_1.Op.lte, filters.allocatedPercent.max),
            ];
        }
    }
    if (filters.tier) {
    ...
    if (filters.ageInDays) {
        ...
        if (filters.ageInDays.max !== undefined) {
            where.createdAt = { [sequelize_1.Op.gte]: ageDate };
        }
    }
...
exports.queryLUNsAdvanced = queryLUNsAdvanced;
/**
 ...
 * ```
 */
const findOrphanedLUNs = async (LUNModel, LUNMappingModel, sequelize, daysUnmapped = 30, transaction) => {
...
exports.findOrphanedLUNs = findOrphanedLUNs;
// ============================================================================
 ...
 * ```
 */
const recordLUNPerformanceMetrics = async (LUNPerformanceModel, metrics, transaction) => {
    const record = await LUNPerformanceModel.create(metrics, {
        transaction,
    });
    return record.toJSON();
};
exports.recordLUNPerformanceMetrics = recordLUNPerformanceMetrics;
/**
 ...
 * ```
 */
const getAggregatedPerformanceMetrics = async (LUNPerformanceModel, sequelize, lunId, startTime, endTime, transaction) => {
...
exports.getAggregatedPerformanceMetrics = getAggregatedPerformanceMetrics;
/**
 ...
 * ```
 */
const identifyHotLUNs = async (LUNPerformanceModel, sequelize, topN = 10, transaction) => {
...
exports.identifyHotLUNs = identifyHotLUNs;
// ============================================================================
// CAPACITY MANAGEMENT
// ============================================================================
/**
 ...
 * ```
 */
const generateCapacityReport = async (LUNModel, sequelize, storagePoolIds, transaction) => {
 ...
 * ```
 */
const predictCapacityExhaustion = async (LUNModel, sequelize, storagePoolId, maxCapacity, transaction) => {
  ...
  `, {
    ...
    const allocations = results.map((r) => parseFloat(r.daily_allocated));
 ...
 * ```
 */
const reclaimUnusedCapacity = async (LUNModel, storagePoolId, thresholdPercent = 50, transaction) => {
    const luns = await LUNModel.findAll({
        where: {
            storagePoolId,
            thinProvisioned: true,
...
exports.reclaimUnusedCapacity = reclaimUnusedCapacity;
// ============================================================================
// SNAPSHOT OPERATIONS
// ============================================================================
/**
 ...
 * ```
 */
const createLUNSnapshot = async (LUNSnapshotModel, LUNModel, lunId, snapshotName, retentionDays = 30, description, transaction) => {
    const lun = await LUNModel.findByPk(lunId, { transaction });
    if (!lun) {
        throw new Error(`LUN ${lunId} not found`);
    }
...
exports.createLUNSnapshot = createLUNSnapshot;
/**
 ...
 * ```
 */
const restoreLUNFromSnapshot = async (LUNModel, LUNSnapshotModel, snapshotId, transaction) => {
...
exports.restoreLUNFromSnapshot = restoreLUNFromSnapshot;
/**
 ...
 * ```
 */
const purgeExpiredSnapshots = async (LUNSnapshotModel, transaction) => {
    const now = new Date();
    const expiredSnapshots = await LUNSnapshotModel.findAll({
        where: {
            status: 'active',
            createdAt: {
                [sequelize_1.Op.lt]: sequelize.literal(`DATE_SUB(NOW(), INTERVAL retention_days DAY)`),
            },
        },
        transaction,
    });
 ...
 * ```
 */
const validateWWN = (wwn) => {
    return /^([0-9a-fA-F]{2}:){7}[0-9a-fA-F]{2}$/.test(wwn);
};
exports.validateWWN = validateWWN;
/**
 ...
 * ```
 */
const validateIQN = (iqn) => {
    return /^iqn\.\d{4}-\d{2}\.[a-z0-9.-]+:[a-z0-9._-]+$/i.test(iqn);
};
exports.validateIQN = validateIQN;
exports.default = {
    // Provisioning and lifecycle
    provisionLUN: exports.provisionLUN,
    expandLUN: exports.expandLUN,
    deleteLUN: exports.deleteLUN,
    cloneLUN: exports.cloneLUN,
    // Masking and mapping
    maskLUNToHosts: exports.maskLUNToHosts,
    unmaskLUNFromHosts: exports.unmaskLUNFromHosts,
    getLUNMappings: exports.getLUNMappings,
    updateLUNAccessMode: exports.updateLUNAccessMode,
    // Multipathing
    configureMultipathing: exports.configureMultipathing,
    registerLUNPaths: exports.registerLUNPaths,
    monitorPathHealth: exports.monitorPathHealth,
    performPathFailover: exports.performPathFailover,
    balanceIOAcrossPaths: exports.balanceIOAcrossPaths,
    // Zoning
    createZone: exports.createZone,
    validateZoneConfig: exports.validateZoneConfig,
    activateZone: exports.activateZone,
    deactivateZone: exports.deactivateZone,
    // Discovery and queries
    discoverLUNs: exports.discoverLUNs,
    findLUNsByIdentifier: exports.findLUNsByIdentifier,
    queryLUNsAdvanced: exports.queryLUNsAdvanced,
    findOrphanedLUNs: exports.findOrphanedLUNs,
    // Performance monitoring
    recordLUNPerformanceMetrics: exports.recordLUNPerformanceMetrics,
    getAggregatedPerformanceMetrics: exports.getAggregatedPerformanceMetrics,
    identifyHotLUNs: exports.identifyHotLUNs,
    // Capacity management
    generateCapacityReport: exports.generateCapacityReport,
    predictCapacityExhaustion: exports.predictCapacityExhaustion,
    reclaimUnusedCapacity: exports.reclaimUnusedCapacity,
    // Snapshot operations
    createLUNSnapshot: exports.createLUNSnapshot,
    restoreLUNFromSnapshot: exports.restoreLUNFromSnapshot,
    purgeExpiredSnapshots: exports.purgeExpiredSnapshots,
    // Utilities
    generateWWID: exports.generateWWID,
    validateWWN: exports.validateWWN,
    validateIQN: exports.validateIQN,
};
//# sourceMappingURL=san-lun-operations-kit.js.map