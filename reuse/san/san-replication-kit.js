...
typescript
    * ;
const san_replication_kit_1 = require("./san-replication-kit");
    *
    * ;
const readiness = await (0, san_replication_kit_1.verifyFailoverReadiness)('repl-job-001');
    * ;
if (readiness.ready) {
        * await (0, san_replication_kit_1.initiateFailover)('repl-job-001', { automatic: true });
        * ;
}
    * `` `
 */
...
replication: $;
{
    config.deltaReplicationEnabled;
}
`);
...
(throughput, { currentThroughputMbps }, Mbps) => ;
`);
...
const readiness = await (0, san_replication_kit_1.verifyFailoverReadiness)('repl-job-001');
    * ;
if (readiness.ready) {
        * await (0, san_replication_kit_1.initiateFailover)(jobId);
        * ;
}
else {
 ...
 * ` ``; typescript
    * )
    const result = await (0, san_replication_kit_1.initiateFailover)('repl-job-001', {});
    * `` `
 */
...
Monitoring;
auto - failover;
for ($; { jobId }(threshold, $, { healthThreshold }) `);
...
;
//# sourceMappingURL=san-replication-kit.js.map