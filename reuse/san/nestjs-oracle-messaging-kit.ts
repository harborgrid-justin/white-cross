...
export enum MessagePriority {
  ...
  HIGH = 7,
  HIGHEST = 9,
  CRITICAL = 10, // Healthcare-specific priority
}
 ...
 * ```
 */
export function createJmsMessage<T>(
  body: T,
 ...
 * ```
 */
export function setMessagePriority<T>(
  message: JmsMessage<T>,
  priority: MessagePriority,
): JmsMessage<T> {
  return {
    ...message,
 ...
 * ```
 */
export function setMessageTTL<T>(message: JmsMessage<T>, ttlMs: number): JmsMessage<T> {
 ...
 * ```
 */
export function setCorrelationId<T>(message: JmsMessage<T>, correlationId: string): JmsMessage<T> {
  return {
    ...message,
    correlationId,
    headers: {
 ...
 * ```
 */
export function setReplyTo<T>(message: JmsMessage<T>, replyTo: string): JmsMessage<T> {
  return {
    ...message,
    headers: {
      ...message.headers,
 ...
 * ```
 */
export function markMessageWithPHI<T>(message: JmsMessage<T>, encrypted: boolean): JmsMessage<T> {
  return {
    ...message,
    properties: {
      ...message.properties,
 ...
 * ```
 */
export function setHealthcareContext<T>(
  message: JmsMessage<T>,
  context: {
    tenantId?: string;
    organizationId?: string;
 ...
 * ```
 */
export function cloneMessageForRedelivery<T>(message: JmsMessage<T>): JmsMessage<T> {
  return {
    ...message,
    redelivered: true,
 ...
 * ```
 */
export function createPriorityQueue(name: string, maxPriority: number = 10): QueueConfig {
 ...
 * ```
 */
export function createDurableSubscription(
  subscriptionName: string,
  topicName: string,
  selector?: string,
): DurableSubscriptionConfig {
  return {
    subscriptionName,
    topicName,
    selector,
 ...
 * ```
 */
export function createNonDurableSubscription(
  subscriptionName: string,
  topicName: string,
  selector?: string,
): DurableSubscriptionConfig {
...
export async function subscribeToTopic(
  subscription: DurableSubscriptionConfig,
  handler: (message: JmsMessage) => Promise<void>,
  channel: any,
): Promise<string> {
  ...
  const { consumerTag } = await channel.consume(queueName, async (msg: any) => {
    if (msg) {
      const message: JmsMessage = JSON.parse(msg.content.toString());
 ...
 * ```
 */
export function compileMessageSelector(expression: string): MessageSelector {
  // Simplified SQL-92 selector compilation
  const compiled = (msg: JmsMessage): boolean => {
    try {
 ...
 * ```
 */
export function filterMessagesByProperty<T>(
  messages: JmsMessage<T>[],
  propertyName: string,
  propertyValue: any,
): JmsMessage<T>[] {
 ...
 * ```
 */
export function filterMessagesByPriority<T>(
  messages: JmsMessage<T>[],
...
export function filterMessages<T>(
  messages: JmsMessage<T>[],
  predicate: MessageFilterPredicate<T>,
): JmsMessage<T>[] {
 ...
 * ```
 */
export function filterExpiredMessages<T>(
  messages: JmsMessage<T>[],
): { expired: JmsMessage<T>[]; valid: JmsMessage<T>[] } {
  const now = Date.now();
  const expired: JmsMessage<T>[] = [];
  const valid: JmsMessage<T>[] = [];
 ...
 * ```
 */
export async function negativeAcknowledge(
  messageId: string,
  channel: any,
  deliveryTag: any,
  requeue: boolean,
 ...
 * ```
 */
export async function autoAcknowledge<T>(
  message: JmsMessage<T>,
  processor: (msg: JmsMessage<T>) => Promise<void>,
  channel: any,
  deliveryTag: any,
): Promise<AckResult> {
  try {
...
export function createDeadLetterQueueConfig(
  ...
  options?: Partial<DeadLetterQueueConfig>,
 ...
 * ```
 */
export async function monitorDeadLetterQueue(
  dlqName: string,
  threshold: number,
  channel: any,
): Promise<{ messageCount: number; alertTriggered: boolean }> {
 ...
 * ```
 */
export function isMessageExpired<T>(message: JmsMessage<T>): boolean {
 ...
 * ```
 */
export function getRemainingTTL<T>(message: JmsMessage<T>): number {
  if (!message.expiresAt) {
    return 0;
  }
 ...
 * ```
 */
export function extendMessageTTL<T>(message: JmsMessage<T>, extensionMs: number): JmsMessage<T> {
 ...
 * ```
 */
export function removeMessageTTL<T>(message: JmsMessage<T>): JmsMessage<T> {
 ...
 * ```
 */
export function sortMessagesByPriority<T>(messages: JmsMessage<T>[]): JmsMessage<T>[] {
 ...
 * ```
 */
export function scheduleMessageDelivery<T>(
  message: JmsMessage<T>,
  timestamp: number,
): JmsMessage<T> {
 ...
 * ```
 */
export function scheduleMessageDelay<T>(message: JmsMessage<T>, delayMs: number): JmsMessage<T> {
 ...
 * ```
 */
export function createCronScheduledMessage<T>(
  message: JmsMessage<T>,
  cronExpression: string,
): JmsMessage<T> {
  return {
 ...
 * ```
 */
export function isScheduledMessageReady<T>(message: JmsMessage<T>): boolean {
 ...
 * ```
 */
export function createReplyMessage<T>(
  reply: T,
  correlationId: string,
  success: boolean,
): JmsMessage<T> {
 ...
 * ```
 */
export function validateReplyMessage<T>(
  reply: JmsMessage<T>,
 ...
 * ```
 */
export async function requestReplyWithTimeout<TReq, TRes>(
  request: JmsMessage<TReq>,
  sendFn: (msg: JmsMessage<TReq>) => Promise<void>,
  receiveFn: (correlationId: string) => Promise<JmsMessage<TRes>>,
  timeout: number,
): Promise<TRes> {
 ...
 * ```
 */
export async function createTemporaryReplyQueue(
  prefix: string,
  channel: any,
): Promise<string> {
  const queueName = `${prefix}.${uuidv4()}`;
