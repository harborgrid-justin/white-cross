...
exports.SANTestingUtilities = void 0;
exports.generateSANVolumeFixture = generateSANVolumeFixture;
exports.generateSANLUNFixture = generateSANLUNFixture;
exports.generateSANSnapshotFixture = generateSANSnapshotFixture;
exports.generateSANReplicationJobFixture = generateSANReplicationJobFixture;
exports.generateSANStoragePoolFixture = generateSANStoragePoolFixture;
exports.generateSANPerformanceMetricsFixture = generateSANPerformanceMetricsFixture;
exports.generateSANVolumeFixtures = generateSANVolumeFixtures;
exports.generateSANLUNFixtures = generateSANLUNFixtures;
exports.generateSANSnapshotFixtures = generateSANSnapshotFixtures;
exports.generateSANHierarchy = generateSANHierarchy;
exports.createMockSANVolumeRepository = createMockSANVolumeRepository;
exports.createMockSANLUNRepository = createMockSANLUNRepository;
exports.createMockSANSnapshotRepository = createMockSANSnapshotRepository;
exports.createMockSANReplicationService = createMockSANReplicationService;
exports.createMockSANStorageService = createMockSANStorageService;
exports.createSANTestingModule = createSANTestingModule;
exports.setupUnitTest = setupUnitTest;
exports.createServiceSpy = createServiceSpy;
exports.verifyMethodCall = verifyMethodCall;
exports.resetAllMocks = resetAllMocks;
exports.createTestDatabaseModule = createTestDatabaseModule;
exports.seedSANTestData = seedSANTestData;
exports.cleanSANTestData = cleanSANTestData;
exports.createIntegrationTestContext = createIntegrationTestContext;
exports.waitForCondition = waitForCondition;
exports.createE2ETestApp = createE2ETestApp;
exports.getE2EAuthToken = getE2EAuthToken;
exports.createE2ETestContext = createE2ETestContext;
exports.cleanupE2ETestResources = cleanupE2ETestResources;
exports.executeAuthenticatedRequest = executeAuthenticatedRequest;
...
exports.assertSANVolumeProperties = assertSANVolumeProperties;
exports.assertSANLUNProperties = assertSANLUNProperties;
exports.assertSANSnapshotProperties = assertSANSnapshotProperties;
exports.assertReplicationJobProperties = assertReplicationJobProperties;
 ...
 */
function generateSANVolumeFixture(overrides) {
 ...
 */
function generateSANLUNFixture(overrides) {
    return {
 ...
 */
function generateSANSnapshotFixture(overrides) {
 ...
 */
function generateSANReplicationJobFixture(overrides) {
    return {
 ...
 */
function generateSANStoragePoolFixture(overrides) {
 ...
 */
function generateSANPerformanceMetricsFixture(overrides) {
 ...
 */
function generateSANVolumeFixtures(count, overrides) {
    return Array.from({ length: count }, (_, index) => generateSANVolumeFixture({
        ...overrides,
 ...
 */
function generateSANLUNFixtures(count, volumeId, overrides) {
    return Array.from({ length: count }, (_, index) => generateSANLUNFixture({
        ...overrides,
        lunNumber: index,
 ...
 */
function generateSANSnapshotFixtures(count, volumeId, overrides) {
    return Array.from({ length: count }, (_, index) => generateSANSnapshotFixture({
        ...overrides,
...
function generateSANHierarchy() {
    ...
    const luns = volumes.flatMap((volume) => generateSANLUNFixtures(2, volume.id));
    const snapshots = volumes.flatMap((volume) => generateSANSnapshotFixtures(5, volume.id));
 ...
 */
function createMockSANVolumeRepository(mockData = []) {
    const data = [...mockData];
    return {
 ...
 */
function createMockSANLUNRepository(mockData = []) {
    const data = [...mockData];
    return {
        find: jest.fn().mockResolvedValue(data),
        findOne: jest.fn().mockImplementation(({ where }) => {
 ...
 */
function createMockSANSnapshotRepository(mockData = []) {
    const data = [...mockData];
    return {
        find: jest.fn().mockResolvedValue(data),
        findOne: jest.fn().mockImplementation(({ where }) => {
 ...
 */
function createMockSANReplicationService() {
    return {
        createReplicationJob: jest.fn().mockImplementation((dto) => Promise.resolve(generateSANReplicationJobFixture(dto))),
        getReplicationJob: jest.fn().mockImplementation((id) => Promise.resolve(generateSANReplicationJobFixture({ id }))),
 ...
 */
function createMockSANStorageService() {
    return {
        createVolume: jest.fn().mockImplementation((dto) => Promise.resolve(generateSANVolumeFixture(dto))),
        getVolume: jest.fn().mockImplementation((id) => Promise.resolve(generateSANVolumeFixture({ id }))),
        updateVolume: jest.fn().mockImplementation((id, dto) => Promise.resolve(generateSANVolumeFixture({ id, ...dto }))),
        deleteVolume: jest.fn().mockResolvedValue(undefined),
        expandVolume: jest.fn().mockResolvedValue({ capacity: 2000 }),
        getVolumeMetrics: jest.fn().mockImplementation((id) => Promise.resolve(generateSANPerformanceMetricsFixture({ volumeId: id }))),
    };
}
// ============================================================================
// Unit Test Utilities
 ...
 */
async function createSANTestingModule(providers, imports = []) {
 ...
 */
async function setupUnitTest(ServiceClass, repositoryToken, mockRepository) {
 ...
 */
function createServiceSpy(service, method, returnValue) {
 ...
 */
function verifyMethodCall(spy, expectedArgs, callIndex = 0) {
 ...
 */
function createTestDatabaseModule(entities) {
 ...
 */
async function seedSANTestData(repositories) {
    ...
    const volumes = repositories.volumeRepo
        ? await repositories.volumeRepo.save(generateSANVolumeFixtures(3, {
            storagePoolId: Array.isArray(pools) ? pools[0]?.id : pools?.id,
        }))
        : [];
    const luns = repositories.lunRepo
        ? await repositories.lunRepo.save(generateSANLUNFixtures(5, volumes[0]?.id))
        : [];
    const snapshots = repositories.snapshotRepo
        ? await repositories.snapshotRepo.save(generateSANSnapshotFixtures(10, volumes[0]?.id))
        : [];
 ...
 */
async function cleanSANTestData(repositories) {
    if (repositories.snapshotRepo)
        await repositories.snapshotRepo.clear();
    if (repositories.lunRepo)
        await repositories.lunRepo.clear();
    if (repositories.volumeRepo)
        await repositories.volumeRepo.clear();
    if (repositories.poolRepo)
        await repositories.poolRepo.clear();
}
/**
 * 24. Create integration test context
 */
async function createIntegrationTestContext(moduleMetadata) {
    const module = await testing_1.Test.createTestingModule(moduleMetadata).compile();
    return module;
}
/**
 ...
 */
async function createE2ETestApp(moduleClass) {
 ...
 */
async function getE2EAuthToken(app, credentials) {
 ...
 */
async function createE2ETestContext(app, credentials) {
    let authToken;
    if (credentials) {
 ...
 */
async function cleanupE2ETestResources(context) {
 ...
 */
async function executeAuthenticatedRequest(app, method, path, authToken, body) {
 ...
 */
async function runPerformanceTest(operationName, operation, iterations = 100) {
 ...
 */
async function runLoadTest(operationName, operation, concurrency = 10, duration = 10000 // milliseconds
) {
    const startTime = Date.now();
    const startMemory = process.memoryUsage().heapUsed;
    let successCount = 0;
 ...
 */
function assertPerformanceThreshold(result, thresholds) {
 ...
 */
function assertSANVolumeProperties(volume, expected) {
    expect(volume).toBeDefined();
    if (expected.id)
        expect(volume.id).toBe(expected.id);
    if (expected.name)
        expect(volume.name).toBe(expected.name);
    if (expected.capacity)
        expect(volume.capacity).toBe(expected.capacity);
    if (expected.capacityUnit)
        expect(volume.capacityUnit).toBe(expected.capacityUnit);
    if (expected.type)
        expect(volume.type).toBe(expected.type);
    if (expected.status)
        expect(volume.status).toBe(expected.status);
    if (expected.encryption !== undefined)
        expect(volume.encryption).toBe(expected.encryption);
}
/**
 * 37. Assert SAN LUN properties
 */
function assertSANLUNProperties(lun, expected) {
    expect(lun).toBeDefined();
    if (expected.id)
        expect(lun.id).toBe(expected.id);
    if (expected.lunNumber !== undefined)
        expect(lun.lunNumber).toBe(expected.lunNumber);
    if (expected.volumeId)
        expect(lun.volumeId).toBe(expected.volumeId);
    if (expected.capacity)
        expect(lun.capacity).toBe(expected.capacity);
    if (expected.status)
        expect(lun.status).toBe(expected.status);
    if (expected.wwn)
        expect(lun.wwn).toBe(expected.wwn);
}
/**
 * 38. Assert SAN snapshot properties
 */
function assertSANSnapshotProperties(snapshot, expected) {
    ...
    if (expected.volumeId)
        expect(snapshot.volumeId).toBe(expected.volumeId);
    if (expected.type)
 ...
 */
function assertReplicationJobProperties(job, expected) {
    expect(job).toBeDefined();
    if (expected.id)
        expect(job.id).toBe(expected.id);
    if (expected.name)
        expect(job.name).toBe(expected.name);
    if (expected.sourceVolumeId)
        expect(job.sourceVolumeId).toBe(expected.sourceVolumeId);
    if (expected.targetVolumeId)
        expect(job.targetVolumeId).toBe(expected.targetVolumeId);
    if (expected.replicationType)
        expect(job.replicationType).toBe(expected.replicationType);
    if (expected.status)
 ...
 */
function assertAPIResponse(response, expectedStatus, expectedProperties) {
 ...
 */
function generateWWN() {
    const segments = Array.from({ length: 8 }, () => Math.floor(Math.random() * 256).toString(16).padStart(2, '0'));
...
exports.default = exports.SANTestingUtilities;
//# sourceMappingURL=san-testing-utilities-kit.js.map