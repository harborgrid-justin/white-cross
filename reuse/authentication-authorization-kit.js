...
exports.MFAMethod = exports.OAuth2GrantType = void 0;
exports.generateRS256Token = generateRS256Token;
exports.verifyRS256Token = verifyRS256Token;
exports.generateHS256Token = generateHS256Token;
exports.verifyHS256Token = verifyHS256Token;
exports.generateES256Token = generateES256Token;
exports.verifyES256Token = verifyES256Token;
exports.generateOAuth2AuthorizationCode = generateOAuth2AuthorizationCode;
exports.verifyPKCEChallenge = verifyPKCEChallenge;
exports.generatePKCEChallenge = generatePKCEChallenge;
exports.generateClientCredentialsToken = generateClientCredentialsToken;
exports.generateDeviceCode = generateDeviceCode;
exports.validateDeviceCodeAuthorization = validateDeviceCodeAuthorization;
...
exports.verifyWebAuthnAssertion = verifyWebAuthnAssertion;
exports.registerBiometricCredential = registerBiometricCredential;
exports.validateBiometricCounter = validateBiometricCounter;
/**
 * File: /reuse/authentication-authorization-kit.ts
...
(function (MFAMethod) {
    ...
    MFAMethod["BACKUP_CODE"] = "backup_code";
 ...
 * );
 */
function generateRS256Token(payload, privateKey, options) {
    const header = {
        alg: 'RS256',
        typ: 'JWT',
    };
 ...
 * });
 */
function verifyRS256Token(token, publicKey, options) {
 ...
 * );
 */
function generateHS256Token(payload, secret, options) {
    const header = {
        alg: 'HS256',
        typ: 'JWT',
    };
 ...
 * );
 */
function generateES256Token(payload, privateKey, options) {
    const header = {
        alg: 'ES256',
        typ: 'JWT',
    };
 ...
 * @throws UnauthorizedException if token is invalid
 */
function verifyES256Token(token, publicKey, options) {
    const parts = token.split('.');
    if (parts.length !== 3) {
        throw new common_1.UnauthorizedException('Invalid token format');
    }
    ...
    verify.update(signatureInput);
    const isValid = verify.verify({ key: publicKey, dsaEncoding: 'der' }, derSignature, 'base64');
    if (!isValid) {
        throw new common_1.UnauthorizedException('Invalid token signature');
    }
    const payload = JSON.parse(base64UrlDecode(encodedPayload));
 ...
 * }, 'user-456');
 */
function generateOAuth2AuthorizationCode(request, userId) {
 ...
 */
function verifyPKCEChallenge(codeVerifier, codeChallenge, method) {
    if (method === 'plain') {
        if (codeVerifier !== codeChallenge) {
            throw new common_1.UnauthorizedException('Invalid PKCE code verifier');
        }
        return true;
    }
    if (method === 'S256') {
    ...
    throw new common_1.UnauthorizedException('Unsupported PKCE method');
}
/**
 ...
 * );
 */
async function generateClientCredentialsToken(clientId, clientSecret, scope) {
 ...
 */
function generateDeviceCode(clientId, scope) {
 ...
 */
async function validateDeviceCodeAuthorization(deviceCode, userCode) {
 ...
 */
async function validateAPIKey(apiKey, ipAddress) {
    ...
    if (config.ipWhitelist && ipAddress) {
        if (!config.ipWhitelist.includes(ipAddress)) {
            throw new common_1.UnauthorizedException('IP address not whitelisted');
        }
    }
    return config;
}
 ...
 */
function rotateAPIKey(oldKey, gracePeriod = 3600) {
 ...
 */
function generateTOTPSecret(userId, issuer = 'White Cross') {
 ...
 */
function generateMFAChallenge(userId, method, target, length = 6) {
 ...
 */
function verifyMFAChallenge(challengeId, code, challenge) {
    if (challenge.challengeId !== challengeId) {
        throw new common_1.UnauthorizedException('Invalid challenge ID');
    }
    if (challenge.expiresAt < new Date()) {
        throw new common_1.UnauthorizedException('Challenge has expired');
    }
 ...
 */
function checkRole(userRole, requiredRoles, roleHierarchy) {
 ...
 * );
 */
function evaluatePermissionConditions(permission, context) {
 ...
 */
function getRolePermissions(role, allRoles) {
...
function calculatePasswordStrength(password) {
    ...
    if (password.length >= 16)
        ...
        feedback.push('Use a longer password (16+ characters)');
    // Character variety
 ...
 */
function trackFailedLogin(identifier, config, tracker) {
    const now = new Date();
    if (!tracker) {
        tracker = {
            identifier,
 ...
 * }
 */
function isAccountLocked(tracker) {
 ...
 */
function resetFailedLoginAttempts(identifier) {
    return {
        identifier,
        attempts: 0,
 ...
 */
function detectSuspiciousActivity(attempts, threshold = 10) {
    ...
    const recentAttempts = attempts.filter((a) => Date.now() - a.timestamp.getTime() < timeWindow);
 ...
 */
function createCORSConfig(allowedOrigins, credentials = true) {
    return {
 ...
 * }
 */
function detectTokenReuse(tokenId, rotationHistory) {
 ...
 */
function validateRefreshToken(token, policy, rotationCount) {
    if (rotationCount >= policy.maxRotations) {
        throw new common_1.UnauthorizedException('Token rotation limit exceeded. Please re-authenticate.');
    }
    return true;
}
// ============================================================================
 ...
 */
function verifyWebAuthnAssertion(assertion, publicKey, challenge) {
 ...
 */
function registerBiometricCredential(userId, credentialId, publicKey, counter, deviceType) {
    return {
        userId,
        credentialId,
        publicKey,
 ...
 */
function validateBiometricCounter(newCounter, storedCounter) {
    if (newCounter <= storedCounter) {
        throw new common_1.UnauthorizedException('Invalid signature counter - possible replay attack');
    }
    return true;
}
// ============================================================================
...
function validateJWTClaims(payload, options) {
    ...
    if (options?.maxAge && payload.iat) {
        ...
        if (now - payload.iat > maxAgeSeconds) {
            throw new common_1.UnauthorizedException('Token is too old');
        }
    }
}
/**
 ...
 */
function trimLeadingZeros(buffer) {
    let i = 0;
 ...
 */
function generateUserFriendlyCode(length) {
...
//# sourceMappingURL=authentication-authorization-kit.js.map