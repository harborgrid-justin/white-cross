 ...
 */
export type ExtractGeometry<T> = T extends GeoJSONFeature<infer _P>
  ? T['geometry']
  : never;
 ...
 */
export function coordinateToUTM(coord: Coordinate): UTMCoordinate {
 ...
 * });
 */
export function utmToCoordinate(utm: UTMCoordinate): Coordinate {
 ...
 */
export function coordinateToWebMercator(coord: Coordinate): [number, number] {
 ...
 */
export function webMercatorToCoordinate(x: number, y: number): Coordinate {
 ...
 * );
 */
export function calculateRhumbLineDistance(point1: Coordinate, point2: Coordinate): number {
  const lat1 = degreesToRadians(point1.lat);
  const lat2 = degreesToRadians(point2.lat);
 ...
 * ); // ~256° (WSW)
 */
export function calculateInitialBearing(point1: Coordinate, point2: Coordinate): number {
  const lat1 = degreesToRadians(point1.lat);
  const lat2 = degreesToRadians(point2.lat);
  const deltaLng = degreesToRadians(point2.lng - point1.lng);
 ...
 */
export function calculateFinalBearing(point1: Coordinate, point2: Coordinate): number {
 ...
 */
export function bearingToCompassDirection(bearing: number, precise: boolean = false): string {
 ...
 */
export function generateCircularBuffer(
  center: Coordinate,
  radius: number,
  segments: number = 32
): Coordinate[] {
 ...
 * ]);
 */
export function optimizeRouteGreedy(
  ...
  while (improved && iteration < maxIterations) {
    ...
    for (let i = 1; i < bestRoute.length - 2; i++) {
      for (let j = i + 1; j < bestRoute.length - 1; j++) {
        ...
        const currentDist =
          calculateHaversineDistance(bestRoute[i - 1], bestRoute[i]) +
          calculateHaversineDistance(bestRoute[j], bestRoute[j + 1]);

        // Calculate distance if we reverse the segment
        const newDist =
          calculateHaversineDistance(bestRoute[i - 1], bestRoute[j]) +
 ...
 * ]);
 */
export function calculateDistanceMatrix(points: Coordinate[]): number[][] {
 ...
 * // Returns: "dr5regw3p"
 */
export function encodeGeohash(coord: Coordinate, precision: GeohashPrecision = 9): string {
  let { lat, lng } = coord;
  let geohash = '';
  let even = true;
  let bit = 0;
 ...
 */
export function decodeGeohash(geohash: string): BoundingBox {
  let even = true;
  let latRange = [-90, 90];
  let lngRange = [-180, 180];
 ...
 */
export function getGeohashNeighbors(geohash: string): Record<string, string> {
  ...
  const precision = geohash.length as GeohashPrecision;
 ...
 * });
 */
export function validateGeoJSONGeometry(geometry: unknown): geometry is GeoJSONGeometry {
  ...
  switch (geo.type) {
    ...
    case 'LineString':
      return Array.isArray(geo.coordinates) &&
             geo.coordinates.length >= 2 &&
             geo.coordinates.every((coord: unknown) =>
 ...
 */
export function coordinateToGeoJSONPoint(coord: Coordinate): GeoJSONPoint {
  return {
    type: 'Point',
 ...
 */
export function coordinatesToGeoJSONLineString(coords: Coordinate[]): GeoJSONLineString {
  return {
    type: 'LineString',
 ...
 */
export function coordinatesToGeoJSONPolygon(coords: Coordinate[]): GeoJSONPolygon {
  const ring = [...coords];
 ...
 */
export function simplifyLine(line: Coordinate[], tolerance: number): Coordinate[] {
  if (line.length <= 2) {
    return [...line];
  }
  ...
  const douglasPeucker = (points: Coordinate[], epsilon: number): Coordinate[] => {
    let dmax = 0;
    let index = 0;
...
export function kMeansClustering(
  ...
  const centroids: Coordinate[] = [points[Math.floor(Math.random() * points.length)]];
  ...
  while (changed && iteration < maxIterations) {
    ...
    const clusters: Coordinate[][] = Array(k).fill(0).map(() => []);
  ...
  const clusterAssignments: Coordinate[][] = Array(k).fill(0).map(() => []);
 ...
 */
export function isGeoJSONFeature<P = Record<string, unknown>>(
  obj: unknown
): obj is GeoJSONFeature<P> {
 ...
 */
export function stDistance(point1: Coordinate, point2: Coordinate): number {
  return calculateHaversineDistance(point1, point2);
}
 ...
 */
export function stDWithin(point1: Coordinate, point2: Coordinate, distance: number): boolean {
 ...
 */
export function stContains(polygon: Coordinate[], point: Coordinate): boolean {
