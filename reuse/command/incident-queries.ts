...
export interface IncidentPattern {
  ...
  timePatterns: { hour: number; dayOfWeek: number; count: number }[];
...
export async function getActiveIncidents(
  IncidentReport: ModelStatic<IncidentReportModel>,
  options: {
    limit?: number;
    offset?: number;
  ...
  return IncidentReport.findAll({
    where: {
      status: {
        [Op.in]: ['PENDING_REVIEW', 'UNDER_INVESTIGATION', 'REQUIRES_ACTION'],
      },
      deletedAt: null,
    },
    include,
 ...
 * ```
 */
export async function getIncidentsPendingParentNotification(
  IncidentReport: ModelStatic<IncidentReportModel>,
  complianceHours: number = 4,
): Promise<IncidentReportModel[]> {
  ...
  return IncidentReport.findAll({
    where: {
      ...
      occurredAt: {
        [Op.lte]: complianceCutoff,
      },
      severity: {
        [Op.in]: ['MEDIUM', 'HIGH', 'CRITICAL'],
      },
      deletedAt: null,
    },
    include: [
 ...
 * ```
 */
export async function getIncidentsRequiringFollowUp(
  IncidentReport: ModelStatic<IncidentReportModel>,
  options: {
    daysOverdue?: number;
    assignedTo?: string;
  ...
  if (daysOverdue) {
    ...
    where.occurredAt = { [Op.lte]: overdueDate };
  }

  const followUpInclude: any = {
...
): Promise<{ rows: IncidentReportModel[]; count: number }> {
...
export async function getIncidentsByType(
  ...
  if (minSeverity) {
    ...
    where.severity = {
      [Op.in]: severityOrder.slice(minIndex),
    };
  }
 ...
 * ```
 */
export async function getIncidentsWithInsuranceClaims(
  IncidentReport: ModelStatic<IncidentReportModel>,
  options: {
 ...
 * ```
 */
export async function getIncidentsWithEvidence(
  IncidentReport: ModelStatic<IncidentReportModel>,
  options: {
  ...
  if (evidenceType === 'photos') {
    where[Op.and] = literal(`array_length("evidencePhotos", 1) >= ${minCount}`);
  } else if (evidenceType === 'videos') {
    where[Op.and] = literal(`array_length("evidenceVideos", 1) >= ${minCount}`);
  } else if (evidenceType === 'attachments') {
    where[Op.and] = literal(`array_length("attachments", 1) >= ${minCount}`);
  } else {
    where[Op.or] = [
      literal(`array_length("evidencePhotos", 1) >= ${minCount}`),
      literal(`array_length("evidenceVideos", 1) >= ${minCount}`),
      literal(`array_length("attachments", 1) >= ${minCount}`),
    ];
  }
 ...
 * ```
 */
export async function getIncidentsByComplianceStatus(
  IncidentReport: ModelStatic<IncidentReportModel>,
 ...
 * ```
 */
export async function searchIncidentsAdvanced(
  IncidentReport: ModelStatic<IncidentReportModel>,
  ...
  if (filters.studentIds && filters.studentIds.length > 0) {
    where.studentId = { [Op.in]: filters.studentIds };
  }

  if (filters.locations && filters.locations.length > 0) {
    where.location = {
      [Op.or]: filters.locations.map(loc => ({ [Op.iLike]: `%${loc}%` })),
    };
  }
  ...
  if (filters.complianceStatus && filters.complianceStatus.length > 0) {
    where.legalComplianceStatus = { [Op.in]: filters.complianceStatus };
  }

  if (filters.insuranceClaimStatus && filters.insuranceClaimStatus.length > 0) {
    where.insuranceClaimStatus = { [Op.in]: filters.insuranceClaimStatus };
  }

  if (filters.searchTerm) {
    where[Op.or] = [
      { description: { [Op.iLike]: `%${filters.searchTerm}%` } },
      { location: { [Op.iLike]: `%${filters.searchTerm}%` } },
      { actionsTaken: { [Op.iLike]: `%${filters.searchTerm}%` } },
      { followUpNotes: { [Op.iLike]: `%${filters.searchTerm}%` } },
    ];
  }

  if (filters.hasEvidence !== undefined && filters.hasEvidence) {
    where[Op.or] = [
      literal(`array_length("evidencePhotos", 1) > 0`),
      literal(`array_length("evidenceVideos", 1) > 0`),
      literal(`array_length("attachments", 1) > 0`),
    ];
  }
 ...
 * ```
 */
export async function getIncidentHistoryByDateRange(
  IncidentReport: ModelStatic<IncidentReportModel>,
  startDate: Date,
  endDate: Date,
  options: {
 ...
 * ```
 */
export async function getClosedIncidentsForArchive(
  IncidentReport: ModelStatic<IncidentReportModel>,
  options: {
 ...
 * ```
 */
export async function searchIncidentsByWitness(
  IncidentReport: ModelStatic<IncidentReportModel>,
  witnessName: string,
  options: {
    daysBack?: number;
...
export async function getSimilarIncidents(
  ...
  if (matchLocation) {
    where.location = {
      [Op.iLike]: `%${referenceIncident.location}%`,
    };
  }
 ...
 * ```
 */
export async function getIncidentsByActionKeywords(
  IncidentReport: ModelStatic<IncidentReportModel>,
  keywords: string[],
  options: {
    daysBack?: number;
 ...
 * ```
 */
export async function getRecentlyModifiedIncidents(
  IncidentReport: ModelStatic<IncidentReportModel>,
  since: Date,
  options: {
 ...
 * ```
 */
export async function searchIncidentsByDescription(
  IncidentReport: ModelStatic<IncidentReportModel>,
  searchQuery: string,
  options: {
    limit?: number;
 ...
 * ```
 */
export async function getDraftIncidents(
  IncidentReport: ModelStatic<IncidentReportModel>,
  options: {
 ...
 * ```
 */
export async function analyzeIncidentTrendsByTimePeriod(
  sequelize: Sequelize,
  options: {
    startDate: Date;
    endDate: Date;
 ...
 * ```
 */
export async function identifyRecurringIncidentPatterns(
  sequelize: Sequelize,
  options: {
    minIncidents?: number;
    daysWindow?: number;
 ...
 * ```
 */
export async function analyzeIncidentHotspotsByLocation(
  sequelize: Sequelize,
  options: {
    daysBack?: number;
    minIncidents?: number;
 ...
 * ```
 */
export async function detectIncidentClusters(
  sequelize: Sequelize,
  options: {
 ...
 * ```
 */
export async function analyzeIncidentTimeOfDayPatterns(
  sequelize: Sequelize,
  options: {
    daysBack?: number;
    transaction?: Transaction;
 ...
 * ```
 */
export async function analyzeIncidentTypeDistribution(
  sequelize: Sequelize,
  options: {
    startDate: Date;
    endDate: Date;
 ...
 * ```
 */
export async function analyzeSeasonalIncidentPatterns(
  sequelize: Sequelize,
  options: {
    years?: number;
    transaction?: Transaction;
 ...
 * ```
 */
export async function analyzeIncidentResolutionPatterns(
  sequelize: Sequelize,
  options: {
    daysBack?: number;
    transaction?: Transaction;
 ...
 * ```
 */
export async function detectAnomalousIncidentPatterns(
  sequelize: Sequelize,
  options: {
 ...
 * ```
 */
export async function buildComplexIncidentFilter(
  IncidentReport: ModelStatic<IncidentReportModel>,
  criteria: {
    severity?: string[];
    types?: string[];
  ...
  if (complianceIssues) {
    where.legalComplianceStatus = {
      [Op.in]: ['NON_COMPLIANT', 'UNDER_REVIEW'],
    };
  }
  ...
  if (locations && locations.length > 0) {
    where.location = {
      [Op.or]: locations.map(loc => ({ [Op.iLike]: `%${loc}%` })),
    };
  }
 ...
 * ```
 */
export async function getIncidentsWithRelationshipFilters(
  IncidentReport: ModelStatic<IncidentReportModel>,
  relationFilters: {
    studentGrade?: string[];
    schoolId?: string;
  ...
  if (studentGrade || schoolId) {
    ...
    if (studentGrade && studentGrade.length > 0) {
      studentWhere.grade = { [Op.in]: studentGrade };
    }
    if (schoolId) {
 ...
 * ```
 */
export async function aggregateIncidentsByDimensions(
  sequelize: Sequelize,
  dimensions: {
 ...
 * ```
 */
export async function getIncidentsWithAdvancedSorting(
  IncidentReport: ModelStatic<IncidentReportModel>,
  sortOptions: {
    primarySort: string;
  ...
  const order: Order = [[primarySort, direction]];
 ...
 * ```
 */
export async function getIncidentSummaryStatistics(
  sequelize: Sequelize,
  options: {
    dateRange?: { start: Date; end: Date };
    transaction?: Transaction;
 ...
 * ```
 */
export async function exportIncidentsForReporting(
  IncidentReport: ModelStatic<IncidentReportModel>,
  exportOptions: {
    dateRange?: { start: Date; end: Date };
    fields?: string[];
  ...
  if (includeRelations) {
    include.push(
      { association: 'student', attributes: ['id', 'firstName', 'lastName', 'studentNumber', 'grade'] },
      { association: 'reporter', attributes: ['id', 'firstName', 'lastName', 'email'] },
    );
  }
 ...
 * ```
 */
export async function getIncidentsRequiringComplianceReview(
  IncidentReport: ModelStatic<IncidentReportModel>,
  options: {
    daysOverdue?: number;
    severity?: string[];
 ...
 * ```
 */
export async function searchIncidentsAcrossSchools(
  sequelize: Sequelize,
  searchCriteria: {
    schoolIds: string[];
    type?: string;
  ...
  const replacements: any = { schoolIds };
 ...
 * ```
 */
export async function getPaginatedIncidentsWithFilters(
  IncidentReport: ModelStatic<IncidentReportModel>,
  options: {
    page?: number;
    pageSize?: number;
...
): Promise<{ rows: IncidentReportModel[]; count: number; page: number; totalPages: number }> {
 ...
 * ```
 */
export async function analyzeIncidentOutcomes(
  sequelize: Sequelize,
  options: {
    daysBack?: number;
    transaction?: Transaction;
 ...
 * ```
 */
export async function calculateIncidentResponseTimeMetrics(
  sequelize: Sequelize,
  options: {
    daysBack?: number;
    transaction?: Transaction;
 ...
 * ```
 */
export async function analyzeFollowUpCompletionRates(
  sequelize: Sequelize,
  options: {
    daysBack?: number;
    transaction?: Transaction;
 ...
 * ```
 */
export async function getIncidentsWithDelayedNotification(
  IncidentReport: ModelStatic<IncidentReportModel>,
  ...
  return IncidentReport.findAll({
    where: {
      ...
      [Op.and]: literal(
        `EXTRACT(EPOCH FROM ("parentNotifiedAt" - "occurredAt")) / 60 > ${thresholdMinutes}`,
      ),
      deletedAt: null,
    },
    include: [
    ...
    order: [
      [literal(`EXTRACT(EPOCH FROM ("parentNotifiedAt" - "occurredAt"))`), 'DESC'],
    ],
  });
}
 ...
 * ```
 */
export async function analyzeIncidentResourceConsumption(
  sequelize: Sequelize,
  options: {
    daysBack?: number;
    transaction?: Transaction;
 ...
 * ```
 */
export async function generateIncidentRecurrencePrediction(
  sequelize: Sequelize,
  options: {
    lookbackDays?: number;
 ...
 * ```
 */
export async function analyzeComplianceViolationPatterns(
  sequelize: Sequelize,
  options: {
    daysBack?: number;
    transaction?: Transaction;
 ...
 * ```
 */
export async function getIncidentDashboardMetrics(
  sequelize: Sequelize,
  options: {
 ...
 * ```
 */
export async function getComprehensiveIncidentReport(
  IncidentReport: ModelStatic<IncidentReportModel>,
  incidentId: string,
): Promise<IncidentReportModel | null> {
