 ...
 */
export type Incident = MedicalIncident | FireIncident | LawEnforcementIncident | HazmatIncident | NaturalDisasterIncident | TechnicalRescueIncident;
/**
 ...
 */
export interface StatusTransitionEvent<TFrom extends IncidentStatus, TTo extends StatusTransitions[TFrom]> {
 ...
 */
export type ValidTransition<T extends IncidentStatus> = StatusTransitions[T] extends never ? never : StatusTransitions[T];
/**
...
export type DeepReadonly<T> = {
    readonly [P in keyof T]: T[P] extends object ? T[P] extends ReadonlyArray<infer U> ? ReadonlyArray<DeepReadonly<U>> : DeepReadonly<T[P]> : T[P];
};
/**
 * Conditional type for extracting incidents by type.
 */
export type IncidentByType<T extends IncidentType> = Extract<Incident, {
    type: T;
}>;
/**
 ...
 */
export type IncidentTypePredicate<T extends IncidentType> = (incident: Incident) => incident is IncidentByType<T>;
/**
 ...
 */
export type FilterByStatus<T extends Incident, S extends IncidentStatus> = T extends unknown ? T['status'] extends S ? T : never : never;
export {};
//# sourceMappingURL=incident-management-types.d.ts.map