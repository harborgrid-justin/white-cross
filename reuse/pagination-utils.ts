 ...
 * ```
 */
export const calculatePageFromOffset = (offset: number, limit: number): number => {
 ...
 * ```
 */
export const createCursorFromItem = (item: any): string => {
 ...
 * ```
 */
export const parseCursor = (cursor: string): { id: string; timestamp?: string } => {
...
): { decodedCursor?: string; limit: number; direction: 'forward' | 'backward' } => {
  return {
    decodedCursor: params.cursor ? decodeCursor(params.cursor) : undefined,
    limit: params.limit,
 ...
 * ```
 */
export const hasMoreCursorResults = (
  fetchedCount: number,
 ...
 * ```
 */
export const createKeysetPagination = (params: KeysetPaginationParams) => {
 ...
 * ```
 */
export const buildKeysetWhereClause = (
  sortField: string,
  lastValue: any,
  lastId?: string | number,
  operator: string = '>',
 ...
 * ```
 */
export const extractKeysetValues = (
  lastItem: any,
  sortField: string,
): { lastId: string | number; lastValue: any } => {
  return {
    lastId: lastItem.id,
 ...
 * ```
 */
export const sanitizeLimit = (
  limit: number | string | undefined,
  defaultLimit: number = 20,
  maxLimit: number = 100,
): number => {
  const parsed = typeof limit === 'string' ? parseInt(limit, 10) : limit;
  if (!parsed || parsed < 1) return defaultLimit;
...
export const createPaginationMetadata = (
  ...
  return {
    ...
    totalItems,
    totalPages,
    hasNextPage: hasNextPage(currentPage, totalItems, pageSize),
...
export const generatePaginationLinks = (
  ...
  const createLink = (page: number): string => {
 ...
 * ```
 */
export const generateCursorLinks = (
  baseUrl: string,
...
export const parsePaginationQuery = (
  ...
  const page = sanitizePageNumber(query.page, defaults?.page);
  const limit = sanitizeLimit(query.limit, defaults?.limit, defaults?.maxLimit);
...
export const parseSortQuery = (
  ...
  return sorts.map((sort) => {
    if (sort.startsWith('-')) {
      return { field: sort.slice(1), order: 'DESC' as const };
    }
    if (sort.includes(':')) {
    ...
    return { field: sort, order: 'ASC' as const };
  });
};
 ...
 * ```
 */
export const convertSortToSequelizeOrder = (sortCriteria: SortCriteria[]): any[] => {
 ...
 * ```
 */
export const createSequelizePaginationOptions = (
 ...
 * ```
 */
export const formatSequelizeResult = <T>(
  sequelizeResult: { rows: T[]; count: number },
  page: number,
  limit: number,
): PaginatedResponse<T> => {
 ...
 * ```
 */
export const shouldSkipTotalCount = (
  page: number,
 ...
 * ```
 */
export const estimateTotalCount = (
  currentPage: number,
  limit: number,
  fetchedCount: number,
): number => {
 ...
 * ```
 */
export const formatInfiniteScrollResponse = <T>(
  data: T[],
  page: number,
  limit: number,
  totalItems?: number,
 ...
 * ```
 */
export const formatCursorResponse = <T extends { id: any }>(
  data: T[],
  limit: number,
  baseUrl?: string,
  ...
  const nextCursor = hasMore && data.length > 0
    ...
    : undefined;
  const previousCursor = data.length > 0 ? encodeCursor(data[0].id) : undefined;
...
export const validatePaginationParams = (
  ...
  constraints?: { maxLimit?: number; minLimit?: number; maxPage?: number },
  ...
  const sanitized: PaginationParams = {
    ...
    limit: sanitizeLimit(limit, 20, maxLimit),
  };
  sanitized.offset = calculateOffset(sanitized.page!, sanitized.limit!);
 ...
 * ```
 */
export const createGraphQLConnection = <T extends { id: any }>(
  nodes: T[],
 ...
 * ```
 */
export const parseGraphQLPaginationArgs = (
  args: GraphQLPaginationArgs,
): { limit: number; cursor?: string; direction: 'forward' | 'backward' } => {
  if (args.first !== undefined) {
    return {
      limit: args.first,
      cursor: args.after ? decodeCursor(args.after) : undefined,
      direction: 'forward',
    };
  }
  ...
  if (args.last !== undefined) {
    return {
      limit: args.last,
      cursor: args.before ? decodeCursor(args.before) : undefined,
      direction: 'backward',
    };
  }
 ...
 * ```
 */
export const getSwaggerPaginationParams = (options?: {
  defaultPage?: number;
  defaultLimit?: number;
  maxLimit?: number;
}): any[] => {
 ...
 * ```
 */
export const getSwaggerPaginatedResponseSchema = (itemSchemaRef: string): any => {
  return {
    type: 'object',
    properties: {
      data: {
...
export default {
  ...
  hasMoreCursorResults,

  // Keyset pagination
  createKeysetPagination,
  ...
  // Total count optimization
  shouldSkipTotalCount,
  estimateTotalCount,

  // Infinite scroll
  formatInfiniteScrollResponse,
  ...
  parseGraphQLPaginationArgs,

  // Swagger/OpenAPI
  getSwaggerPaginationParams,
  getSwaggerPaginatedResponseSchema,
};
