...
export interface RoutingOperation {
  ...
  parallelOperations?: string[]; // Operation IDs that can run in parallel
  prerequisites?: string[]; // Operation IDs that must complete first
  alternativeWorkCenters?: string[]; // Alternative work center IDs
  instructions?: string;
...
export interface RoutingConfigSchema {
  routing: {
    ...
    capacityThreshold: number; // 0-1 (80% = 0.8)
  };
  workCenter: {
 ...
 * ```
 */
export function createRouting(config: RoutingBuilderConfig): Routing {
...
export function addOperation(
  routing: Routing,
  operation: Partial<RoutingOperation>
): Routing {
  const operationId = crypto.randomUUID();
  const sequence = routing.operations.length + 1;

  const newOperation: RoutingOperation = {
    operationId,
 ...
 * ```
 */
export function removeOperation(routing: Routing, operationId: string): Routing {
  const updatedRouting = {
    ...routing,
 ...
 * ```
 */
export function activateRouting(routing: Routing, approvedBy: string): Routing {
 ...
 * ```
 */
export function recalculateRouting(routing: Routing): Routing {
  const totalSetupTime = routing.operations.reduce((sum, op) => sum + op.setupTime, 0);
  const totalRunTime = routing.operations.reduce(
    (sum, op) => sum + op.runTime * (routing.lotSize || 1),
    0
  );
 ...
 * ```
 */
export function addOperationDependency(
  routing: Routing,
  operationId: string,
 ...
 * ```
 */
export function removeOperationDependency(
  routing: Routing,
  operationId: string,
  prerequisiteId: string
): Routing {
 ...
 * ```
 */
export function setParallelOperations(
  routing: Routing,
  operationIds: string[]
): Routing {
  return {
 ...
 * ```
 */
export function buildDependencyGraph(routing: Routing): OperationDependency[] {
 ...
 * ```
 */
export function findCriticalPath(routing: Routing): {
  operations: string[];
  totalTime: number;
  bottleneck?: string;
} {
  ...
  const operationMap = new Map(
    routing.operations.map(op => [op.operationId, op])
  );
 ...
 * ```
 */
export function validateOperationSequence(routing: Routing): {
  valid: boolean;
  errors: string[];
  warnings: string[];
} {
    ...
    if (op.prerequisites) {
      for (const prereqId of op.prerequisites) {
        if (!operationIds.has(prereqId)) {
          errors.push(
            `Operation ${op.operationNumber}: prerequisite ${prereqId} not found`
          );
        } else {
          const prereqOp = routing.operations.find(o => o.operationId === prereqId);
          if (prereqOp && prereqOp.sequence >= op.sequence) {
            warnings.push(
              `Operation ${op.operationNumber}: prerequisite ${prereqOp.operationNumber} comes after in sequence`
            );
          }
        }
      }
    ...
    if (op.parallelOperations) {
      for (const parallelId of op.parallelOperations) {
        if (!operationIds.has(parallelId)) {
          errors.push(
            `Operation ${op.operationNumber}: parallel operation ${parallelId} not found`
          );
        }
      }
    }
 ...
 * ```
 */
export function autoSequenceOperations(routing: Routing): Routing {
  const graph = buildDependencyGraph(routing);
  const sequenced: RoutingOperation[] = [];
  const processed = new Set<string>();
  const operationMap = new Map(
    routing.operations.map(op => [op.operationId, op])
  );

  function canProcess(opId: string): boolean {
    const deps = graph.find(d => d.operationId === opId);
    if (!deps || !deps.dependsOn.length) return true;
  ...
  while (sequenced.length < routing.operations.length) {
    const candidates = routing.operations.filter(
      op => !processed.has(op.operationId) && canProcess(op.operationId)
    );
 ...
 * ```
 */
export function assignWorkCenter(
  routing: Routing,
  operationId: string,
  workCenterId: string,
  workCenter: WorkCenter
 ...
 * ```
 */
export function addAlternativeWorkCenter(
  routing: Routing,
  operationId: string,
 ...
 * ```
 */
export function createWorkCenterAssignment(
  operationId: string,
  workCenter: WorkCenter,
 ...
 * ```
 */
export function findAvailableWorkCenters(
  workCenters: WorkCenter[],
  operationType: OperationType,
  scheduledTime: Date
): WorkCenter[] {
  // Map operation types to work center types
  const typeMapping: Record<OperationType, WorkCenterType[]> = {
 ...
 * ```
 */
export function analyzeCapacityConstraints(
  routing: Routing,
  workCenters: WorkCenter[],
  quantity: number
): CapacityAnalysis[] {
  ...
  for (const [workCenterId, required] of Object.entries(capacityRequired)) {
    ...
    if (overloaded) {
      recommendations.push(
        `Add ${Math.ceil(required - available)} additional hours or use alternative work center`
      );
    }
 ...
 * ```
 */
export function findBestWorkCenter(
  operation: RoutingOperation,
  workCenters: WorkCenter[],
  strategy: OptimizationStrategy = OptimizationStrategy.MINIMIZE_COST
): WorkCenter | null {
  const available = findAvailableWorkCenters(
    workCenters,
    operation.type,
  ...
  switch (strategy) {
    case OptimizationStrategy.MINIMIZE_COST:
      return available.reduce((best, wc) =>
        wc.costPerHour < best.costPerHour ? wc : best
      );

    case OptimizationStrategy.MINIMIZE_TIME:
      return available.reduce((best, wc) =>
        wc.efficiency > best.efficiency ? wc : best
      );
 ...
 * ```
 */
export function createRoutingVersion(
  routing: Routing,
  changeDescription: string,
  changedBy: string
): RoutingVersion {
 ...
 * ```
 */
export function rollbackRoutingVersion(
 ...
 * ```
 */
export function mergeRoutingChanges(
  baseRouting: Routing,
  alternativeRouting: Routing,
  operationIdsToMerge: string[]
): Routing {
  const alternativeOpMap = new Map(
    alternativeRouting.operations.map(op => [op.operationId, op])
  );
 ...
 * ```
 */
export function getRoutingVersionHistory(
  allRoutings: Routing[],
  routingCode: string
): Routing[] {
  return allRoutings
 ...
 * ```
 */
export function approveRouting(
  routing: Routing,
  approvedBy: string,
 ...
 * ```
 */
export function archiveRouting(routing: Routing, reason: string): Routing {
  return {
    ...routing,
 ...
 * ```
 */
export function calculateLeadTime(routing: Routing): LeadTimeBreakdown {
  const criticalPath = findCriticalPath(routing);
  const operationMap = new Map(
    routing.operations.map(op => [op.operationId, op])
  );
 ...
 * ```
 */
export function validateRouting(routing: Routing): RoutingValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];
  const recommendations: string[] = [];
  ...
  for (const op of routing.operations) {
    ...
    if (op.setupTime > 480) {
      // > 8 hours
      warnings.push(
        `Operation ${op.operationNumber} has unusually long setup time (${op.setupTime} min)`
      );
    }
  }
  ...
  const hasQualityCheck = routing.operations.some(
    op => op.type === OperationType.QUALITY_CHECK || op.qualityCheckRequired
  );
  if (!hasQualityCheck) {
  ...
  if (routing.operations.length >= 5) {
    const hasParallel = routing.operations.some(
      op => op.parallelOperations && op.parallelOperations.length > 0
    );
    if (!hasParallel) {
 ...
 * ```
 */
export function suggestRoutingImprovements(
  routing: Routing,
  workCenters: WorkCenter[]
): string[] {
  ...
  if (bottlenecks.length > 0) {
    suggestions.push(
      `${bottlenecks.length} potential bottleneck(s) identified - review critical path`
    );
  }
  ...
  if (overloaded.length > 0) {
    suggestions.push(
      `${overloaded.length} work center(s) overloaded - consider alternatives or additional capacity`
    );
  }
  ...
  if (totalSetup > totalRun * 0.5) {
    suggestions.push(
      'High setup time ratio - consider setup reduction initiatives or larger lot sizes'
    );
  }
  ...
  const sequential = routing.operations.filter(
    op => !op.parallelOperations || op.parallelOperations.length === 0
  );
 ...
 */
function generateRoutingCode(itemCode: string): string {
 ...
 */
function calculateOperationCost(operation: RoutingOperation): number {
  return (
    operation.laborCost +
...
function hasCircularDependency(
  ...
  if (operationId === prerequisiteId) return true;
  if (visited.has(prerequisiteId)) return false;

  visited.add(prerequisiteId);

  const prereqOp = routing.operations.find(op => op.operationId === prerequisiteId);
  if (!prereqOp || !prereqOp.prerequisites) return false;
 ...
 */
function optimizeForCost(routing: Routing, workCenters: WorkCenter[]): Routing {
  let optimized = { ...routing };

  for (const op of routing.operations) {
    const bestWC = findBestWorkCenter(
      op,
      workCenters,
 ...
 */
function optimizeForTime(routing: Routing, workCenters: WorkCenter[]): Routing {
  let optimized = { ...routing };
 ...
 */
function minimizeSetupChanges(routing: Routing): Routing {
 ...
 */
function maximizeThroughput(routing: Routing, workCenters: WorkCenter[]): Routing {
  let optimized = { ...routing };
  ...
  for (const op of routing.operations) {
    ...
    if (available.length > 0) {
      ...
      const highest = available.reduce((best, wc) =>
        wc.capacity > best.capacity ? wc : best
      );
 ...
 */
function calculateEfficiencyGain(oldRouting: Routing, newRouting: Routing): number {
...
function generateOptimizationRecommendations(
  ...
  const originalParallel = original.operations.filter(
    op => op.parallelOperations && op.parallelOperations.length > 0
  ).length;
  const optimizedParallel = optimized.operations.filter(
    op => op.parallelOperations && op.parallelOperations.length > 0
  ).length;

  if (optimizedParallel > originalParallel) {
    recommendations.push(
      `${optimizedParallel - originalParallel} additional parallel operation(s) identified`
    );
  }
 ...
 */
function validateOptimizationConstraints(
  routing: Routing,
  config: OptimizationConfig
): string[] {
  const warnings: string[] = [];
  ...
  if (config.constraints.maxLeadTime && routing.totalLeadTime > config.constraints.maxLeadTime) {
    warnings.push(
      `Lead time ${routing.totalLeadTime} exceeds constraint ${config.constraints.maxLeadTime}`
    );
  }

  if (config.constraints.maxCost && routing.totalCost > config.constraints.maxCost) {
    warnings.push(
      `Cost ${routing.totalCost.toFixed(2)} exceeds constraint ${config.constraints.maxCost.toFixed(2)}`
    );
  }
