...
exports.DemandSignal = exports.ABCClassification = exports.TrendDirection = exports.SeasonalityPattern = exports.ForecastMethod = void 0;
exports.createDemandHistory = createDemandHistory;
exports.validateDemandData = validateDemandData;
exports.cleanDemandData = cleanDemandData;
exports.normalizeDemandData = normalizeDemandData;
exports.aggregateDemandData = aggregateDemandData;
exports.splitDemandData = splitDemandData;
exports.fillMissingDemandValues = fillMissingDemandValues;
exports.calculateStatisticalSummary = calculateStatisticalSummary;
exports.detectOutliers = detectOutliers;
exports.forecastSimpleMovingAverage = forecastSimpleMovingAverage;
exports.forecastWeightedMovingAverage = forecastWeightedMovingAverage;
exports.forecastExponentialSmoothing = forecastExponentialSmoothing;
exports.forecastDoubleExponentialSmoothing = forecastDoubleExponentialSmoothing;
exports.forecastLinearRegression = forecastLinearRegression;
exports.selectBestForecastMethod = selectBestForecastMethod;
exports.createEnsembleForecast = createEnsembleForecast;
exports.optimizeForecastParameters = optimizeForecastParameters;
...
exports.identifyCyclicalPatterns = identifyCyclicalPatterns;
exports.removeSeasonality = removeSeasonality;
exports.applySeasonalAdjustment = applySeasonalAdjustment;
exports.detectStructuralBreaks = detectStructuralBreaks;
exports.generateForecastWithConfidence = generateForecastWithConfidence;
exports.generateSeasonalTrendForecast = generateSeasonalTrendForecast;
exports.generateAdaptiveForecast = generateAdaptiveForecast;
exports.generateIntermittentDemandForecast = generateIntermittentDemandForecast;
exports.applyForecastConstraints = applyForecastConstraints;
exports.generateHierarchicalForecast = generateHierarchicalForecast;
exports.generateProbabilisticForecast = generateProbabilisticForecast;
exports.generateLongTermForecast = generateLongTermForecast;
exports.generateNewProductForecast = generateNewProductForecast;
exports.calculateForecastAccuracy = calculateForecastAccuracy;
exports.applyForecastAdjustment = applyForecastAdjustment;
exports.calculateForecastBias = calculateForecastBias;
exports.detectDemandSignals = detectDemandSignals;
exports.calculateSafetyStock = calculateSafetyStock;
exports.performABCClassification = performABCClassification;
exports.recommendForecastActions = recommendForecastActions;
...
(function (ForecastMethod) {
    ForecastMethod["SIMPLE_MOVING_AVERAGE"] = "SIMPLE_MOVING_AVERAGE";
    ForecastMethod["WEIGHTED_MOVING_AVERAGE"] = "WEIGHTED_MOVING_AVERAGE";
    ForecastMethod["EXPONENTIAL_SMOOTHING"] = "EXPONENTIAL_SMOOTHING";
    ForecastMethod["DOUBLE_EXPONENTIAL_SMOOTHING"] = "DOUBLE_EXPONENTIAL_SMOOTHING";
    ForecastMethod["TRIPLE_EXPONENTIAL_SMOOTHING"] = "TRIPLE_EXPONENTIAL_SMOOTHING";
 ...
 */
var SeasonalityPattern;
(function (SeasonalityPattern) {
    SeasonalityPattern["NONE"] = "NONE";
    SeasonalityPattern["WEEKLY"] = "WEEKLY";
    SeasonalityPattern["MONTHLY"] = "MONTHLY";
    SeasonalityPattern["QUARTERLY"] = "QUARTERLY";
    SeasonalityPattern["YEARLY"] = "YEARLY";
    SeasonalityPattern["CUSTOM"] = "CUSTOM";
})(SeasonalityPattern || (exports.SeasonalityPattern = SeasonalityPattern = {}));
/**
 * Trend direction
 */
var TrendDirection;
...
(function (DemandSignal) {
    DemandSignal["SPIKE"] = "SPIKE";
    DemandSignal["DROP"] = "DROP";
    DemandSignal["TREND_CHANGE"] = "TREND_CHANGE";
    DemandSignal["SEASONALITY_SHIFT"] = "SEASONALITY_SHIFT";
    DemandSignal["OUTLIER"] = "OUTLIER";
    DemandSignal["NORMAL"] = "NORMAL";
})(DemandSignal || (exports.DemandSignal = DemandSignal = {}));
// ============================================================================
 ...
 * ```
 */
function createDemandHistory(productId, dataPoints, locationId) {
 ...
 * ```
 */
function validateDemandData(history) {
    const issues = [];
    const warnings = [];
 ...
 * ```
 */
function cleanDemandData(history, options = {}) {
    const { removeOutliers = true, outlierThreshold = 3.0, fillMissing = true, fillMethod = 'INTERPOLATE', } = options;
 ...
 * ```
 */
function aggregateDemandData(history, aggregationLevel) {
    ...
    const aggregatedPoints = Array.from(aggregated.entries()).map(([key, bucket], index) => ({
        date: bucket.dates[0],
        period: index + 1,
 ...
 * ```
 */
function splitDemandData(history, trainingRatio = 0.8) {
 ...
 * ```
 */
function fillMissingDemandValues(dataPoints, fillMethod = 'INTERPOLATE') {
    if (dataPoints.length === 0)
        return dataPoints;
 ...
 * ```
 */
function calculateStatisticalSummary(values) {
    if (values.length === 0) {
 ...
 * ```
 */
function detectOutliers(dataPoints, threshold = 3.0) {
 ...
 * ```
 */
function forecastSimpleMovingAverage(history, periods, windowSize = 3) {
 ...
 * ```
 */
function forecastWeightedMovingAverage(history, periods, weights = [0.5, 0.3, 0.2]) {
    // Normalize weights to sum to 1
    const totalWeight = weights.reduce((sum, w) => sum + w, 0);
    const normalizedWeights = weights.map(w => w / totalWeight);
    const recentData = history.dataPoints.slice(-weights.length);
    const weightedAverage = recentData.reduce((sum, dp, index) => sum + dp.actualDemand * normalizedWeights[weights.length - 1 - index], 0);
    const forecastPeriods = [];
    const lastDate = history.dataPoints[history.dataPoints.length - 1].date;
    for (let i = 1; i <= periods; i++) {
        forecastPeriods.push({
 ...
 * ```
 */
function forecastExponentialSmoothing(history, periods, alpha = 0.3) {
    if (alpha <= 0 || alpha >= 1) {
 ...
 * ```
 */
function forecastDoubleExponentialSmoothing(history, periods, alpha = 0.3, beta = 0.1) {
 ...
 * ```
 */
function forecastLinearRegression(history, periods) {
 ...
 * ```
 */
function selectBestForecastMethod(history, methods = [
    ForecastMethod.SIMPLE_MOVING_AVERAGE,
    ForecastMethod.EXPONENTIAL_SMOOTHING,
    ForecastMethod.LINEAR_REGRESSION,
]) {
 ...
 * ```
 */
function createEnsembleForecast(history, periods, config) {
    const forecasts = [];
 ...
 * ```
 */
function optimizeForecastParameters(history, method, parameterRanges) {
    const { training, testing } = splitDemandData(history, 0.8);
        ...
        try {
            switch (method) {
                case ForecastMethod.EXPONENTIAL_SMOOTHING:
                    ...
                    break;
                case ForecastMethod.DOUBLE_EXPONENTIAL_SMOOTHING:
                    forecast = forecastDoubleExponentialSmoothing(training, testing.totalPeriods, params.alpha, params.beta);
                    break;
                default:
                    continue;
            }
 ...
 * ```
 */
function detectSeasonality(history, maxPeriod = 12) {
    const values = history.dataPoints.map(dp => dp.actualDemand);
    let bestPeriod = 0;
    let bestStrength = 0;
    ...
    if (detected) {
        if (bestPeriod === 7)
            pattern = SeasonalityPattern.WEEKLY;
        else if (bestPeriod === 12)
            pattern = SeasonalityPattern.MONTHLY;
        else if (bestPeriod === 4)
            pattern = SeasonalityPattern.QUARTERLY;
        else
            pattern = SeasonalityPattern.CUSTOM;
    }
 ...
 * ```
 */
function decomposeTimeSeries(history, seasonalPeriod) {
    const values = history.dataPoints.map(dp => dp.actualDemand);
    const n = values.length;
 ...
 * ```
 */
function calculateSeasonalIndices(values, seasonalPeriod) {
    const seasonalSums = Array(seasonalPeriod).fill(0);
    const seasonalCounts = Array(seasonalPeriod).fill(0);
    ...
    const seasonalAverages = seasonalSums.map((sum, idx) => seasonalCounts[idx] > 0 ? sum / seasonalCounts[idx] : 0);
 ...
 * ```
 */
function identifyCyclicalPatterns(history) {
    const values = history.dataPoints.map(dp => dp.actualDemand);
    const n = values.length;
    const peaks = [];
    const troughs = [];
    ...
    if (peaks.length >= 2) {
        ...
        cyclePeriod = Math.round(peakDistances.reduce((sum, d) => sum + d, 0) / peakDistances.length);
 ...
 * ```
 */
function removeSeasonality(history, seasonalPeriod) {
    const values = history.dataPoints.map(dp => dp.actualDemand);
    const seasonalIndices = calculateSeasonalIndices(values, seasonalPeriod);
 ...
 * ```
 */
function applySeasonalAdjustment(forecast, seasonalIndices) {
 ...
 * ```
 */
function detectStructuralBreaks(history) {
    const values = history.dataPoints.map(dp => dp.actualDemand);
    const breakPoints = [];
 ...
 * ```
 */
function generateForecastWithConfidence(history, config) {
    ...
    switch (config.method) {
        ...
        case ForecastMethod.LINEAR_REGRESSION:
            ...
            break;
        case ForecastMethod.DOUBLE_EXPONENTIAL_SMOOTHING:
            baseForecast = forecastDoubleExponentialSmoothing(history, config.periods, config.alpha || 0.3, config.beta || 0.1);
            break;
        default:
 ...
 * ```
 */
function generateSeasonalTrendForecast(history, periods, seasonalPeriod) {
    ...
    // Calculate seasonal indices
    const seasonalIndices = calculateSeasonalIndices(history.dataPoints.map(dp => dp.actualDemand), seasonalPeriod);
 ...
 * ```
 */
function generateAdaptiveForecast(history, periods, adaptationRate = 0.3) {
    ...
    const recentForecast = forecastExponentialSmoothing(recentHistory, periods, adaptationRate);
    const fullForecast = forecastExponentialSmoothing(history, periods, adaptationRate * 0.5);
 ...
 * ```
 */
function generateIntermittentDemandForecast(history, periods) {
    const values = history.dataPoints.map(dp => dp.actualDemand);
 ...
 * ```
 */
function applyForecastConstraints(forecast, constraints) {
 ...
 * ```
 */
function generateHierarchicalForecast(familyHistories, proportions, periods) {
    const skuForecasts = {};
 ...
 * ```
 */
function generateProbabilisticForecast(history, periods) {
    const stats = calculateStatisticalSummary(history.dataPoints.map(dp => dp.actualDemand));
 ...
 * ```
 */
function generateLongTermForecast(history, years) {
 ...
 * ```
 */
function generateNewProductForecast(similarProductHistories, periods, scaleFactor = 1.0) {
    ...
    const similarForecasts = similarProductHistories.map(history => forecastExponentialSmoothing(history, periods));
    // Average the forecasts
    const avgForecastPeriods = [];
    for (let i = 0; i < periods; i++) {
 ...
 * ```
 */
function calculateForecastAccuracy(forecast, actual) {
 ...
 * ```
 */
function applyForecastAdjustment(forecast, period, adjustedValue, reason, appliedBy) {
 ...
 * ```
 */
function calculateForecastBias(historicalForecasts, actual) {
    let totalBias = 0;
    let totalAbsBias = 0;
    let count = 0;
 ...
 * ```
 */
function detectDemandSignals(history, forecast) {
    const signals = [];
    const values = history.dataPoints.map(dp => dp.actualDemand);
    const stats = calculateStatisticalSummary(values);
 ...
 * ```
 */
function calculateSafetyStock(history, leadTime, serviceLevel = 0.95) {
    const values = history.dataPoints.map(dp => dp.actualDemand);
    const stats = calculateStatisticalSummary(values);
 ...
 * ```
 */
function performABCClassification(products) {
 ...
 * ```
 */
function recommendForecastActions(accuracy) {
    const actions = [];
    let priority = 'LOW';
 ...
 * ```
 */
function generateAccuracyReport(forecasts, actual) {
    const accuracyByMethod = new Map();
 ...
 * ```
 */
function exportForecast(forecast, format) {
    switch (format) {
        case 'CSV':
 ...
 */
function addPeriods(date, periods) {
 ...
 */
function getAggregationKey(date, level) {
 ...
 */
function interpolateValue(complete, period, sorted) {
 ...
 */
function estimateDate(sorted, period) {
    if (sorted.length < 2)
 ...
 */
function generateParameterCombinations(parameterRanges) {
...
//# sourceMappingURL=demand-forecasting-kit.js.map