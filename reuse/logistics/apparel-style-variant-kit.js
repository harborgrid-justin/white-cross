...
exports.PatternType = exports.SizeSystem = exports.Gender = exports.Season = exports.StyleStatus = void 0;
exports.createApparelStyle = createApparelStyle;
exports.generateStyleHierarchy = generateStyleHierarchy;
exports.updateStyleStatus = updateStyleStatus;
exports.addColorsToStyle = addColorsToStyle;
exports.addSizesToStyle = addSizesToStyle;
exports.cloneStyleForNewSeason = cloneStyleForNewSeason;
exports.validateStyleDefinition = validateStyleDefinition;
exports.searchStyles = searchStyles;
exports.createColorDefinition = createColorDefinition;
exports.createColorPalette = createColorPalette;
exports.determineColorFamily = determineColorFamily;
exports.findComplementaryColors = findComplementaryColors;
exports.addPatternToStyle = addPatternToStyle;
exports.generateColorCode = generateColorCode;
exports.validateColorPalette = validateColorPalette;
exports.groupColorsByFamily = groupColorsByFamily;
exports.hexToRgb = hexToRgb;
exports.createMaterialSpec = createMaterialSpec;
exports.validateMaterialComposition = validateMaterialComposition;
exports.addMaterialsToStyle = addMaterialsToStyle;
exports.generateCareInstructions = generateCareInstructions;
exports.calculateFabricCost = calculateFabricCost;
exports.findSustainableMaterialAlternatives = findSustainableMaterialAlternatives;
exports.generateMaterialCode = generateMaterialCode;
exports.compareMaterialSustainability = compareMaterialSustainability;
exports.generateStyleVariants = generateStyleVariants;
exports.generateSKU = generateSKU;
exports.parseSKU = parseSKU;
exports.generateUPC = generateUPC;
exports.validateUPC = validateUPC;
exports.filterVariants = filterVariants;
exports.updateVariantPrice = updateVariantPrice;
exports.calculateVariantMargin = calculateVariantMargin;
exports.generateSizeRun = generateSizeRun;
exports.createCollection = createCollection;
exports.addStylesToCollection = addStylesToCollection;
exports.removeStylesFromCollection = removeStylesFromCollection;
exports.calculateCollectionStats = calculateCollectionStats;
exports.generateCollectionCode = generateCollectionCode;
exports.validateCollectionForLaunch = validateCollectionForLaunch;
exports.exportCollectionToCatalog = exportCollectionToCatalog;
exports.generateCollectionLookbook = generateCollectionLookbook;
/**
 * File: /reuse/logistics/apparel-style-variant-kit.ts
...
(function (StyleStatus) {
    StyleStatus["DRAFT"] = "DRAFT";
    StyleStatus["ACTIVE"] = "ACTIVE";
    StyleStatus["SEASONAL"] = "SEASONAL";
    StyleStatus["DISCONTINUED"] = "DISCONTINUED";
    StyleStatus["ARCHIVED"] = "ARCHIVED";
    StyleStatus["PENDING_APPROVAL"] = "PENDING_APPROVAL";
})(StyleStatus || (exports.StyleStatus = StyleStatus = {}));
/**
 * Season enumeration
 */
var Season;
(function (Season) {
    ...
    Season["PRE_SPRING"] = "PRE_SPRING";
    Season["PRE_FALL"] = "PRE_FALL";
    Season["RESORT"] = "RESORT";
...
(function (SizeSystem) {
    ...
    SizeSystem["UK"] = "UK";
    SizeSystem["EU"] = "EU";
    SizeSystem["ALPHA"] = "ALPHA";
    SizeSystem["NUMERIC"] = "NUMERIC";
    SizeSystem["INCHES"] = "INCHES";
    SizeSystem["CM"] = "CM";
...
(function (PatternType) {
    ...
    PatternType["STRIPE"] = "STRIPE";
    PatternType["PLAID"] = "PLAID";
    PatternType["CHECK"] = "CHECK";
    PatternType["FLORAL"] = "FLORAL";
    PatternType["GEOMETRIC"] = "GEOMETRIC";
    PatternType["ANIMAL_PRINT"] = "ANIMAL_PRINT";
    PatternType["PAISLEY"] = "PAISLEY";
    PatternType["POLKA_DOT"] = "POLKA_DOT";
    PatternType["ABSTRACT"] = "ABSTRACT";
    PatternType["CAMOUFLAGE"] = "CAMOUFLAGE";
    PatternType["TIE_DYE"] = "TIE_DYE";
 ...
 * ```
 */
function createApparelStyle(styleData) {
 ...
 * ```
 */
function generateStyleHierarchy(division, department, category, subcategory) {
    return [
        {
            level: 1,
 ...
 * ```
 */
function updateStyleStatus(style, newStatus, updatedBy) {
    ...
    if (!validTransitions[style.status].includes(newStatus)) {
        throw new Error(`Invalid status transition from ${style.status} to ${newStatus}`);
    }
    return {
        ...style,
        status: newStatus,
 ...
 * ```
 */
function addColorsToStyle(style, colors) {
 ...
 * ```
 */
function addSizesToStyle(style, sizes) {
 ...
 * ```
 */
function cloneStyleForNewSeason(originalStyle, newSeason, newYear) {
    const newStyleId = generateStyleId();
    const newStyleNumber = generateStyleNumber(originalStyle.category, originalStyle.gender, newSeason, newYear);
    return {
        ...originalStyle,
        styleId: newStyleId,
 ...
 * ```
 */
function validateStyleDefinition(style) {
    const errors = [];
    const warnings = [];
    // Required fields
    if (!style.styleName)
    ...
    if (!style.basePrice || style.basePrice <= 0)
        errors.push('Valid base price is required');
    // Business logic validations
    if (style.colors.length === 0)
        warnings.push('No colors defined');
    if (style.sizes.length === 0)
        warnings.push('No sizes defined');
 ...
 * ```
 */
function searchStyles(styles, criteria) {
    return styles.filter(style => {
        if (criteria.gender && style.gender !== criteria.gender)
            return false;
        if (criteria.season && style.season !== criteria.season)
            return false;
        if (criteria.year && style.year !== criteria.year)
            return false;
        if (criteria.category && style.category !== criteria.category)
            return false;
        if (criteria.status && style.status !== criteria.status)
            return false;
        if (criteria.minPrice && style.basePrice < criteria.minPrice)
            return false;
        if (criteria.maxPrice && style.basePrice > criteria.maxPrice)
            return false;
        if (criteria.tags && !criteria.tags.some(tag => style.tags.includes(tag)))
            return false;
        return true;
    });
}
 ...
 * ```
 */
function createColorDefinition(colorData) {
 ...
 * ```
 */
function createColorPalette(paletteData) {
 ...
 * ```
 */
function determineColorFamily(hexValue) {
 ...
 * ```
 */
function findComplementaryColors(baseColor, availableColors) {
    const baseRgb = hexToRgb(baseColor.hexValue);
    if (!baseRgb)
        return [];
    const complementaryColors = [];
    for (const color of availableColors) {
        if (color.colorId === baseColor.colorId)
            continue;
 ...
 * ```
 */
function addPatternToStyle(style, patterns) {
 ...
 * ```
 */
function createMaterialSpec(materialData) {
 ...
 * ```
 */
function validateMaterialComposition(composition) {
 ...
 * ```
 */
function addMaterialsToStyle(style, materials) {
 ...
 * ```
 */
function generateCareInstructions(composition) {
 ...
 * ```
 */
function calculateFabricCost(material, yardagePerGarment) {
    if (!material.cost)
        return 0;
 ...
 * ```
 */
function findSustainableMaterialAlternatives(currentMaterial, availableMaterials) {
    ...
    return availableMaterials.filter(material => {
        ...
        const hasSustainableCert = material.certifications.some(cert => sustainableCertifications.some(sus => cert.includes(sus)));
 ...
 * ```
 */
function generateMaterialCode(materialName) {
 ...
 * ```
 */
function compareMaterialSustainability(material1, material2) {
 ...
 * ```
 */
function generateStyleVariants(style, config) {
    const variants = [];
 ...
 * ```
 */
function generateSKU(styleNumber, colorCode, sizeCode, config) {
    const defaultConfig = {
        includeStyle: true,
        includeColor: true,
        includeSize: true,
    ...
    if (finalConfig.includeStyle)
        parts.push(styleNumber);
    if (finalConfig.includeColor)
        parts.push(colorCode);
    if (finalConfig.includeSize)
        parts.push(sizeCode);
    if (finalConfig.suffix)
        parts.push(finalConfig.suffix);
 ...
 * ```
 */
function filterVariants(variants, criteria) {
    return variants.filter(variant => {
        if (criteria.colorCode && variant.colorCode !== criteria.colorCode)
            return false;
        if (criteria.sizeCode && variant.sizeCode !== criteria.sizeCode)
            return false;
        if (criteria.minPrice && variant.price < criteria.minPrice)
            return false;
        if (criteria.maxPrice && variant.price > criteria.maxPrice)
            return false;
        if (criteria.isActive !== undefined && variant.isActive !== criteria.isActive)
            return false;
        if (criteria.inStock && (!variant.stockLevel || variant.stockLevel <= 0))
            return false;
        return true;
    });
}
 ...
 * ```
 */
function updateVariantPrice(variant, newPrice, compareAtPrice) {
    return {
        ...variant,
        price: newPrice,
        compareAtPrice,
 ...
 * ```
 */
function calculateVariantMargin(variant) {
 ...
 * ```
 */
function generateSizeRun(matrix, totalQuantity) {
    const sizeRun = {};
    if (!matrix.ratios) {
        // Equal distribution
...
function createCollection(collectionData) {
    ...
    const collectionCode = generateCollectionCode(collectionData.collectionName || '', collectionData.season, collectionData.year);
    return {
        collectionId,
        collectionCode,
 ...
 * ```
 */
function addStylesToCollection(collection, styleIds) {
 ...
 * ```
 */
function removeStylesFromCollection(collection, styleIds) {
 ...
 * ```
 */
function calculateCollectionStats(collection, styles) {
    const collectionStyles = styles.filter(s => collection.styles.includes(s.styleId));
    const totalVariants = collectionStyles.reduce((sum, s) => sum + s.colors.length * s.sizes.length, 0);
    const prices = collectionStyles.map(s => s.basePrice);
    const averagePrice = prices.reduce((sum, p) => sum + p, 0) / prices.length || 0;
    const allColors = new Set(collectionStyles.flatMap(s => s.colors.map(c => c.colorId)));
 ...
 * ```
 */
function generateCollectionCode(collectionName, season, year) {
 ...
 * ```
 */
function validateCollectionForLaunch(collection, styles) {
    const errors = [];
    const warnings = [];
 ...
 * ```
 */
function exportCollectionToCatalog(collection, styles, variants) {
    const collectionStyles = styles.filter(s => collection.styles.includes(s.styleId));
 ...
 * ```
 */
function generateCollectionLookbook(collection, styles) {
    const collectionStyles = styles.filter(s => collection.styles.includes(s.styleId));
    // Group by category
 ...
 */
function generateStyleNumber(category, gender, season, year) {
 ...
 */
function calculateHueDifference(rgb1, rgb2) {
 ...
 */
function calculateSustainabilityScore(material) {
    let score = 0;
    // Certification score
...
//# sourceMappingURL=apparel-style-variant-kit.js.map