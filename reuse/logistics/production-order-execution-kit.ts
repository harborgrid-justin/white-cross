...
export interface SequelizeProductionOrderModel {
  // One-to-Many: ProductionOrder has many WorkOrders
  workOrders?: any[];
  getWorkOrders?: () => Promise<any[]>;
  addWorkOrder?: (workOrder: any) => Promise<void>;

  // One-to-Many: ProductionOrder has many MaterialIssues
  materialIssues?: any[];
  getMaterialIssues?: () => Promise<any[]>;

  // One-to-Many: ProductionOrder has many QualityCheckpoints
  qualityCheckpoints?: any[];
  getQualityCheckpoints?: () => Promise<any[]>;

  // BelongsTo: ProductionOrder belongs to Facility
  facility?: any;
  getFacility?: () => Promise<any>;

  // BelongsTo: ProductionOrder belongs to Item
  item?: any;
  getItem?: () => Promise<any>;

  // BelongsTo: ProductionOrder belongs to SalesOrder (optional)
  salesOrder?: any;
...
export interface SequelizeWorkOrderModel {
  // BelongsTo: WorkOrder belongs to ProductionOrder
  productionOrder?: any;
  getProductionOrder?: () => Promise<any>;

  // BelongsTo: WorkOrder belongs to WorkCenter
  workCenter?: any;
  getWorkCenter?: () => Promise<any>;

  // One-to-Many: WorkOrder has many LaborEntries
  laborEntries?: any[];
  getLaborEntries?: () => Promise<any[]>;
  addLaborEntry?: (entry: any) => Promise<void>;

  // One-to-Many: WorkOrder has many MaterialIssues
  materialIssues?: any[];
  getMaterialIssues?: () => Promise<any[]>;

  // One-to-Many: WorkOrder has many QualityCheckpoints
  qualityCheckpoints?: any[];
  getQualityCheckpoints?: () => Promise<any[]>;

  // BelongsToMany: WorkOrder belongs to many Operators through WorkOrderOperators
  operators?: any[];
 ...
 * ```
 */
export function releaseProductionOrder(order: ProductionOrder): ProductionOrder {
 ...
 * ```
 */
export function startProductionOrder(order: ProductionOrder): ProductionOrder {
 ...
 * ```
 */
export function holdProductionOrder(
 ...
 * ```
 */
export function resumeProductionOrder(order: ProductionOrder): ProductionOrder {
 ...
 * ```
 */
export function completeProductionOrder(order: ProductionOrder): ProductionOrder {
  if (order.status !== ProductionOrderStatus.IN_PROGRESS) {
    throw new Error('Can only complete in-progress orders');
  }

  // Validate all work orders are completed
  const incompleteWorkOrders = order.workOrders.filter(
    wo => wo.status !== WorkOrderStatus.COMPLETED && wo.status !== WorkOrderStatus.SKIPPED
  );
 ...
 * ```
 */
export function closeProductionOrder(order: ProductionOrder): ProductionOrder {
 ...
 * ```
 */
export function updateProductionOrderQuantity(
  order: ProductionOrder,
  newQuantity: number
): ProductionOrder {
  if (order.status === ProductionOrderStatus.COMPLETED ||
 ...
 * ```
 */
export function createMaterialIssues(
  order: ProductionOrder,
  workOrder: WorkOrder
): MaterialIssue[] {
 ...
 * ```
 */
export function backflushMaterials(
  workOrder: WorkOrder,
  order: ProductionOrder
): MaterialIssue[] {
 ...
 * ```
 */
export function generateMaterialShortageReport(order: ProductionOrder): {
  orderId: string;
  orderNumber: string;
 ...
 * ```
 */
export function createWorkOrders(order: ProductionOrder): WorkOrder[] {
 ...
 * ```
 */
export function resumeWorkOrder(workOrder: WorkOrder): WorkOrder {
 ...
 * ```
 */
export function calculateLaborHours(entries: LaborEntry[]): {
  totalHours: number;
  setupHours: number;
  runHours: number;
 ...
 * ```
 */
export function calculateLaborCost(entries: LaborEntry[]): number {
 ...
 * ```
 */
export function createQualityCheckpoints(
  workOrder: WorkOrder,
 ...
 * ```
 */
export function recordSampleInspection(
  checkpoint: QualityCheckpoint,
  sampleSize: number,
  acceptedQuantity: number,
  rejectedQuantity: number
 ...
 * ```
 */
export function validateQualityCheckpoints(workOrder: WorkOrder): {
  allPassed: boolean;
 ...
 * ```
 */
export function calculateFirstPassYield(workOrder: WorkOrder): number {
 ...
 * ```
 */
export function generateQualityReport(order: ProductionOrder): {
  orderId: string;
  orderNumber: string;
  totalCheckpoints: number;
  passedCheckpoints: number;
 ...
 * ```
 */
export function waiveQualityCheckpoint(
  checkpoint: QualityCheckpoint,
  approver: string,
  reason: string
): QualityCheckpoint {
 ...
 * ```
 */
export function reportProductionQuantity(
  workOrder: WorkOrder,
  quantity: number,
  isGood: boolean = true
): WorkOrder {
 ...
 * ```
 */
export function calculateYieldPercentage(order: ProductionOrder): number {
 ...
 * ```
 */
export function generateProductionSummary(order: ProductionOrder): {
  orderId: string;
  orderNumber: string;
  itemCode: string;
  status: ProductionOrderStatus;
 ...
 * ```
 */
export function searchProductionOrders(
  orders: ProductionOrder[],
  criteria: ProductionOrderSearchCriteria
): ProductionOrder[] {
  return orders.filter(order => {
    if (criteria.facilityId && order.facilityId !== criteria.facilityId) return false;
    if (criteria.status && !criteria.status.includes(order.status)) return false;
    if (criteria.priority && !criteria.priority.includes(order.priority)) return false;
    if (criteria.itemCode && order.itemCode !== criteria.itemCode) return false;
    if (criteria.salesOrderId && order.salesOrderId !== criteria.salesOrderId) return false;
    if (criteria.customerId && order.customerId !== criteria.customerId) return false;
    if (criteria.plannedStartFrom && order.plannedStartDate < criteria.plannedStartFrom) return false;
    if (criteria.plannedStartTo && order.plannedStartDate > criteria.plannedStartTo) return false;
    if (criteria.orderNumber && order.orderNumber !== criteria.orderNumber) return false;
 ...
 * ```
 */
export function exportProductionOrdersToCSV(orders: ProductionOrder[]): string {
  const headers = [
    'Order Number',
    'Item Code',
    'Item Description',
 ...
 * ```
 */
export function validateProductionOrderCompletion(order: ProductionOrder): {
  canComplete: boolean;
  issues: string[];
  warnings: string[];
} {
 ...
 */
export function generateProductionOrderId(): string {
 ...
 */
export function generateOrderNumber(facilityId: string): string {
  const date = new Date();
 ...
 */
export function generateWorkOrderNumber(orderNumber: string, operationNumber: number): string {
 ...
 */
export function generateMaterialIssueId(): string {
 ...
 */
export function generateLaborEntryId(): string {
