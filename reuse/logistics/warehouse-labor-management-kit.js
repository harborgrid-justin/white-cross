...
exports.IncentiveType = exports.PerformanceRating = exports.SkillLevel = exports.TaskStatus = exports.TaskCategory = void 0;
exports.createLaborStandard = createLaborStandard;
exports.calculateStandardTime = calculateStandardTime;
exports.updateLaborStandard = updateLaborStandard;
exports.validateLaborStandard = validateLaborStandard;
exports.applyEfficiencyFactor = applyEfficiencyFactor;
exports.calculateLaborCost = calculateLaborCost;
exports.compareLaborStandards = compareLaborStandards;
exports.exportLaborStandards = exportLaborStandards;
exports.createTaskAssignment = createTaskAssignment;
exports.assignBulkTasks = assignBulkTasks;
exports.reassignTask = reassignTask;
exports.prioritizeTask = prioritizeTask;
exports.balanceWorkload = balanceWorkload;
exports.matchTaskToSkill = matchTaskToSkill;
exports.scheduleTask = scheduleTask;
exports.cancelTaskAssignment = cancelTaskAssignment;
exports.startTimeEntry = startTimeEntry;
exports.stopTimeEntry = stopTimeEntry;
exports.pauseTimeEntry = pauseTimeEntry;
exports.resumeTimeEntry = resumeTimeEntry;
exports.calculateActualTime = calculateActualTime;
exports.calculateBreakTime = calculateBreakTime;
exports.validateTimeEntry = validateTimeEntry;
exports.adjustTimeEntry = adjustTimeEntry;
...
exports.calculateUnitsPerHour = calculateUnitsPerHour;
exports.calculateIdleTime = calculateIdleTime;
exports.generateProductivityReport = generateProductivityReport;
exports.compareWorkerPerformance = compareWorkerPerformance;
exports.calculateTeamProductivity = calculateTeamProductivity;
exports.createIncentivePlan = createIncentivePlan;
exports.calculateIncentivePay = calculateIncentivePay;
exports.evaluatePerformance = evaluatePerformance;
exports.trackPerformanceTrend = trackPerformanceTrend;
exports.identifyTopPerformers = identifyTopPerformers;
exports.identifyUnderperformers = identifyUnderperformers;
exports.generatePerformanceReview = generatePerformanceReview;
exports.calculateTeamIncentives = calculateTeamIncentives;
/**
 * File: /reuse/logistics/warehouse-labor-management-kit.ts
...
(function (TaskCategory) {
    TaskCategory["RECEIVING"] = "RECEIVING";
    TaskCategory["PUTAWAY"] = "PUTAWAY";
    TaskCategory["PICKING"] = "PICKING";
    TaskCategory["PACKING"] = "PACKING";
    TaskCategory["LOADING"] = "LOADING";
    TaskCategory["REPLENISHMENT"] = "REPLENISHMENT";
    TaskCategory["CYCLE_COUNT"] = "CYCLE_COUNT";
    TaskCategory["QUALITY_CHECK"] = "QUALITY_CHECK";
    TaskCategory["RETURNS_PROCESSING"] = "RETURNS_PROCESSING";
...
(function (PerformanceRating) {
    ...
    PerformanceRating["ABOVE_AVERAGE"] = "ABOVE_AVERAGE";
...
(function (IncentiveType) {
    IncentiveType["PRODUCTIVITY"] = "PRODUCTIVITY";
    IncentiveType["QUALITY"] = "QUALITY";
    IncentiveType["ATTENDANCE"] = "ATTENDANCE";
    IncentiveType["SAFETY"] = "SAFETY";
    IncentiveType["TEAM_PERFORMANCE"] = "TEAM_PERFORMANCE";
 ...
 * ```
 */
function createLaborStandard(standard) {
 ...
 * ```
 */
function calculateStandardTime(standard, quantity, complexityMultiplier = 1.0) {
 ...
 * ```
 */
function updateLaborStandard(standard, updates) {
    const updated = {
        ...standard,
        ...updates,
        updatedAt: new Date(),
 ...
 * ```
 */
function validateLaborStandard(standard) {
    const errors = [];
    const warnings = [];
 ...
 * ```
 */
function applyEfficiencyFactor(standard, newEfficiencyFactor, reason) {
 ...
 * ```
 */
function calculateLaborCost(standard, quantity, hourlyRate) {
 ...
 * ```
 */
function compareLaborStandards(standards) {
    ...
    return {
        ...
        averageStandardTime: Number((standardTimes.reduce((sum, t) => sum + t, 0) / standards.length).toFixed(2)),
 ...
 * ```
 */
function exportLaborStandards(standards) {
    const headers = [
        'Standard ID',
        'Task Category',
        'Task Name',
 ...
 * ```
 */
function createTaskAssignment(assignment, standard) {
 ...
 * ```
 */
function assignBulkTasks(assignments, standards) {
    return assignments.map(assignment => {
        const standard = standards.find(s => s.taskCategory === assignment.taskCategory && s.standardId === assignment.standardId);
 ...
 * ```
 */
function reassignTask(assignment, newWorkerId, newWorkerName, reason) {
 ...
 * ```
 */
function prioritizeTask(assignment, newPriority, reason) {
 ...
 * ```
 */
function balanceWorkload(unassignedTasks, workers, config) {
 ...
 * ```
 */
function matchTaskToSkill(task, standard, workers) {
 ...
 * ```
 */
function scheduleTask(assignment, startTime, endTime) {
    const durationMinutes = (endTime.getTime() - startTime.getTime()) / (1000 * 60);
    if (durationMinutes < assignment.estimatedDuration) {
        throw new Error(`Scheduled time window (${durationMinutes} min) is less than estimated duration (${assignment.estimatedDuration} min)`);
    }
    return {
        ...assignment,
 ...
 * ```
 */
function cancelTaskAssignment(assignment, reason) {
    if (assignment.status === TaskStatus.COMPLETED) {
 ...
 * ```
 */
function startTimeEntry(workerId, assignmentId, taskCategory) {
 ...
 * ```
 */
function pauseTimeEntry(entry, breakType = 'REST') {
 ...
 * ```
 */
function resumeTimeEntry(entry) {
    if (entry.status !== 'PAUSED') {
 ...
 * ```
 */
function calculateActualTime(entry) {
 ...
 * ```
 */
function calculateBreakTime(entry) {
    const breaksByType = {};
 ...
 * ```
 */
function adjustTimeEntry(entry, adjustments, reason) {
    return {
        ...entry,
        ...adjustments,
        metadata: {
 ...
 * ```
 */
function calculateProductivity(assignments, timeEntries) {
 ...
 * ```
 */
function calculateEfficiency(assignment, standard, actualMinutes) {
 ...
 * ```
 */
function calculateUtilization(productiveMinutes, availableMinutes) {
 ...
 * ```
 */
function calculateUnitsPerHour(unitsProcessed, hoursWorked) {
 ...
 * ```
 */
function calculateIdleTime(shift) {
 ...
 * ```
 */
function generateProductivityReport(workerId, workerName, assignments, timeEntries, periodStart, periodEnd) {
    const completedTasks = assignments.filter(a => a.status === TaskStatus.COMPLETED);
 ...
 * ```
 */
function compareWorkerPerformance(metrics) {
    if (metrics.length === 0) {
 ...
 * ```
 */
function calculateTeamProductivity(workerMetrics) {
 ...
 * ```
 */
function createIncentivePlan(plan) {
 ...
 * ```
 */
function calculateIncentivePay(workerId, workerName, plan, metrics, baseHourlyRate) {
 ...
 * ```
 */
function evaluatePerformance(workerId, workerName, metrics, evaluatedBy) {
    ...
    if (averageScore >= 90)
        ...
        overallRating = PerformanceRating.ABOVE_AVERAGE;
    ...
    if (metrics.efficiency >= 100)
        strengths.push('Consistently meets or exceeds productivity standards');
    if (metrics.utilization >= 85)
        strengths.push('Excellent time utilization');
    if (metrics.efficiency < 80)
        areasForImprovement.push('Productivity below expectations');
    if (metrics.utilization < 70)
        areasForImprovement.push('Time management needs improvement');
    return {
        evaluationId,
        workerId,
        workerName,
 ...
 * ```
 */
function trackPerformanceTrend(historicalMetrics) {
    if (historicalMetrics.length < 2) {
        throw new Error('At least 2 periods of data required for trend analysis');
    }
    const sortedMetrics = [...historicalMetrics].sort((a, b) => a.period.start.getTime() - b.period.start.getTime());
    ...
    const stdDev = Math.sqrt(uphs.reduce((sum, u) => sum + Math.pow(u - averageUPH, 2), 0) / uphs.length);
 ...
 * ```
 */
function identifyTopPerformers(allMetrics, topN = 5) {
    // Calculate composite score (weighted average of key metrics)
    const scored = allMetrics.map(m => {
        const compositeScore = m.unitsPerHour * 0.4 + // 40% weight on UPH
    ...
    const averageCompositeScore = scored.reduce((sum, s) => sum + s.compositeScore, 0) / scored.length;
 ...
 * ```
 */
function identifyUnderperformers(allMetrics, thresholdPercent = 10) {
    // Calculate composite scores
    const scored = allMetrics.map(m => {
        const compositeScore = m.unitsPerHour * 0.4 + m.efficiency * 0.35 + m.utilization * 0.25;
        return { ...m, compositeScore };
    });
    const teamAverage = scored.reduce((sum, s) => sum + s.compositeScore, 0) / scored.length;
    ...
    const underperformers = scored.slice(0, count).map(m => {
        ...
        if (m.unitsPerHour < teamAverage * 0.4)
            improvementAreas.push('Productivity (UPH)');
        if (m.efficiency < 80)
            improvementAreas.push('Efficiency vs. Standard');
        if (m.utilization < 70)
            improvementAreas.push('Time Utilization');
        return {
            workerId: m.workerId,
            workerName: m.workerName,
            unitsPerHour: m.unitsPerHour,
    ...
    const improvementOpportunity = underperformers.reduce((sum, u) => sum + u.gapFromAverage, 0);
    return {
        underperformers,
 ...
 * ```
 */
function generatePerformanceReview(evaluation) {
    let review = '';
    review += '='.repeat(60) + '\n';
 ...
 * ```
 */
function calculateTeamIncentives(teamMetrics, plan, totalIncentivePool) {
...
//# sourceMappingURL=warehouse-labor-management-kit.js.map