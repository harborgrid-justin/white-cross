/**
 * LOC: CEFMS-UBS-030
 * File: /reuse/financial/cefms/composites/downstream/utility-backend-service.ts
 *
 * UPSTREAM (imports from):
 *   - ../cefms-utility-billing-collections-composite.ts
 *
 * DOWNSTREAM (imported by):
 *   - Utility billing APIs, meter reading, rate management
 */

/**
 * File: /reuse/financial/cefms/composites/downstream/utility-backend-service.ts
 * Locator: WC-CEFMS-UBS-030
 * Purpose: USACE CEFMS Utility billing, meter reading, rate calculations
 *
 * Upstream: Imports from cefms-utility-billing-collections-composite.ts
 * Downstream: Utility billing APIs, meter reading, rate management
 * Dependencies: TypeScript 5.x, Node 18+, NestJS 10.x, Sequelize 6.x
 * Exports: Complete utility service with 48+ production-ready functions
 *
 * LLM Context: Production-ready USACE CEFMS utility service for comprehensive
 * utility billing, meter reading, rate calculations.
 */
...
// ============================================================================
// TYPE DEFINITIONS
interface Utility_Backend_ServiceSearchCriteria {
  startDate?: Date;
  endDate?: Date;
  status?: string;
  category?: string;
...
interface Utility_Backend_ServiceStatistics {
  total: number;
...
interface Utility_Backend_ServiceValidationResult {
  valid: boolean;
...
interface Utility_Backend_ServiceBulkOperationResult {
...
interface Utility_Backend_ServiceAuditLog {
  logId: string;
  action: string;
  performedBy: string;
  performedAt: Date;
...
interface Utility_Backend_ServiceReport {
  reportId: string;
  reportType: string;
  generatedAt: Date;
  generatedBy: string;
...
interface Utility_Backend_ServiceExport {
  exportId: string;
  exportType: string;
  startDate: Date;
  endDate: Date;
...
interface Utility_Backend_ServiceNotification {
...
/**
 * Sequelize model for UtilityAccount.
 * Utility accounts
 */
export const createUtilityAccountModel = (sequelize: Sequelize) => {
  ...
  UtilityAccount.init(
    ...
    {
      sequelize,
      tableName: 'cefms_utilityaccount_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for MeterReading.
 * Meter readings
 */
export const createMeterReadingModel = (sequelize: Sequelize) => {
  ...
  MeterReading.init(
    ...
    {
      sequelize,
      tableName: 'cefms_meterreading_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for BillingCycle.
 * Billing cycles
 */
export const createBillingCycleModel = (sequelize: Sequelize) => {
  ...
  BillingCycle.init(
    ...
    {
      sequelize,
      tableName: 'cefms_billingcycle_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for RateSchedule.
 * Rate schedules
 */
export const createRateScheduleModel = (sequelize: Sequelize) => {
  ...
  RateSchedule.init(
    ...
    {
      sequelize,
      tableName: 'cefms_rateschedule_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * USACE CEFMS UtilityBackendService
 *
 * Provides comprehensive utility billing, meter reading, rate calculations.
 *
 * @class UtilityBackendService
 */
@Injectable()
export class UtilityBackendService {
  private readonly logger = new Logger(UtilityBackendService.name);
  ...
  async createRecord(data: any, transaction?: Transaction): Promise<any> {
    try {
      this.logger.log(`Creating record in utility`);
