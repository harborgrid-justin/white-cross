/**
 * LOC: CEFMS-GRM-016
 * File: /reuse/financial/cefms/composites/downstream/grant-reporting-module.ts
 *
 * UPSTREAM (imports from):
 *   - ../cefms-grant-lifecycle-tracking-composite.ts
 *
 * DOWNSTREAM (imported by):
 *   - Grant reporting APIs, FFR submissions, performance reporting
 */

/**
 * File: /reuse/financial/cefms/composites/downstream/grant-reporting-module.ts
 * Locator: WC-CEFMS-GRM-016
 * Purpose: USACE CEFMS Grant reporting, FFR, performance reports
 *
 * Upstream: Imports from cefms-grant-lifecycle-tracking-composite.ts
 * Downstream: Grant reporting APIs, FFR submissions, performance reporting
 * Dependencies: TypeScript 5.x, Node 18+, NestJS 10.x, Sequelize 6.x
 * Exports: Complete grant_reporting service with 48+ production-ready functions
 *
 * LLM Context: Production-ready USACE CEFMS grant_reporting service for comprehensive
 * grant reporting, ffr, performance reports.
 */
...
// ============================================================================
// TYPE DEFINITIONS
interface Grant_Reporting_ModuleSearchCriteria {
  startDate?: Date;
  endDate?: Date;
  status?: string;
  category?: string;
...
interface Grant_Reporting_ModuleStatistics {
  total: number;
...
interface Grant_Reporting_ModuleValidationResult {
  valid: boolean;
...
interface Grant_Reporting_ModuleBulkOperationResult {
...
interface Grant_Reporting_ModuleAuditLog {
  logId: string;
  action: string;
  performedBy: string;
  performedAt: Date;
...
interface Grant_Reporting_ModuleReport {
  reportId: string;
  reportType: string;
  generatedAt: Date;
  generatedBy: string;
...
interface Grant_Reporting_ModuleExport {
  exportId: string;
  exportType: string;
  startDate: Date;
  endDate: Date;
...
interface Grant_Reporting_ModuleNotification {
...
/**
 * Sequelize model for FFRReport.
 * Federal Financial Report
 */
export const createFFRReportModel = (sequelize: Sequelize) => {
  class FFRReport extends Model {
    public id!: string;
  ...
  FFRReport.init(
    ...
    {
      sequelize,
      tableName: 'cefms_ffrreport_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for PerformanceReport.
 * Performance reports
 */
export const createPerformanceReportModel = (sequelize: Sequelize) => {
  ...
  PerformanceReport.init(
    ...
    {
      sequelize,
      tableName: 'cefms_performancereport_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for GrantExpenditure.
 * Grant expenditures
 */
export const createGrantExpenditureModel = (sequelize: Sequelize) => {
  ...
  GrantExpenditure.init(
    ...
    {
      sequelize,
      tableName: 'cefms_grantexpenditure_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for MatchingFunds.
 * Matching funds
 */
export const createMatchingFundsModel = (sequelize: Sequelize) => {
  class MatchingFunds extends Model {
    public id!: string;
    public readonly createdAt!: Date;
    public readonly updatedAt!: Date;
  }
  ...
  MatchingFunds.init(
    ...
    {
      sequelize,
      tableName: 'cefms_matchingfunds_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for IndirectCosts.
 * Indirect costs
 */
export const createIndirectCostsModel = (sequelize: Sequelize) => {
  ...
  IndirectCosts.init(
    ...
    {
      sequelize,
      tableName: 'cefms_indirectcosts_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * USACE CEFMS GrantReportingService
 *
 * Provides comprehensive grant reporting, ffr, performance reports.
 *
 * @class GrantReportingService
 */
@Injectable()
export class GrantReportingService {
  ...
  async createRecord(data: any, transaction?: Transaction): Promise<any> {
    try {
      this.logger.log(`Creating record in grant_reporting`);
