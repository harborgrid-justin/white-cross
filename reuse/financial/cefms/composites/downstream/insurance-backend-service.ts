/**
 * LOC: CEFMS-ICBS-024
 * File: /reuse/financial/cefms/composites/downstream/insurance-backend-service.ts
 *
 * UPSTREAM (imports from):
 *   - ../cefms-insurance-claims-processing-composite.ts
 *
 * DOWNSTREAM (imported by):
 *   - Insurance APIs, claims management, loss tracking
 */

/**
 * File: /reuse/financial/cefms/composites/downstream/insurance-backend-service.ts
 * Locator: WC-CEFMS-ICBS-024
 * Purpose: USACE CEFMS Claims processing, loss tracking
 *
 * Upstream: Imports from cefms-insurance-claims-processing-composite.ts
 * Downstream: Insurance APIs, claims management, loss tracking
 * Dependencies: TypeScript 5.x, Node 18+, NestJS 10.x, Sequelize 6.x
 * Exports: Complete insurance service with 50+ production-ready functions
 *
 * LLM Context: Production-ready USACE CEFMS insurance service for comprehensive
...
// ============================================================================
// TYPE DEFINITIONS
interface Insurance_Backend_ServiceSearchCriteria {
  startDate?: Date;
  endDate?: Date;
  status?: string;
  category?: string;
...
interface Insurance_Backend_ServiceStatistics {
  total: number;
...
interface Insurance_Backend_ServiceValidationResult {
  valid: boolean;
...
interface Insurance_Backend_ServiceBulkOperationResult {
...
interface Insurance_Backend_ServiceAuditLog {
  logId: string;
  action: string;
  performedBy: string;
  performedAt: Date;
...
interface Insurance_Backend_ServiceReport {
  reportId: string;
  reportType: string;
  generatedAt: Date;
  generatedBy: string;
...
interface Insurance_Backend_ServiceExport {
  exportId: string;
  exportType: string;
  startDate: Date;
  endDate: Date;
...
interface Insurance_Backend_ServiceNotification {
...
/**
 * Sequelize model for InsurancePolicy.
 * Insurance policies
 */
export const createInsurancePolicyModel = (sequelize: Sequelize) => {
  ...
  InsurancePolicy.init(
    ...
    {
      sequelize,
      tableName: 'cefms_insurancepolicy_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for Claim.
 * Insurance claims
 */
export const createClaimModel = (sequelize: Sequelize) => {
...
/**
 * Sequelize model for ClaimPayment.
 * Claim payments
 */
export const createClaimPaymentModel = (sequelize: Sequelize) => {
  ...
  ClaimPayment.init(
    ...
    {
      sequelize,
      tableName: 'cefms_claimpayment_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for Loss.
 * Loss records
 */
export const createLossModel = (sequelize: Sequelize) => {
...
/**
 * Sequelize model for Coverage.
 * Coverage details
 */
export const createCoverageModel = (sequelize: Sequelize) => {
  ...
  Coverage.init(
    ...
    {
      sequelize,
      tableName: 'cefms_coverage_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * USACE CEFMS InsuranceBackendService
 *
 * Provides comprehensive claims processing, loss tracking.
 *
 * @class InsuranceBackendService
 */
@Injectable()
export class InsuranceBackendService {
  private readonly logger = new Logger(InsuranceBackendService.name);
  ...
  async createRecord(data: any, transaction?: Transaction): Promise<any> {
    try {
      this.logger.log(`Creating record in insurance`);
