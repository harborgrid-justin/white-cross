/**
 * LOC: CEFMS-IABS-027
 * File: /reuse/financial/cefms/composites/downstream/interagency-backend-service.ts
 *
 * UPSTREAM (imports from):
 *   - ../cefms-interagency-agreements-composite.ts
 *
 * DOWNSTREAM (imported by):
 *   - Interagency APIs, agreement tracking, billing systems
 */

/**
 * File: /reuse/financial/cefms/composites/downstream/interagency-backend-service.ts
 * Locator: WC-CEFMS-IABS-027
 * Purpose: USACE CEFMS Interagency agreement tracking, billing
 *
 * Upstream: Imports from cefms-interagency-agreements-composite.ts
 * Downstream: Interagency APIs, agreement tracking, billing systems
 * Dependencies: TypeScript 5.x, Node 18+, NestJS 10.x, Sequelize 6.x
 * Exports: Complete interagency service with 48+ production-ready functions
 *
 * LLM Context: Production-ready USACE CEFMS interagency service for comprehensive
 * interagency agreement tracking, billing.
 */
...
// ============================================================================
// TYPE DEFINITIONS
interface Interagency_Backend_ServiceSearchCriteria {
  startDate?: Date;
  endDate?: Date;
  status?: string;
  category?: string;
...
interface Interagency_Backend_ServiceStatistics {
  total: number;
...
interface Interagency_Backend_ServiceValidationResult {
  valid: boolean;
...
interface Interagency_Backend_ServiceBulkOperationResult {
...
interface Interagency_Backend_ServiceAuditLog {
  logId: string;
  action: string;
  performedBy: string;
  performedAt: Date;
...
interface Interagency_Backend_ServiceReport {
  reportId: string;
  reportType: string;
  generatedAt: Date;
  generatedBy: string;
...
interface Interagency_Backend_ServiceExport {
  exportId: string;
  exportType: string;
  startDate: Date;
  endDate: Date;
...
interface Interagency_Backend_ServiceNotification {
...
/**
 * Sequelize model for InteragencyAgreement.
...
export const createInteragencyAgreementModel = (sequelize: Sequelize) => {
  ...
  InteragencyAgreement.init(
    ...
    {
      sequelize,
      tableName: 'cefms_interagencyagreement_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for IAAPurchaseOrder.
 * IAA purchase orders
 */
export const createIAAPurchaseOrderModel = (sequelize: Sequelize) => {
  class IAAPurchaseOrder extends Model {
    public id!: string;
    public readonly createdAt!: Date;
    public readonly updatedAt!: Date;
  }

  IAAPurchaseOrder.init(
    {
      id: {
        type: DataTypes.UUID,
        defaultValue: DataTypes.UUIDV4,
    ...
    {
      sequelize,
      tableName: 'cefms_iaapurchaseorder_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
  ...
  return IAAPurchaseOrder;
};

/**
 * Sequelize model for IAAModification.
 * IAA modifications
 */
export const createIAAModificationModel = (sequelize: Sequelize) => {
  class IAAModification extends Model {
    public id!: string;
    public readonly createdAt!: Date;
    public readonly updatedAt!: Date;
  }

  IAAModification.init(
    {
      id: {
        type: DataTypes.UUID,
        defaultValue: DataTypes.UUIDV4,
    ...
    {
      sequelize,
      tableName: 'cefms_iaamodification_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
  ...
  return IAAModification;
};

/**
 * Sequelize model for ServiceDelivery.
 * Service deliveries
 */
export const createServiceDeliveryModel = (sequelize: Sequelize) => {
  ...
  ServiceDelivery.init(
    ...
    {
      sequelize,
      tableName: 'cefms_servicedelivery_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * USACE CEFMS InteragencyBackendService
 *
 * Provides comprehensive interagency agreement tracking, billing.
 *
 * @class InteragencyBackendService
 */
@Injectable()
export class InteragencyBackendService {
  private readonly logger = new Logger(InteragencyBackendService.name);
  ...
  async createRecord(data: any, transaction?: Transaction): Promise<any> {
    try {
      this.logger.log(`Creating record in interagency`);
