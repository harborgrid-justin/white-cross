/**
 * LOC: CEFMS-CPM-025
 * File: /reuse/financial/cefms/composites/downstream/claims-processing-module.ts
 *
 * UPSTREAM (imports from):
 *   - ../cefms-insurance-claims-processing-composite.ts
 *
 * DOWNSTREAM (imported by):
 *   - Claims processing APIs, loss tracking, payment processing
 */

/**
 * File: /reuse/financial/cefms/composites/downstream/claims-processing-module.ts
 * Locator: WC-CEFMS-CPM-025
 * Purpose: USACE CEFMS Claims processing, loss tracking
 *
 * Upstream: Imports from cefms-insurance-claims-processing-composite.ts
 * Downstream: Claims processing APIs, loss tracking, payment processing
 * Dependencies: TypeScript 5.x, Node 18+, NestJS 10.x, Sequelize 6.x
 * Exports: Complete claims_processing service with 46+ production-ready functions
 *
 * LLM Context: Production-ready USACE CEFMS claims_processing service for comprehensive
 * claims processing, loss tracking.
 */
...
// ============================================================================
// TYPE DEFINITIONS
interface Claims_Processing_ModuleSearchCriteria {
  startDate?: Date;
  endDate?: Date;
  status?: string;
  category?: string;
...
interface Claims_Processing_ModuleStatistics {
  total: number;
...
interface Claims_Processing_ModuleValidationResult {
  valid: boolean;
...
interface Claims_Processing_ModuleBulkOperationResult {
...
interface Claims_Processing_ModuleAuditLog {
  logId: string;
  action: string;
  performedBy: string;
  performedAt: Date;
...
interface Claims_Processing_ModuleReport {
  reportId: string;
  reportType: string;
  generatedAt: Date;
  generatedBy: string;
...
interface Claims_Processing_ModuleExport {
  exportId: string;
  exportType: string;
  startDate: Date;
  endDate: Date;
...
interface Claims_Processing_ModuleNotification {
...
/**
 * Sequelize model for ClaimIntake.
 * Claim intake
 */
export const createClaimIntakeModel = (sequelize: Sequelize) => {
  class ClaimIntake extends Model {
    public id!: string;
  ...
  ClaimIntake.init(
    ...
    {
      sequelize,
      tableName: 'cefms_claimintake_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for ClaimAssessment.
 * Claim assessments
 */
export const createClaimAssessmentModel = (sequelize: Sequelize) => {
  ...
  ClaimAssessment.init(
    ...
    {
      sequelize,
      tableName: 'cefms_claimassessment_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for LossValuation.
 * Loss valuations
 */
export const createLossValuationModel = (sequelize: Sequelize) => {
  class LossValuation extends Model {
    public id!: string;
    public readonly createdAt!: Date;
    public readonly updatedAt!: Date;
  }
  ...
  LossValuation.init(
    ...
    {
      sequelize,
      tableName: 'cefms_lossvaluation_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
  ...
  return LossValuation;
};

/**
 * Sequelize model for ClaimStatus.
 * Claim status tracking
 */
export const createClaimStatusModel = (sequelize: Sequelize) => {
  ...
  ClaimStatus.init(
    ...
    {
      sequelize,
      tableName: 'cefms_claimstatus_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * USACE CEFMS ClaimsProcessingService
 *
 * Provides comprehensive claims processing, loss tracking.
 *
...
export class ClaimsProcessingService {
  ...
  async createRecord(data: any, transaction?: Transaction): Promise<any> {
    try {
      this.logger.log(`Creating record in claims_processing`);
