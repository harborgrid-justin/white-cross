/**
 * LOC: CEFMS-FCBS-022
 * File: /reuse/financial/cefms/composites/downstream/foreign-currency-backend-service.ts
 *
 * UPSTREAM (imports from):
 *   - ../cefms-foreign-currency-transactions-composite.ts
 *
 * DOWNSTREAM (imported by):
 *   - Currency APIs, exchange rate management, revaluation processing
 */

/**
 * File: /reuse/financial/cefms/composites/downstream/foreign-currency-backend-service.ts
 * Locator: WC-CEFMS-FCBS-022
 * Purpose: USACE CEFMS Exchange rate management, currency conversion, revaluation
 *
 * Upstream: Imports from cefms-foreign-currency-transactions-composite.ts
 * Downstream: Currency APIs, exchange rate management, revaluation processing
 * Dependencies: TypeScript 5.x, Node 18+, NestJS 10.x, Sequelize 6.x
 * Exports: Complete foreign_currency service with 48+ production-ready functions
 *
 * LLM Context: Production-ready USACE CEFMS foreign_currency service for comprehensive
 * exchange rate management, currency conversion, revaluation.
 */
...
// ============================================================================
// TYPE DEFINITIONS
interface Foreign_Currency_Backend_ServiceSearchCriteria {
  startDate?: Date;
  endDate?: Date;
  status?: string;
  category?: string;
...
interface Foreign_Currency_Backend_ServiceStatistics {
  total: number;
...
interface Foreign_Currency_Backend_ServiceValidationResult {
  valid: boolean;
...
interface Foreign_Currency_Backend_ServiceBulkOperationResult {
...
interface Foreign_Currency_Backend_ServiceAuditLog {
  logId: string;
  action: string;
  performedBy: string;
  performedAt: Date;
...
interface Foreign_Currency_Backend_ServiceReport {
  reportId: string;
  reportType: string;
  generatedAt: Date;
  generatedBy: string;
...
interface Foreign_Currency_Backend_ServiceExport {
  exportId: string;
  exportType: string;
  startDate: Date;
  endDate: Date;
...
interface Foreign_Currency_Backend_ServiceNotification {
...
/**
 * Sequelize model for CurrencyTransaction.
 * Currency transactions
 */
export const createCurrencyTransactionModel = (sequelize: Sequelize) => {
  ...
  CurrencyTransaction.init(
    ...
    {
      sequelize,
      tableName: 'cefms_currencytransaction_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for ExchangeRate.
 * Exchange rates
 */
export const createExchangeRateModel = (sequelize: Sequelize) => {
  ...
  ExchangeRate.init(
    ...
    {
      sequelize,
      tableName: 'cefms_exchangerate_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for CurrencyRevaluation.
...
export const createCurrencyRevaluationModel = (sequelize: Sequelize) => {
  ...
  CurrencyRevaluation.init(
    ...
    {
      sequelize,
      tableName: 'cefms_currencyrevaluation_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for ForexGainLoss.
 * Foreign exchange gains/losses
 */
export const createForexGainLossModel = (sequelize: Sequelize) => {
  ...
  ForexGainLoss.init(
    ...
    {
      sequelize,
      tableName: 'cefms_forexgainloss_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for CurrencyHolding.
 * Currency holdings
 */
export const createCurrencyHoldingModel = (sequelize: Sequelize) => {
  class CurrencyHolding extends Model {
    public id!: string;
    public readonly createdAt!: Date;
    public readonly updatedAt!: Date;
  }

  CurrencyHolding.init(
    {
      id: {
        type: DataTypes.UUID,
        defaultValue: DataTypes.UUIDV4,
    ...
    {
      sequelize,
      tableName: 'cefms_currencyholding_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * USACE CEFMS ForeignCurrencyBackendService
 *
 * Provides comprehensive exchange rate management, currency conversion, revaluation.
 *
 * @class ForeignCurrencyBackendService
 */
@Injectable()
export class ForeignCurrencyBackendService {
  private readonly logger = new Logger(ForeignCurrencyBackendService.name);
  ...
  async createRecord(data: any, transaction?: Transaction): Promise<any> {
    try {
      this.logger.log(`Creating record in foreign_currency`);
