/**
 * LOC: CEFMS-GMBS-014
 * File: /reuse/financial/cefms/composites/downstream/grant-management-backend-service.ts
 *
 * UPSTREAM (imports from):
 *   - ../cefms-grant-lifecycle-tracking-composite.ts
 *
 * DOWNSTREAM (imported by):
 *   - Grant management APIs, award tracking, lifecycle management
 */

/**
 * File: /reuse/financial/cefms/composites/downstream/grant-management-backend-service.ts
 * Locator: WC-CEFMS-GMBS-014
 * Purpose: USACE CEFMS Grant lifecycle tracking, award management
 *
 * Upstream: Imports from cefms-grant-lifecycle-tracking-composite.ts
 * Downstream: Grant management APIs, award tracking, lifecycle management
 * Dependencies: TypeScript 5.x, Node 18+, NestJS 10.x, Sequelize 6.x
 * Exports: Complete grants service with 52+ production-ready functions
 *
 * LLM Context: Production-ready USACE CEFMS grants service for comprehensive
 * grant lifecycle tracking, award management.
 */
...
// ============================================================================
// TYPE DEFINITIONS
interface Grant_Management_Backend_ServiceSearchCriteria {
  startDate?: Date;
  endDate?: Date;
  status?: string;
  category?: string;
...
interface Grant_Management_Backend_ServiceStatistics {
  total: number;
...
interface Grant_Management_Backend_ServiceValidationResult {
  valid: boolean;
...
interface Grant_Management_Backend_ServiceBulkOperationResult {
...
interface Grant_Management_Backend_ServiceAuditLog {
  logId: string;
  action: string;
  performedBy: string;
  performedAt: Date;
...
interface Grant_Management_Backend_ServiceReport {
  reportId: string;
  reportType: string;
  generatedAt: Date;
  generatedBy: string;
...
interface Grant_Management_Backend_ServiceExport {
  exportId: string;
  exportType: string;
  startDate: Date;
  endDate: Date;
...
interface Grant_Management_Backend_ServiceNotification {
...
/**
 * Sequelize model for Grant.
 * Grant records
 */
export const createGrantModel = (sequelize: Sequelize) => {
...
/**
 * Sequelize model for GrantAward.
 * Grant awards
 */
export const createGrantAwardModel = (sequelize: Sequelize) => {
  ...
  GrantAward.init(
    ...
    {
      sequelize,
      tableName: 'cefms_grantaward_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for GrantModification.
 * Grant modifications
 */
export const createGrantModificationModel = (sequelize: Sequelize) => {
  class GrantModification extends Model {
    public id!: string;
    public readonly createdAt!: Date;
    public readonly updatedAt!: Date;
  }

  GrantModification.init(
    {
      id: {
        type: DataTypes.UUID,
        defaultValue: DataTypes.UUIDV4,
    ...
    {
      sequelize,
      tableName: 'cefms_grantmodification_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for GrantDrawdown.
 * Grant drawdowns
 */
export const createGrantDrawdownModel = (sequelize: Sequelize) => {
  class GrantDrawdown extends Model {
    public id!: string;
    public readonly createdAt!: Date;
    public readonly updatedAt!: Date;
  }
  ...
  GrantDrawdown.init(
    ...
    {
      sequelize,
      tableName: 'cefms_grantdrawdown_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
  ...
  return GrantDrawdown;
};

/**
 * Sequelize model for GrantCloseout.
 * Grant closeouts
 */
export const createGrantCloseoutModel = (sequelize: Sequelize) => {
  ...
  GrantCloseout.init(
    ...
    {
      sequelize,
      tableName: 'cefms_grantcloseout_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
  ...
  return GrantCloseout;
};

/**
 * Sequelize model for Grantee.
 * Grantee information
 */
export const createGranteeModel = (sequelize: Sequelize) => {
  ...
  Grantee.init(
    ...
    {
      sequelize,
      tableName: 'cefms_grantee_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * USACE CEFMS GrantManagementBackendService
 *
 * Provides comprehensive grant lifecycle tracking, award management.
 *
 * @class GrantManagementBackendService
 */
@Injectable()
export class GrantManagementBackendService {
  private readonly logger = new Logger(GrantManagementBackendService.name);
  ...
  async createRecord(data: any, transaction?: Transaction): Promise<any> {
    try {
      this.logger.log(`Creating record in grants`);
