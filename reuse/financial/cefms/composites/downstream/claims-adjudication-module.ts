/**
 * LOC: CEFMS-CAM-026
 * File: /reuse/financial/cefms/composites/downstream/claims-adjudication-module.ts
 *
 * UPSTREAM (imports from):
 *   - ../cefms-insurance-claims-processing-composite.ts
 *
 * DOWNSTREAM (imported by):
 *   - Adjudication APIs, settlement processing, recovery tracking
 */

/**
 * File: /reuse/financial/cefms/composites/downstream/claims-adjudication-module.ts
 * Locator: WC-CEFMS-CAM-026
 * Purpose: USACE CEFMS Claims adjudication, settlement, recovery
 *
 * Upstream: Imports from cefms-insurance-claims-processing-composite.ts
 * Downstream: Adjudication APIs, settlement processing, recovery tracking
 * Dependencies: TypeScript 5.x, Node 18+, NestJS 10.x, Sequelize 6.x
 * Exports: Complete claims_adjudication service with 44+ production-ready functions
 *
 * LLM Context: Production-ready USACE CEFMS claims_adjudication service for comprehensive
 * claims adjudication, settlement, recovery.
 */
...
// ============================================================================
// TYPE DEFINITIONS
interface Claims_Adjudication_ModuleSearchCriteria {
  startDate?: Date;
  endDate?: Date;
  status?: string;
  category?: string;
...
interface Claims_Adjudication_ModuleStatistics {
  total: number;
...
interface Claims_Adjudication_ModuleValidationResult {
  valid: boolean;
...
interface Claims_Adjudication_ModuleBulkOperationResult {
...
interface Claims_Adjudication_ModuleAuditLog {
  logId: string;
  action: string;
  performedBy: string;
  performedAt: Date;
...
interface Claims_Adjudication_ModuleReport {
  reportId: string;
  reportType: string;
  generatedAt: Date;
  generatedBy: string;
...
interface Claims_Adjudication_ModuleExport {
  exportId: string;
  exportType: string;
  startDate: Date;
  endDate: Date;
...
interface Claims_Adjudication_ModuleNotification {
...
/**
 * Sequelize model for ClaimAdjudication.
 * Claim adjudications
 */
export const createClaimAdjudicationModel = (sequelize: Sequelize) => {
  ...
  ClaimAdjudication.init(
    ...
    {
      sequelize,
      tableName: 'cefms_claimadjudication_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for Settlement.
 * Settlements
 */
export const createSettlementModel = (sequelize: Sequelize) => {
  ...
  Settlement.init(
    ...
    {
      sequelize,
      tableName: 'cefms_settlement_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for Recovery.
 * Recoveries
 */
export const createRecoveryModel = (sequelize: Sequelize) => {
  ...
  Recovery.init(
    ...
    {
      sequelize,
      tableName: 'cefms_recovery_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for Subrogation.
 * Subrogations
 */
export const createSubrogationModel = (sequelize: Sequelize) => {
  ...
  Subrogation.init(
    ...
    {
      sequelize,
      tableName: 'cefms_subrogation_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * USACE CEFMS ClaimsAdjudicationService
 *
 * Provides comprehensive claims adjudication, settlement, recovery.
 *
...
export class ClaimsAdjudicationService {
  ...
  async createRecord(data: any, transaction?: Transaction): Promise<any> {
    try {
      this.logger.log(`Creating record in claims_adjudication`);
