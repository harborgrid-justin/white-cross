/**
 * LOC: CEFMS-BEC-008
 * File: /reuse/financial/cefms/composites/downstream/budget-execution-controller.ts
 *
 * UPSTREAM (imports from):
 *   - ../cefms-appropriations-budget-execution-composite.ts
 *
 * DOWNSTREAM (imported by):
 *   - Budget execution controllers, allotment APIs
 */

/**
 * File: /reuse/financial/cefms/composites/downstream/budget-execution-controller.ts
 * Locator: WC-CEFMS-BEC-008
 ...
 * Downstream: Budget execution controllers, allotment APIs
 * Dependencies: TypeScript 5.x, Node 18+, NestJS 10.x, Sequelize 6.x
 * Exports: Complete budget_execution service with 48+ production-ready functions
 *
 * LLM Context: Production-ready USACE CEFMS budget_execution service for comprehensive
...
// ============================================================================
// TYPE DEFINITIONS
interface Budget_Execution_ControllerSearchCriteria {
  startDate?: Date;
  endDate?: Date;
  status?: string;
  category?: string;
...
interface Budget_Execution_ControllerStatistics {
  total: number;
...
interface Budget_Execution_ControllerValidationResult {
  valid: boolean;
...
interface Budget_Execution_ControllerBulkOperationResult {
...
interface Budget_Execution_ControllerAuditLog {
  logId: string;
  action: string;
  performedBy: string;
  performedAt: Date;
...
interface Budget_Execution_ControllerReport {
  reportId: string;
  reportType: string;
  generatedAt: Date;
  generatedBy: string;
...
interface Budget_Execution_ControllerExport {
  exportId: string;
  exportType: string;
  startDate: Date;
  endDate: Date;
...
interface Budget_Execution_ControllerNotification {
...
/**
 * Sequelize model for BudgetExecution.
 * Budget execution records
 */
export const createBudgetExecutionModel = (sequelize: Sequelize) => {
  ...
  BudgetExecution.init(
    ...
    {
      sequelize,
      tableName: 'cefms_budgetexecution_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for ExecutionSchedule.
 * Execution schedules
 */
export const createExecutionScheduleModel = (sequelize: Sequelize) => {
  class ExecutionSchedule extends Model {
    public id!: string;
    public readonly createdAt!: Date;
    public readonly updatedAt!: Date;
  }

  ExecutionSchedule.init(
    {
      id: {
        type: DataTypes.UUID,
        defaultValue: DataTypes.UUIDV4,
    ...
    {
      sequelize,
      tableName: 'cefms_executionschedule_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for AllotmentAdvice.
 * Allotment advice
 */
export const createAllotmentAdviceModel = (sequelize: Sequelize) => {
  class AllotmentAdvice extends Model {
    public id!: string;
    public readonly createdAt!: Date;
    public readonly updatedAt!: Date;
  }

  AllotmentAdvice.init(
    {
      id: {
        type: DataTypes.UUID,
        defaultValue: DataTypes.UUIDV4,
    ...
    {
      sequelize,
      tableName: 'cefms_allotmentadvice_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
  ...
  return AllotmentAdvice;
};

/**
 * Sequelize model for FundControl.
 * Fund controls
 */
export const createFundControlModel = (sequelize: Sequelize) => {
  class FundControl extends Model {
    public id!: string;
    public readonly createdAt!: Date;
    public readonly updatedAt!: Date;
  }
  ...
  FundControl.init(
    ...
    {
      sequelize,
      tableName: 'cefms_fundcontrol_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for ExecutionReport.
 * Execution reports
 */
export const createExecutionReportModel = (sequelize: Sequelize) => {
  ...
  ExecutionReport.init(
    ...
    {
      sequelize,
      tableName: 'cefms_executionreport_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * USACE CEFMS BudgetExecutionService
 *
 ...
 */
@Injectable()
export class BudgetExecutionService {
  private readonly logger = new Logger(BudgetExecutionService.name);
  ...
  async createRecord(data: any, transaction?: Transaction): Promise<any> {
    try {
      this.logger.log(`Creating record in budget_execution`);
