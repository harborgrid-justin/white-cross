/**
 * LOC: CEFMS-TCM-035
 * File: /reuse/financial/cefms/composites/downstream/tax-collection-module.ts
 *
 * UPSTREAM (imports from):
 *   - ../cefms-tax-assessment-collection-composite.ts
 *
 * DOWNSTREAM (imported by):
 *   - Collection APIs, payment plan management, enforcement
 */

/**
 * File: /reuse/financial/cefms/composites/downstream/tax-collection-module.ts
 * Locator: WC-CEFMS-TCM-035
 * Purpose: USACE CEFMS Tax collection, payment plans, enforcement
 *
 * Upstream: Imports from cefms-tax-assessment-collection-composite.ts
 * Downstream: Collection APIs, payment plan management, enforcement
 * Dependencies: TypeScript 5.x, Node 18+, NestJS 10.x, Sequelize 6.x
 * Exports: Complete tax_collection service with 42+ production-ready functions
 *
 * LLM Context: Production-ready USACE CEFMS tax_collection service for comprehensive
 * tax collection, payment plans, enforcement.
 */
...
// ============================================================================
// TYPE DEFINITIONS
interface Tax_Collection_ModuleSearchCriteria {
  startDate?: Date;
  endDate?: Date;
  status?: string;
  category?: string;
...
interface Tax_Collection_ModuleStatistics {
  total: number;
...
interface Tax_Collection_ModuleValidationResult {
  valid: boolean;
...
interface Tax_Collection_ModuleBulkOperationResult {
...
interface Tax_Collection_ModuleAuditLog {
  logId: string;
  action: string;
  performedBy: string;
  performedAt: Date;
...
interface Tax_Collection_ModuleReport {
  reportId: string;
  reportType: string;
  generatedAt: Date;
  generatedBy: string;
...
interface Tax_Collection_ModuleExport {
  exportId: string;
  exportType: string;
  startDate: Date;
  endDate: Date;
...
interface Tax_Collection_ModuleNotification {
...
/**
 * Sequelize model for TaxCollection.
 * Tax collections
 */
export const createTaxCollectionModel = (sequelize: Sequelize) => {
  ...
  TaxCollection.init(
    ...
    {
      sequelize,
      tableName: 'cefms_taxcollection_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for PaymentPlan.
 * Payment plans
 */
export const createPaymentPlanModel = (sequelize: Sequelize) => {
  ...
  PaymentPlan.init(
    ...
    {
      sequelize,
      tableName: 'cefms_paymentplan_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for EnforcementAction.
 * Enforcement actions
 */
export const createEnforcementActionModel = (sequelize: Sequelize) => {
  ...
  EnforcementAction.init(
    ...
    {
      sequelize,
      tableName: 'cefms_enforcementaction_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * USACE CEFMS TaxCollectionService
 *
 * Provides comprehensive tax collection, payment plans, enforcement.
 *
 * @class TaxCollectionService
 */
@Injectable()
export class TaxCollectionService {
  private readonly logger = new Logger(TaxCollectionService.name);
  ...
  async createRecord(data: any, transaction?: Transaction): Promise<any> {
    try {
      this.logger.log(`Creating record in tax_collection`);
