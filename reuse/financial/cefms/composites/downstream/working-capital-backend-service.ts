/**
 * LOC: CEFMS-WCBS-036
 * File: /reuse/financial/cefms/composites/downstream/working-capital-backend-service.ts
 *
 * UPSTREAM (imports from):
 *   - ../cefms-working-capital-fund-composite.ts
 *
 * DOWNSTREAM (imported by):
 *   - Working capital APIs, fund management, rate setting
 */

/**
 * File: /reuse/financial/cefms/composites/downstream/working-capital-backend-service.ts
 * Locator: WC-CEFMS-WCBS-036
 * Purpose: USACE CEFMS Working capital fund operations, rate setting, cost recovery
 *
 * Upstream: Imports from cefms-working-capital-fund-composite.ts
 * Downstream: Working capital APIs, fund management, rate setting
 * Dependencies: TypeScript 5.x, Node 18+, NestJS 10.x, Sequelize 6.x
 * Exports: Complete working_capital service with 46+ production-ready functions
 *
 * LLM Context: Production-ready USACE CEFMS working_capital service for comprehensive
 * working capital fund operations, rate setting, cost recovery.
 */
...
// ============================================================================
// TYPE DEFINITIONS
interface Working_Capital_Backend_ServiceSearchCriteria {
  startDate?: Date;
  endDate?: Date;
  status?: string;
  category?: string;
...
interface Working_Capital_Backend_ServiceStatistics {
  total: number;
...
interface Working_Capital_Backend_ServiceValidationResult {
  valid: boolean;
...
interface Working_Capital_Backend_ServiceBulkOperationResult {
...
interface Working_Capital_Backend_ServiceAuditLog {
  logId: string;
  action: string;
  performedBy: string;
  performedAt: Date;
...
interface Working_Capital_Backend_ServiceReport {
  reportId: string;
  reportType: string;
  generatedAt: Date;
  generatedBy: string;
...
interface Working_Capital_Backend_ServiceExport {
  exportId: string;
  exportType: string;
  startDate: Date;
  endDate: Date;
...
interface Working_Capital_Backend_ServiceNotification {
...
/**
 * Sequelize model for WorkingCapitalFund.
...
export const createWorkingCapitalFundModel = (sequelize: Sequelize) => {
  ...
  WorkingCapitalFund.init(
    ...
    {
      sequelize,
      tableName: 'cefms_workingcapitalfund_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for FundOperation.
 * Fund operations
 */
export const createFundOperationModel = (sequelize: Sequelize) => {
  ...
  FundOperation.init(
    ...
    {
      sequelize,
      tableName: 'cefms_fundoperation_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
export const createCostPoolModel = (sequelize: Sequelize) => {
  ...
  CostPool.init(
    ...
    {
      sequelize,
      tableName: 'cefms_costpool_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * USACE CEFMS WorkingCapitalBackendService
 *
 * Provides comprehensive working capital fund operations, rate setting, cost recovery.
 *
 * @class WorkingCapitalBackendService
 */
@Injectable()
export class WorkingCapitalBackendService {
  private readonly logger = new Logger(WorkingCapitalBackendService.name);
  ...
  async createRecord(data: any, transaction?: Transaction): Promise<any> {
    try {
      this.logger.log(`Creating record in working_capital`);
