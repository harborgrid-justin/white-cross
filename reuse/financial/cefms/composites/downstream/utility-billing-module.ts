/**
 * LOC: CEFMS-UBM-031
 * File: /reuse/financial/cefms/composites/downstream/utility-billing-module.ts
 *
 * UPSTREAM (imports from):
 *   - ../cefms-utility-billing-collections-composite.ts
 *
 * DOWNSTREAM (imported by):
 *   - Billing APIs, rate calculation, invoice generation
 */

/**
 * File: /reuse/financial/cefms/composites/downstream/utility-billing-module.ts
 * Locator: WC-CEFMS-UBM-031
 ...
 * Downstream: Billing APIs, rate calculation, invoice generation
 * Dependencies: TypeScript 5.x, Node 18+, NestJS 10.x, Sequelize 6.x
 * Exports: Complete utility_billing service with 44+ production-ready functions
 *
 * LLM Context: Production-ready USACE CEFMS utility_billing service for comprehensive
...
// ============================================================================
// TYPE DEFINITIONS
interface Utility_Billing_ModuleSearchCriteria {
  startDate?: Date;
  endDate?: Date;
  status?: string;
  category?: string;
...
interface Utility_Billing_ModuleStatistics {
  total: number;
...
interface Utility_Billing_ModuleValidationResult {
  valid: boolean;
...
interface Utility_Billing_ModuleBulkOperationResult {
...
interface Utility_Billing_ModuleAuditLog {
  logId: string;
  action: string;
  performedBy: string;
  performedAt: Date;
...
interface Utility_Billing_ModuleReport {
  reportId: string;
  reportType: string;
  generatedAt: Date;
  generatedBy: string;
...
interface Utility_Billing_ModuleExport {
  exportId: string;
  exportType: string;
  startDate: Date;
  endDate: Date;
...
interface Utility_Billing_ModuleNotification {
...
/**
 * Sequelize model for UtilityBill.
 * Utility bills
 */
export const createUtilityBillModel = (sequelize: Sequelize) => {
  ...
  UtilityBill.init(
    ...
    {
      sequelize,
      tableName: 'cefms_utilitybill_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for ConsumptionData.
 * Consumption data
 */
export const createConsumptionDataModel = (sequelize: Sequelize) => {
  ...
  ConsumptionData.init(
    ...
    {
      sequelize,
      tableName: 'cefms_consumptiondata_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for BillingAdjustment.
 * Billing adjustments
 */
export const createBillingAdjustmentModel = (sequelize: Sequelize) => {
  ...
  BillingAdjustment.init(
    ...
    {
      sequelize,
      tableName: 'cefms_billingadjustment_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * USACE CEFMS UtilityBillingService
 *
...
export class UtilityBillingService {
  ...
  async createRecord(data: any, transaction?: Transaction): Promise<any> {
    try {
      this.logger.log(`Creating record in utility_billing`);
