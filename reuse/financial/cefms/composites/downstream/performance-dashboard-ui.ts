/**
 * LOC: CEFMS-PDU-012
 * File: /reuse/financial/cefms/composites/downstream/performance-dashboard-ui.ts
 *
 * UPSTREAM (imports from):
 *   - ../cefms-performance-metrics-dashboard-composite.ts
 *
 * DOWNSTREAM (imported by):
 *   - Dashboard UIs, visualization components, reporting widgets
 */

/**
 * File: /reuse/financial/cefms/composites/downstream/performance-dashboard-ui.ts
 * Locator: WC-CEFMS-PDU-012
 ...
 * Downstream: Dashboard UIs, visualization components, reporting widgets
 * Dependencies: TypeScript 5.x, Node 18+, NestJS 10.x, Sequelize 6.x
 * Exports: Complete dashboard service with 45+ production-ready functions
 *
 * LLM Context: Production-ready USACE CEFMS dashboard service for comprehensive
...
// ============================================================================
// TYPE DEFINITIONS
interface Performance_Dashboard_UiSearchCriteria {
  startDate?: Date;
  endDate?: Date;
  status?: string;
  category?: string;
...
interface Performance_Dashboard_UiStatistics {
  total: number;
...
interface Performance_Dashboard_UiValidationResult {
  valid: boolean;
...
interface Performance_Dashboard_UiBulkOperationResult {
...
interface Performance_Dashboard_UiAuditLog {
  logId: string;
  action: string;
  performedBy: string;
  performedAt: Date;
...
interface Performance_Dashboard_UiReport {
  reportId: string;
  reportType: string;
  generatedAt: Date;
  generatedBy: string;
...
interface Performance_Dashboard_UiExport {
  exportId: string;
  exportType: string;
  startDate: Date;
  endDate: Date;
...
interface Performance_Dashboard_UiNotification {
...
/**
 * Sequelize model for DashboardWidget.
 * Dashboard widgets
 */
export const createDashboardWidgetModel = (sequelize: Sequelize) => {
  ...
  DashboardWidget.init(
    ...
    {
      sequelize,
      tableName: 'cefms_dashboardwidget_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for ChartConfiguration.
 * Chart configurations
 */
export const createChartConfigurationModel = (sequelize: Sequelize) => {
  ...
  ChartConfiguration.init(
    ...
    {
      sequelize,
      tableName: 'cefms_chartconfiguration_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for DashboardLayout.
 * Dashboard layouts
 */
export const createDashboardLayoutModel = (sequelize: Sequelize) => {
  ...
  DashboardLayout.init(
    ...
    {
      sequelize,
      tableName: 'cefms_dashboardlayout_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for UserPreference.
 * User preferences
 */
export const createUserPreferenceModel = (sequelize: Sequelize) => {
  ...
  UserPreference.init(
    ...
    {
      sequelize,
      tableName: 'cefms_userpreference_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * Sequelize model for DataRefresh.
 * Data refresh schedules
 */
export const createDataRefreshModel = (sequelize: Sequelize) => {
  ...
  DataRefresh.init(
    ...
    {
      sequelize,
      tableName: 'cefms_datarefresh_records',
      timestamps: true,
      indexes: [
        { fields: ['id'] },
        { fields: ['createdAt'] },
...
/**
 * USACE CEFMS PerformanceDashboardService
 *
...
export class PerformanceDashboardService {
  ...
  async createRecord(data: any, transaction?: Transaction): Promise<any> {
    try {
      this.logger.log(`Creating record in dashboard`);
