...
exports.BudgetingForecastingKit = exports.VarianceAnalysisType = exports.ScenarioType = exports.ForecastPeriod = exports.ForecastMethod = exports.BudgetStatus = exports.BudgetMethodology = exports.BudgetType = void 0;
exports.createBudget = createBudget;
exports.createBudgetFromTemplate = createBudgetFromTemplate;
exports.addBudgetLineItems = addBudgetLineItems;
exports.updateBudgetStatus = updateBudgetStatus;
exports.getBudgetWithDetails = getBudgetWithDetails;
exports.allocateBudgetTopDown = allocateBudgetTopDown;
exports.consolidateBudgets = consolidateBudgets;
exports.calculateBudgetVariance = calculateBudgetVariance;
exports.decomposeVariance = decomposeVariance;
exports.analyzeVarianceTrends = analyzeVarianceTrends;
exports.generateMultiYearForecast = generateMultiYearForecast;
exports.createRollingForecast = createRollingForecast;
exports.updateRollingForecast = updateRollingForecast;
exports.generateSeasonalForecast = generateSeasonalForecast;
exports.createBudgetScenario = createBudgetScenario;
...
exports.getAllocationPlan = getAllocationPlan;
exports.trackBudgetUtilization = trackBudgetUtilization;
exports.createDriverBasedForecast = createDriverBasedForecast;
exports.calculateDriverBasedForecast = calculateDriverBasedForecast;
...
var BudgetType;
(function (BudgetType) {
    BudgetType["OPERATING"] = "operating";
    BudgetType["CAPITAL"] = "capital";
    BudgetType["CASH_FLOW"] = "cash_flow";
 ...
 */
var BudgetMethodology;
(function (BudgetMethodology) {
    BudgetMethodology["TOP_DOWN"] = "top_down";
    BudgetMethodology["BOTTOM_UP"] = "bottom_up";
    BudgetMethodology["ZERO_BASED"] = "zero_based";
    BudgetMethodology["INCREMENTAL"] = "incremental";
    BudgetMethodology["ACTIVITY_BASED"] = "activity_based";
    BudgetMethodology["VALUE_PROPOSITION"] = "value_proposition";
})(BudgetMethodology || (exports.BudgetMethodology = BudgetMethodology = {}));
/**
...
(function (BudgetStatus) {
    ...
    BudgetStatus["IN_REVIEW"] = "in_review";
...
(function (ForecastMethod) {
    ...
    ForecastMethod["WEIGHTED_MOVING_AVERAGE"] = "weighted_moving_average";
    ForecastMethod["SEASONAL_DECOMPOSITION"] = "seasonal_decomposition";
    ForecastMethod["ARIMA"] = "arima";
    ForecastMethod["DRIVER_BASED"] = "driver_based";
    ForecastMethod["STRAIGHT_LINE"] = "straight_line";
...
(function (ForecastPeriod) {
    ...
    ForecastPeriod["ANNUALLY"] = "annually";
 ...
 */
var VarianceAnalysisType;
(function (VarianceAnalysisType) {
    VarianceAnalysisType["SIMPLE"] = "simple";
    VarianceAnalysisType["DETAILED"] = "detailed";
    VarianceAnalysisType["DECOMPOSED"] = "decomposed";
    VarianceAnalysisType["TREND"] = "trend";
})(VarianceAnalysisType || (exports.VarianceAnalysisType = VarianceAnalysisType = {}));
// ============================================================================
 ...
 * ```
 */
async function createBudget(sequelize, params) {
 ...
 */
async function createBudgetFromTemplate(sequelize, templateId, params, adjustmentFactor = 1.0) {
    const transaction = await sequelize.transaction();
    try {
 ...
 */
async function addBudgetLineItems(sequelize, budgetId, lineItems) {
    const transaction = await sequelize.transaction();
    try {
    ...
    `, {
      ...
      '${item.period}',
      ${new decimal_js_1.Decimal(item.amount.toString()).toFixed(2)},
      ${item.quantity || 'NULL'},
      ${item.unitPrice ? new decimal_js_1.Decimal(item.unitPrice.toString()).toFixed(2) : 'NULL'},
 ...
 */
async function updateBudgetStatus(sequelize, budgetId, newStatus, userId, notes) {
    const transaction = await sequelize.transaction();
    try {
 ...
 */
async function getBudgetWithDetails(sequelize, budgetId, includeLineItems = true) {
    // Get budget header
    const [budget] = await sequelize.query(`
    SELECT
      b.*,
 ...
 */
async function allocateBudgetTopDown(sequelize, budgetId, totalAmount, allocationRules) {
    const transaction = await sequelize.transaction();
    try {
        ...
        const allocations = [];
        let remainingAmount = new decimal_js_1.Decimal(totalAmount);
        // Process fixed amounts first
        for (const rule of allocationRules.filter(r => r.fixedAmount)) {
            const allocated = new decimal_js_1.Decimal(rule.fixedAmount);
            allocations.push({
 ...
 */
async function consolidateBudgets(sequelize, masterBudgetId, sourceBudgetIds, consolidationRules) {
    const transaction = await sequelize.transaction();
    try {
        const rules = {
 ...
 */
async function calculateBudgetVariance(sequelize, budgetId, periodStart, periodEnd, analysisType = VarianceAnalysisType.DETAILED) {
  ...
  `, {
    ...
    const totals = variances.reduce((acc, v) => ({
        ...
        totalBudget: new decimal_js_1.Decimal(0),
        totalActual: new decimal_js_1.Decimal(0),
        totalVariance: new decimal_js_1.Decimal(0),
        favorableVariance: new decimal_js_1.Decimal(0),
        unfavorableVariance: new decimal_js_1.Decimal(0)
    });
    // Group by category
    const byCategory = {};
    variances.forEach((v) => {
        if (!byCategory[v.category]) {
            byCategory[v.category] = new decimal_js_1.Decimal(0);
        }
    ...
    variances.forEach((v) => {
        if (v.department_id) {
            if (!byDepartment[v.department_id]) {
                byDepartment[v.department_id] = new decimal_js_1.Decimal(0);
            }
    ...
    return {
        ...
        lineItems: variances.map((v) => ({
            accountId: v.account_id,
            accountName: v.account_name,
            budgeted: new decimal_js_1.Decimal(v.budgeted || 0),
            actual: new decimal_js_1.Decimal(v.actual || 0),
            variance: new decimal_js_1.Decimal(v.variance || 0),
 ...
 */
async function decomposeVariance(sequelize, budgetId, accountId, periodStart, periodEnd) {
  ...
  `, {
    ...
    const volumeVariance = new decimal_js_1.Decimal(data.volume_variance || 0);
    const priceVariance = new decimal_js_1.Decimal(data.price_variance || 0);
    const totalVariance = new decimal_js_1.Decimal(data.total_variance || 0);
 ...
 */
async function analyzeVarianceTrends(sequelize, budgetId, periods = 12) {
  ...
  `, {
    ...
    return trends.map((t) => ({
        period: t.period,
        value: new decimal_js_1.Decimal(t.value || 0),
        forecast: new decimal_js_1.Decimal(t.forecast || 0),
 ...
 */
async function generateMultiYearForecast(sequelize, params) {
    const transaction = await sequelize.transaction();
    try {
    ...
    `, {
        ...
        historicalData.forEach((row) => {
            ...
            accountData[row.account_id].push({
                period: row.period,
                amount: new decimal_js_1.Decimal(row.amount)
            });
        });
        // Generate forecasts for each account
        const forecastData = [];
        for (const [accountId, history] of Object.entries(accountData)) {
            const forecasted = applyForecastMethod(history, params.method, params.forecastYears, params.period);
 ...
 */
function applyForecastMethod(historicalData, method, forecastYears, period) {
    const results = [];
    switch (method) {
        case ForecastMethod.LINEAR_REGRESSION:
            ...
            const sumY = historicalData.reduce((sum, d) => sum.plus(d.amount), new decimal_js_1.Decimal(0));
            const sumXY = historicalData.reduce((sum, d, i) => sum.plus(new decimal_js_1.Decimal(i).mul(d.amount)), new decimal_js_1.Decimal(0));
        ...
        case ForecastMethod.EXPONENTIAL_SMOOTHING:
            ...
            for (let i = 1; i < historicalData.length; i++) {
                lastSmoothed = new decimal_js_1.Decimal(alpha).mul(historicalData[i].amount)
                    .plus(new decimal_js_1.Decimal(1 - alpha).mul(lastSmoothed));
            }
            // Project forward
            for (let i = 0; i < forecastYears * (period === ForecastPeriod.MONTHLY ? 12 : 1); i++) {
                results.push({
        ...
        case ForecastMethod.MOVING_AVERAGE:
            ...
            const average = lastValues.reduce((sum, d) => sum.plus(d.amount), new decimal_js_1.Decimal(0))
                .div(windowSize);
            for (let i = 0; i < forecastYears * (period === ForecastPeriod.MONTHLY ? 12 : 1); i++) {
                results.push({
                    period: getNextPeriod(historicalData[historicalData.length - 1].period, i + 1, period),
...
function calculateStdDev(values) {
    ...
    const mean = values.reduce((sum, v) => sum.plus(v), new decimal_js_1.Decimal(0)).div(n);
    const variance = values.reduce((sum, v) => {
        const diff = v.minus(mean);
        return sum.plus(diff.mul(diff));
    }, new decimal_js_1.Decimal(0)).div(n);
    return variance.sqrt();
}
/**
 * Helper to get next period date
 */
function getNextPeriod(lastPeriod, offset, period) {
 ...
 */
async function createRollingForecast(sequelize, params) {
    const transaction = await sequelize.transaction();
    try {
    ...
    `, {
        ...
        for (const period of forecastPeriods) {
            for (const budgetLine of budgetData.filter((b) => b.period === period)) {
                const actual = actuals.find((a) => a.account_id === budgetLine.account_id && a.period === period);
                forecastData.push({
 ...
 */
async function updateRollingForecast(sequelize, forecastId) {
    const transaction = await sequelize.transaction();
    try {
 ...
 */
async function generateSeasonalForecast(sequelize, accountId, years = 3, forecastPeriods = 12) {
  ...
  `, {
    ...
    historicalData.forEach((row) => {
        ...
        if (!monthlyAverages[month]) {
            monthlyAverages[month] = new decimal_js_1.Decimal(0);
            monthCounts[month] = 0;
        }
    ...
    let overallAverage = new decimal_js_1.Decimal(0);
    ...
    for (let month = 1; month <= 12; month++) {
        if (seasonalIndices[month]) {
            ...
            seasonalIndices[month] = new decimal_js_1.Decimal(1);
        }
    }
    // Calculate trend using linear regression on deseasonalized data
    const deseasonalized = historicalData.map((row) => ({
        period: row.period,
        value: new decimal_js_1.Decimal(row.amount).div(seasonalIndices[row.month])
    }));
    const n = deseasonalized.length;
    const sumX = deseasonalized.reduce((sum, _, i) => sum + i, 0);
    const sumY = deseasonalized.reduce((sum, d) => sum.plus(d.value), new decimal_js_1.Decimal(0));
    const sumXY = deseasonalized.reduce((sum, d, i) => sum.plus(new decimal_js_1.Decimal(i).mul(d.value)), new decimal_js_1.Decimal(0));
 ...
 */
async function createBudgetScenario(sequelize, params) {
    const transaction = await sequelize.transaction();
    try {
 ...
 */
async function compareScenarios(sequelize, budgetId, scenarioIds) {
  ...
  `, {
    ...
    const sensitivity = scenarios.map((s) => ({
        ...
        baseTotal: new decimal_js_1.Decimal(s.base_total || 0),
        adjustedTotal: new decimal_js_1.Decimal(s.adjusted_total || 0),
        totalImpact: new decimal_js_1.Decimal(s.total_impact || 0),
        impactPercentage: s.base_total > 0
            ? new decimal_js_1.Decimal(s.total_impact || 0).div(s.base_total).mul(100).toNumber()
            : 0
    }));
    // Calculate weighted average if probabilities provided
    const weightedAverage = sensitivity
        .filter(s => s.probability)
        .reduce((sum, s) => sum.plus(s.adjustedTotal.mul(s.probability)), new decimal_js_1.Decimal(0));
    return {
        budgetId,
 ...
 */
async function performWhatIfAnalysis(sequelize, budgetId, parameter, valueRange) {
    const results = [];
    // Get baseline
  ...
  `, {
    ...
    `, {
        ...
        const adjustedTotal = new decimal_js_1.Decimal(result.adjusted_total || 0);
 ...
 */
async function runMonteCarloSimulation(sequelize, budgetId, iterations = 1000, variabilityFactors) {
    // Get budget baseline
    const budgetData = await sequelize.query(`
    SELECT
      account_id,
  ...
  `, {
    ...
    for (let i = 0; i < iterations; i++) {
        ...
        for (const line of budgetData) {
            const factor = variabilityFactors.find(f => f.accountId === line.account_id);
            let multiplier = 1;
            if (factor) {
                ...
                switch (factor.distribution) {
                    ...
                    case 'uniform':
                        ...
                        break;
                    case 'triangular':
                        multiplier = generateTriangular(factor.min || 0.8, factor.max || 1.2, factor.mode || 1);
                        break;
                }
            }
            total = total.plus(new decimal_js_1.Decimal(line.base_amount).mul(multiplier));
        }
 ...
 */
function generateTriangular(min, max, mode) {
    const u = Math.random();
 ...
 */
async function optimizeResourceAllocation(sequelize, params) {
    const transaction = await sequelize.transaction();
    try {
        // Validate total budget
        const totalRequested = params.departments.reduce((sum, d) => sum.plus(d.requestedAmount), new decimal_js_1.Decimal(0));
    ...
    `, {
            replacements: {
                ...
                totalAllocated: allocations.reduce((sum, a) => sum.plus(a.allocated), new decimal_js_1.Decimal(0)).toString(),
      ...
      `, {
        ...
        return {
            ...
            summary: {
                ...
                totalAllocated: allocations.reduce((sum, a) => sum.plus(a.allocated), new decimal_js_1.Decimal(0)),
                totalRequested: totalRequested,
                utilizationRate: allocations.reduce((sum, a) => sum.plus(a.allocated), new decimal_js_1.Decimal(0))
 ...
 */
function allocateByROI(departments, totalBudget, constraints) {
    const allocations = [];
    let remainingBudget = new decimal_js_1.Decimal(totalBudget);
    // Sort by projected ROI
    const sorted = departments.sort((a, b) => (b.projectedROI || 0) - (a.projectedROI || 0));
    for (const dept of sorted) {
        let allocated = new decimal_js_1.Decimal(0);
        // Check minimum allocation constraint
        const minConstraint = constraints?.find(c => c.type === 'min_percentage' && c.departmentId === dept.departmentId);
        if (minConstraint) {
            allocated = totalBudget.mul(minConstraint.value / 100);
        }
        else if (dept.minAllocation) {
            allocated = new decimal_js_1.Decimal(dept.minAllocation);
        }
        // Try to allocate requested amount up to max
        const maxAllocation = dept.maxAllocation
            ? decimal_js_1.Decimal.min(dept.requestedAmount, dept.maxAllocation)
            : dept.requestedAmount;
        allocated = decimal_js_1.Decimal.min(maxAllocation, decimal_js_1.Decimal.max(allocated, dept.requestedAmount), remainingBudget);
        allocations.push({
 ...
 */
function allocateByMinimumCost(departments, totalBudget, constraints) {
    const allocations = [];
    let remainingBudget = new decimal_js_1.Decimal(totalBudget);
    for (const dept of departments) {
        const minAllocation = dept.minAllocation || new decimal_js_1.Decimal(0);
        const allocated = decimal_js_1.Decimal.min(minAllocation, remainingBudget);
        allocations.push({
            departmentId: dept.departmentId,
            departmentName: dept.departmentName,
            requested: dept.requestedAmount,
 ...
 */
function allocateBalanced(departments, totalBudget, constraints) {
    const allocations = [];
    // Calculate total weighted requests
    const totalWeightedRequests = departments.reduce((sum, d) => sum.plus(new decimal_js_1.Decimal(d.requestedAmount).mul(d.priority)), new decimal_js_1.Decimal(0));
    for (const dept of departments) {
        const weightedRequest = new decimal_js_1.Decimal(dept.requestedAmount).mul(dept.priority);
        ...
        if (dept.minAllocation) {
            allocated = decimal_js_1.Decimal.max(allocated, dept.minAllocation);
        }
        if (dept.maxAllocation) {
            allocated = decimal_js_1.Decimal.min(allocated, dept.maxAllocation);
        }
        allocations.push({
            departmentId: dept.departmentId,
            departmentName: dept.departmentName,
 ...
 */
async function getAllocationPlan(sequelize, allocationId) {
 ...
 */
async function trackBudgetUtilization(sequelize, budgetId, periodStart, periodEnd) {
  ...
  `, {
    ...
    return utilization.map((u) => ({
        ...
        allocatedAmount: new decimal_js_1.Decimal(u.allocated_amount),
        spentAmount: new decimal_js_1.Decimal(u.spent_amount),
        remainingAmount: new decimal_js_1.Decimal(u.remaining_amount),
        utilizationPercentage: u.utilization_percentage,
        isOverspent: u.is_overspent,
        overspentAmount: new decimal_js_1.Decimal(u.overspent_amount)
    }));
}
// ============================================================================
 ...
 */
async function createDriverBasedForecast(sequelize, budgetId, drivers) {
    const transaction = await sequelize.transaction();
    try {
 ...
 */
async function calculateDriverBasedForecast(sequelize, forecastModelId, driverValues) {
  ...
  `, {
    ...
    const forecasts = [];
    for (const driver of drivers) {
        const driverValue = driverValues[driver.driver_metric];
        if (driverValue === undefined)
            continue;
        let forecastedAmount = new decimal_js_1.Decimal(0);
        switch (driver.formula_type) {
            case 'linear':
                forecastedAmount = new decimal_js_1.Decimal(driverValue).mul(driver.coefficient || 1);
                break;
            case 'proportional':
                // Get baseline ratio
                const [baseline] = await sequelize.query(`
        ...
        `, {
                    replacements: {
                        driverMetric: driver.driver_metric,
                        accountId: driver.account_id
                    },
                    type: sequelize_1.QueryTypes.SELECT
                });
                if (baseline && baseline.ratio) {
                    forecastedAmount = new decimal_js_1.Decimal(driverValue).mul(baseline.ratio);
                }
                break;
            case 'custom':
                // Evaluate custom formula (simplified - would need proper parser in production)
                try {
                    const formula = driver.formula.replace(/\$driver/g, driverValue.toString());
                    forecastedAmount = new decimal_js_1.Decimal(eval(formula));
                }
                catch (error) {
 ...
 */
async function generateCashFlowForecast(sequelize, budgetId, periods = 12, paymentTerms = { receivables: 30, payables: 45 }) {
  ...
  `, {
    ...
    let cumulativeCash = new decimal_js_1.Decimal(0);
    const cashFlowForecast = cashFlowData.map((row) => {
        const netCashFlow = new decimal_js_1.Decimal(row.net_cash_flow || 0);
        cumulativeCash = cumulativeCash.plus(netCashFlow);
        return {
            period: row.period,
            totalInflows: new decimal_js_1.Decimal(row.total_inflows || 0),
            totalOutflows: new decimal_js_1.Decimal(row.total_outflows || 0),
            netCashFlow,
            cumulativeCash: cumulativeCash,
            daysOfCash: cumulativeCash.div(new decimal_js_1.Decimal(row.total_outflows || 1).div(30)).toNumber()
        };
    });
    return {
        budgetId,
        ...
        summary: {
            totalInflows: cashFlowForecast.reduce((sum, f) => sum.plus(f.totalInflows), new decimal_js_1.Decimal(0)),
            totalOutflows: cashFlowForecast.reduce((sum, f) => sum.plus(f.totalOutflows), new decimal_js_1.Decimal(0)),
 ...
 */
exports.BudgetingForecastingKit = {
    // Budget Management
    createBudget,
...
//# sourceMappingURL=budgeting-forecasting-kit.js.map