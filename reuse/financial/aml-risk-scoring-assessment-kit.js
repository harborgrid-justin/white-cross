...
exports.createRiskScore = createRiskScore;
exports.scoreToRiskTier = scoreToRiskTier;
exports.validateGeographicData = validateGeographicData;
exports.normalizeRiskScore = normalizeRiskScore;
exports.calculateCustomerProfileRisk = calculateCustomerProfileRisk;
exports.calculateCustomerTenureRisk = calculateCustomerTenureRisk;
exports.calculateBeneficialOwnershipRisk = calculateBeneficialOwnershipRisk;
exports.calculateBusinessActivityRisk = calculateBusinessActivityRisk;
exports.calculateCounterpartyGeographicRisk = calculateCounterpartyGeographicRisk;
exports.calculateTransactionAmountRisk = calculateTransactionAmountRisk;
exports.calculateTransactionFrequencyRisk = calculateTransactionFrequencyRisk;
exports.calculateTransactionTypeRisk = calculateTransactionTypeRisk;
exports.calculateTransactionTimingRisk = calculateTransactionTimingRisk;
exports.calculateSettlementSpeedRisk = calculateSettlementSpeedRisk;
exports.calculateTransactionPatternAnomalyRisk = calculateTransactionPatternAnomalyRisk;
exports.calculateProductInherentRisk = calculateProductInherentRisk;
exports.calculateProductUsageRisk = calculateProductUsageRisk;
exports.calculateProductControlAdequacy = calculateProductControlAdequacy;
exports.calculateJurisdictionRisk = calculateJurisdictionRisk;
exports.calculateHighRiskJurisdictionExposure = calculateHighRiskJurisdictionExposure;
exports.calculateSanctionRegimeExposure = calculateSanctionRegimeExposure;
exports.calculateChannelRisk = calculateChannelRisk;
exports.calculateThirdPartyRisk = calculateThirdPartyRisk;
exports.calculateDigitalChannelRisk = calculateDigitalChannelRisk;
exports.calculateCompositeRiskScore = calculateCompositeRiskScore;
exports.applyTemporalRiskModifier = applyTemporalRiskModifier;
exports.createRiskMatrix = createRiskMatrix;
exports.updateFactorWeights = updateFactorWeights;
exports.detectModelDrift = detectModelDrift;
exports.adaptWeightsToRecentActivity = adaptWeightsToRecentActivity;
exports.assignRiskTierWithControls = assignRiskTierWithControls;
exports.applyDynamicControlAdjustments = applyDynamicControlAdjustments;
exports.generateMitigationStrategies = generateMitigationStrategies;
exports.calculateMitigationEffectiveness = calculateMitigationEffectiveness;
exports.calculateInherentRisk = calculateInherentRisk;
exports.calculateResidualRisk = calculateResidualRisk;
exports.analyzeRiskReduction = analyzeRiskReduction;
exports.createRiskAppetiteFramework = createRiskAppetiteFramework;
exports.evaluateAppetiteAlignment = evaluateAppetiteAlignment;
exports.evaluatePortfolioVsAppetite = evaluatePortfolioVsAppetite;
exports.aggregateEnterpriseRisk = aggregateEnterpriseRisk;
exports.calculateConcentrationRisk = calculateConcentrationRisk;
exports.generateEnterpriseRiskReport = generateEnterpriseRiskReport;
exports.performPortfolioStressTest = performPortfolioStressTest;
exports.identifySystemicRiskFactors = identifySystemicRiskFactors;
/**
 * Risk category enumeration
 */
var RiskCategory;
(function (RiskCategory) {
    RiskCategory["CustomerProfile"] = "CUSTOMER_PROFILE";
    RiskCategory["TransactionBehavior"] = "TRANSACTION_BEHAVIOR";
    RiskCategory["ProductService"] = "PRODUCT_SERVICE";
    RiskCategory["Geographic"] = "GEOGRAPHIC";
    RiskCategory["ChannelDelivery"] = "CHANNEL_DELIVERY";
    RiskCategory["ComplianceHistory"] = "COMPLIANCE_HISTORY";
    RiskCategory["BusinessStructure"] = "BUSINESS_STRUCTURE";
 ...
 */
function createRiskScore(score) {
 ...
 */
function validateGeographicData(region) {
    const errors = [];
    ...
    if (region.corruptionIndex !== undefined &&
 ...
 */
function normalizeRiskScore(score, min, max) {
    if (max <= min) {
 ...
 */
function calculateCustomerProfileRisk(profile) {
    let score = 0;
    // Customer type risk
    const customerTypeRisk = {
 ...
 * @returns Risk score component (0-100)
 */
function calculateCustomerTenureRisk(incorporationDate) {
    const now = new Date();
 ...
 * @returns Risk score component (0-100)
 */
function calculateBeneficialOwnershipRisk(ownershipLayers, jurisdictionCount) {
    let score = 0;
    // Layer complexity
    if (ownershipLayers > 5)
        score += 30;
    else if (ownershipLayers > 3)
        score += 20;
 ...
 * @returns Risk score component (0-100)
 */
function calculateBusinessActivityRisk(activities) {
    ...
    for (const activity of activities) {
        if (highRiskActivities.has(activity))
            score += 40;
        else if (mediumRiskActivities.has(activity))
            score += 15;
    }
 ...
 * @returns Risk score component (0-100)
 */
function calculateCounterpartyGeographicRisk(counterpartyCountries, geographicRegions) {
    let totalRisk = 0;
 ...
 * @returns Risk score component (0-100)
 */
function calculateTransactionAmountRisk(amount, customerAverageTransaction) {
    if (customerAverageTransaction === 0)
        return 50;
 ...
 * @returns Risk score component (0-100)
 */
function calculateTransactionFrequencyRisk(transactionFrequency, historicalFrequency) {
 ...
 * @returns Risk score component (0-100)
 */
function calculateTransactionTypeRisk(transactionType) {
 ...
 * @returns Risk score component (0-100)
 */
function calculateTransactionTimingRisk(transactionDate) {
 ...
 * @returns Risk score component (0-100)
 */
function calculateSettlementSpeedRisk(settlementHours) {
    if (settlementHours < 1)
        return 70; // Instant/too fast
    if (settlementHours < 4)
        return 40;
    if (settlementHours < 24)
        return 20;
    if (settlementHours < 72)
        return 15;
    if (settlementHours > 30 * 24)
        return 65; // Unusually delayed
    return 10;
}
/**
 ...
 * @returns Risk score component (0-100)
 */
function calculateTransactionPatternAnomalyRisk(currentTransaction, historicalAverages) {
    let anomalyScore = 0;
    // Amount anomaly
    const amountZScore = (currentTransaction.amount - historicalAverages.avgAmount) /
 ...
 * @returns Risk score component (0-100)
 */
function calculateProductInherentRisk(product) {
    let score = 0;
    // Base risk by level
 ...
 * @returns Risk score component (0-100)
 */
function calculateProductUsageRisk(transactionVolume, avgTransactionSize, typicalMarketBehavior) {
    let score = 0;
    // Volume anomaly
    const [minVol, maxVol] = typicalMarketBehavior.typicalVolumeRange;
    if (transactionVolume > maxVol * 2)
        score += 30;
    else if (transactionVolume < minVol / 2)
        score += 20;
    // Size anomaly
    const [minSize, maxSize] = typicalMarketBehavior.typicalSizeRange;
    if (avgTransactionSize > maxSize * 2)
        score += 25;
    else if (avgTransactionSize < minSize / 2)
        score += 15;
    return Math.min(score, 100);
}
/**
 ...
 */
function calculateProductControlAdequacy(product, appliedControls) {
    const requiredControls = new Set(product.typicalAmlControls);
    const appliedSet = new Set(appliedControls);
    let score = 0;
    // Coverage gap
    const missingControls = product.typicalAmlControls.filter((c) => !appliedSet.has(c)).length;
 ...
 * @returns Risk score component (0-100)
 */
function calculateJurisdictionRisk(regions) {
    if (regions.length === 0)
        return 50;
    let totalRisk = 0;
    for (const region of regions) {
        ...
        if (region.financialCenterStatus)
            regionRisk += 15; // More scrutiny needed
 ...
 * @returns Risk score component (0-100)
 */
function calculateHighRiskJurisdictionExposure(jurisdictions, highRiskList) {
 ...
 * @returns Risk score component (0-100)
 */
function calculateSanctionRegimeExposure(exposedCountries, sanctionedCountries) {
 ...
 * @returns Risk score component (0-100)
 */
function calculateChannelRisk(channel) {
    const channelRisks = {
        'Online': 35,
        'Branch': 20,
        'Mobile': 45,
 ...
 * @returns Risk score component (0-100)
 */
function calculateThirdPartyRisk(thirdPartyName, riskRating, complianceTrack) {
    let score = riskRating * 0.6; // Weight third-party's own risk
    // Compliance track record
    if (complianceTrack === 'Poor')
        score += 25;
    else if (complianceTrack === 'Fair')
        score += 12;
    return Math.min(score, 100);
}
/**
 ...
 * @returns Risk score component (0-100)
 */
function calculateDigitalChannelRisk(isOnline, encryptionLevel, authenticationFactor) {
    if (!isOnline)
        return 15; // Offline is lower risk
    let score = 0;
    // Encryption
    const encryptionRisks = {
        None: 70,
        Basic: 45,
        Standard: 20,
        Enhanced: 5,
    ...
    // Authentication
    if (authenticationFactor < 1)
        score += 50;
    else if (authenticationFactor === 1)
        score += 25;
    else if (authenticationFactor === 2)
        score += 10;
    return Math.min(score, 100);
}
 ...
 */
function calculateCompositeRiskScore(componentScores, weights) {
 ...
 */
function applyTemporalRiskModifier(baseScore, recentAnomalies, monthsSinceLastReview) {
    ...
    if (monthsSinceLastReview > 12)
        adjustedScore += 10;
    else if (monthsSinceLastReview > 6)
        adjustedScore += 5;
 ...
 */
function createRiskMatrix(factors) {
 ...
 */
function updateFactorWeights(currentWeights, performanceMetrics) {
    // Normalize accuracies
    const totalAccuracy = performanceMetrics.customerProfileAccuracy +
    ...
    return {
        customerProfileWeight: (performanceMetrics.customerProfileAccuracy / totalAccuracy) * 0.3,
        transactionBehaviorWeight: (performanceMetrics.transactionBehaviorAccuracy / totalAccuracy) * 0.3,
        productServiceWeight: (performanceMetrics.productServiceAccuracy / totalAccuracy) * 0.18,
 ...
 */
function detectModelDrift(historicalAccuracy, recentAccuracy, threshold = 0.05) {
 ...
 */
function adaptWeightsToRecentActivity(baseWeights, recentTransactions) {
    if (recentTransactions.length === 0)
        return baseWeights;
    // Analyze transaction characteristics
    const cryptoTransactions = recentTransactions.filter((t) => t.transactionType === 'Crypto').length;
    const highAmountTransactions = recentTransactions.filter((t) => t.amount > 100000).length;
 ...
 */
function assignRiskTierWithControls(riskScore) {
 ...
 */
function applyDynamicControlAdjustments(baseControls, riskFactors) {
 ...
 */
function generateMitigationStrategies(riskScore, riskFactors, customerType) {
    const strategies = [];
 ...
 */
function calculateMitigationEffectiveness(baseRiskScore, appliedMitigations) {
 ...
 */
function calculateInherentRisk(profile, transactions, products) {
    const componentScores = {};
    componentScores[RiskCategory.CustomerProfile] = calculateCustomerProfileRisk(profile);
    // Average transaction risk
    if (transactions.length > 0) {
        const transactionRisks = transactions.map((t) => calculateTransactionTypeRisk(t.transactionType));
 ...
 * @returns Residual risk score
 */
function calculateResidualRisk(inherentRisk, appliedControls, controlEffectiveness = new Map()) {
 ...
 */
function analyzeRiskReduction(inherentRisk, residualRisk, acceptableResidualLevel) {
 ...
 */
function createRiskAppetiteFramework(riskTolerance) {
    const acceptableRiskTiers = riskTolerance > 70
        ? ['Low', 'Medium']
        : riskTolerance > 40
            ? ['Low']
            : [];
 ...
 */
function evaluateAppetiteAlignment(riskScore, appetite) {
 ...
 */
function evaluatePortfolioVsAppetite(customerRisks, appetite) {
    ...
    const avgRisk = customerRisks.reduce((a, b) => a + b, 0) / customerRisks.length;
    const portfolioRiskScore = createRiskScore(avgRisk);
    const withinAppetite = customerRisks.filter((r) => r <= appetite.maxAcceptableRiskScore).length;
 ...
 */
function aggregateEnterpriseRisk(customerAssessments, assessmentDate) {
    ...
    let totalScore = 0;
    const sortedByRisk = [...customerAssessments].sort((a, b) => b.overallRiskScore - a.overallRiskScore);
    ...
    const complianceStatus = criticalCount > 0
        ? 'Fail'
 ...
 */
function calculateConcentrationRisk(assessments, concentrationThreshold = 0.3) {
    ...
    if (customerConcentration.length > 0) {
        concernAreas.push(`Concentration risk: ${customerConcentration[0].customerId} represents ${(customerConcentration[0].proportion * 100).toFixed(1)}% of portfolio risk`);
    }
 ...
 */
function generateEnterpriseRiskReport(enterpriseView, appetite) {
 ...
 */
function performPortfolioStressTest(assessments, scenario) {
    ...
    const avgStressedRisk = (stressedRisks.reduce((a, b) => a + b, 0) +
        assessments
            .slice(affectedCount)
 ...
 */
function identifySystemicRiskFactors(assessments, timeWindow = 90) {
    ...
    affectedCount = recentAssessments.filter((a) => a.assessment.overallRiskScore > 50).length;
...
//# sourceMappingURL=aml-risk-scoring-assessment-kit.js.map