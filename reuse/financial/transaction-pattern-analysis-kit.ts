...
interface BehavioralProfile {
  ...
  typicalTransactionFrequency: number; // per day
...
/**
 * Machine learning pattern metadata
 */
 ...
 */
export function detectLinearProgressionPattern(
  ...
  let bestSequenceIndices: number[] = [];
 ...
 */
export function detectComplexSequencePattern(
  transactions: TransactionRecord[],
  patterns: string[] = ['layering', 'integration', 'smurfing']
): PatternDetectionResult {
 ...
 */
export function detectFractalPattern(
  transactions: TransactionRecord[],
  scales: number[] = [1, 7, 30],
  ...
  for (const scale of scales) {
    const scaledPatterns: number[] = [];
    const scaledMs = scale * msPerDay;
    const buckets: Map<number, TransactionRecord[]> = new Map();
...
export function detectContextualAnomalies(
  ...
  contextWindow: number = 7 // days
): AnomalyResult {
  const contextMs = contextWindow * 24 * 60 * 60 * 1000;
  const relevantTxns = recentTransactions.filter(
    t => Math.abs(t.timestamp - transaction.timestamp) <= contextMs && t.accountId === transaction.accountId
  );
...
export function generateTimeSeries(
  transactions: TransactionRecord[],
  intervalMinutes: number = 1440, // 1 day
  aggregateType: 'sum' | 'count' | 'avg' | 'max' | 'min' = 'sum'
): TimeSeriesPoint[] {
  if (!transactions || transactions.length === 0) {
    return [];
  }
...
export function seasonalDecomposition(
  series: TimeSeriesPoint[],
  seasonalPeriod: number = 7 // weekly
): { trend: TimeSeriesPoint[]; seasonal: TimeSeriesPoint[]; residual: TimeSeriesPoint[] } {
  ...
  // Seasonal component
  const seasonal: TimeSeriesPoint[] = Array(series.length).fill(null).map((_, i) => ({
    ...series[i],
    value: 0,
  }));
...
export function calculateAutocorrelation(
  ...
  const autocorr: Record<number, number> = {};
 ...
 */
export function differenceTimeSeries(
  series: TimeSeriesPoint[],
  order: number = 1
): TimeSeriesPoint[] {
  let current = [...series];
  ...
  for (let d = 0; d < order; d++) {
    const differenced: TimeSeriesPoint[] = [];
...
export function performKMeansClustering(
    ...
    const newAssignments = features.map(feature => {
      ...
      for (let c = 0; c < centroids.length; c++) {
        const dist = Math.sqrt(
          feature.reduce((acc, val, idx) => acc + Math.pow(val - centroids[c][idx], 2), 0)
        );
        if (dist < minDist) {
          minDist = dist;
          cluster = c;
  ...
  return transactions.map((t, idx) => {
    ...
    const distance = Math.sqrt(
      feature.reduce((acc, val, i) => acc + Math.pow(val - centroid[i], 2), 0)
    );
...
export function performDBSCANClustering(
  transactions: TransactionRecord[],
  epsilon: number = 100, // distance threshold
  minPoints: number = 3
): ClusterAssignment[] {
  ...
  function getNeighbors(idx: number): number[] {
    ...
    for (let i = 0; i < features.length; i++) {
      if (i !== idx) {
        const dist = Math.sqrt(
          features[idx].reduce((acc, val, j) => acc + Math.pow(val - features[i][j], 2), 0)
        );
        if (dist <= epsilon) {
          neighbors.push(i);
        }
  ...
  while (clusters.length > numClusters) {
    ...
    for (let i = 0; i < clusters.length; i++) {
      for (let j = i + 1; j < clusters.length; j++) {
        const dist = Math.sqrt(
          clusters[i].centroid.reduce((acc, val, k) => acc + Math.pow(val - clusters[j].centroid[k], 2), 0)
        );
        if (dist < minDist) {
          minDist = dist;
          mergeI = i;
 ...
 */
export function detectDormantAccountActivation(
  transactions: TransactionRecord[],
  dormancyThresholdDays: number = 180,
  reactivationAmountMultiplier: number = 2
): PatternDetectionResult {
  if (!transactions || transactions.length < 2) {
    return {
 ...
 */
export function analyzeActivitySeasonality(
  transactions: TransactionRecord[],
...
export function detectSequentialPatterns(
  ...
  maxPatternLength: number = 4
): Array<{ pattern: string[]; support: number; confidence: number }> {
  ...
  // Filter by support
  const result: Array<{ pattern: string[]; support: number; confidence: number }> = [];
 ...
 */
export function detectActivityBursts(
  transactions: TransactionRecord[],
  baselineWindowDays: number = 30,
  burstMultiplier: number = 3,
  detectionWindowDays: number = 7
): PatternDetectionResult {
  if (!transactions || transactions.length < 10) {
    return {
...
export function detectCircularTransactions(
  ...
  circulationThresholdPercent: number = 80
): PatternDetectionResult {
    ...
    for (const [to, amount] of toMap.entries()) {
      if (to !== 'external' && graph.has(to)) {
        ...
        if (reverseAmount > 0) {
          ...
          circularTxns.push(...relevant.filter(
            t => (t.originatorInfo.accountNumber === from && t.beneficiaryInfo.accountNumber === to) ||
                 (t.originatorInfo.accountNumber === to && t.beneficiaryInfo.accountNumber === from)
 ...
 */
export function detectMultiLegCircularPaths(
  transactions: TransactionRecord[],
  ...
  const graph: Map<string, Array<{ to: string; txn: TransactionRecord }>> = new Map();
  ...
  // DFS to find cycles
  function findCycles(start: string, path: string[], visited: Set<string>, cycle: TransactionRecord[]): void {
    if (path.length === 0) {
      visited.clear();
    }
 ...
 */
export function detectCheckKiting(
  transactions: TransactionRecord[],
  checkClearanceDays: number = 2,
  lookbackDays: number = 30
): PatternDetectionResult {
  if (!transactions || transactions.length < 4) {
    return {
 ...
 */
export function detectACHKiting(
  transactions: TransactionRecord[],
  lookbackDays: number = 30,
  rapidityThresholdHours: number = 24,
  ...
  const relevant = transactions.filter(
    t => (now - t.timestamp) <= lookbackMs && (t.transactionType === 'ach' || t.transactionType === 'transfer')
  );
  ...
  for (let i = 0; i < relevant.length - 1; i++) {
    ...
    for (let j = i + 1; j < relevant.length; j++) {
      ...
      if (
        Math.abs(txn1.amount - txn2.amount) < txn1.amount * 0.05 &&
 ...
 */
export function detectPassThroughAccounts(
  transactions: TransactionRecord[],
  lookbackDays: number = 30,
  passThresholdPercent: number = 80,
  ...
  let passedAmount = 0;
  const passTxns: string[] = [];

  // Group by account
  const byAccount: Map<string, TransactionRecord[]> = new Map();
  for (const txn of relevant) {
  ...
  for (const txns of byAccount.values()) {
    ...
    for (const inTxn of inbound) {
      const closeExits = outbound.filter(
        t => (t.timestamp - inTxn.timestamp) >= 0 && (t.timestamp - inTxn.timestamp) <= timeWindowMs
      );
 ...
 */
export function detectShellAccountPatterns(
  transactions: TransactionRecord[],
  profile: BehavioralProfile,
  lookbackDays: number = 90
): PatternDetectionResult {
...
export function detectFrequencyAnomalies(
  transactions: TransactionRecord[],
  lookbackDays: number = 30,
  frequencyThresholdMultiplier: number = 2.5
): AnomalyResult[] {
  if (!transactions || transactions.length < 5) {
    return [];
  }
  ...
  const dailyFrequencies: Record<string, number> = {};
  for (const txn of relevantTxns) {
 ...
 */
export function analyzePeerGroupComparison(
  customerProfile: BehavioralProfile,
  peerProfiles: BehavioralProfile[],
  ...
  // Outlier detection
  let outlierStatus: 'not_outlier' | 'mild_outlier' | 'moderate_outlier' | 'severe_outlier' = 'not_outlier';
 ...
 */
export function detectMultivariateOutliers(
  transactions: TransactionRecord[],
  threshold: number = 3,
  lookbackDays: number = 90
): AnomalyResult[] {
 ...
 */
export function detectQuantileOutliers(
  transactions: TransactionRecord[],
  lowerQuantile: number = 0.25,
  upperQuantile: number = 0.75,
...
export function detectMLPatterns(
  ...
  const clusterProfiles: Map<number, any> = new Map();
  ...
  for (const [clusterId, profile] of clusterProfiles.entries()) {
    ...
    const amountVariance = profile.amounts.length > 1
      ? Math.sqrt(
          profile.amounts.reduce((acc, val) => {
 ...
 */
export function detectEnsemblePatterns(
  transactions: TransactionRecord[],
  numTrees: number = 10,
  ...
  const results: PatternDetectionResult[] = [];
  const voteMatrix: Map<string, number> = new Map();
 ...
 */
export function rankAnomaliesByRiskScore(
  anomalies: AnomalyResult[],
  weightFactors: Record<string, number> = {}
): Array<AnomalyResult & { finalRiskScore: number }> {
  const defaults = {
    anomalyScore: 0.7,
    zScore: 0.2,
    frequencyWeight: 0.1,
  ...
  return anomalies
    .map(anomaly => ({
      ...anomaly,
      finalRiskScore:
        (Math.min(Math.abs(anomaly.zScore) / 4, 1) * weights.zScore +
 ...
 */
export function consolidateDetectionResults(
  ...
  const highRiskTransactions = new Set<string>();
 ...
 */
export function generatePatternComparisonMatrix(
  detectorResults: Map<string, PatternDetectionResult[]>,
 ...
 */
export function calculatePatternPersistence(
  historicalPatterns: Array<{ timestamp: Date; pattern: PatternDetectionResult }>,
  patternType: string,
  timeWindowDays: number = 30
): number {
  ...
  const relevantPatterns = historicalPatterns.filter(
    hp => hp.pattern.patternType === patternType && (now - hp.timestamp.getTime()) <= windowMs
  );
