...
exports.generateFilingForm = generateFilingForm;
exports.createFilingFromTemplate = createFilingFromTemplate;
exports.generateFilingPreview = generateFilingPreview;
exports.extractFilingData = extractFilingData;
exports.validateFilingData = validateFilingData;
exports.validateFieldValue = validateFieldValue;
exports.validateFilingCompleteness = validateFilingCompleteness;
exports.submitFilingElectronically = submitFilingElectronically;
exports.retryFailedSubmission = retryFailedSubmission;
exports.trackSubmissionProgress = trackSubmissionProgress;
exports.cancelSubmission = cancelSubmission;
exports.getFilingStatus = getFilingStatus;
exports.updateFilingStatus = updateFilingStatus;
exports.trackMultipleFilings = trackMultipleFilings;
exports.getStatusHistory = getStatusHistory;
exports.processAcknowledgment = processAcknowledgment;
exports.extractAcknowledgmentData = extractAcknowledgmentData;
exports.handleRejection = handleRejection;
exports.createAmendment = createAmendment;
exports.submitAmendment = submitAmendment;
exports.trackAmendmentStatus = trackAmendmentStatus;
exports.generateCorrectionForm = generateCorrectionForm;
exports.calculateDeadlines = calculateDeadlines;
exports.getUpcomingDeadlines = getUpcomingDeadlines;
exports.createRegulatoryCalendarEntry = createRegulatoryCalendarEntry;
exports.batchSubmitFilings = batchSubmitFilings;
exports.batchValidateFilings = batchValidateFilings;
exports.generateBatchReport = generateBatchReport;
exports.convertToXML = convertToXML;
exports.convertToJSON = convertToJSON;
exports.convertToCSV = convertToCSV;
exports.signFiling = signFiling;
exports.verifySignature = verifySignature;
exports.generateSignatureToken = generateSignatureToken;
exports.generateFilingAnalytics = generateFilingAnalytics;
exports.getComplianceMetrics = getComplianceMetrics;
exports.validateMultiJurisdictionFiling = validateMultiJurisdictionFiling;
exports.mapFilingToJurisdiction = mapFilingToJurisdiction;
exports.getJurisdictionRequirements = getJurisdictionRequirements;
exports.generateRejectionReport = generateRejectionReport;
exports.createResubmissionWorkflow = createResubmissionWorkflow;
 ...
 */
function generateFilingForm(jurisdiction, filingType, templateVersion = '1.0') {
 ...
 */
function createFilingFromTemplate(template, initialData = {}) {
 ...
 */
function generateFilingPreview(filing, format = 'text') {
 ...
 */
function extractFilingData(filing, fieldsToExtract) {
 ...
 */
function validateFilingData(filing, template) {
    if (!filing || !template) {
    ...
    const completionPercentage = (validatedFieldCount / (template.requiredFields.length + template.optionalFields.length)) * 100;
    return {
 ...
 */
function validateFieldValue(field, value, rule) {
    const errors = [];
 ...
 */
function validateFilingCompleteness(filing, template) {
    if (!filing || !template) {
        throw new Error('Filing and template are required');
    }
    const missingFields = [];
 ...
 */
async function submitFilingElectronically(filing, endpoint, credentials) {
 ...
 */
async function retryFailedSubmission(filing, endpoint, credentials, maxRetries = 3) {
 ...
 */
async function trackSubmissionProgress(submissionId, endpoint) {
 ...
 */
async function cancelSubmission(submissionId, endpoint, reason) {
 ...
 */
function getFilingStatus(filingId, statusRepository) {
 ...
 */
function updateFilingStatus(filingId, newStatus, statusRepository, details) {
 ...
 */
function trackMultipleFilings(filingIds, statusRepository) {
 ...
 */
function getStatusHistory(filingId, historyRepository) {
    if (!filingId || filingId.length === 0) {
        throw new Error('Filing ID is required');
    }
 ...
 */
function processAcknowledgment(acknowledgmentData, filingId) {
 ...
 */
function extractAcknowledgmentData(acknowledgment) {
    if (!acknowledgment) {
 ...
 */
function handleRejection(rejectionData, originalFilingId) {
    ...
    if (rejectionData.reasons && Array.isArray(rejectionData.reasons)) {
        rejectionData.reasons.forEach((reason) => {
            reasons.push({
    ...
    const requiredCorrections = rejectionData.corrections || {};
    return {
        rejectionId,
 ...
 */
function createAmendment(originalFilingId, changes, reason) {
 ...
 */
async function submitAmendment(amendment, endpoint, credentials) {
 ...
 */
function trackAmendmentStatus(amendmentId, amendmentRepository) {
 ...
 */
function generateCorrectionForm(rejection, originalFiling) {
    ...
    rejection.reasons.forEach((reason) => {
        if (reason.affectedFields) {
            reason.affectedFields.forEach((field) => {
                correctionForm.rejectedFields.add(field);
            });
        }
    });
    ...
    correctionForm.rejectedFields = Array.from(correctionForm.rejectedFields);
 ...
 */
function calculateDeadlines(jurisdiction, filingType, referenceDate = new Date()) {
    ...
    const dueDate = (0, date_fns_1.addDays)(referenceDate, primaryRule.daysFromEvent);
    return {
        dueDate,
        warningDate: (0, date_fns_1.addDays)(dueDate, -14), // Warning 2 weeks before
        criticalDate: (0, date_fns_1.addDays)(dueDate, -3), // Critical 3 days before
    };
}
/**
 ...
 */
function getUpcomingDeadlines(jurisdiction, daysAhead = 90, eventRepository) {
    if (!jurisdiction) {
        throw new Error('Jurisdiction is required');
    }
    ...
    return events
        ...
        return (0, date_fns_1.isBefore)(eventDate, futureDate) && (0, date_fns_1.isAfter)(eventDate, now);
    })
 ...
 */
function createRegulatoryCalendarEntry(event, eventRepository) {
 ...
 */
async function batchSubmitFilings(batch, endpoint, credentials) {
 ...
 */
function batchValidateFilings(batch, template) {
 ...
 */
function convertToCSV(filings, fields) {
 ...
 */
function signFiling(filing, signingKey, signedBy) {
    if (!filing || !signingKey || !signedBy) {
        throw new Error('Filing, signing key, and signer identity are required');
    }
    const filingHash = (0, crypto_1.createHash)('sha256').update(JSON.stringify(filing)).digest('hex');
    try {
 ...
 */
function verifySignature(signature, filing, publicKey) {
 ...
 */
function generateSignatureToken(filing, secret) {
    if (!filing || !secret) {
    ...
    const hash = (0, crypto_1.createHash)('sha256')
        ...
        .digest('hex');
    const expiresAt = (0, date_fns_1.addDays)(new Date(), 30); // Token valid for 30 days
    return {
        token: hash,
        expiresAt,
 ...
 */
function generateFilingAnalytics(period, filingRepository) {
    ...
    const periodEnd = (0, date_fns_1.addDays)(periodStart, 30);
    const periodFilings = filings.filter((f) => {
        const subDate = new Date(f.submissionDate);
        return (0, date_fns_1.isBefore)(subDate, periodEnd) && (0, date_fns_1.isAfter)(subDate, periodStart);
    });
    return {
        period,
 ...
 */
function getComplianceMetrics(filingRepository, statusRepository) {
    const filings = Array.from(filingRepository.values());
    ...
    const onTimeCount = filings.filter((f) => (0, date_fns_1.isBefore)(new Date(f.submissionDate), new Date(f.dueDate))).length;
    return {
        ...
        averageCorrectionTime: corrections.length > 0
            ? corrections.reduce((sum, c) => {
                const correctionDays = Math.floor((new Date(c.correctedAt).getTime() - new Date(c.createdAt).getTime()) / 86400000);
 ...
 */
function validateMultiJurisdictionFiling(filing, jurisdictions, requirementsRepository) {
 ...
 */
function mapFilingToJurisdiction(filing, sourceJurisdiction, targetJurisdiction, requirementsRepository) {
 ...
 */
function getJurisdictionRequirements(jurisdiction, filingType, requirementsRepository) {
    if (!jurisdiction || !filingType) {
        throw new Error('Jurisdiction and filing type are required');
    }
    const requirements = requirementsRepository.get(jurisdiction);
 ...
 */
function generateRejectionReport(rejection) {
    if (!rejection) {
 ...
 */
function createResubmissionWorkflow(rejectionId, originalFilingId, maxRetries = 3) {
 ...
 */
function getSubmissionHistory(filingId, historyRepository) {
    if (!filingId || filingId.length === 0) {
        throw new Error('Filing ID is required');
    }
    const history = historyRepository.get(filingId) || [];
 ...
 */
function generateRequiredFields(jurisdiction, filingType) {
 ...
 */
function generateOptionalFields(jurisdiction, filingType) {
    return ['notes', 'attachments', 'metadata', 'additionalComments'];
}
/**
 * Generates validation rules for filing
 */
function generateValidationRules(jurisdiction, filingType) {
    return {
        entityName: {
            type: 'string',
            required: true,
 ...
 */
function generateDeadlineRules(jurisdiction, filingType) {
    return [
        {
 ...
 */
function getTypeSpecificRequiredFields(jurisdiction, filingType) {
    const typeMap = {
 ...
 */
function calculateDueDate(template) {
    const now = new Date();
    const deadlineRule = template.deadlineRules[0];
    return deadlineRule ? (0, date_fns_1.addDays)(now, deadlineRule.daysFromEvent) : (0, date_fns_1.addDays)(now, 90);
}
/**
 * Initializes template structure
 */
function initializeTemplateStructure(template) {
    const structure = {};
 ...
 */
function generateTextPreview(filing) {
 ...
 */
function generateHtmlPreview(filing) {
 ...
 */
async function secureTransmit(endpoint, payload, credentials) {
    // Simulate secure transmission - in production would use actual HTTP/SFTP
 ...
 */
async function querySubmissionStatus(submissionId, endpoint) {
 ...
 */
async function performCancellation(submissionId, endpoint, reason) {
 ...
 */
function getDeadlineRulesForFiling(jurisdiction, filingType) {
    return [
        {
            ruleId: 'PRIMARY',
...
//# sourceMappingURL=regulatory-filing-submission-kit.js.map