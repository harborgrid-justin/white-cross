...
/**
 * Production-ready Sequelize model for Ceremony
 *
 * Features:
...
export const createCeremonyModel = (sequelize: Sequelize) => {
  ...
  Ceremony.init(
    ...
    },
    {
      ...
      hooks: {
        beforeCreate: async (record: Ceremony, options: any) => {
        ...
        afterCreate: async (record: Ceremony, options: any) => {
          console.log(`[AUDIT] Ceremony created: ${record.id}`);
        },
        beforeUpdate: async (record: Ceremony, options: any) => {
        ...
        afterUpdate: async (record: Ceremony, options: any) => {
          console.log(`[AUDIT] Ceremony updated: ${record.id}`);
        },
        beforeDestroy: async (record: Ceremony, options: any) => {
          if (options.transaction) {
            await sequelize.query(
              `INSERT INTO audit_logs (action, table_name, record_id, user_id, data, created_at)
               VALUES (:action, :tableName, :recordId, :userId, :data, NOW())`,
        ...
        afterDestroy: async (record: Ceremony, options: any) => {
          console.log(`[AUDIT] Ceremony deleted: ${record.id}`);
        },
      },
      scopes: {
        defaultScope: {
