/**
 * LOC: EDU-COMP-DOWN-FUNDRAISING-PLATFORMS
 * File: /reuse/education/composites/downstream/fundraising-platforms.ts
 * Purpose: Production-grade composite for fundraising-platforms
 */
...
/**
 * Production-ready Sequelize model for FundraisingPlatformsRecord
 *
 * Features:
 ...
 */
export const createFundraisingPlatformsRecordModel = (sequelize: Sequelize) => {
  class FundraisingPlatformsRecord extends Model {
    public id!: string;
  ...
  FundraisingPlatformsRecord.init(
    {
      id: {
        type: DataTypes.UUID,
    ...
    {
      sequelize,
      tableName: 'fundraising_platforms_records',
      timestamps: true,
      paranoid: true,
      underscored: true,
      indexes: [
      ...
      hooks: {
        beforeCreate: async (record: FundraisingPlatformsRecord, options: any) => {
          ...
          if (options.transaction) {
            await sequelize.query(
              ...
              {
                replacements: {
                  action: 'CREATE_FUNDRAISINGPLATFORMSRECORD',
                  tableName: 'fundraising_platforms_records',
                  recordId: record.id,
        ...
        afterCreate: async (record: FundraisingPlatformsRecord, options: any) => {
          console.log(`[AUDIT] FundraisingPlatformsRecord created: ${record.id}`);
        },
        beforeUpdate: async (record: FundraisingPlatformsRecord, options: any) => {
          ...
          if (changed && options.transaction) {
            await sequelize.query(
              ...
              {
                replacements: {
                  action: 'UPDATE_FUNDRAISINGPLATFORMSRECORD',
                  tableName: 'fundraising_platforms_records',
                  recordId: record.id,
                  userId: options.userId || 'system',
        ...
        afterUpdate: async (record: FundraisingPlatformsRecord, options: any) => {
          console.log(`[AUDIT] FundraisingPlatformsRecord updated: ${record.id}`);
        },
        beforeDestroy: async (record: FundraisingPlatformsRecord, options: any) => {
          if (options.transaction) {
            await sequelize.query(
              `INSERT INTO audit_logs (action, table_name, record_id, user_id, data, created_at)
               VALUES (:action, :tableName, :recordId, :userId, :data, NOW())`,
              {
                replacements: {
                  action: 'DELETE_FUNDRAISINGPLATFORMSRECORD',
                  tableName: 'fundraising_platforms_records',
                  recordId: record.id,
                  userId: options.userId || 'system',
                  data: JSON.stringify(record.toJSON()),
        ...
        afterDestroy: async (record: FundraisingPlatformsRecord, options: any) => {
          console.log(`[AUDIT] FundraisingPlatformsRecord deleted: ${record.id}`);
        },
      },
      scopes: {
        defaultScope: {
  ...
  return FundraisingPlatformsRecord;
};
...
export class FundraisingPlatformsService {
  private readonly logger = new Logger(FundraisingPlatformsService.name);
...
export default FundraisingPlatformsService;
