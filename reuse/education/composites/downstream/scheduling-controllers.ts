...
/**
 * Sequelize model for Term Schedules.
 *
 * @swagger
 * @openapi
 * components:
 ...
 * @returns {Model} TermSchedule model
 */
  ...
  TermSchedule.init(
    {
      ...
      scheduleType: {
        type: DataTypes.ENUM('regular', 'summer', 'winter', 'special'),
        allowNull: false,
        defaultValue: 'regular',
        comment: 'Schedule type',
      },
      publicationStatus: {
        type: DataTypes.ENUM('draft', 'preview', 'published', 'archived'),
        allowNull: false,
        defaultValue: 'draft',
...
/**
 * Sequelize model for Schedule Conflicts.
 *
 * @param {Sequelize} sequelize - Sequelize instance
 * @returns {Model} ScheduleConflict model
 */
export const createScheduleConflictModel = (sequelize: Sequelize) => {
  class ScheduleConflict extends Model {
    ...
    public conflictData!: Record<string, any>;
  ...
  ScheduleConflict.init(
    {
      ...
      conflictType: {
        type: DataTypes.ENUM('room', 'instructor', 'time', 'student'),
        allowNull: false,
        comment: 'Type of conflict',
      },
      severity: {
        type: DataTypes.ENUM('low', 'medium', 'high', 'critical'),
        allowNull: false,
        comment: 'Conflict severity',
      },
      conflictData: {
        type: DataTypes.JSON,
        allowNull: false,
        defaultValue: {},
        comment: 'Conflict details',
      },
      resolvedAt: {
        type: DataTypes.DATE,
        allowNull: true,
