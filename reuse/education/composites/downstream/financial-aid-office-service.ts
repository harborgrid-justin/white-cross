/**
 * LOC: EDU-COMP-DOWN-FINANCIAL-AID-OFFICE-CONTROLLERS
 * File: /reuse/education/composites/downstream/financial-aid-office-controllers.ts
 * Purpose: Production-grade composite for financial-aid-office-controllers
 */
...
/**
 * Production-ready Sequelize model for FinancialAidApplication
 *
 * Features:
 ...
 */
export const createFinancialAidApplicationModel = (sequelize: Sequelize) => {
  ...
  FinancialAidApplication.init(
    ...
    {
      sequelize,
      tableName: 'financial_aid_applications',
      timestamps: true,
      paranoid: true,
      underscored: true,
      indexes: [
      ...
      hooks: {
        beforeCreate: async (record: FinancialAidApplication, options: any) => {
          ...
          if (options.transaction) {
            await sequelize.query(
              ...
              {
                replacements: {
                  action: 'CREATE_FINANCIALAIDAPPLICATION',
                  tableName: 'financial_aid_applications',
                  recordId: record.id,
        ...
        afterCreate: async (record: FinancialAidApplication, options: any) => {
          console.log(`[AUDIT] FinancialAidApplication created: ${record.id}`);
        },
        beforeUpdate: async (record: FinancialAidApplication, options: any) => {
          ...
          if (changed && options.transaction) {
            await sequelize.query(
              ...
              {
                replacements: {
                  action: 'UPDATE_FINANCIALAIDAPPLICATION',
                  tableName: 'financial_aid_applications',
                  recordId: record.id,
                  userId: options.userId || 'system',
        ...
        afterUpdate: async (record: FinancialAidApplication, options: any) => {
          console.log(`[AUDIT] FinancialAidApplication updated: ${record.id}`);
        },
        beforeDestroy: async (record: FinancialAidApplication, options: any) => {
          if (options.transaction) {
            await sequelize.query(
              `INSERT INTO audit_logs (action, table_name, record_id, user_id, data, created_at)
               VALUES (:action, :tableName, :recordId, :userId, :data, NOW())`,
              {
                replacements: {
                  action: 'DELETE_FINANCIALAIDAPPLICATION',
                  tableName: 'financial_aid_applications',
                  recordId: record.id,
                  userId: options.userId || 'system',
                  data: JSON.stringify(record.toJSON()),
        ...
        afterDestroy: async (record: FinancialAidApplication, options: any) => {
          console.log(`[AUDIT] FinancialAidApplication deleted: ${record.id}`);
        },
      },
      scopes: {
        defaultScope: {
...
/**
 * Production-ready Sequelize model for AwardPackage
 *
 * Features:
 * - Lifecycle hooks for FERPA/HIPAA compliance auditing
 * - Comprehensive validations with custom validators
 ...
 * - Optimized indexes (simple and compound)
 */
export const createAwardPackageModel = (sequelize: Sequelize) => {
  ...
  AwardPackage.init(
    ...
    },
    {
      ...
      hooks: {
        beforeCreate: async (record: AwardPackage, options: any) => {
          // Audit logging for FERPA/HIPAA compliance
          if (options.transaction) {
            await sequelize.query(
              `INSERT INTO audit_logs (action, table_name, record_id, user_id, data, created_at)
        ...
        afterCreate: async (record: AwardPackage, options: any) => {
          console.log(`[AUDIT] AwardPackage created: ${record.id}`);
        },
        beforeUpdate: async (record: AwardPackage, options: any) => {
          const changed = record.changed();
          if (changed && options.transaction) {
            await sequelize.query(
              `INSERT INTO audit_logs (action, table_name, record_id, user_id, data, created_at)
        ...
        afterUpdate: async (record: AwardPackage, options: any) => {
          console.log(`[AUDIT] AwardPackage updated: ${record.id}`);
        },
        beforeDestroy: async (record: AwardPackage, options: any) => {
          if (options.transaction) {
            await sequelize.query(
              `INSERT INTO audit_logs (action, table_name, record_id, user_id, data, created_at)
               VALUES (:action, :tableName, :recordId, :userId, :data, NOW())`,
        ...
        afterDestroy: async (record: AwardPackage, options: any) => {
          console.log(`[AUDIT] AwardPackage deleted: ${record.id}`);
        },
      },
      scopes: {
        defaultScope: {
...
export class FinancialAidOfficeServiceService {
  private readonly logger = new Logger(FinancialAidOfficeServiceService.name);
...
export default FinancialAidOfficeServiceService;
