/**
 * LOC: EDU-COMP-DOWNSTREAM-RES-020
 * File: /reuse/education/composites/downstream/residential-life-services.ts
 *
...
/**
 * Production-grade ResidentialLifeServicesComposite for Ellucian SIS competitors.
 */
...
/**
 * Production-ready Sequelize model for ResidentialLifeServicesRecord
 *
 * Features:
 ...
 */
export const createResidentialLifeServicesRecordModel = (sequelize: Sequelize) => {
  class ResidentialLifeServicesRecord extends Model {
    public id!: string;
  ...
  ResidentialLifeServicesRecord.init(
    {
      id: {
        type: DataTypes.UUID,
    ...
    {
      sequelize,
      tableName: 'residential_life_services_records',
      timestamps: true,
      paranoid: true,
      underscored: true,
      indexes: [
      ...
      hooks: {
        beforeCreate: async (record: ResidentialLifeServicesRecord, options: any) => {
          ...
          if (options.transaction) {
            await sequelize.query(
              ...
              {
                replacements: {
                  action: 'CREATE_RESIDENTIALLIFESERVICESRECORD',
                  tableName: 'residential_life_services_records',
                  recordId: record.id,
        ...
        afterCreate: async (record: ResidentialLifeServicesRecord, options: any) => {
          console.log(`[AUDIT] ResidentialLifeServicesRecord created: ${record.id}`);
        },
        beforeUpdate: async (record: ResidentialLifeServicesRecord, options: any) => {
          ...
          if (changed && options.transaction) {
            await sequelize.query(
              ...
              {
                replacements: {
                  action: 'UPDATE_RESIDENTIALLIFESERVICESRECORD',
                  tableName: 'residential_life_services_records',
                  recordId: record.id,
                  userId: options.userId || 'system',
        ...
        afterUpdate: async (record: ResidentialLifeServicesRecord, options: any) => {
          console.log(`[AUDIT] ResidentialLifeServicesRecord updated: ${record.id}`);
        },
        beforeDestroy: async (record: ResidentialLifeServicesRecord, options: any) => {
          if (options.transaction) {
            await sequelize.query(
              `INSERT INTO audit_logs (action, table_name, record_id, user_id, data, created_at)
               VALUES (:action, :tableName, :recordId, :userId, :data, NOW())`,
              {
                replacements: {
                  action: 'DELETE_RESIDENTIALLIFESERVICESRECORD',
                  tableName: 'residential_life_services_records',
                  recordId: record.id,
                  userId: options.userId || 'system',
                  data: JSON.stringify(record.toJSON()),
        ...
        afterDestroy: async (record: ResidentialLifeServicesRecord, options: any) => {
          console.log(`[AUDIT] ResidentialLifeServicesRecord deleted: ${record.id}`);
        },
      },
      scopes: {
        defaultScope: {
  ...
  return ResidentialLifeServicesRecord;
};
...
export class ResidentialLifeServicesComposite {
  private readonly logger = new Logger(ResidentialLifeServicesComposite.name);
...
export default ResidentialLifeServicesComposite;
