...
/**
 * Production-ready Sequelize model for RegistrarOfficeControllersRecord
 * Features: lifecycle hooks, validations, scopes, virtual attributes, paranoid mode, indexes
 */
export const createRegistrarOfficeControllersRecordModel = (sequelize: Sequelize) => {
  class RegistrarOfficeControllersRecord extends Model {
    public id!: string;
  ...
  RegistrarOfficeControllersRecord.init(
    {
      id: {
        type: DataTypes.UUID,
    ...
    {
      sequelize,
      tableName: 'registrar_office_controllers_records',
      timestamps: true,
      paranoid: true,
      underscored: true,
      indexes: [
      ...
      hooks: {
        beforeCreate: async (record: RegistrarOfficeControllersRecord, options: any) => {
          console.log(`[AUDIT] Creating RegistrarOfficeControllersRecord: ${record.id}`);
        },
        afterCreate: async (record: RegistrarOfficeControllersRecord, options: any) => {
          console.log(`[AUDIT] RegistrarOfficeControllersRecord created: ${record.id}`);
        },
        beforeUpdate: async (record: RegistrarOfficeControllersRecord, options: any) => {
          console.log(`[AUDIT] Updating RegistrarOfficeControllersRecord: ${record.id}`);
        },
        afterUpdate: async (record: RegistrarOfficeControllersRecord, options: any) => {
          console.log(`[AUDIT] RegistrarOfficeControllersRecord updated: ${record.id}`);
        },
        beforeDestroy: async (record: RegistrarOfficeControllersRecord, options: any) => {
          console.log(`[AUDIT] Deleting RegistrarOfficeControllersRecord: ${record.id}`);
        },
        afterDestroy: async (record: RegistrarOfficeControllersRecord, options: any) => {
          console.log(`[AUDIT] RegistrarOfficeControllersRecord deleted: ${record.id}`);
        },
      },
      scopes: {
  ...
  return RegistrarOfficeControllersRecord;
};
...
export default RegistrarOfficeControllersComposite;
