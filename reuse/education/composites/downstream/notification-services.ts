import { Injectable, Scope, Logger, Inject, BadRequestException, InternalServerErrorException } from '@nestjs/common';
import { ConfigService } from '@nestjs/config';
import { Sequelize, Model, DataTypes, Transaction } from 'sequelize';
import { DATABASE_CONNECTION } from './common/tokens/database.tokens';
import { EducationException, ValidationException } from './common/exceptions/education.exceptions';
...
/**
 * Production-ready Sequelize model for NotificationServicesRecord
 *
 * Features:
 ...
 */
export const createNotificationServicesRecordModel = (sequelize: Sequelize) => {
  class NotificationServicesRecord extends Model {
    public id!: string;
  ...
  NotificationServicesRecord.init(
    {
      id: {
        type: DataTypes.UUID,
    ...
    {
      sequelize,
      tableName: 'notification_services_records',
      timestamps: true,
      paranoid: true,
      underscored: true,
      indexes: [
      ...
      hooks: {
        beforeCreate: async (record: NotificationServicesRecord, options: any) => {
          ...
          if (options.transaction) {
            await sequelize.query(
              ...
              {
                replacements: {
                  action: 'CREATE_NOTIFICATIONSERVICESRECORD',
                  tableName: 'notification_services_records',
                  recordId: record.id,
        ...
        afterCreate: async (record: NotificationServicesRecord, options: any) => {
          console.log(`[AUDIT] NotificationServicesRecord created: ${record.id}`);
        },
        beforeUpdate: async (record: NotificationServicesRecord, options: any) => {
          ...
          if (changed && options.transaction) {
            await sequelize.query(
              ...
              {
                replacements: {
                  action: 'UPDATE_NOTIFICATIONSERVICESRECORD',
                  tableName: 'notification_services_records',
                  recordId: record.id,
                  userId: options.userId || 'system',
        ...
        afterUpdate: async (record: NotificationServicesRecord, options: any) => {
          console.log(`[AUDIT] NotificationServicesRecord updated: ${record.id}`);
        },
        beforeDestroy: async (record: NotificationServicesRecord, options: any) => {
          if (options.transaction) {
            await sequelize.query(
              `INSERT INTO audit_logs (action, table_name, record_id, user_id, data, created_at)
               VALUES (:action, :tableName, :recordId, :userId, :data, NOW())`,
              {
                replacements: {
                  action: 'DELETE_NOTIFICATIONSERVICESRECORD',
                  tableName: 'notification_services_records',
                  recordId: record.id,
                  userId: options.userId || 'system',
                  data: JSON.stringify(record.toJSON()),
        ...
        afterDestroy: async (record: NotificationServicesRecord, options: any) => {
          console.log(`[AUDIT] NotificationServicesRecord deleted: ${record.id}`);
        },
      },
      scopes: {
        defaultScope: {
  ...
  return NotificationServicesRecord;
};
...
export class NotificationServicesComposite {
  ...
  constructor(
    ...
    this.batchSize = this.configService.get<number>('notification.batchSize', 100);
    this.retryAttempts = this.configService.get<number>('notification.retryAttempts', 3);
    this.retryDelayMs = this.configService.get<number>('notification.retryDelayMs', 5000);
  }
