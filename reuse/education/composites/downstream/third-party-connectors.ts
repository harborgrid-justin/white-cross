 ...
 * File: /reuse/education/composites/downstream/third-party-connectors.ts
 *
 * UPSTREAM (imports from):
 *   - @nestjs/common (v10.x)
 *   - sequelize (v6.x)
*   - ../../integration-data-exchange-kit
 *
 ...
 * Purpose: Third Party Connectors - Production-grade external integrations
 *
 ...
 * Composes functions to provide external integrations with full operational capabilities.
 */
...
export const createServiceModel = (sequelize: Sequelize) => {
  ...
  ServiceModel.init(
    ...
    {
      sequelize,
      tableName: 'third_party_connectors',
      timestamps: true,
...
@Injectable()
export class ThirdPartyConnectorsCompositeService {
  private readonly logger = new Logger(ThirdPartyConnectorsCompositeService.name);
...
export default ThirdPartyConnectorsCompositeService;
