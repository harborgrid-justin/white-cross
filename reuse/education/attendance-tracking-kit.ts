...
interface AttendancePolicyData {
  ...
  chronicAbsenceThreshold: number; // percentage
  tardyThreshold: number; // number of tardies before action
...
export class Attendance extends Model {
  ...
  public minutesLate!: number | null;
  public minutesAbsent!: number | null;
  public excuseDocumentUrl!: string | null;
  ...
  Attendance.init(
    {
      ...
      periodType: {
        type: DataTypes.ENUM(...Object.values(PeriodType)),
        allowNull: false,
        defaultValue: PeriodType.FULL_DAY,
      },
      absenceType: {
        type: DataTypes.ENUM(...Object.values(AbsenceType)),
        allowNull: true,
      },
      notes: {
        type: DataTypes.TEXT,
    ...
    },
    {
      ...
      indexes: [
        ...
        },
        {
          ...
          where: {
            status: {
              [Op.in]: [AttendanceStatus.ABSENT, AttendanceStatus.UNEXCUSED],
            },
          },
...
export class AttendanceRecord extends Model {
  ...
  public excusedAbsences!: number;
  public unexcusedAbsences!: number;
  public tardies!: number;
  public earlyDismissals!: number;
 ...
 */
export function initAttendanceRecordModel(sequelize: Sequelize): typeof AttendanceRecord {
...
export class AttendancePolicy extends Model {
  ...
  public academicYear!: string;
  public maxExcusedAbsences!: number;
  public maxUnexcusedAbsences!: number;
  public chronicAbsenceThreshold!: number;
  public tardyThreshold!: number;
  public excusedAbsenceTypes!: AbsenceType[];
  public requiresDocumentation!: boolean;
  public documentationDeadlineDays!: number;
  public truancyReferralThreshold!: number;
 ...
 */
export function initAttendancePolicyModel(sequelize: Sequelize): typeof AttendancePolicy {
 ...
 */
export class AttendanceAlert extends Model {
  public id!: string;
  public studentId!: string;
 ...
 */
export function initAttendanceAlertModel(sequelize: Sequelize): typeof AttendanceAlert {
 ...
 */
export class MakeupClass extends Model {
  public id!: string;
  public studentId!: string;
  public originalAbsenceId!: string;
 ...
 */
export function initMakeupClassModel(sequelize: Sequelize): typeof MakeupClass {
  MakeupClass.init(
    {
      id: {
        type: DataTypes.UUID,
...
export async function recordAttendance(
  ...
  try {
    ...
    // Create new record
    const attendance = await Attendance.create(data as any, { transaction: t });
...
export async function recordBulkAttendance(
  ...
  try {
    ...
    for (const record of bulkData.records) {
      try {
        const attendanceData: AttendanceRecordData = {
          ...record,
        ...
        if (existingRecord) {
          ...
          await Attendance.create(attendanceData as any, { transaction: t });
          results.created++;
        }
      } catch (error: any) {
 ...
 */
export async function markStudentTardy(
  sequelize: Sequelize,
  studentId: string,
  classId: string,
  date: Date,
 ...
 */
export async function recordEarlyDismissal(
  sequelize: Sequelize,
  studentId: string,
  schoolId: string,
  date: Date,
 ...
 */
export async function recordExcusedAbsence(
  sequelize: Sequelize,
  studentId: string,
  schoolId: string,
  date: Date,
 ...
 */
export async function recordUnexcusedAbsence(
  sequelize: Sequelize,
  studentId: string,
  schoolId: string,
  date: Date,
 ...
 */
export async function convertToExcusedAbsence(
  sequelize: Sequelize,
  attendanceId: string,
  absenceType: AbsenceType,
  excuseDocumentUrl: string,
...
export async function getStudentAbsences(
  ...
  const whereClause: WhereOptions = {
    ...
    status: includeExcused
      ? { [Op.in]: [AttendanceStatus.ABSENT, AttendanceStatus.EXCUSED, AttendanceStatus.UNEXCUSED] }
      : { [Op.in]: [AttendanceStatus.ABSENT, AttendanceStatus.UNEXCUSED] },
  };
...
export async function getConsecutiveAbsences(
...
): Promise<{ count: number; startDate: Date; absences: Attendance[] }> {
  const absences = await Attendance.findAll({
    where: {
      studentId,
      date: { [Op.lte]: asOfDate },
      status: { [Op.in]: [AttendanceStatus.ABSENT, AttendanceStatus.UNEXCUSED] },
    },
    order: [['date', 'DESC']],
  ...
  const consecutiveAbsences: Attendance[] = [];
...
export async function generateAttendanceReport(
  ...
  if (options.schoolId) whereClause.schoolId = options.schoolId;
  if (options.classId) whereClause.classId = options.classId;
  if (options.studentIds) whereClause.studentId = { [Op.in]: options.studentIds };
...
export async function createAttendanceAlert(
  sequelize: Sequelize,
  alertData: AttendanceAlertData,
  transaction?: Transaction
): Promise<AttendanceAlert> {
  return AttendanceAlert.create(alertData as any, { transaction });
}

/**
 * 17. Checks for chronic absenteeism and creates alerts
 */
export async function checkChronicAbsenteeism(
  sequelize: Sequelize,
  schoolId: string,
  academicYear: string,
  transaction?: Transaction
  ...
  for (const student of chronicStudents as any[]) {
    const alert = await createAttendanceAlert(
      sequelize,
      {
 ...
 */
export async function getUnresolvedAlertsForStudent(
  sequelize: Sequelize,
  studentId: string
): Promise<AttendanceAlert[]> {
 ...
 */
export async function getStudentsAtRiskOfTruancy(
  sequelize: Sequelize,
  schoolId: string,
  academicYear: string,
  threshold: number = 5
 ...
 */
export async function calculateBatchAttendanceRates(
  sequelize: Sequelize,
  studentIds: string[],
  startDate: Date,
  endDate: Date
  ...
  return new Map(
    (results as any[]).map(r => [r.student_id, parseFloat(r.attendance_rate)])
  );
}
 ...
 */
export async function getAttendanceByGradeLevel(
  sequelize: Sequelize,
  schoolId: string,
  academicYear: string
): Promise<any[]> {
...
export async function scheduleMakeupClass(
  sequelize: Sequelize,
  makeupData: MakeupClassData,
  transaction?: Transaction
): Promise<MakeupClass> {
  return MakeupClass.create(makeupData as any, { transaction });
}

/**
 * 27. Gets scheduled makeup classes for a student
 */
export async function getStudentMakeupClasses(
  sequelize: Sequelize,
  studentId: string,
  status?: 'scheduled' | 'completed' | 'cancelled' | 'no_show'
): Promise<MakeupClass[]> {
  const whereClause: WhereOptions = { studentId };
 ...
 */
export async function completeMakeupClass(
  sequelize: Sequelize,
 ...
 */
export async function getMakeupClassStatistics(
  sequelize: Sequelize,
  studentId: string,
  academicYear: string
): Promise<{
...
export async function createAttendancePolicy(
  sequelize: Sequelize,
  policyData: AttendancePolicyData,
  transaction?: Transaction
): Promise<AttendancePolicy> {
  return AttendancePolicy.create(policyData as any, { transaction });
}

/**
 * 32. Gets active attendance policy for a school
 */
export async function getActiveAttendancePolicy(
  sequelize: Sequelize,
  schoolId: string,
  academicYear: string,
  asOfDate: Date = new Date()
 ...
 */
export async function validateAttendanceAgainstPolicy(
  sequelize: Sequelize,
  studentId: string,
  schoolId: string,
  academicYear: string
  ...
  if (stats.unexcusedAbsences > policy.maxUnexcusedAbsences) {
    violations.push(
      `Exceeded maximum unexcused absences: ${stats.unexcusedAbsences} > ${policy.maxUnexcusedAbsences}`
    );
  }

  if (stats.excusedAbsences > policy.maxExcusedAbsences) {
    warnings.push(
      `Exceeded maximum excused absences: ${stats.excusedAbsences} > ${policy.maxExcusedAbsences}`
    );
  }

  if (stats.attendanceRate < (100 - policy.chronicAbsenceThreshold)) {
    violations.push(
      `Chronic absenteeism: ${stats.attendanceRate}% < ${100 - policy.chronicAbsenceThreshold}%`
    );
  }

  if (stats.tardies >= policy.tardyThreshold) {
    warnings.push(
      `Exceeded tardy threshold: ${stats.tardies} >= ${policy.tardyThreshold}`
    );
  }

  if (stats.unexcusedAbsences >= policy.truancyReferralThreshold) {
    violations.push(
      `Truancy referral required: ${stats.unexcusedAbsences} >= ${policy.truancyReferralThreshold}`
    );
  }
 ...
 */
export async function generateStateComplianceReport(
  sequelize: Sequelize,
  schoolId: string,
  academicYear: string
): Promise<any> {
  const totalStudents = await sequelize.query(
    `SELECT COUNT(*) as count FROM students WHERE school_id = :schoolId AND active = true`,
    {
  ...
  return {
    schoolId,
    academicYear,
    totalStudents: (totalStudents[0] as any).count,
    chronicAbsenteeismCount: (chronicAbsenteeism[0] as any).count,
    truancyReferralsCount: (truancyReferrals[0] as any).count,
    averageAttendanceRate: (avgAttendanceRate[0] as any).avg_rate,
 ...
 */
export async function updateAttendanceRecordSummaries(
  sequelize: Sequelize,
  schoolId: string,
  academicYear: string,
  transaction?: Transaction
 ...
 */
export async function getAttendanceWithRelations(
  sequelize: Sequelize,
 ...
 */
export async function bulkFetchStudentAttendance(
  sequelize: Sequelize,
  studentIds: string[],
  startDate: Date,
  endDate: Date
 ...
 */
export async function getAttendanceWithAdvancedFilters(
  sequelize: Sequelize,
  filters: {
    schoolIds?: string[];
    gradeLevel?: string;
 ...
 */
export async function getAttendanceOutliers(
  sequelize: Sequelize,
  schoolId: string,
  academicYear: string
): Promise<any[]> {
 ...
 */
export async function getPeriodBasedAttendanceSummary(
  sequelize: Sequelize,
  classId: string,
  date: Date
): Promise<any[]> {
 ...
 */
export async function archiveOldAttendanceRecords(
  sequelize: Sequelize,
  olderThanYears: number = 7,
  transaction?: Transaction
 ...
 */
async function getSchoolIdFromClass(
  sequelize: Sequelize,
  classId: string,
  transaction?: Transaction
): Promise<string | null> {
  ...
  return result.length > 0 ? (result[0] as any).school_id : null;
}

/**
 * Helper: Calculates minutes early for dismissal
 */
function calculateMinutesEarly(dismissalTime: Date): number {
 ...
 */
function detectBiweeklyPattern(absences: Attendance[]): boolean {
  if (absences.length < 4) return false;
 ...
 */
function detectPatternType(absences: Attendance[]): 'weekly' | 'biweekly' | 'monthly' | 'random' {
  if (absences.length < 3) return 'random';
 ...
 */
function calculatePatternConfidence(absences: Attendance[], patterns: string[]): number {
  if (absences.length < 3) return 0;
 ...
 */
function generatePatternSuggestions(patterns: string[]): string[] {
