...
exports.createQueryStatisticsTracker = createQueryStatisticsTracker;
exports.detectQueryRegressions = detectQueryRegressions;
...
exports.detectRedundantIndexes = detectRedundantIndexes;
exports.generateCompositeIndexRecommendations = generateCompositeIndexRecommendations;
exports.calculateIndexSelectivity = calculateIndexSelectivity;
exports.detectNPlusOneQueries = detectNPlusOneQueries;
exports.preventNPlusOne = preventNPlusOne;
exports.createDataLoader = createDataLoader;
exports.prefetchAssociations = prefetchAssociations;
exports.analyzeLoadingStrategy = analyzeLoadingStrategy;
exports.cacheAssociation = cacheAssociation;
exports.optimizeIncludeDepth = optimizeIncludeDepth;
exports.determineEagerLoadingStrategy = determineEagerLoadingStrategy;
exports.applySelectiveEagerLoading = applySelectiveEagerLoading;
exports.parallelLoadIncludes = parallelLoadIncludes;
...
exports.warmConnectionPool = warmConnectionPool;
exports.detectPoolExhaustion = detectPoolExhaustion;
exports.createQueryCache = createQueryCache;
exports.createCacheInvalidation = createCacheInvalidation;
exports.createDistributedCacheSync = createDistributedCacheSync;
exports.calculateOptimalCacheTTL = calculateOptimalCacheTTL;
exports.warmCache = warmCache;
exports.monitorCachePerformance = monitorCachePerformance;
exports.routeToShard = routeToShard;
exports.selectReadReplica = selectReadReplica;
exports.monitorReplicationLag = monitorReplicationLag;
exports.createFailoverManager = createFailoverManager;
exports.distributeWrites = distributeWrites;
exports.aggregateCrossShard = aggregateCrossShard;
exports.createPerformanceDashboard = createPerformanceDashboard;
exports.detectPerformanceRegressions = detectPerformanceRegressions;
...
exports.setupPerformanceAlerting = setupPerformanceAlerting;
/**
 * File: /reuse/sequelize-optimization-kit.ts
 ...
 * ```
 */
async function analyzeQueryPerformance(sequelize, query, options = {}) {
 ...
 * ```
 */
async function profileQuery(queryFn, options = {}) {
    ...
    if (captureStackTrace && queryCount > 1) {
        console.warn(`Potential N+1 query detected: ${queryCount} queries executed in ${totalTime}ms`);
    }
 ...
 * ```
 */
async function compareQueryPerformance(scenarios, options = {}) {
 ...
 * ```
 */
function monitorSlowQueries(sequelize, thresholdMs, callback) {
 ...
 * ```
 */
function createQueryStatisticsTracker() {
    const timings = [];
    let count = 0;
    let sum = 0;
    let min = Infinity;
 ...
 * ```
 */
function detectQueryRegressions(baseline, current, threshold = 20) {
    const regressions = [];
 ...
 * ```
 */
async function estimateQueryCost(sequelize, query, replacements = {}) {
    const dialect = sequelize.getDialect();
    if (dialect === 'postgres') {
        const [plan] = await sequelize.query(`EXPLAIN (FORMAT JSON) ${query}`, {
            replacements,
        ...
        const planData = plan?.query_block || {};
 ...
 * ```
 */
async function suggestIndexes(sequelize, metrics) {
    const suggestions = [];
 ...
 * ```
 */
async function analyzeIndexUsage(sequelize, tableName, metrics) {
    const dialect = sequelize.getDialect();
    const indexes = [];
    // Get existing indexes
    if (dialect === 'postgres') {
        const result = await sequelize.query(`SELECT indexname, indexdef FROM pg_indexes WHERE tablename = :tableName`, { replacements: { tableName }, type: sequelize_1.QueryTypes.SELECT });
        indexes.push(...result);
    }
    else if (dialect === 'mysql') {
        const result = await sequelize.query(`SHOW INDEX FROM ${tableName}`, { type: sequelize_1.QueryTypes.SELECT });
        indexes.push(...result);
    }
    // Analyze usage
    return indexes.map((idx) => {
        const indexName = idx.indexname || idx.Key_name;
        const usageCount = metrics.filter(m => m.indexNames?.includes(indexName)).length;
 ...
 * ```
 */
async function detectRedundantIndexes(sequelize, tableName) {
    const dialect = sequelize.getDialect();
    const redundancies = [];
    // Get all indexes
    let indexes = [];
    if (dialect === 'postgres') {
        indexes = await sequelize.query(`SELECT indexname, indexdef FROM pg_indexes WHERE tablename = :tableName`, { replacements: { tableName }, type: sequelize_1.QueryTypes.SELECT });
    }
    else if (dialect === 'mysql') {
        indexes = await sequelize.query(`SHOW INDEX FROM ${tableName}`, { type: sequelize_1.QueryTypes.SELECT });
    }
 ...
 * ```
 */
function generateCompositeIndexRecommendations(metrics, options = {}) {
 ...
 * ```
 */
async function calculateIndexSelectivity(sequelize, tableName, columnName) {
    // Get total rows
    const [totalResult] = await sequelize.query(`SELECT COUNT(*) as count FROM ${tableName}`, { type: sequelize_1.QueryTypes.SELECT });
    const totalRows = parseInt(totalResult.count, 10);
    // Get unique values
    const [uniqueResult] = await sequelize.query(`SELECT COUNT(DISTINCT ${columnName}) as count FROM ${tableName}`, { type: sequelize_1.QueryTypes.SELECT });
 ...
 * ```
 */
function detectNPlusOneQueries(metrics, options = {}) {
    ...
    for (const window of windows) {
        ...
        const similarQueries = window.slice(1).filter(q => queriesAreSimilar(firstQuery.query, q.query));
 ...
 * ```
 */
function preventNPlusOne(options, config = {}) {
    const { useSeparate = false, useSubQuery = false, maxDepth = 3 } = config;
    const optimized = { ...options };
    if (optimized.include) {
        optimized.include = optimizeIncludesForNPlusOne(Array.isArray(optimized.include) ? optimized.include : [optimized.include], { useSeparate, maxDepth, currentDepth: 0 });
    }
 ...
 * ```
 */
function createDataLoader(batchLoadFn, config = {}) {
    const { batchSize = 50, maxBatchSize = 1000, batchScheduleFn = (callback) => process.nextTick(callback), cacheKeyFn = (key) => String(key), } = config;
    const cache = new Map();
    const queue = [];
    let scheduled = false;
 ...
 * ```
 */
async function prefetchAssociations(instances, associations) {
    ...
    for (const associationName of associations) {
        ...
        const associatedRecords = await association.target.findAll({
            where: {
                [association.foreignKey]: { [sequelize_1.Op.in]: ids },
            },
        });
        // Map records to instances
        const recordsMap = new Map();
        for (const record of associatedRecords) {
            const foreignKeyValue = record[association.foreignKey];
        ...
        for (const instance of instances) {
            const records = recordsMap.get(instance.id) || [];
            instance[associationName] = association.isMultiAssociation ? records : records[0];
        }
    }
    return instances;
}
 ...
 * ```
 */
async function analyzeLoadingStrategy(model, baseQuery) {
    const strategies = {};
    // Test eager loading (join)
    const eagerStart = Date.now();
    const { metrics: eagerMetrics } = await profileQuery(() => model.findAll({ ...baseQuery, subQuery: false }));
    strategies.eager = {
    ...
    const { metrics: separateMetrics } = await profileQuery(() => model.findAll({
        ...baseQuery,
        include: baseQuery.include
            ? (Array.isArray(baseQuery.include) ? baseQuery.include : [baseQuery.include]).map(inc => typeof inc === 'object' ? { ...inc, separate: true } : inc)
            : undefined,
    }));
    strategies.separate = {
 ...
 * ```
 */
function cacheAssociation(model, associationName, cacheOptions = {}) {
    ...
    return async (instanceId) => {
        ...
        if (!instance)
            return null;
        const data = await instance[`get${associationName.charAt(0).toUpperCase()}${associationName.slice(1)}`]();
        // Store in cache
        const entry = {
            key: cacheKey,
            data,
 ...
 * ```
 */
function optimizeIncludeDepth(includes, maxDepth = 3) {
    const optimize = (includes, currentDepth) => {
 ...
 * ```
 */
function determineEagerLoadingStrategy(config) {
    const { parentCount = 100, childrenPerParent = 10, associationType = 'hasMany', dataSize = 1024, } = config;
 ...
 * ```
 */
function applySelectiveEagerLoading(options, accessPatterns, threshold = 0.5) {
    const selective = { ...options };
    ...
    if (selective.include) {
        ...
        selective.include = includes.filter(include => {
            ...
            const associationName = include.as || include.association;
            return (accessPatterns[associationName] || 0) >= threshold;
        });
    }
    return selective;
}
 ...
 * ```
 */
async function parallelLoadIncludes(model, options) {
    ...
    const ids = baseRecords.map((r) => r.id);
        ...
        if (typeof include === 'string') {
            ...
            if (association) {
                ...
                for (const record of associated) {
                    const fkValue = record[association.foreignKey];
                ...
                for (const instance of baseRecords) {
                    const records = associatedMap.get(instance.id) || [];
                    instance[associationName] = association.isMultiAssociation
                        ? records
                        : records[0];
                }
            }
 ...
 * ```
 */
function createLazyLoader(model, associationName, options = {}) {
    ...
    const batchLoad = async () => {
        ...
        const instances = await model.findAll({
            where: { id: { [sequelize_1.Op.in]: idsToLoad } },
            include: [{ association: associationName }],
        });
        // Cache results
        if (cache) {
            for (const instance of instances) {
                const cacheKey = `${model.name}:${instance.id}:${associationName}`;
                cacheMap.set(cacheKey, {
                    key: cacheKey,
                    data: instance[associationName],
                    createdAt: new Date(),
                    expiresAt: new Date(Date.now() + ttl * 1000),
                    hits: 0,
                    size: 0,
 ...
 * ```
 */
function getConnectionPoolStats(sequelize) {
 ...
 * ```
 */
function optimizeConnectionPool(currentStats, workloadMetrics) {
 ...
 * ```
 */
async function warmConnectionPool(sequelize, targetSize = 5) {
    const warmQueries = [];
    for (let i = 0; i < targetSize; i++) {
        warmQueries.push(sequelize.query('SELECT 1', { type: sequelize_1.QueryTypes.SELECT }));
    }
 ...
 * ```
 */
function detectPoolExhaustion(sequelize, options = {}) {
...
function createQueryCache(options = {}) {
    ...
    return {
        ...
        set: (key, value, ttl = defaultTtl, tags = []) => {
            evictOldest();
            const entry = {
                key,
                data: value,
        ...
        wrap: async (key, fn, ttl = defaultTtl, tags = []) => {
 ...
 * ```
 */
function createCacheInvalidation(cache, strategy = {}) {
    return {
 ...
 * ```
 */
function createDistributedCacheSync(options = {}) {
 ...
 * ```
 */
function calculateOptimalCacheTTL(metrics) {
    const { updateFrequency = 0.1, readFrequency = 10, dataSize = 1024, staleTolerance = 0.1, } = metrics;
 ...
 * ```
 */
async function warmCache(cache, loaders) {
    await Promise.all(loaders.map(({ key, loader, ttl, tags }) => cache.wrap(key, loader, ttl, tags)));
}
/**
 ...
 * ```
 */
function monitorCachePerformance(cache, options = {}) {
 ...
 * ```
 */
function routeToShard(shards, shardKey) {
    if (shards.length === 0) {
 ...
 * ```
 */
function selectReadReplica(replicas, options = {}) {
 ...
 * ```
 */
async function monitorReplicationLag(replicas) {
    const results = await Promise.all(replicas.map(async (replica, index) => {
        try {
            ...
            const [result] = await replica.sequelize.query("SELECT EXTRACT(EPOCH FROM (now() - pg_last_xact_replay_timestamp())) * 1000 AS lag_ms", { type: sequelize_1.QueryTypes.SELECT });
            return {
                name: `replica-${index}`,
                healthy: true,
...
function createFailoverManager(config) {
    const { primary, replicas, healthCheckInterval = 10000, onFailover, } = config;
 ...
 * ```
 */
async function distributeWrites(shards, data, shardKeyFn) {
    // Group data by shard
    ...
    let failed = 0;
    const errors = [];
    await Promise.all(Array.from(shardGroups.entries()).map(async ([shardIndex, items]) => {
        try {
 ...
 * ```
 */
async function aggregateCrossShard(shards, queryFn, aggregateFn) {
    const results = await Promise.all(shards.map(shard => queryFn(shard.sequelize)));
 ...
 * ```
 */
function createPerformanceDashboard(sources) {
    const { queryMetrics = [], poolStats, cacheStats, nPlusOneDetections = [], } = sources;
    ...
    if (poolStats)
        score -= Math.min(20, (poolStats.utilizationPercent - 50) / 2);
    if (cacheStats)
        score -= Math.min(20, (1 - cacheStats.hitRate) * 40);
 ...
 * ```
 */
function detectPerformanceRegressions(baseline, current) {
    const regressions = [];
    // Check query time
 ...
 * ```
 */
function exportPerformanceMetrics(metrics, format = 'json') {
    if (format === 'json') {
 ...
 * ```
 */
function setupPerformanceAlerting(thresholds, alertFn) {
    const { slowQueryMs = 1000, poolUtilization = 90, cacheHitRate = 0.3, nPlusOneCount = 10, } = thresholds;
    // This is a simplified example
 ...
 */
function extractIndexUsage(queryPlan) {
 ...
 */
function extractRowsExamined(queryPlan) {
    // Simplified - would parse actual plan structure
    return 0;
}
/**
 ...
 */
function extractModelNames(query) {
 ...
 */
function queriesAreSimilar(query1, query2) {
    // Remove literals and normalize
    const normalize = (q) => q.replace(/\d+/g, 'N')
        .replace(/'[^']*'/g, 'S')
        .replace(/\s+/g, ' ')
        .trim();
...
function extractIndexColumns(indexDef) {
    ...
    if (indexDef.Column_name)
        return [indexDef.Column_name];
    return [];
}
/**
 ...
 */
function optimizeIncludesForNPlusOne(includes, options) {
    ...
    return includes.map(include => {
        ...
        if (optimized.include) {
            optimized.include = optimizeIncludesForNPlusOne(Array.isArray(optimized.include) ? optimized.include : [optimized.include], { ...options, currentDepth: currentDepth + 1 });
        }
        return optimized;
    });
}
//# sourceMappingURL=sequelize-optimization-kit.js.map