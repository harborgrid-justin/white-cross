...
exports.ShippingCarrier = exports.ERPSystem = exports.EDITransactionSet = exports.EDISegmentType = void 0;
exports.parseEDI850PurchaseOrder = parseEDI850PurchaseOrder;
exports.generateEDI850PurchaseOrder = generateEDI850PurchaseOrder;
exports.validateEDI850PurchaseOrder = validateEDI850PurchaseOrder;
exports.generateEDI855POAcknowledgment = generateEDI855POAcknowledgment;
exports.parseEDI855POAcknowledgment = parseEDI855POAcknowledgment;
exports.createPOAcknowledgment = createPOAcknowledgment;
exports.generateEDI856ShipNotice = generateEDI856ShipNotice;
exports.parseEDI856ShipNotice = parseEDI856ShipNotice;
exports.createASNFromShipment = createASNFromShipment;
exports.generateEDI810Invoice = generateEDI810Invoice;
exports.parseEDI810Invoice = parseEDI810Invoice;
...
exports.parseAmazonOrder = parseAmazonOrder;
exports.parseShopifyOrder = parseShopifyOrder;
exports.transformMarketplaceOrder = transformMarketplaceOrder;
exports.submitMarketplaceFulfillment = submitMarketplaceFulfillment;
exports.syncOrderToERP = syncOrderToERP;
exports.fetchERPOrderStatus = fetchERPOrderStatus;
exports.transformDataWithMappings = transformDataWithMappings;
exports.createERPAuthToken = createERPAuthToken;
exports.submitOrderToWMS = submitOrderToWMS;
exports.fetchWMSFulfillmentStatus = fetchWMSFulfillmentStatus;
exports.cancelWMSOrder = cancelWMSOrder;
...
exports.validateAndTransformOrder = validateAndTransformOrder;
exports.checkIntegrationHealth = checkIntegrationHealth;
 ...
 */
var EDISegmentType;
(function (EDISegmentType) {
    EDISegmentType["ISA"] = "ISA";
    EDISegmentType["GS"] = "GS";
    EDISegmentType["ST"] = "ST";
    EDISegmentType["BEG"] = "BEG";
    EDISegmentType["REF"] = "REF";
    EDISegmentType["PER"] = "PER";
    EDISegmentType["DTM"] = "DTM";
    EDISegmentType["N1"] = "N1";
    EDISegmentType["N3"] = "N3";
    EDISegmentType["N4"] = "N4";
    EDISegmentType["PO1"] = "PO1";
    EDISegmentType["CTT"] = "CTT";
    EDISegmentType["SE"] = "SE";
    EDISegmentType["GE"] = "GE";
    EDISegmentType["IEA"] = "IEA";
    EDISegmentType["BAK"] = "BAK";
    EDISegmentType["BSN"] = "BSN";
    EDISegmentType["HL"] = "HL";
    EDISegmentType["TD5"] = "TD5";
    EDISegmentType["BIG"] = "BIG";
})(EDISegmentType || (exports.EDISegmentType = EDISegmentType = {}));
/**
 * EDI transaction set types
 */
var EDITransactionSet;
(function (EDITransactionSet) {
    EDITransactionSet["PO850"] = "850";
    EDITransactionSet["PO_ACK_855"] = "855";
    EDITransactionSet["ASN_856"] = "856";
    EDITransactionSet["INVOICE_810"] = "810";
})(EDITransactionSet || (exports.EDITransactionSet = EDITransactionSet = {}));
/**
 * ERP system types
 */
var ERPSystem;
(function (ERPSystem) {
    ...
    ERPSystem["DYNAMICS"] = "DYNAMICS";
    ERPSystem["EPICOR"] = "EPICOR";
 ...
 * ```
 */
function parseEDI850PurchaseOrder(ediContent) {
    ...
    const lineItems = po1Segments.map((seg, idx) => ({
        lineNumber: idx + 1,
 ...
 * ```
 */
function generateEDI850PurchaseOrder(po, senderId, receiverId) {
 ...
 * ```
 */
function validateEDI850PurchaseOrder(po) {
    const errors = [];
 ...
 * ```
 */
function generateEDI855POAcknowledgment(ack, senderId, receiverId) {
    const controlNumber = generateControlNumber();
    const segments = [];
    segments.push(`ISA*00*          *00*          *ZZ*${senderId.padEnd(15)}*ZZ*${receiverId.padEnd(15)}*${formatEDIDate(new Date())}*${formatEDITime(new Date())}*U*00401*${controlNumber}*0*P*>`);
 ...
 * ```
 */
function parseEDI855POAcknowledgment(ediContent) {
    const segments = parseEDISegments(ediContent);
    ...
    const acknowledgmentType = ackCode === 'AC' ? 'ACCEPT' : ackCode === 'RJ' ? 'REJECT' : 'CHANGE';
    const lineItems = [];
    const po1Segments = segments.filter(s => s.type === EDISegmentType.PO1);
 ...
 * ```
 */
function createPOAcknowledgment(po, confirmations) {
    const lineItems = po.lineItems.map((item, idx) => {
 ...
 * ```
 */
function generateEDI856ShipNotice(asn, senderId, receiverId) {
    const controlNumber = generateControlNumber();
    const segments = [];
    segments.push(`ISA*00*          *00*          *ZZ*${senderId.padEnd(15)}*ZZ*${receiverId.padEnd(15)}*${formatEDIDate(new Date())}*${formatEDITime(new Date())}*U*00401*${controlNumber}*0*P*>`);
 ...
 * ```
 */
function parseEDI856ShipNotice(ediContent) {
    const segments = parseEDISegments(ediContent);
 ...
 * ```
 */
function createASNFromShipment(orderId, packages, carrier, trackingNumber) {
    return {
 ...
 * ```
 */
function generateEDI810Invoice(invoice, senderId, receiverId) {
    const controlNumber = generateControlNumber();
    const segments = [];
    segments.push(`ISA*00*          *00*          *ZZ*${senderId.padEnd(15)}*ZZ*${receiverId.padEnd(15)}*${formatEDIDate(new Date())}*${formatEDITime(new Date())}*U*00401*${controlNumber}*0*P*>`);
 ...
 * ```
 */
function parseEDI810Invoice(ediContent) {
    const segments = parseEDISegments(ediContent);
    ...
    const lineItems = [];
    const it1Segments = segments.filter(s => s.type === 'IT1');
 ...
 * ```
 */
function createInvoiceFromOrder(order, shipment, taxRate = 0) {
 ...
 * ```
 */
async function sendWebhookEvent(webhook, event) {
    if (!webhook.active)
        return false;
    const payload = {
 ...
 * ```
 */
function parseWebhookEvent(requestBody, signature, secret) {
 ...
 * ```
 */
function parseAmazonOrder(amazonOrder) {
    return {
 ...
 * ```
 */
function transformMarketplaceOrder(marketplaceOrder) {
    return {
 ...
 * ```
 */
async function submitMarketplaceFulfillment(marketplace, orderId, trackingNumber, carrier) {
 ...
 * ```
 */
async function syncOrderToERP(erpConfig, order) {
    try {
 ...
 * ```
 */
async function fetchERPOrderStatus(erpConfig, erpOrderId) {
    // Implementation would call ERP-specific API
    return {
        orderId: erpOrderId,
        status: 'PROCESSING',
 ...
 * ```
 */
function transformDataWithMappings(sourceData, mappings) {
    const result = {};
 ...
 * ```
 */
async function createERPAuthToken(erpSystem, credentials) {
 ...
 * ```
 */
async function submitOrderToWMS(wmsEndpoint, order) {
    const wmsOrder = {
 ...
 * ```
 */
async function fetchWMSFulfillmentStatus(wmsEndpoint, wmsOrderId) {
    // Implementation would call WMS API
    return {
        wmsOrderId,
        status: 'PICKING',
 ...
 * ```
 */
async function cancelWMSOrder(wmsEndpoint, wmsOrderId) {
 ...
 * ```
 */
async function authorizePayment(gateway, orderId, amount, currency) {
 ...
 * ```
 */
async function capturePayment(gateway, transactionId, amount) {
 ...
 * ```
 */
async function processRefund(gateway, transactionId, amount) {
 ...
 * ```
 */
async function createShippingLabel(request) {
 ...
 * ```
 */
async function calculateShippingRates(fromAddress, toAddress, packages) {
 ...
 * ```
 */
function validateAndTransformOrder(rawData, schema) {
    const errors = [];
    const warnings = [];
 ...
 * ```
 */
async function checkIntegrationHealth(integrationName, endpoint) {
 ...
 * ```
 */
function logIntegrationEvent(integration, eventType, data) {
    const logEntry = {
        id: `evt-${Date.now()}`,
        integration,
        eventType,
...
// HELPER FUNCTIONS
// ============================================================================
function parseEDISegments(ediContent) {
    const segments = [];
...
//# sourceMappingURL=order-integration-kit.js.map