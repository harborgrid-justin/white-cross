...
exports.recordSLAPerformance = recordSLAPerformance;
exports.getSLAComplianceSummary = getSLAComplianceSummary;
exports.monitorActiveSLAs = monitorActiveSLAs;
exports.generatePaymentSchedule = generatePaymentSchedule;
exports.createPaymentScheduleEntry = createPaymentScheduleEntry;
exports.recordPayment = recordPayment;
exports.getUpcomingPayments = getUpcomingPayments;
exports.processOverduePayments = processOverduePayments;
exports.createContractAmendment = createContractAmendment;
exports.approveAndExecuteAmendment = approveAndExecuteAmendment;
exports.getContractAmendments = getContractAmendments;
exports.calculateVendorPerformance = calculateVendorPerformance;
exports.compareVendors = compareVendors;
exports.performComplianceCheck = performComplianceCheck;
exports.generateComplianceReport = generateComplianceReport;
exports.getContractsForRenewal = getContractsForRenewal;
exports.generateRenewalRecommendation = generateRenewalRecommendation;
exports.processAutoRenewal = processAutoRenewal;
exports.convertCurrency = convertCurrency;
exports.getContractValueInCurrency = getContractValueInCurrency;
 ...
 * ```
 */
async function createContract(sequelize, contractData, transaction) {
 ...
 */
async function updateContract(sequelize, contractId, updates, updatedBy, transaction) {
    // Build dynamic update query based on provided fields
    const updateFields = [];
    const replacements = { contractId, updatedBy };
 ...
 */
async function getContractById(sequelize, contractId) {
    const query = `
    SELECT
      id, contract_number, contract_name, contract_type, status,
 ...
 */
async function listContracts(sequelize, filters, options = {}) {
 ...
 * @returns Updated contract
 */
async function activateContract(sequelize, contractId, approvedBy, transaction) {
    const query = `
    UPDATE property_contracts
    SET status = 'active',
 ...
 * @returns Updated contract
 */
async function terminateContract(sequelize, contractId, reason, terminatedBy, effectiveDate, transaction) {
 ...
 * @returns Created SLA
 */
async function createSLA(sequelize, slaData, transaction) {
    const query = `
 ...
 */
async function recordSLAPerformance(sequelize, slaId, actualValue, measurementPeriod, reportedBy, transaction) {
    const sla = await getSLAById(sequelize, slaId);
    // Calculate compliance
    const compliancePercentage = (actualValue / sla.targetValue) * 100;
    const complianceStatus = determineComplianceStatus(compliancePercentage, sla.warningThreshold, sla.criticalThreshold);
 ...
 */
async function getSLAComplianceSummary(sequelize, contractId, periodStart, periodEnd) {
    const query = `
    SELECT
      s.sla_name,
      s.metric_name,
  ...
  `;
    ...
    const slaResults = results.map(row => {
        ...
        return {
            ...
            status: determineComplianceStatus(avgCompliance, 90, 80),
 ...
 */
async function monitorActiveSLAs(sequelize) {
    const query = `
    SELECT
      c.id as contract_id,
      c.contract_name,
  ...
  `;
    ...
    return results.map(row => ({
        ...
        slaName: String(row.sla_name),
        currentStatus: String(row.compliance_status),
 ...
 */
async function generatePaymentSchedule(sequelize, contractId, transaction) {
    const contract = await getContractById(sequelize, contractId);
    const payments = [];
    const totalMonths = Math.ceil((contract.endDate.getTime() - contract.startDate.getTime()) / (1000 * 60 * 60 * 24 * 30));
    let paymentCount = 0;
    let paymentAmount = 0;
 ...
 */
async function createPaymentScheduleEntry(sequelize, paymentData, transaction) {
    const query = `
 ...
 */
async function recordPayment(sequelize, paymentId, paidAmount, paymentMethod, paymentReference, transaction) {
    const query = `
 ...
 */
async function getUpcomingPayments(sequelize, daysAhead = 30) {
    const query = `
 ...
 */
async function processOverduePayments(sequelize, lateFeePercentage = 5, transaction) {
 ...
 */
async function createContractAmendment(sequelize, amendmentData, transaction) {
 ...
 */
async function approveAndExecuteAmendment(sequelize, amendmentId, approvedBy, transaction) {
  ...
  `;
    ...
    const contract = await updateContract(sequelize, amendment.contractId, updates, approvedBy, transaction);
 ...
 */
async function getContractAmendments(sequelize, contractId) {
    const query = `
    SELECT *
 ...
 */
async function calculateVendorPerformance(sequelize, vendorId, evaluationPeriod) {
    // Get vendor details
    const vendorQuery = `
    SELECT vendor_name
    FROM property_contracts
 ...
 */
async function compareVendors(sequelize, vendorIds, serviceType, evaluationPeriod) {
    const vendorPerformances = [];
 ...
 */
async function performComplianceCheck(sequelize, contractId, checkType, checkedBy) {
    const contract = await getContractById(sequelize, contractId);
    const issues = [];
    ...
    if (checkType === 'insurance') {
        ...
        requiredInsurance.forEach(insuranceType => {
            const hasValidInsurance = insuranceDocs.some(doc => doc.tags.includes(insuranceType) &&
    ...
    if (checkType === 'licensing') {
        ...
        const expiredLicenses = licenses.filter(doc => doc.expirationDate && doc.expirationDate < new Date());
    ...
    if (checkType === 'performance') {
        const slaCompliance = await getSLAComplianceSummary(sequelize, contractId, new Date(Date.now() - 90 * 24 * 60 * 60 * 1000), new Date());
 ...
 */
async function generateComplianceReport(sequelize, filters) {
 ...
 */
async function getContractsForRenewal(sequelize, daysAhead = 90) {
    const query = `
    SELECT *
    FROM property_contracts
    WHERE status = 'active'
 ...
 */
async function generateRenewalRecommendation(sequelize, contractId) {
    const contract = await getContractById(sequelize, contractId);
    // Get vendor performance
    const vendorPerformance = await calculateVendorPerformance(sequelize, contract.vendorId, {
        start: new Date(Date.now() - 180 * 24 * 60 * 60 * 1000),
        end: new Date(),
    });
    // Get SLA compliance
    const slaCompliance = await getSLAComplianceSummary(sequelize, contractId, new Date(Date.now() - 180 * 24 * 60 * 60 * 1000), new Date());
 ...
 */
async function processAutoRenewal(sequelize, contractId, transaction) {
    const contract = await getContractById(sequelize, contractId);
    ...
    const amendment = await createContractAmendment(sequelize, {
        contractId,
    ...
    const updatedContract = await updateContract(sequelize, contractId, {
 ...
 */
async function convertCurrency(sequelize, amount, fromCurrency, toCurrency) {
 ...
 */
async function getContractValueInCurrency(sequelize, contractId, targetCurrency) {
    const contract = await getContractById(sequelize, contractId);
    const convertedValue = await convertCurrency(sequelize, contract.totalValue, contract.currency, targetCurrency);
    return {
 ...
 */
async function updateExchangeRates(sequelize, rates, transaction) {
    let updatedCount = 0;
 ...
 */
async function uploadContractDocument(sequelize, contractId, documentData, transaction) {
    const query = `
 ...
 */
async function getContractDocuments(sequelize, contractId, documentType) {
    const whereClauses = ['contract_id = :contractId'];
    const replacements = { contractId };
    if (documentType) {
 ...
 */
async function generateContractNumber(sequelize, contractType) {
 ...
 */
async function cancelFuturePayments(sequelize, contractId, effectiveDate, transaction) {
    const query = `
    UPDATE contract_payment_schedules
 ...
 */
async function applyPenaltyToNextPayment(sequelize, contractId, penaltyAmount, reason, transaction) {
    const query = `
    UPDATE contract_payment_schedules
 ...
 */
function determineComplianceStatus(compliancePercentage, warningThreshold, criticalThreshold) {
    if (compliancePercentage >= warningThreshold)
        return 'compliant';
    if (compliancePercentage >= criticalThreshold)
        return 'warning';
 ...
 */
function calculateSLAPenalty(sla, compliancePercentage) {
    if (!sla.penaltyEnabled)
        return 0;
 ...
 */
function generateSLAActionRecommendation(data) {
 ...
 */
function generateRiskFactors(slaCompliance, breachCount, criticalBreaches) {
    const factors = [];
    if (slaCompliance < 80)
        factors.push('Consistently low SLA compliance');
    if (breachCount > 5)
        factors.push('Frequent SLA breaches');
    if (criticalBreaches > 0)
        factors.push('Critical SLA violations occurred');
    return factors;
}
/**
 * Generates vendor recommendations
 */
function generateVendorRecommendations(overallRating, slaCompliance, breachCount) {
 ...
 */
function generateVendorActionItems(riskLevel, criticalBreaches) {
    const actions = [];
 ...
 */
function generateRenewalSuggestions(recommendation, performanceScore, serviceQuality) {
    const suggestions = [];
    if (recommendation === 'renew_with_modifications') {
        if (performanceScore < 80)
            suggestions.push('Add performance guarantees');
        if (serviceQuality < 85)
            suggestions.push('Strengthen SLA requirements');
 ...
 */
function mapContractFromDb(row) {
    return {
        id: String(row.id),
...
function mapSLAFromDb(row) {
    return {
        ...
        penaltyTiers: row.penalty_tiers,
 ...
 * Retrieves SLA by ID
 */
async function getSLAById(sequelize, slaId) {
 ...
 */
async function getSLAPerformanceById(sequelize, id) {
    ...
    return {
        ...
        contractId: String(row.contract_id),
        measurementPeriod: {
            start: new Date(row.measurement_period_start),
            end: new Date(row.measurement_period_end),
        },
        ...
        breachDetails: row.breach_details,
 ...
 */
function mapPaymentScheduleFromDb(row) {
    return {
        id: String(row.id),
        contractId: String(row.contract_id),
 ...
 */
async function getPaymentScheduleById(sequelize, id) {
 ...
 */
async function getPaymentScheduleByContract(sequelize, contractId) {
    const query = `
 ...
 */
function mapAmendmentFromDb(row) {
    return {
        id: String(row.id),
        contractId: String(row.contract_id),
        ...
        fieldsModified: row.fields_modified,
 ...
 */
async function getAmendmentById(sequelize, id) {
 ...
 */
async function getDocumentById(sequelize, id) {
...
//# sourceMappingURL=property-contract-management-kit.js.map