...
exports.DepreciationType = exports.MarketCondition = exports.AppraisalPurpose = exports.PropertyCondition = exports.ValuationMethod = void 0;
exports.conductPropertyAppraisal = conductPropertyAppraisal;
exports.performSalesComparisonApproach = performSalesComparisonApproach;
exports.performIncomeApproach = performIncomeApproach;
exports.performCostApproach = performCostApproach;
...
exports.calculatePropertyAdjustments = calculatePropertyAdjustments;
exports.analyzeComparableSalesTrends = analyzeComparableSalesTrends;
exports.calculateNetOperatingIncome = calculateNetOperatingIncome;
exports.calculateMarketCapRate = calculateMarketCapRate;
exports.performDCFAnalysis = performDCFAnalysis;
exports.calculateGrossIncomeMultiplier = calculateGrossIncomeMultiplier;
exports.calculateDebtServiceCoverageRatio = calculateDebtServiceCoverageRatio;
exports.estimateLandValue = estimateLandValue;
exports.calculateReplacementCost = calculateReplacementCost;
exports.calculateAccruedDepreciation = calculateAccruedDepreciation;
exports.performRegressionAnalysis = performRegressionAnalysis;
exports.estimateMarketValue = estimateMarketValue;
exports.calculateValueConfidenceInterval = calculateValueConfidenceInterval;
exports.performAutomatedValuation = performAutomatedValuation;
exports.performHedonicPricing = performHedonicPricing;
exports.calculateAVMConfidence = calculateAVMConfidence;
exports.applyMarketConditionsAdjustment = applyMarketConditionsAdjustment;
exports.calculateLocationAdjustment = calculateLocationAdjustment;
exports.applyFunctionalObsolescenceAdjustment = applyFunctionalObsolescenceAdjustment;
exports.generateAppraisalReport = generateAppraisalReport;
exports.exportAppraisalReportToPDF = exportAppraisalReportToPDF;
exports.getPropertyValuationHistory = getPropertyValuationHistory;
exports.trackPropertyValueChanges = trackPropertyValueChanges;
exports.compareToHistoricalValues = compareToHistoricalValues;
exports.analyzeMarketTrends = analyzeMarketTrends;
exports.forecastPropertyValue = forecastPropertyValue;
exports.identifyComparableMarkets = identifyComparableMarkets;
/**
 * File: /reuse/property/property-valuation-kit.ts
...
(function (ValuationMethod) {
    ValuationMethod["SALES_COMPARISON"] = "sales_comparison";
    ValuationMethod["INCOME_APPROACH"] = "income_approach";
    ValuationMethod["COST_APPROACH"] = "cost_approach";
    ValuationMethod["DISCOUNTED_CASH_FLOW"] = "discounted_cash_flow";
    ValuationMethod["AUTOMATED_VALUATION"] = "automated_valuation";
...
(function (PropertyCondition) {
    ...
    PropertyCondition["VERY_GOOD"] = "very_good";
    PropertyCondition["GOOD"] = "good";
    PropertyCondition["FAIR"] = "fair";
    PropertyCondition["POOR"] = "poor";
    PropertyCondition["VERY_POOR"] = "very_poor";
})(PropertyCondition || (exports.PropertyCondition = PropertyCondition = {}));
/**
 * Appraisal purpose enumeration
 */
var AppraisalPurpose;
(function (AppraisalPurpose) {
    AppraisalPurpose["PURCHASE"] = "purchase";
    AppraisalPurpose["SALE"] = "sale";
    AppraisalPurpose["FINANCING"] = "financing";
    AppraisalPurpose["INSURANCE"] = "insurance";
    AppraisalPurpose["TAXATION"] = "taxation";
    AppraisalPurpose["PORTFOLIO_REVIEW"] = "portfolio_review";
    AppraisalPurpose["LITIGATION"] = "litigation";
    AppraisalPurpose["PARTNERSHIP_DISSOLUTION"] = "partnership_dissolution";
})(AppraisalPurpose || (exports.AppraisalPurpose = AppraisalPurpose = {}));
/**
 * Market condition enumeration
 */
var MarketCondition;
(function (MarketCondition) {
    MarketCondition["SELLERS_MARKET"] = "sellers_market";
    MarketCondition["BUYERS_MARKET"] = "buyers_market";
    MarketCondition["BALANCED"] = "balanced";
    MarketCondition["DECLINING"] = "declining";
    MarketCondition["APPRECIATING"] = "appreciating";
})(MarketCondition || (exports.MarketCondition = MarketCondition = {}));
/**
 * Depreciation type enumeration
 */
var DepreciationType;
(function (DepreciationType) {
    DepreciationType["PHYSICAL"] = "physical";
    DepreciationType["FUNCTIONAL"] = "functional";
    DepreciationType["EXTERNAL"] = "external";
    DepreciationType["ECONOMIC"] = "economic";
})(DepreciationType || (exports.DepreciationType = DepreciationType = {}));
// ============================================================================
 ...
 * ```
 */
async function conductPropertyAppraisal(propertyId, purpose, effectiveDate) {
 ...
 * ```
 */
async function performSalesComparisonApproach(propertyId, effectiveDate) {
    ...
    const adjustedComparables = comparables.map(comp => {
        ...
        const totalAdjustment = adjustments.reduce((sum, adj) => sum.plus(adj.value), new decimal_js_1.default(0));
        return {
    ...
    const indicatedValue = adjustedComparables.reduce((sum, comp) => sum.plus(comp.adjustedPrice.mul((comp.similarityScore || 50) / totalWeight)), new decimal_js_1.default(0));
    return {
 ...
 * ```
 */
async function performIncomeApproach(propertyId, effectiveDate) {
    const propertyData = await fetchPropertyData(propertyId);
    ...
    const grossScheduledIncome = incomeData.annualRent || new decimal_js_1.default(0);
    ...
    const totalOperatingExpenses = operatingExpenses.reduce((sum, exp) => sum.plus(exp.amount), new decimal_js_1.default(0));
 ...
 * ```
 */
async function performCostApproach(propertyId, effectiveDate) {
    const propertyData = await fetchPropertyData(propertyId);
    ...
    const totalImprovementCost = improvementCosts.reduce((sum, cost) => sum.plus(cost.totalCost), new decimal_js_1.default(0));
    ...
    const totalDepreciation = depreciation.reduce((sum, dep) => sum.plus(dep.amount), new decimal_js_1.default(0));
 ...
 * ```
 */
function calculateDepreciation(propertyData, effectiveAge, economicLife, replacementCost) {
    const depreciation = [];
 ...
 * ```
 */
async function findComparableProperties(propertyId, effectiveDate, maxComparables = 5) {
    const subjectProperty = await fetchPropertyData(propertyId);
    // Search criteria
 ...
 * ```
 */
function calculateSimilarityScore(subjectProperty, comparable) {
    let score = 100;
    ...
    const conditionDiff = Math.abs((conditionScore[subjectProperty.condition] || 4) - (conditionScore[comparable.condition] || 4));
 ...
 * ```
 */
function calculatePropertyAdjustments(subjectProperty, comparable) {
    const adjustments = [];
    // Size adjustment
 ...
 * ```
 */
async function analyzeComparableSalesTrends(propertyType, location, months = 12) {
    ...
    return {
        ...
        averagePrice: sales.reduce((sum, s) => sum.plus(s.price), new decimal_js_1.default(0)).div(sales.length),
 ...
 * ```
 */
async function calculateNetOperatingIncome(propertyId, year) {
    const incomeData = await fetchPropertyIncomeData(propertyId, new Date(year, 0, 1));
    const grossScheduledIncome = incomeData.annualRent || new decimal_js_1.default(0);
    const otherIncome = incomeData.otherIncome || new decimal_js_1.default(0);
    ...
    const operatingExpenses = incomeData.expenses?.reduce((sum, exp) => sum.plus(exp.amount), new decimal_js_1.default(0)) || new decimal_js_1.default(0);
 ...
 * ```
 */
async function calculateMarketCapRate(propertyType, location) {
    ...
    if (comparableSales.length === 0) {
        ...
        const defaultCapRates = {
            hospital: new decimal_js_1.default(0.065),
            medical_office: new decimal_js_1.default(0.070),
            clinic: new decimal_js_1.default(0.075),
            surgical_center: new decimal_js_1.default(0.068),
            diagnostic_center: new decimal_js_1.default(0.072),
        };
        return defaultCapRates[propertyType] || new decimal_js_1.default(0.070);
    }
    // Calculate cap rates from comparable sales
    const capRates = comparableSales.map(sale => sale.noi.div(sale.salePrice));
 ...
 * ```
 */
async function performDCFAnalysis(propertyId, holdingPeriod = 10, discountRate = new decimal_js_1.default(0.10)) {
    const propertyData = await fetchPropertyData(propertyId);
    const currentNOI = await calculateNetOperatingIncome(propertyId, new Date().getFullYear());
    // Project cash flows
    const annualGrowthRate = new decimal_js_1.default(0.03); // 3% NOI growth
    const cashFlows = [];
    for (let year = 1; year <= holdingPeriod; year++) {
        const projectedNOI = currentNOI.mul(decimal_js_1.default.pow(1 + annualGrowthRate.toNumber(), year));
        const discountFactor = decimal_js_1.default.pow(1 + discountRate.toNumber(), year);
    ...
    const terminalNOI = currentNOI.mul(decimal_js_1.default.pow(1 + annualGrowthRate.toNumber(), holdingPeriod));
    const terminalCapRate = new decimal_js_1.default(0.08); // Exit cap rate
    const terminalValue = terminalNOI.div(terminalCapRate);
    const terminalPV = terminalValue.div(decimal_js_1.default.pow(1 + discountRate.toNumber(), holdingPeriod));
    const totalPV = cashFlows.reduce((sum, cf) => sum.plus(cf.pv), new decimal_js_1.default(0)).plus(terminalPV);
    return {
        propertyId,
        holdingPeriod,
        discountRate,
 ...
 * ```
 */
function calculateGrossIncomeMultiplier(propertyValue, grossIncome) {
    if (grossIncome.isZero()) {
        throw new common_1.BadRequestException('Gross income cannot be zero');
    }
 ...
 * ```
 */
function calculateDebtServiceCoverageRatio(netOperatingIncome, annualDebtService) {
    if (annualDebtService.isZero()) {
        throw new common_1.BadRequestException('Annual debt service cannot be zero');
    }
 ...
 * ```
 */
async function estimateLandValue(propertyId, effectiveDate) {
    const propertyData = await fetchPropertyData(propertyId);
    ...
    if (landSales.length === 0) {
        ...
        const defaultLandValuePerAcre = new decimal_js_1.default(500000);
 ...
 * ```
 */
async function calculateReplacementCost(propertyData) {
    const costData = await fetchConstructionCosts(propertyData.propertyType, propertyData.location);
    const costs = [];
    // Building structure
    const baseCostPerSF = costData.baseCost || new decimal_js_1.default(250);
    costs.push({
        category: 'structure',
    ...
    if (propertyData.propertyType.includes('hospital') || propertyData.propertyType.includes('medical')) {
        const medicalSystemsCost = new decimal_js_1.default(75);
        costs.push({
    ...
    const mepCostPerSF = new decimal_js_1.default(50);
    costs.push({
        category: 'mep',
    ...
    const totalHardCosts = costs.reduce((sum, c) => sum.plus(c.totalCost), new decimal_js_1.default(0));
    const softCostsRate = new decimal_js_1.default(0.15); // 15% of hard costs
    costs.push({
        category: 'soft_costs',
    ...
    // Developer profit
    const developerProfitRate = new decimal_js_1.default(0.10); // 10%
    const totalCosts = costs.reduce((sum, c) => sum.plus(c.totalCost), new decimal_js_1.default(0));
    costs.push({
 ...
 * ```
 */
async function calculateAccruedDepreciation(propertyId, replacementCost) {
    const propertyData = await fetchPropertyData(propertyId);
    const effectiveAge = new Date().getFullYear() - propertyData.yearBuilt;
    const economicLife = 50;
    const depreciation = calculateDepreciation(propertyData, effectiveAge, economicLife, replacementCost);
    const totalDepreciation = depreciation.reduce((sum, d) => sum.plus(d.amount), new decimal_js_1.default(0));
    return {
        propertyId,
        effectiveAge,
        economicLife,
 ...
 * ```
 */
async function performRegressionAnalysis(propertyType, location, propertyCharacteristics) {
 ...
 * ```
 */
async function estimateMarketValue(propertyId, effectiveDate) {
    const salesComparison = await performSalesComparisonApproach(propertyId, effectiveDate);
    const incomeApproach = await performIncomeApproach(propertyId, effectiveDate);
    const costApproach = await performCostApproach(propertyId, effectiveDate);
 ...
 * ```
 */
function calculateValueConfidenceInterval(estimatedValue, standardError, confidenceLevel = 95) {
    ...
    const margin = new decimal_js_1.default(standardError).mul(zScore);
    return {
 ...
 * ```
 */
async function performAutomatedValuation(propertyId, modelName = 'hedonic_pricing') {
    const propertyData = await fetchPropertyData(propertyId);
 ...
 * ```
 */
function performHedonicPricing(propertyData, comparables) {
    // Base value from comparables
    const baseValue = comparables.length > 0
        ? comparables.reduce((sum, c) => sum.plus(c.salePrice), new decimal_js_1.default(0)).div(comparables.length)
    ...
    const totalContribution = factors.reduce((sum, f) => sum.plus(f.contribution), new decimal_js_1.default(0));
 ...
 * ```
 */
function calculateAVMConfidence(propertyData, comparableCount) {
    let confidence = 100;
 ...
 * ```
 */
function applyMarketConditionsAdjustment(baseValue, marketCondition, monthsSinceComparable) {
    const monthlyRates = {
 ...
 * ```
 */
function calculateLocationAdjustment(subjectLocation, comparableLocation, comparableValue) {
 ...
 * ```
 */
function applyFunctionalObsolescenceAdjustment(propertyData, replacementCost) {
    let adjustmentRate = 0;
    const issues = [];
    // Outdated systems
 ...
 * ```
 */
async function generateAppraisalReport(propertyId, purpose, effectiveDate) {
    ...
    const finalValue = valuations.reduce((sum, v) => sum.plus(v.indicatedValue.mul(v.weight)), new decimal_js_1.default(0));
    const report = {
        reportId,
        propertyId,
        tenantId: propertyData.tenantId,
 ...
 * ```
 */
async function exportAppraisalReportToPDF(reportId) {
 ...
 * ```
 */
async function getPropertyValuationHistory(propertyId) {
    ...
    const trends = Object.keys(yearlyGroups).map(year => {
        ...
        const avgValue = yearValuations.reduce((sum, v) => sum.plus(v.estimatedValue), new decimal_js_1.default(0)).div(yearValuations.length);
        return {
            period: year,
 ...
 * ```
 */
async function trackPropertyValueChanges(propertyId, years = 5) {
    ...
    return {
        ...
        peakValue: recentValuations.reduce((max, v) => v.estimatedValue.gt(max) ? v.estimatedValue : max, new decimal_js_1.default(0)),
        averageValue: recentValuations.reduce((sum, v) => sum.plus(v.estimatedValue), new decimal_js_1.default(0)).div(recentValuations.length),
    };
}
/**
 ...
 * ```
 */
async function compareToHistoricalValues(propertyId, currentValue) {
    ...
    const averageValue = historicalValues.reduce((sum, v) => sum.plus(v), new decimal_js_1.default(0))
        .div(historicalValues.length);
    const maxValue = historicalValues.reduce((max, v) => v.gt(max) ? v : max, new decimal_js_1.default(0));
 ...
 * ```
 */
async function analyzeMarketTrends(propertyType, market, months = 12) {
    ...
    if (sales.length === 0) {
        throw new common_1.NotFoundException('No sales data available for market analysis');
    }
    const prices = sales.map(s => s.price);
    const medianPrice = calculateMedian(prices);
    const averagePrice = prices.reduce((sum, p) => sum.plus(p), new decimal_js_1.default(0)).div(prices.length);
    ...
    const firstHalfAvg = firstHalfSales.reduce((sum, s) => sum.plus(s.price), new decimal_js_1.default(0)).div(firstHalfSales.length || 1);
    const secondHalfAvg = secondHalfSales.reduce((sum, s) => sum.plus(s.price), new decimal_js_1.default(0)).div(secondHalfSales.length || 1);
 ...
 * ```
 */
async function forecastPropertyValue(propertyId, months = 12) {
    const propertyData = await fetchPropertyData(propertyId);
    const history = await getPropertyValuationHistory(propertyId);
 ...
 * ```
 */
async function identifyComparableMarkets(market, propertyType) {
 ...
 */
function reconcileValuations(valuations) {
    return valuations.reduce((sum, v) => sum.plus(v.value.mul(v.weight)), new decimal_js_1.default(0));
}
/**
 * Determines appropriate weights for valuation approaches.
 */
function determineApproachWeights(propertyId, dataQuality) {
 ...
 */
function calculateAveragePricePerSF(sales) {
    if (sales.length === 0)
        return new decimal_js_1.default(0);
    const totalPricePerSF = sales.reduce((sum, s) => sum.plus(s.price.div(s.squareFeet || 1)), new decimal_js_1.default(0));
    return totalPricePerSF.div(sales.length);
}
/**
 * Calculates data quality score.
 */
function calculateDataQualityScore(propertyData) {
    let score = 100;
 ...
 */
function groupValuationsByYear(valuations) {
 ...
 */
function calculateValuationVolatility(valuations) {
...
function groupSalesByMonth(sales) {
    ...
    return Object.keys(groups).sort().map(month => ({
        month,
        sales: groups[month],
        averagePrice: groups[month].reduce((sum, s) => sum.plus(s.price), new decimal_js_1.default(0)).div(groups[month].length),
    }));
}
/**
 ...
 */
function calculatePriceVolatility(monthlyData) {
    if (monthlyData.length < 2)
        return 0;
 ...
 */
function generateMarketForecast(sales, forecastMonths) {
    const monthlyData = groupSalesByMonth(sales);
    const trendLine = calculateTrendLine(monthlyData);
    const forecast = [];
 ...
 */
function performMultipleLinearRegression(data, variables) {
    // Simplified regression - in production would use proper statistical library
    return {
        rSquared: 0.85,
        adjustedRSquared: 0.83,
        standardError: new decimal_js_1.default(50000),
        coefficients: variables.map(v => ({
            variable: v,
            coefficient: new decimal_js_1.default(Math.random() * 1000),
            standardError: new decimal_js_1.default(100),
            tStatistic: 2.5,
            pValue: 0.01,
            significance: true,
        })),
 ...
 */
function calculatePrediction(model, characteristics) {
    // Simplified prediction
    return new decimal_js_1.default(5000000);
}
/**
 * Performs repeat sales analysis.
 */
async function performRepeatSalesAnalysis(propertyId) {
 ...
 */
async function performNeuralNetworkValuation(propertyData) {
 ...
 */
function calculateMarketSimilarity(market1, market2) {
    let similarity = 100;
...
async function fetchPropertyData(propertyId) {
    ...
    async function fetchPropertyIncomeData(propertyId, effectiveDate) {
        return {
            annualRent: new decimal_js_1.default(1000000),
            vacancyRate: 5,
            otherIncome: new decimal_js_1.default(50000),
            expenses: [
                { category: 'maintenance', amount: new decimal_js_1.default(100000) },
                { category: 'utilities', amount: new decimal_js_1.default(75000) },
                { category: 'insurance', amount: new decimal_js_1.default(50000) },
                { category: 'property_tax', amount: new decimal_js_1.default(125000) },
            ],
        };
    }
    async function searchComparableSales(criteria) {
        return [
            {
                comparableId: 'comp-1',
                ...
                salePrice: new decimal_js_1.default(9500000),
                condition: PropertyCondition.GOOD,
                location: { latitude: 40.7200, longitude: -74.0100, zipCode: '10001', neighborhood: 'Midtown' },
                features: { parkingSpaces: 150, stories: 5 },
                pricePerSquareFoot: new decimal_js_1.default(100),
            },
        ];
    }
    async function fetchSalesHistory(propertyType, location, months) {
        return [];
    }
    async function fetchComparableSalesWithIncome(propertyType, location) {
        return [
            {
                salePrice: new decimal_js_1.default(10000000),
                noi: new decimal_js_1.default(700000),
            },
        ];
    }
    async function fetchConstructionCosts(propertyType, location) {
        return {
            baseCost: new decimal_js_1.default(250),
        };
    }
    async function fetchComparableLandSales(location, effectiveDate) {
        return [];
    }
    async function fetchPropertySalesData(propertyType, location) {
        return [];
    }
    async function fetchPropertyValuations(propertyId) {
        return [];
    }
    async function fetchMarketSales(propertyType, market, startDate, endDate) {
        return [
            {
                price: new decimal_js_1.default(10000000),
                saleDate: new Date(),
                squareFeet: 100000,
                daysOnMarket: 90,
            },
    ...
    async function getMarketInventory(propertyType, market) {
        return 50;
    }
    async function fetchMarketData(market, propertyType) {
        return {
            marketId: market,
            medianPrice: new decimal_js_1.default(10000000),
            priceAppreciation: 3.5,
        };
    }
    async function fetchAllMarkets(propertyType) {
        return [];
    }
    async function fetchAppraisalReport(reportId) {
        throw new Error('Not implemented');
    }
    async function saveValuationRecord(valuation) {
        // Save to database
    }
    async function saveAppraisalReport(report) {
        // Save to database
    }
    async function logValuationAudit(valuationId, propertyId, action) {
        // Log audit event
    }
...
//# sourceMappingURL=property-valuation-kit.js.map