...
exports.calculateSpaceTurnover = exports.exportOccupancyData = exports.compareOccupancyMetrics = exports.generateOccupancyHeatmap = exports.validateSensorHealth = exports.calculateRealtimeOccupancy = exports.aggregateMultiSensorReadings = exports.generateUtilizationReport = exports.resolveOccupancyAlert = exports.generateOccupancyAlert = exports.createCapacityPlan = exports.generateOccupancyForecast = exports.identifyPeakOccupancy = exports.identifyOccupancyPatterns = exports.analyzeOccupancyTrends = exports.calculateSpaceDensity = exports.calculateDeskUtilization = exports.updateDeskOccupancy = exports.aggregateBadgeOccupancy = exports.processBadgeSwipe = exports.detectOccupancyAnomaly = exports.calculateSpaceOccupancy = exports.recordOccupancyReading = exports.registerOccupancySensor = void 0;
// ============================================================================
// UTILITY FUNCTIONS
// ============================================================================
/**
 ...
 * ```
 */
const registerOccupancySensor = (sensorData) => {
    return {
...
exports.registerOccupancySensor = registerOccupancySensor;
/**
 ...
 * ```
 */
const recordOccupancyReading = (sensorId, spaceId, occupantCount, capacity, confidence, isEstimated = false) => {
...
exports.recordOccupancyReading = recordOccupancyReading;
/**
 ...
 * ```
 */
const calculateSpaceOccupancy = (readings, spaceId, spaceName, spaceType, capacity, floor = 'Unknown', building = 'Unknown') => {
    ...
    // Find peak occupancy
    const peakReading = sortedReadings.reduce((max, r) => r.occupantCount > max.occupantCount ? r : max, sortedReadings[0]);
...
exports.calculateSpaceOccupancy = calculateSpaceOccupancy;
/**
 ...
 * ```
 */
const detectOccupancyAnomaly = (reading, historicalReadings) => {
    const anomalies = [];
    ...
    if (historicalReadings.length >= 10) {
        ...
        const stdDev = Math.sqrt(historicalReadings.reduce((sum, r) => sum + Math.pow(r.occupantCount - avgOccupancy, 2), 0) / historicalReadings.length);
    ...
    const severity = anomalies.length >= 3 ? 'high' : anomalies.length >= 2 ? 'medium' : 'low';
    return {
        isAnomaly,
        anomalies,
        severity,
...
exports.detectOccupancyAnomaly = detectOccupancyAnomaly;
/**
 ...
 * ```
 */
const processBadgeSwipe = (eventData) => {
    return {
...
exports.processBadgeSwipe = processBadgeSwipe;
/**
 ...
 * ```
 */
const aggregateBadgeOccupancy = (swipeEvents, spaceId, timeRange) => {
    const relevantEvents = swipeEvents.filter(e => (!e.spaceId || e.spaceId === spaceId) &&
...
exports.aggregateBadgeOccupancy = aggregateBadgeOccupancy;
/**
 ...
 * ```
 */
const updateDeskOccupancy = (deskData) => {
    return {
...
exports.updateDeskOccupancy = updateDeskOccupancy;
/**
 ...
 * ```
 */
const calculateDeskUtilization = (desks, period) => {
    ...
    const desksByZone = {};
    desks.forEach(desk => {
        if (!desksByZone[desk.zone])
            desksByZone[desk.zone] = [];
    ...
    // Group by floor
    const utilizationByFloor = {};
    const desksByFloor = {};
    desks.forEach(desk => {
        if (!desksByFloor[desk.floor])
            desksByFloor[desk.floor] = [];
...
exports.calculateDeskUtilization = calculateDeskUtilization;
/**
 ...
 * ```
 */
const calculateSpaceDensity = (spaceId, spaceName, area, occupantCount, capacity) => {
    ...
    let densityRating;
    if (sqFtPerPerson < 50)
        densityRating = 'overcrowded';
    else if (sqFtPerPerson < 100)
        densityRating = 'very_high';
    else if (sqFtPerPerson < 150)
        densityRating = 'high';
    else if (sqFtPerPerson < 200)
        densityRating = 'moderate';
    else if (sqFtPerPerson < 300)
        densityRating = 'low';
    else
        densityRating = 'very_low';
...
exports.calculateSpaceDensity = calculateSpaceDensity;
/**
 ...
 * ```
 */
const analyzeOccupancyTrends = (spaceId, spaceName, readings, period) => {
    const filteredReadings = readings.filter(r => r.spaceId === spaceId && r.timestamp >= period.start && r.timestamp <= period.end);
    ...
    const direction = slope > 0.5 ? 'increasing' : slope < -0.5 ? 'decreasing' : 'stable';
    ...
    const ssResidual = dataPoints.reduce((sum, dp, i) => sum + Math.pow(dp.utilizationRate - predictedValues[i], 2), 0);
    const rSquared = ssTotal > 0 ? 1 - (ssResidual / ssTotal) : 0;
    const confidence = Math.max(0, Math.min(100, rSquared * 100));
    // Identify patterns
    const patterns = (0, exports.identifyOccupancyPatterns)(dataPoints);
    return {
        spaceId,
        spaceName,
        period,
...
exports.analyzeOccupancyTrends = analyzeOccupancyTrends;
/**
 ...
 * ```
 */
const identifyOccupancyPatterns = (dataPoints) => {
    const patterns = [];
...
exports.identifyOccupancyPatterns = identifyOccupancyPatterns;
/**
 ...
 * ```
 */
const identifyPeakOccupancy = (readings, period, spaceId, spaceName, capacity) => {
    const filteredReadings = readings.filter(r => r.spaceId === spaceId && r.timestamp >= period.start && r.timestamp <= period.end);
    if (filteredReadings.length === 0) {
        return {
            spaceId,
    ...
    // Find absolute peak
    const peakReading = filteredReadings.reduce((max, r) => r.occupantCount > max.occupantCount ? r : max, filteredReadings[0]);
    // Group by day of week and hour
    const peaksByDayHour = {};
    filteredReadings.forEach(reading => {
    ...
    let capacityAdequacy;
    if (peakUtilization > 100)
        capacityAdequacy = 'insufficient';
    else if (peakUtilization > 90)
        capacityAdequacy = 'adequate';
    else
        capacityAdequacy = 'excessive';
...
exports.identifyPeakOccupancy = identifyPeakOccupancy;
/**
 ...
 * ```
 */
const generateOccupancyForecast = (spaceId, spaceName, historicalReadings, forecastPeriod, capacity) => {
...
exports.generateOccupancyForecast = generateOccupancyForecast;
/**
 ...
 * ```
 */
const createCapacityPlan = (propertyId, planName, spaces, targetUtilization) => {
...
exports.createCapacityPlan = createCapacityPlan;
/**
 ...
 * ```
 */
const generateOccupancyAlert = (spaceId, spaceName, alertType, currentValue, threshold, severity) => {
    const messages = {
...
exports.generateOccupancyAlert = generateOccupancyAlert;
/**
 ...
 * ```
 */
const resolveOccupancyAlert = (alert, actionTaken) => {
    return {
        ...alert,
        isResolved: true,
        resolvedAt: new Date(),
...
exports.resolveOccupancyAlert = resolveOccupancyAlert;
/**
 ...
 * ```
 */
const generateUtilizationReport = (propertyId, spaces, reportPeriod) => {
    const totalSpaces = spaces.length;
    const averageUtilization = spaces.reduce((sum, s) => sum + s.utilizationRate, 0) / totalSpaces;
    ...
    const totalOccupancyHours = spaces.reduce((sum, s) => sum + (s.utilizationRate / 100) * periodHours, 0);
    // Group by space type
    const utilizationBySpaceType = {};
    const spacesByType = {};
    spaces.forEach(space => {
        if (!spacesByType[space.spaceType])
            spacesByType[space.spaceType] = [];
    ...
    Object.entries(spacesByType).forEach(([type, typeSpaces]) => {
        ...
        utilizationBySpaceType[type] = Math.round(avgUtil * 100) / 100;
    });
    // Group by floor
    const utilizationByFloor = {};
    const spacesByFloor = {};
    spaces.forEach(space => {
        if (!spacesByFloor[space.floor])
            spacesByFloor[space.floor] = [];
 ...
 * ```
 */
const aggregateMultiSensorReadings = (readings, spaceId) => {
    if (readings.length === 0) {
    ...
    const weightedOccupancy = readings.reduce((sum, r) => sum + (r.occupantCount * r.confidence / totalConfidence), 0);
...
exports.aggregateMultiSensorReadings = aggregateMultiSensorReadings;
/**
 ...
 * ```
 */
const calculateRealtimeOccupancy = (sensors, recentReadings) => {
    const activeSensors = sensors.filter(s => s.status === 'active').length;
    const inactiveSensors = sensors.length - activeSensors;
    // Group readings by space
    const readingsBySpace = {};
    recentReadings.forEach(reading => {
        if (!readingsBySpace[reading.spaceId])
            readingsBySpace[reading.spaceId] = [];
    ...
    Object.entries(readingsBySpace).forEach(([spaceId, readings]) => {
        ...
        const latestReading = readings.reduce((latest, r) => r.timestamp > latest.timestamp ? r : latest, readings[0]);
...
exports.calculateRealtimeOccupancy = calculateRealtimeOccupancy;
/**
 ...
 * ```
 */
const validateSensorHealth = (sensor, recentReadings) => {
    const issues = [];
    const recommendations = [];
    let healthScore = 100;
    // Check sensor status
    ...
    if (sensorReadings.length > 0) {
        const lastReading = sensorReadings.reduce((latest, r) => r.timestamp > latest.timestamp ? r : latest, sensorReadings[0]);
...
exports.validateSensorHealth = validateSensorHealth;
/**
 ...
 * ```
 */
const generateOccupancyHeatmap = (propertyId, spaces, timeRange, resolution, floor) => {
...
exports.generateOccupancyHeatmap = generateOccupancyHeatmap;
/**
 ...
 * ```
 */
const compareOccupancyMetrics = (spaces, metric) => {
    if (spaces.length === 0) {
...
exports.compareOccupancyMetrics = compareOccupancyMetrics;
/**
 ...
 * ```
 */
const exportOccupancyData = (spaces, format) => {
    if (format === 'json') {
...
exports.exportOccupancyData = exportOccupancyData;
/**
 ...
 * ```
 */
const calculateSpaceTurnover = (swipeEvents, spaceId, period) => {
    const relevantEvents = swipeEvents.filter(e => (!e.spaceId || e.spaceId === spaceId) &&
...
exports.calculateSpaceTurnover = calculateSpaceTurnover;
//# sourceMappingURL=property-occupancy-analytics-kit.js.map