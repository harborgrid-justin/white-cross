...
exports.getAllConnectedSockets = getAllConnectedSockets;
...
exports.getNamespaceConnectionCount = getNamespaceConnectionCount;
exports.hasPermission = hasPermission;
exports.hasAnyPermission = hasAnyPermission;
exports.hasAllPermissions = hasAllPermissions;
exports.storeAuthMetadata = storeAuthMetadata;
...
exports.validateAndTransformEvent = validateAndTransformEvent;
exports.validateMessageSize = validateMessageSize;
exports.emitToUser = emitToUser;
exports.emitToRooms = emitToRooms;
...
exports.broadcastWithPriority = broadcastWithPriority;
exports.broadcastToNamespaceCompressed = broadcastToNamespaceCompressed;
...
exports.formatWsError = formatWsError;
exports.handleWsError = handleWsError;
exports.formatWsSuccess = formatWsSuccess;
exports.setupHeartbeat = setupHeartbeat;
exports.measureRTT = measureRTT;
exports.handleReconnection = handleReconnection;
exports.storeClientSession = storeClientSession;
 ...
 * ```
 */
function createWsException(message, code, metadata) {
    const error = {
        message,
        code: code || 'WS_ERROR',
 ...
 * ```
 */
async function authenticateSocket(client, jwtService) {
    const token = extractToken(client);
    if (!token) {
        throw createWsException('Missing authentication token', 'UNAUTHORIZED');
    }
 ...
 * ```
 */
function hasRole(client, allowedRoles) {
    const user = getSocketUser(client);
    if (!user)
        return false;
 ...
 * ```
 */
async function getAllConnectedSockets(server) {
 ...
 * ```
 */
async function getSocketById(server, socketId) {
    const sockets = await server.fetchSockets();
 ...
 * ```
 */
async function getSocketsByUserId(server, userId) {
    const sockets = await server.fetchSockets();
 ...
 * ```
 */
async function disconnectUserSockets(server, userId, reason) {
 ...
 * ```
 */
async function getConnectionCount(server) {
    const sockets = await server.fetchSockets();
 ...
 * ```
 */
async function joinRoom(client, roomId, metadata) {
 ...
 * ```
 */
async function leaveRoom(client, roomId, notify = false) {
    if (notify) {
        const user = getSocketUser(client);
 ...
 * ```
 */
async function getRoomSize(client, roomId) {
 ...
 * ```
 */
async function getRoomMembers(client, roomId) {
    const sockets = await client.to(roomId).fetchSockets();
    return sockets.map((socket) => socket.id);
}
/**
 ...
 * ```
 */
function broadcastToRoom(client, roomId, event, data) {
 ...
 * ```
 */
function getNamespace(server, namespacePath) {
 ...
 * ```
 */
function broadcastToNamespace(server, namespacePath, event, data) {
 ...
 * ```
 */
async function getNamespaceConnectionCount(server, namespacePath) {
    const namespace = getNamespace(server, namespacePath);
 ...
 * ```
 */
function hasPermission(client, requiredPermission) {
    const user = getSocketUser(client);
    if (!user?.permissions)
        return false;
 ...
 * ```
 */
function hasAnyPermission(client, requiredPermissions) {
    const user = getSocketUser(client);
    if (!user?.permissions)
        return false;
 ...
 * ```
 */
function hasAllPermissions(client, requiredPermissions) {
    const user = getSocketUser(client);
    if (!user?.permissions)
        return false;
 ...
 * ```
 */
async function storeAuthMetadata(redis, socketId, user, ttlSeconds = 3600) {
 ...
 * ```
 */
async function getAuthMetadata(redis, socketId) {
    const key = `ws:auth:${socketId}`;
 ...
 * ```
 */
function validateMessagePayload(payload, requiredFields) {
    const errors = [];
 ...
 */
function sanitizeMessageContent(content, maxLength = 5000) {
 ...
 * ```
 */
function validateAndTransformEvent(data, requiredFields, transformers) {
 ...
 * ```
 */
function validateMessageSize(payload, maxSizeBytes = 10240) {
 ...
 * ```
 */
async function emitToUser(server, userId, event, data) {
    const userSockets = await getSocketsByUserId(server, userId);
    userSockets.forEach((socket) => {
 ...
 * ```
 */
function emitToRooms(server, roomIds, event, data) {
 ...
 * ```
 */
async function emitWithAck(client, event, data, timeoutMs = 5000) {
 ...
 * ```
 */
function broadcastGlobally(server, event, data) {
 ...
 * ```
 */
function broadcastToRoomExcept(server, roomId, event, data, excludeSocketIds) {
 ...
 * ```
 */
async function broadcastToRole(server, roles, event, data) {
 ...
 */
function broadcastWithPriority(server, target, event, data, priority = 'normal') {
    const payload = {
        ...data,
        priority,
        timestamp: new Date(),
 ...
 * ```
 */
function broadcastToNamespaceCompressed(server, namespacePath, event, data, compress = true) {
    const namespace = getNamespace(server, namespacePath);
    if (compress) {
 ...
 * ```
 */
async function trackConnection(redis, socketId, metadata, ttlSeconds = 86400) {
 ...
 * ```
 */
async function updateLastActivity(redis, socketId) {
    const key = `ws:connection:${socketId}`;
    const data = await redis.get(key);
    if (data) {
 ...
 * ```
 */
function formatWsError(error, context) {
 ...
 */
function handleWsError(error, client, event, payload) {
    const user = getSocketUser(client);
 ...
 * ```
 */
function formatWsSuccess(data, message) {
    return {
        success: true,
        data,
        ...(message && { message }),
 ...
 * ```
 */
function setupHeartbeat(client, config) {
 ...
 * ```
 */
async function measureRTT(client) {
 ...
 * ```
 */
async function handleReconnection(redis, client, previousSocketId) {
    try {
 ...
 * ```
 */
async function storeClientSession(redis, client, ttlSeconds = 3600) {
 ...
 * ```
 */
async function queueMessageForUser(redis, userId, message, ttlSeconds = 604800) {
 ...
 * ```
 */
async function deliverQueuedMessages(redis, client, userId, removeAfterDelivery = true) {
    const queueKey = `ws:queue:${userId}`;
 ...
 * ```
 */
async function checkRateLimit(redis, identifier, config) {
 ...
 * ```
 */
async function getRateLimitStatus(redis, identifier, config) {
    const key = `ws:ratelimit:${identifier}`;
    const blockKey = `ws:ratelimit:blocked:${identifier}`;
    // Check if blocked
...
//# sourceMappingURL=nest-websockets-utils.js.map