...
interface SubmittalData {
  ...
  specSection: string;
  title: string;
  type: 'shop_drawing' | 'product_data' | 'sample' | 'test_report' | 'material_cert' | 'other';
  description: string;
  submittedBy: string;
  ...
  status: 'draft' | 'submitted' | 'under_review' | 'approved' | 'approved_as_noted' | 'revise_resubmit' | 'rejected' | 'closed';
...
interface ReviewRoutingStep {
  stepNumber: number;
  reviewerRole: string;
  reviewerName: string;
  reviewerEmail: string;
  isParallel: boolean;
  requiredAction: 'review' | 'approve' | 'comment' | 'acknowledge';
  daysAllowed: number;
...
interface ReviewActionData {
  ...
  reviewerName: string;
  reviewDate: Date;
  action: 'approved' | 'approved_as_noted' | 'revise_resubmit' | 'rejected' | 'no_exception_taken';
  comments: string;
  markupUrls?: string[];
  deficiencies?: string[];
  nextAction?: string;
...
interface SubmittalComplianceCheck {
  submittalId: string;
  specSection: string;
  ...
  specificationMatches: SpecificationMatch[];
...
interface SpecificationMatch {
  ...
  matches: boolean;
  variance?: string;
  acceptabilityRating: 'compliant' | 'acceptable_variance' | 'non_compliant';
}

interface NonCompliantItem {
  itemDescription: string;
  requirement: string;
  actualProvided: string;
  severity: 'minor' | 'major' | 'critical';
  impact: string;
  recommendedAction: string;
}

interface SubmittalMetrics {
  projectId: string;
  periodStart: Date;
  periodEnd: Date;
  ...
  topDelayedSpecs: SpecDelayMetric[];
}

interface SpecDelayMetric {
  specSection: string;
...
interface SubmittalNotification {
  submittalId: string;
  notificationType: 'new_submittal' | 'review_assigned' | 'review_completed' | 'approval' | 'rejection' | 'overdue_alert' | 'escalation';
  recipients: string[];
  subject: string;
  message: string;
...
interface SubmittalAuditEntry {
  submittalId: string;
  action: 'created' | 'submitted' | 'reviewed' | 'approved' | 'rejected' | 'resubmitted' | 'closed' | 'status_change';
  performedBy: string;
  performedAt: Date;
  fromStatus?: string;
  toStatus?: string;
...
interface SubmittalSchedule {
  projectId: string;
  specSection: string;
  submittalDescription: string;
  responsibleParty: string;
  plannedSubmitDate: Date;
  actualSubmitDate?: Date;
  requiredApprovalDate: Date;
  actualApprovalDate?: Date;
  leadTimeRequired: number;
  status: 'not_started' | 'in_preparation' | 'submitted' | 'approved' | 'overdue';
  criticalPath: boolean;
  dependencies: string[];
}

interface SubmittalDashboard {
  projectId: string;
  asOfDate: Date;
  totalSubmittals: number;
  pendingReview: number;
  awaitingResubmittal: number;
  approved: number;
  rejected: number;
  overdueSubmittals: SubmittalRegisterEntry[];
  criticalPathItems: SubmittalSchedule[];
  reviewBacklog: { reviewer: string; count: number }[];
  recentActivity: SubmittalAuditEntry[];
}
...
/**
 * Sequelize model for Construction Submittals with comprehensive tracking.
 *
 * @swagger
 * @openapi
 * components:
 ...
 *         status:
 *           type: string
 *           enum: [draft, submitted, under_review, approved, approved_as_noted, revise_resubmit, rejected, closed]
 *
 * @param {Sequelize} sequelize - Sequelize instance
 * @returns {Model} ConstructionSubmittal model
 *
 * @example
 * ```typescript
 * const ConstructionSubmittal = createConstructionSubmittalModel(sequelize);
 ...
 * ```
 */
export const createConstructionSubmittalModel = (sequelize: Sequelize) => {
  class ConstructionSubmittal extends Model {
    public id!: string;
    public projectId!: string;
    public submittalNumber!: string;
    public specSection!: string;
    public title!: string;
    public type!: string;
    public description!: string;
    public submittedBy!: string;
    public submittedByCompany!: string;
    ...
    public originalSubmittalId!: string | null;
    public ballInCourt!: string;
    public assignedReviewer!: string | null;
    public assignedReviewerEmail!: string | null;
    public reviewStartDate!: Date | null;
    public reviewCompletedDate!: Date | null;
    public finalAction!: string | null;
    public finalActionBy!: string | null;
    public finalActionDate!: Date | null;
    public leadTimeDays!: number;
    public daysInReview!: number;
    public isOverdue!: boolean;
    public closedDate!: Date | null;
    public closedBy!: string | null;
    public documentUrls!: string[];
    public markupUrls!: string[];
    public complianceVerified!: boolean;
  ...
  ConstructionSubmittal.init(
    {
      ...
      submittalNumber: {
        type: DataTypes.STRING(50),
        allowNull: false,
        comment: 'Submittal tracking number',
        validate: {
          notEmpty: true,
        },
      },
      specSection: {
        type: DataTypes.STRING(50),
        allowNull: false,
        comment: 'Specification section number',
      },
      title: {
        type: DataTypes.STRING(500),
        allowNull: false,
        comment: 'Submittal title/description',
      },
      type: {
        type: DataTypes.ENUM('shop_drawing', 'product_data', 'sample', 'test_report', 'material_cert', 'other'),
        allowNull: false,
        comment: 'Type of submittal',
      },
      description: {
        type: DataTypes.TEXT,
        allowNull: false,
      ...
      dateRequired: {
        type: DataTypes.DATE,
        allowNull: false,
        comment: 'Required approval date',
      },
      dateReceived: {
        type: DataTypes.DATE,
        allowNull: true,
        comment: 'Date received by reviewer',
      },
      priority: {
        type: DataTypes.ENUM('low', 'medium', 'high', 'critical'),
        allowNull: false,
        defaultValue: 'medium',
        comment: 'Submittal priority',
      },
      status: {
        type: DataTypes.ENUM('draft', 'submitted', 'under_review', 'approved', 'approved_as_noted', 'revise_resubmit', 'rejected', 'closed'),
        allowNull: false,
        defaultValue: 'draft',
        comment: 'Current submittal status',
      },
      revisionNumber: {
      ...
      originalSubmittalId: {
        type: DataTypes.UUID,
        allowNull: true,
        comment: 'Original submittal if resubmission',
        references: {
          model: 'construction_submittals',
          key: 'id',
        },
      },
      ballInCourt: {
        type: DataTypes.STRING(100),
        allowNull: false,
        comment: 'Current responsible party',
      },
      assignedReviewer: {
        type: DataTypes.STRING(100),
        allowNull: true,
        comment: 'Assigned reviewer name',
      },
      ...
      reviewStartDate: {
        type: DataTypes.DATE,
        allowNull: true,
        comment: 'Review start date',
      },
      reviewCompletedDate: {
        type: DataTypes.DATE,
        allowNull: true,
        comment: 'Review completion date',
      },
      finalAction: {
        type: DataTypes.STRING(50),
        allowNull: true,
        comment: 'Final review action',
      },
      finalActionBy: {
        type: DataTypes.STRING(100),
        allowNull: true,
        comment: 'Final action by',
      },
      finalActionDate: {
        type: DataTypes.DATE,
        allowNull: true,
        comment: 'Final action date',
      },
      leadTimeDays: {
        type: DataTypes.INTEGER,
        allowNull: false,
      ...
      daysInReview: {
        ...
        allowNull: false,
        defaultValue: 0,
        comment: 'Days spent in review',
      },
      isOverdue: {
        type: DataTypes.BOOLEAN,
        allowNull: false,
        defaultValue: false,
        comment: 'Is submittal overdue',
      },
      closedDate: {
        type: DataTypes.DATE,
        allowNull: true,
      ...
      documentUrls: {
        ...
        allowNull: false,
        defaultValue: [],
        comment: 'Submitted document URLs',
      },
      markupUrls: {
        type: DataTypes.JSON,
        allowNull: false,
        defaultValue: [],
        comment: 'Markup/redline URLs',
      },
      complianceVerified: {
        type: DataTypes.BOOLEAN,
        allowNull: false,
        defaultValue: false,
        comment: 'Compliance verification complete',
      },
      complianceScore: {
  ...
  return ConstructionSubmittal;
};

/**
 * Sequelize model for Submittal Review History with detailed tracking.
 *
 * @swagger
 * @openapi
 * components:
 ...
 *         action:
 *           type: string
 *           enum: [approved, approved_as_noted, revise_resubmit, rejected, no_exception_taken]
 *
 * @param {Sequelize} sequelize - Sequelize instance
 * @returns {Model} SubmittalReview model
 *
 * @example
 * ```typescript
 * const SubmittalReview = createSubmittalReviewModel(sequelize);
 ...
 * ```
 */
export const createSubmittalReviewModel = (sequelize: Sequelize) => {
  class SubmittalReview extends Model {
    ...
    public markupUrls!: string[];
    public deficiencies!: string[];
    public nextAction!: string | null;
    public daysToReview!: number;
    public signature!: string | null;
    public isLatest!: boolean;
    public reviewStepNumber!: number;
    public metadata!: Record<string, any>;
    public readonly createdAt!: Date;
    public readonly updatedAt!: Date;
  }

  SubmittalReview.init(
    {
      id: {
        type: DataTypes.UUID,
        defaultValue: DataTypes.UUIDV4,
      ...
      action: {
        type: DataTypes.ENUM('approved', 'approved_as_noted', 'revise_resubmit', 'rejected', 'no_exception_taken'),
        allowNull: false,
      ...
      markupUrls: {
        ...
        allowNull: false,
        defaultValue: [],
        comment: 'Markup document URLs',
      },
      deficiencies: {
        type: DataTypes.JSON,
        allowNull: false,
        defaultValue: [],
        comment: 'List of deficiencies found',
      },
      nextAction: {
        type: DataTypes.STRING(200),
        allowNull: true,
      ...
      isLatest: {
        ...
        allowNull: false,
        defaultValue: true,
        comment: 'Is latest review for this submittal',
      },
      reviewStepNumber: {
        type: DataTypes.INTEGER,
        allowNull: false,
        defaultValue: 1,
        comment: 'Review workflow step number',
      },
      metadata: {
        type: DataTypes.JSON,
        allowNull: false,
  ...
  return SubmittalReview;
};

/**
 * Sequelize model for Submittal Workflow tracking.
 *
 * @swagger
 * @openapi
 * components:
 ...
 *
 * @param {Sequelize} sequelize - Sequelize instance
 * @returns {Model} SubmittalWorkflow model
 *
 * @example
 * ```typescript
 * const SubmittalWorkflow = createSubmittalWorkflowModel(sequelize);
 ...
 * ```
 */
export const createSubmittalWorkflowModel = (sequelize: Sequelize) => {
  class SubmittalWorkflow extends Model {
    public id!: string;
    public submittalId!: string;
    public workflowType!: string;
    public steps!: ReviewRoutingStep[];
    ...
    public escalatedTo!: string | null;
    public pausedAt!: Date | null;
    public pauseReason!: string | null;
    public resumedAt!: Date | null;
    public totalDaysActive!: number;
    public metadata!: Record<string, any>;
    public readonly createdAt!: Date;
    public readonly updatedAt!: Date;
  }

  SubmittalWorkflow.init(
    {
      id: {
        type: DataTypes.UUID,
        defaultValue: DataTypes.UUIDV4,
      ...
      workflowType: {
        type: DataTypes.ENUM('sequential', 'parallel', 'hybrid'),
        allowNull: false,
        comment: 'Workflow execution type',
      },
      steps: {
        type: DataTypes.JSON,
        allowNull: false,
      ...
      currentStepIndex: {
        ...
        allowNull: false,
        defaultValue: 0,
        comment: 'Current workflow step index',
      },
      overallStatus: {
      ...
      escalationRequired: {
        ...
        allowNull: false,
        defaultValue: false,
        comment: 'Escalation required flag',
      },
      escalatedAt: {
        type: DataTypes.DATE,
        allowNull: true,
      ...
      pausedAt: {
        type: DataTypes.DATE,
        allowNull: true,
        comment: 'Workflow pause timestamp',
      },
      pauseReason: {
        type: DataTypes.TEXT,
        allowNull: true,
      ...
      resumedAt: {
        type: DataTypes.DATE,
        allowNull: true,
        comment: 'Workflow resume timestamp',
      },
      totalDaysActive: {
        type: DataTypes.INTEGER,
        allowNull: false,
        defaultValue: 0,
        comment: 'Total days workflow active',
      },
      metadata: {
        type: DataTypes.JSON,
        allowNull: false,
