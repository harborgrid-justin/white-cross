...
/**
 * Labor Plan Model - Sequelize
 * Manages labor planning and forecasting for construction projects
 */
...
/**
 * Timesheet Model - Sequelize
 * Tracks worker time entries and approval workflow
 */
...
/**
 * Worker Model - Sequelize
 * Manages worker profiles, certifications, and compliance
 */
...
/**
 * Labor plan interface
 */
export interface ILaborPlan {
  id: string;
  projectId: string;
  planName: string;
  startDate: Date;
...
/**
 * Certified payroll report interface
 */
...
/**
 * Union compliance record interface
 */
export interface UnionComplianceRecord {
  id: string;
  projectId: string;
  unionLocal: string;
  complianceDate: Date;
  ...
  requiredApprenticeRatio: number;
  isCompliant: boolean;
  violations?: string[];
...
/**
 * Create labor plan DTO
 */
export class CreateLaborPlanDto {
  ...
  @ApiProperty({ description: 'Budgeted labor cost' })
  @IsNumber()
  @Min(0)
  budgetedLaborCost: number;

  @ApiProperty({ description: 'Craft mix percentages' })
  @IsOptional()
  craftMix?: Record<string, number>;

  @ApiProperty({ description: 'Is prevailing wage project' })
  @IsBoolean()
  @IsOptional()
  isPrevailingWage?: boolean;
}
...
export class CreateCrewAssignmentDto {
  ...
  @ApiProperty({ description: 'Foreman worker ID' })
  @IsUUID()
  foremanId: string;

  @ApiProperty({ description: 'Worker IDs', type: [String] })
  @IsArray()
  @IsUUID('4', { each: true })
  workers: string[];

  @ApiProperty({ enum: LaborCraft })
  @IsEnum(LaborCraft)
  craft: LaborCraft;
...
export class SubmitTimesheetDto {
  ...
  @ApiProperty({ description: 'Week ending date' })
  @Type(() => Date)
  @IsDate()
  weekEnding: Date;

  @ApiProperty({ description: 'Daily time entries', type: [Object] })
  @IsArray()
...
export class CreateWorkerDto {
  ...
  @ApiProperty({ description: 'Is apprentice' })
  @IsBoolean()
  @IsOptional()
  isApprentice?: boolean;
}
