...
exports.identifySeasonalPatterns = exports.analyzeTrend = exports.detectRealtimeAnomaly = exports.aggregateRealtimeEvents = exports.trackRealtimeMetric = exports.invalidateReportCache = exports.getCacheStrategy = exports.generateReportCacheKey = exports.validateReportParameters = exports.calculateNextRun = exports.scheduleReport = exports.executeReport = exports.compareFunnels = exports.analyzeFunnel = exports.formatFunnelChartData = exports.generateHeatmapData = exports.formatTimeSeriesForChart = exports.formatChartData = exports.calculateHistogram = exports.performLinearRegression = exports.calculateCorrelation = exports.calculatePercentile = exports.calculateStatisticalSummary = exports.calculateKPITrend = exports.generateKPIScorecard = exports.trackKPI = exports.formatMetricValue = exports.evaluateMetricThreshold = exports.calculateDashboardMetrics = exports.calculateDashboardMetric = exports.exportPivotTable = exports.generatePivotTable = exports.forecastTimeSeries = exports.detectTimeSeriesAnomalies = exports.calculateMovingAverage = exports.aggregateTimeSeries = exports.buildCohortAggregation = exports.buildWindowAggregation = exports.buildMultiDimensionalAggregation = exports.createAggregationBuilder = exports.buildAggregationQuery = exports.createAnalyticsEventModel = exports.createMetricModel = exports.createDashboardModel = exports.createReportTemplateModel = void 0;
/**
 * File: /reuse/reporting-analytics-kit.ts
 ...
 * ```
 */
const createReportTemplateModel = (sequelize) => {
    class ReportTemplate extends sequelize_1.Model {
    }
    ReportTemplate.init({
        id: {
        ...
        type: {
            type: sequelize_1.DataTypes.ENUM('tabular', 'chart', 'pivot', 'custom', 'dashboard'),
            allowNull: false,
        ...
        format: {
            type: sequelize_1.DataTypes.ENUM('pdf', 'excel', 'csv', 'json', 'html'),
            allowNull: false,
            defaultValue: 'pdf',
...
exports.createReportTemplateModel = createReportTemplateModel;
/**
 ...
 * ```
 */
const createMetricModel = (sequelize) => {
 ...
 * ```
 */
const buildAggregationQuery = async (config) => {
    ...
    metrics.forEach(metric => {
        ...
        if (metric.aggregation === 'distinct') {
            attributes.push([
                sequelize_1.Sequelize.fn('COUNT', sequelize_1.Sequelize.fn('DISTINCT', sequelize_1.Sequelize.col(metric.field))),
                alias,
            ]);
        }
            ...
            attributes.push([
                sequelize_1.Sequelize.fn('PERCENTILE_CONT', percentile / 100, sequelize_1.Sequelize.literal(`WITHIN GROUP (ORDER BY "${metric.field}")`)),
                alias,
            ]);
        }
        else {
            attributes.push([
                sequelize_1.Sequelize.fn(metric.aggregation.toUpperCase(), sequelize_1.Sequelize.col(metric.field)),
                alias,
            ]);
        }
    });
 ...
 * ```
 */
const createAggregationBuilder = (model) => {
    const state = {
        attributes: [],
        groupByFields: [],
        whereClause: {},
    ...
    return {
        ...
        sum(field, alias) {
            state.attributes.push([sequelize_1.Sequelize.fn('SUM', sequelize_1.Sequelize.col(field)), alias]);
            return this;
        },
        avg(field, alias) {
            state.attributes.push([sequelize_1.Sequelize.fn('AVG', sequelize_1.Sequelize.col(field)), alias]);
            return this;
        },
        count(field, alias) {
            state.attributes.push([sequelize_1.Sequelize.fn('COUNT', sequelize_1.Sequelize.col(field)), alias]);
            return this;
        },
        countDistinct(field, alias) {
            state.attributes.push([
                sequelize_1.Sequelize.fn('COUNT', sequelize_1.Sequelize.fn('DISTINCT', sequelize_1.Sequelize.col(field))),
                alias,
            ]);
            return this;
        },
        min(field, alias) {
            state.attributes.push([sequelize_1.Sequelize.fn('MIN', sequelize_1.Sequelize.col(field)), alias]);
            return this;
        },
        max(field, alias) {
            state.attributes.push([sequelize_1.Sequelize.fn('MAX', sequelize_1.Sequelize.col(field)), alias]);
            return this;
        },
        where(conditions) {
 ...
 * ```
 */
const buildMultiDimensionalAggregation = async (model, dimensions, metrics, filters) => {
    const results = [];
    ...
    for (const combo of dimensionCombinations) {
        const result = await (0, exports.buildAggregationQuery)({
            model,
...
exports.buildMultiDimensionalAggregation = buildMultiDimensionalAggregation;
/**
 ...
 * ```
 */
const buildWindowAggregation = async (sequelize, tableName, config) => {
...
exports.buildWindowAggregation = buildWindowAggregation;
/**
 ...
 * ```
 */
const buildCohortAggregation = async (model, config) => {
...
exports.buildCohortAggregation = buildCohortAggregation;
// ============================================================================
 ...
 * ```
 */
const aggregateTimeSeries = async (config) => {
    const { model, dateField, valueField, interval, aggregation, dateRange, timezone = 'UTC', fillGaps = false, groupBy = [], } = config;
    const intervalMap = {
        hour: 'hour',
        day: 'day',
        week: 'week',
    ...
    const timeGroup = sequelize_1.Sequelize.fn('DATE_TRUNC', intervalMap[interval], sequelize_1.Sequelize.col(dateField));
    const attributes = [[timeGroup, 'timestamp'], ...groupBy];
    if (aggregation === 'count') {
        attributes.push([sequelize_1.Sequelize.fn('COUNT', sequelize_1.Sequelize.col(valueField)), 'value']);
    }
    else {
        attributes.push([
            sequelize_1.Sequelize.fn(aggregation.toUpperCase(), sequelize_1.Sequelize.col(valueField)),
            'value',
        ]);
    }
    const results = await model.findAll({
    ...
    while (currentDate <= endDate) {
        const existing = results.find((r) => new Date(r.timestamp).getTime() === currentDate.getTime());
        filledResults.push({
 ...
 * ```
 */
const detectTimeSeriesAnomalies = (data, config = {}) => {
 ...
 * ```
 */
const forecastTimeSeries = (data, periods, alpha = 0.3) => {
    if (data.length === 0)
        return [];
    const forecast = [];
 ...
 * ```
 */
const generatePivotTable = async (config) => {
    const { model, rows, columns, values, filters, totals = false } = config;
    // Get aggregated data
    const aggregated = await (0, exports.buildAggregationQuery)({
        model,
    ...
    const result = { data: pivotData };
    if (totals) {
        // Calculate row totals
        result.rowTotals = {};
        values.forEach(v => {
            const metricAlias = v.alias || v.name;
            result.rowTotals[metricAlias] = pivotData.reduce((sum, row) => sum +
                Object.keys(row)
                    .filter(k => k.endsWith(`_${metricAlias}`))
                    .reduce((s, k) => s + (row[k] || 0), 0), 0);
        });
        ...
        Array.from(columnValues).forEach(colKey => {
            values.forEach(v => {
                const valueKey = `${colKey}_${v.alias || v.name}`;
                result.columnTotals[valueKey] = pivotData.reduce((sum, row) => sum + (row[valueKey] || 0), 0);
            });
        });
        ...
        values.forEach(v => {
            const metricAlias = v.alias || v.name;
            result.grandTotal[metricAlias] = pivotData.reduce((sum, row) => sum +
                Object.keys(row)
                    .filter(k => k.endsWith(`_${metricAlias}`))
                    .reduce((s, k) => s + (row[k] || 0), 0), 0);
        });
 ...
 * ```
 */
const exportPivotTable = (pivotData, format) => {
    if (format === 'json') {
...
exports.exportPivotTable = exportPivotTable;
// ============================================================================
 ...
 * ```
 */
const calculateDashboardMetric = async (config) => {
...
exports.calculateDashboardMetric = calculateDashboardMetric;
/**
 ...
 * ```
 */
const calculateDashboardMetrics = async (metricConfigs) => {
    const metrics = await Promise.all(metricConfigs.map(config => (0, exports.calculateDashboardMetric)(config)));
    return metrics;
};
 ...
 * ```
 */
const evaluateMetricThreshold = (value, threshold) => {
...
exports.evaluateMetricThreshold = evaluateMetricThreshold;
/**
 ...
 * ```
 */
const formatMetricValue = (value, format, options) => {
 ...
 * ```
 */
const trackKPI = async (definition) => {
 ...
 * ```
 */
const generateKPIScorecard = async (definitions) => {
    const kpis = await Promise.all(definitions.map(def => (0, exports.trackKPI)(def)));
    const summary = {
    ...
    const overallScore = kpis.reduce((sum, k) => sum + (k.achievement || 0), 0) / kpis.length;
    return { kpis, overallScore, summary };
};
exports.generateKPIScorecard = generateKPIScorecard;
/**
 ...
 * ```
 */
const calculateKPITrend = async (definition, periods) => {
    const results = [];
    for (const period of periods) {
        const kpi = await (0, exports.trackKPI)(definition);
        results.push({
            date: period,
            value: kpi.value,
...
exports.calculateKPITrend = calculateKPITrend;
// ============================================================================
 ...
 * ```
 */
const calculateStatisticalSummary = (values, percentiles = [25, 50, 75, 90, 95, 99]) => {
    if (values.length === 0) {
    ...
    // Quartiles
    const q1 = (0, exports.calculatePercentile)(sorted, 25);
    const q2 = median;
    const q3 = (0, exports.calculatePercentile)(sorted, 75);
    // Custom percentiles
    const percentileResults = {};
    percentiles.forEach(p => {
        percentileResults[p] = (0, exports.calculatePercentile)(sorted, p);
    });
    return {
        count,
        sum,
...
const performLinearRegression = (x, y) => {
    ...
    const correlation = (0, exports.calculateCorrelation)(x, y);
 ...
 * ```
 */
const calculateHistogram = (values, bins = 10) => {
    if (values.length === 0)
        return [];
 ...
 * ```
 */
const formatTimeSeriesForChart = (data, options) => {
...
exports.formatTimeSeriesForChart = formatTimeSeriesForChart;
/**
 ...
 * ```
 */
const generateHeatmapData = (matrix, rowLabels, columnLabels) => {
    const heatmapData = [];
 ...
 * ```
 */
const formatFunnelChartData = (funnelData) => {
...
exports.formatFunnelChartData = formatFunnelChartData;
// ============================================================================
 ...
 * ```
 */
const analyzeFunnel = async (model, stages, config) => {
    const { dateRange, filters = {}, sessionField = 'sessionId' } = config;
    const where = { ...filters };
    if (dateRange) {
        where.timestamp = { [sequelize_1.Op.between]: [dateRange.start, dateRange.end] };
    }
    const funnelStages = [];
    let previousCount = 0;
    ...
    const bottleneck = funnelStages.reduce((max, stage) => (stage.dropoffPercent || 0) > (max.dropoffPercent || 0) ? stage : max).name;
    return {
        stages: funnelStages,
        totalEntries,
        totalConversions,
...
exports.analyzeFunnel = analyzeFunnel;
/**
 ...
 * ```
 */
const compareFunnels = async (funnelConfigs) => {
    const results = await Promise.all(funnelConfigs.map(async (fc) => ({
        name: fc.name,
        funnel: await (0, exports.analyzeFunnel)(fc.model, fc.stages, fc.config),
    })));
    return results;
};
exports.compareFunnels = compareFunnels;
// ============================================================================
 ...
 * ```
 */
const executeReport = async (definition, parameters) => {
    ...
    switch (type) {
        case 'tabular':
            results = await (0, exports.buildAggregationQuery)({
                ...query,
                filters: { ...query.filters, ...parameters },
            });
            break;
        case 'pivot':
            results = await (0, exports.generatePivotTable)({
                ...query,
                filters: { ...query.filters, ...parameters },
            });
            break;
        case 'chart':
            const chartData = await (0, exports.buildAggregationQuery)({
                ...query,
                filters: { ...query.filters, ...parameters },
            });
            results = (0, exports.formatChartData)({
 ...
 * ```
 */
const scheduleReport = (definition, schedule) => {
    const nextRun = (0, exports.calculateNextRun)(schedule);
    return {
 ...
 * ```
 */
const validateReportParameters = (paramDefs, values) => {
    const errors = [];
 ...
 * ```
 */
const generateReportCacheKey = (reportId, parameters) => {
...
exports.generateReportCacheKey = generateReportCacheKey;
/**
 ...
 * ```
 */
const getCacheStrategy = (definition) => {
 ...
 * ```
 */
const invalidateReportCache = (reportId, parameters) => {
    if (parameters) {
        return [(0, exports.generateReportCacheKey)(reportId, parameters)];
    }
 ...
 * ```
 */
const trackRealtimeMetric = (metric, value, metadata) => {
    return {
        metric,
        value,
        timestamp: new Date(),
...
exports.trackRealtimeMetric = trackRealtimeMetric;
/**
 ...
 * ```
 */
const aggregateRealtimeEvents = (events, windowSeconds) => {
    if (events.length === 0)
        return [];
    ...
    for (let windowStart = firstTimestamp; windowStart <= lastTimestamp; windowStart += windowMs) {
        const windowEnd = windowStart + windowMs;
        const windowEvents = sorted.filter(e => e.timestamp.getTime() >= windowStart && e.timestamp.getTime() < windowEnd);
...
exports.aggregateRealtimeEvents = aggregateRealtimeEvents;
/**
 ...
 * ```
 */
const detectRealtimeAnomaly = (recentValues, currentValue, config = {}) => {
    ...
    const stats = (0, exports.calculateStatisticalSummary)(recentValues);
...
exports.detectRealtimeAnomaly = detectRealtimeAnomaly;
// ============================================================================
 ...
 * ```
 */
const analyzeTrend = (data, options) => {
    const { smoothing = false, confidenceLevel = 0.95 } = options || {};
    let values = data.map(d => d.value);
    if (smoothing && values.length >= 7) {
        const smoothed = (0, exports.calculateMovingAverage)(data, 7);
    ...
    const regression = (0, exports.performLinearRegression)(values.map((_, i) => i), values);
 ...
 * ```
 */
const identifySeasonalPatterns = (data, seasonLength) => {
    ...
    const pattern = new Array(seasonLength).fill(0);
    ...
    const seasonalVariance = pattern.reduce((acc, v) => acc + Math.pow(v - overallMean, 2), 0);
...
// HELPER FUNCTIONS
// ============================================================================
function formatTimeSeriesLabel(timestamp, interval) {
    switch (interval) {
        case 'hour':
