 ...
 * ```
 */
export const loadBookmarkConfig = (): BookmarkConfig => {
  return {
 ...
 * ```
 */
export const validateBookmarkConfig = (config: BookmarkConfig): string[] => {
  const errors: string[] = [];
 ...
 * ```
 */
export const createDocumentBookmarkModel = (sequelize: Sequelize): any => {
  const attributes: ModelAttributes = {
    ...
    destinationType: {
      type: DataTypes.ENUM('page', 'named', 'url'),
      allowNull: false,
      defaultValue: 'page',
    },
    destination: {
    ...
    style: {
      type: DataTypes.ENUM('normal', 'bold', 'italic', 'bold-italic'),
      allowNull: true,
      defaultValue: 'normal',
    },
    isOpen: {
 ...
 * ```
 */
export const createNamedDestinationModel = (sequelize: Sequelize): any => {
  const attributes: ModelAttributes = {
    id: {
      type: DataTypes.UUID,
      defaultValue: DataTypes.UUIDV4,
    ...
    viewType: {
      type: DataTypes.ENUM('fit', 'fitH', 'fitV', 'fitR', 'xyz'),
      allowNull: false,
      defaultValue: 'fit',
    },
    coordinates: {
 ...
 * ```
 */
export const createBookmark = (config: Partial<Bookmark>): Bookmark => {
  return {
 ...
 * ```
 */
export const createBookmarkWithChildren = (
  parent: Partial<Bookmark>,
  children: Partial<Bookmark>[],
): Bookmark => {
  const parentBookmark = createBookmark(parent);

  parentBookmark.children = children.map((child) =>
    createBookmark({
      ...child,
 ...
 * ```
 */
export const createBookmarksFromHeadings = (
...
export const addBookmark = (
  ...
  return bookmarks.map((bookmark) => {
    ...
    if (bookmark.children && bookmark.children.length > 0) {
      return {
        ...bookmark,
        children: addBookmark(bookmark.children, newBookmark, parentId),
      };
    }
    return bookmark;
  });
...
export const updateBookmark = (
  ...
  return bookmarks.map((bookmark) => {
    ...
    if (bookmark.children && bookmark.children.length > 0) {
      return {
        ...bookmark,
        children: updateBookmark(bookmark.children, bookmarkId, updates),
      };
    }
    return bookmark;
  });
 ...
 * ```
 */
export const removeBookmark = (bookmarks: Bookmark[], bookmarkId: string): Bookmark[] => {
  return bookmarks
    ...
    .map((bookmark) => {
      if (bookmark.children && bookmark.children.length > 0) {
        return {
          ...bookmark,
          children: removeBookmark(bookmark.children, bookmarkId),
        };
      }
      return bookmark;
    });
 ...
 * ```
 */
export const buildBookmarkTree = (bookmarks: Bookmark[]): BookmarkNode[] => {
 ...
 * ```
 */
export const flattenBookmarkTree = (tree: BookmarkNode[]): Bookmark[] => {
 ...
 * ```
 */
export const getBookmarkDepth = (bookmark: Bookmark, allBookmarks: Bookmark[]): number => {
  let depth = 0;
 ...
 * ```
 */
export const getBookmarkAncestors = (bookmark: Bookmark, allBookmarks: Bookmark[]): Bookmark[] => {
  const ancestors: Bookmark[] = [];
  let current = bookmark;
 ...
 * ```
 */
export const getBookmarkDescendants = (bookmark: Bookmark): Bookmark[] => {
  const descendants: Bookmark[] = [];

  const traverse = (children: Bookmark[]) => {
 ...
 * ```
 */
export const getBookmarkSiblings = (bookmark: Bookmark, allBookmarks: Bookmark[]): Bookmark[] => {
...
export const generateTableOfContents = (
  ...
  const convertToTOCEntry = (bookmark: Bookmark): TOCEntry => {
 ...
 * ```
 */
export const renderTOCAsText = (toc: TOCEntry[], config?: Partial<TOCConfig>): string => {
  const lines: string[] = [];
 ...
 * ```
 */
export const createTOCPage = (toc: TOCEntry[], config: Partial<TOCConfig>): Record<string, any> => {
  return {
 ...
 * ```
 */
export const updateTOCPageNumbers = (
  toc: TOCEntry[],
  pageMapping: Map<string, number>,
): TOCEntry[] => {
  return toc.map((entry) => {
    const newPageNumber = pageMapping.get(entry.id) || entry.pageNumber;
    const children = entry.children ? updateTOCPageNumbers(entry.children, pageMapping) : undefined;
 ...
 * ```
 */
export const createNamedDestination = (config: Partial<NamedDestination>): NamedDestination => {
  return {
 ...
 * ```
 */
export const createXYZDestination = (
  name: string,
  pageNumber: number,
  left?: number,
  top?: number,
 ...
 * ```
 */
export const createFitRDestination = (
  name: string,
  pageNumber: number,
  left: number,
  bottom: number,
 ...
 * ```
 */
export const validateDestinationName = (name: string): boolean => {
 ...
 * ```
 */
export const getNamedDestination = (
 ...
 * ```
 */
export const createInternalLink = (config: Partial<DocumentLink>): DocumentLink => {
  return {
    id: config.id || crypto.randomUUID(),
 ...
 * ```
 */
export const getLinksForPage = (links: DocumentLink[], pageNumber: number): DocumentLink[] => {
 ...
 * ```
 */
export const validateLinkTarget = (link: DocumentLink, totalPages: number): boolean => {
 ...
 * ```
 */
export const exportBookmarksToJSON = (bookmarks: Bookmark[]): string => {
 ...
 * ```
 */
export const exportBookmarksToXML = (bookmarks: Bookmark[]): string => {
  const lines: string[] = ['<?xml version="1.0" encoding="UTF-8"?>', '<bookmarks>'];

  const renderBookmark = (bookmark: Bookmark, indent: string = '  ') => {
 ...
 * ```
 */
export const importBookmarksFromJSON = (json: string): BookmarkImportResult => {
  try {
 ...
 * ```
 */
export const importBookmarksWithValidation = (
  ...
  data.forEach((item, index) => {
    try {
      ...
      bookmarks.push(createBookmark(item));
    } catch (error) {
 ...
 * ```
 */
export const findBookmarkByTitle = (
  bookmarks: Bookmark[],
  title: string,
  ...
  for (const bookmark of bookmarks) {
    ...
    if (bookmark.children && bookmark.children.length > 0) {
      const found = findBookmarkByTitle(bookmark.children, title, caseSensitive);
 ...
 * ```
 */
export const findBookmarksByPage = (bookmarks: Bookmark[], pageNumber: number): Bookmark[] => {
  const results: Bookmark[] = [];

  const search = (items: Bookmark[]) => {
 ...
 * ```
 */
export const sortBookmarksByPage = (bookmarks: Bookmark[]): Bookmark[] => {
 ...
 * ```
 */
export const sortBookmarksByTitle = (bookmarks: Bookmark[], ascending: boolean = true): Bookmark[] => {
 ...
 * ```
 */
export const countBookmarks = (bookmarks: Bookmark[]): number => {
 ...
 * ```
 */
export const validateBookmarkStructure = (bookmarks: Bookmark[], totalPages: number): string[] => {
  const errors: string[] = [];
  const seenIds = new Set<string>();

  const validate = (items: Bookmark[], parentLevel: number = 0) => {
 ...
 * ```
 */
export const cloneBookmarks = (bookmarks: Bookmark[], generateNewIds: boolean = false): Bookmark[] => {
  return bookmarks.map((bookmark) => {
    const cloned: Bookmark = {
      ...bookmark,
      id: generateNewIds ? crypto.randomUUID() : bookmark.id,
    };

    if (bookmark.children && bookmark.children.length > 0) {
      cloned.children = cloneBookmarks(bookmark.children, generateNewIds);
    }
...
export default {
  // Configuration
  loadBookmarkConfig,
  validateBookmarkConfig,

  // Models
  createDocumentBookmarkModel,
  createNamedDestinationModel,

  // Bookmark Creation
  createBookmark,
  createBookmarkWithChildren,
  createBookmarksFromHeadings,
  addBookmark,
  updateBookmark,
  removeBookmark,
  ...
  // Named Destinations
  createNamedDestination,
  createXYZDestination,
  createFitRDestination,
