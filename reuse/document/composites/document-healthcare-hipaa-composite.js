...
exports.HealthcareHipaaCompositeService = void 0;
/**
 * File: /reuse/document/composites/document-healthcare-hipaa-composite.ts
...
const document_healthcare_integration_kit_1 = require("../document-healthcare-integration-kit");
// Import from security kit
const document_security_kit_1 = require("../document-security-kit");
// Import from audit trail kit (via grep results)
const document_audit_trail_advanced_kit_1 = require("../document-audit-trail-advanced-kit");
// ============================================================================
 ...
 */
let HealthcareHipaaCompositeService = (() => {
    let _classDecorators = [(0, common_1.Injectable)()];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    var HealthcareHipaaCompositeService = _classThis = class {
        constructor(sequelize) {
            this.sequelize = sequelize;
            this.logger = new common_1.Logger(HealthcareHipaaCompositeService.name);
        }
        // ============================================================================
         ...
         * ```
         */
        async createFhirPatientResource(patientData) {
            this.logger.log(`Creating FHIR patient resource for ${patientData.patientId}`);
            // Create audit log
            await (0, document_audit_trail_advanced_kit_1.createBlockchainAuditLog)({
            ...
            const fhirPatient = await (0, document_healthcare_integration_kit_1.createFhirPatient)(patientData);
            const isValid = await (0, document_healthcare_integration_kit_1.validateFhirResource)(fhirPatient, 'Patient');
            if (!isValid) {
         ...
         * ```
         */
        async searchFhirResourcesSecure(resourceType, criteria) {
            this.logger.log(`Searching FHIR resources: ${resourceType}`);
            const results = await (0, document_healthcare_integration_kit_1.searchFhirResources)(resourceType, criteria);
         ...
         * ```
         */
        async getFhirResourceSecure(resourceType, resourceId, userId) {
            ...
            const resource = await (0, document_healthcare_integration_kit_1.getFhirResource)(resourceType, resourceId);
            return {
                resourceType,
                id: resourceId,
                resource,
         ...
         * ```
         */
        async createFhirTransactionBundle(resources) {
            this.logger.log(`Creating FHIR bundle with ${resources.length} resources`);
            return await (0, document_healthcare_integration_kit_1.createFhirBundle)(resources, 'transaction');
        }
        /**
         ...
         * ```
         */
        async validateFhirResourceCompliance(resource, resourceType) {
            const isValid = await (0, document_healthcare_integration_kit_1.validateFhirResource)(resource, resourceType);
         ...
         * ```
         */
        async testFhirServerConnectivity(serverUrl) {
            return await (0, document_healthcare_integration_kit_1.testFhirConnectivity)(serverUrl);
        }
        /**
         ...
         * ```
         */
        async convertFhirToCdaDocument(fhirResource) {
            this.logger.log('Converting FHIR to CDA');
            return await (0, document_healthcare_integration_kit_1.convertFhirToCda)(fhirResource);
        }
        /**
         ...
         * ```
         */
        async generateC32ContinuityOfCare(patientId) {
            this.logger.log(`Generating C32 for patient ${patientId}`);
            return await (0, document_healthcare_integration_kit_1.generateC32Document)(patientId);
        }
        // ============================================================================
         ...
         * ```
         */
        async parseDicomStudySecure(dicomBuffer) {
            this.logger.log('Parsing DICOM study');
            const study = await (0, document_healthcare_integration_kit_1.parseDicomStudy)(dicomBuffer);
            // Create forensic fingerprint
            const fingerprint = await (0, document_audit_trail_advanced_kit_1.createDigitalFingerprint)(dicomBuffer, {
         ...
         * ```
         */
        async generateDicomWadoUrl(studyUid, seriesUid, instanceUid) {
            return await (0, document_healthcare_integration_kit_1.generateWadoUrl)(studyUid, seriesUid, instanceUid);
        }
        /**
         ...
         * ```
         */
        async queryDicomStudiesSecure(filters) {
            this.logger.log('Querying DICOM studies');
            const studies = await (0, document_healthcare_integration_kit_1.queryDicomStudies)(filters);
            // Audit the query
            await (0, document_audit_trail_advanced_kit_1.createBlockchainAuditLog)({
         ...
         * ```
         */
        async convertDicomToJpegImage(dicomBuffer, quality) {
            return await (0, document_healthcare_integration_kit_1.convertDicomToJpeg)(dicomBuffer, quality);
        }
        /**
         ...
         * ```
         */
        async anonymizeDicomImage(dicomBuffer) {
            this.logger.log('Anonymizing DICOM image');
            const anonymized = await (0, document_healthcare_integration_kit_1.anonymizeDicom)(dicomBuffer);
            // Verify anonymization
            const study = await (0, document_healthcare_integration_kit_1.parseDicomStudy)(anonymized);
         ...
         * ```
         */
        async testDicomWadoConnectivity(wadoUrl) {
            return await (0, document_healthcare_integration_kit_1.testDicomWadoEndpoint)(wadoUrl);
        }
        // ============================================================================
         ...
         * ```
         */
        async authenticateEpicEhr(config) {
            this.logger.log('Authenticating with Epic EHR');
            const auth = await (0, document_healthcare_integration_kit_1.authenticateEpicOAuth)({
         ...
         * ```
         */
        async getEpicPatientData(patientId, accessToken) {
            this.logger.log(`Getting Epic patient data: ${patientId}`);
            const patient = await (0, document_healthcare_integration_kit_1.getEpicPatient)(patientId, accessToken);
            // Audit access
            await (0, document_audit_trail_advanced_kit_1.createBlockchainAuditLog)({
         ...
         * ```
         */
        async getEpicPatientMedications(patientId, accessToken) {
            return await (0, document_healthcare_integration_kit_1.getEpicMedications)(patientId, accessToken);
        }
        /**
         ...
         * ```
         */
        async syncCernerPatientObservations(patientId, since) {
            this.logger.log(`Syncing Cerner observations for patient ${patientId}`);
            return await (0, document_healthcare_integration_kit_1.syncCernerObservations)(patientId, since);
        }
        /**
         ...
         * ```
         */
        async validateHl7MessageFormat(hl7Message) {
            return await (0, document_healthcare_integration_kit_1.validateHl7Message)(hl7Message);
        }
        /**
         ...
         * ```
         */
        async validateIheProfileCompliance(profile, transaction) {
            return await (0, document_healthcare_integration_kit_1.validateIheProfile)(profile, transaction);
        }
        /**
         ...
         * ```
         */
        async parseCdaDocumentToFhir(cdaXml) {
            return await (0, document_healthcare_integration_kit_1.parseCdaToFhir)(cdaXml);
        }
        /**
         ...
         * ```
         */
        async validateCdaDocumentStructure(cdaXml) {
            return await (0, document_healthcare_integration_kit_1.validateCdaDocument)(cdaXml);
        }
        // ============================================================================
         ...
         * ```
         */
        async performProbabilisticPatientMatch(criteria) {
            this.logger.log('Performing patient matching');
            const matches = await (0, document_healthcare_integration_kit_1.performPatientMatching)(criteria);
         ...
         * ```
         */
        async createMasterPatientIndexEntry(patientData) {
            this.logger.log('Creating MPI entry');
            return await (0, document_healthcare_integration_kit_1.createMpiEntry)(patientData);
        }
        /**
         ...
         * ```
         */
        async linkPatientRecordsAcrossSystems(mpiId, records) {
            this.logger.log(`Linking patient records for MPI ${mpiId}`);
            for (const record of records) {
                await (0, document_healthcare_integration_kit_1.linkPatientRecords)(mpiId, record.system, record.patientId);
            }
        }
        /**
         ...
         * ```
         */
        async calculatePatientMatchConfidence(patient1, patient2) {
            return await (0, document_healthcare_integration_kit_1.calculateMatchConfidence)(patient1, patient2);
        }
        /**
         ...
         * ```
         */
        async resolvePatientIdentityAcrossEhrs(patientId, sourceSystem) {
         ...
         * ```
         */
        async encryptPhiData(phiData, encryptionKey) {
            this.logger.log('Encrypting PHI data');
            const dataString = JSON.stringify(phiData);
            const encrypted = await (0, document_security_kit_1.encryptData)(Buffer.from(dataString), encryptionKey);
            return encrypted;
        }
        /**
         ...
         * ```
         */
        async decryptPhiData(encryptedData, iv, encryptionKey) {
            const decrypted = await (0, document_security_kit_1.decryptData)(encryptedData, iv, encryptionKey);
         ...
         * ```
         */
        async generatePhiEncryptionKey() {
            return await (0, document_security_kit_1.generateEncryptionKey)(256);
        }
        /**
         ...
         * ```
         */
        async rotatePhiEncryptionKeys(oldKey, newKey) {
            this.logger.log('Rotating PHI encryption keys');
            await (0, document_security_kit_1.rotateEncryptionKeys)(oldKey, newKey);
        }
        /**
         ...
         * ```
         */
        async validatePhiEncryptionStrength(encryptionKey) {
            const strength = await (0, document_security_kit_1.validateEncryptionStrength)(encryptionKey);
         ...
         * ```
         */
        async createSecurePhiVault(vaultName) {
            return await (0, document_security_kit_1.createSecureVault)(vaultName);
        }
        // ============================================================================
        ...
        async createBlockchainAuditEntry(auditEntry) {
            return await (0, document_audit_trail_advanced_kit_1.createBlockchainAuditLog)(auditEntry);
        }
        /**
         ...
         * ```
         */
        async anchorAuditToBlockchain(auditIds) {
            return await (0, document_audit_trail_advanced_kit_1.anchorToBlockchain)(auditIds);
        }
        /**
         ...
         * ```
         */
        async buildAuditMerkleTreeStructure(auditEntries) {
            return await (0, document_audit_trail_advanced_kit_1.buildAuditMerkleTree)(auditEntries);
        }
        /**
         ...
         * ```
         */
        async verifyAuditChainIntegrityCheck(auditIds) {
            return await (0, document_audit_trail_advanced_kit_1.verifyAuditChainIntegrity)(auditIds);
        }
        /**
         ...
         * ```
         */
        async performDocumentForensicAnalysis(documentBuffer) {
            this.logger.log('Performing forensic analysis');
            return await (0, document_audit_trail_advanced_kit_1.performForensicAnalysis)(documentBuffer, 'comprehensive');
        }
        /**
         ...
         * ```
         */
        async createPhiDigitalFingerprint(documentBuffer) {
            return await (0, document_audit_trail_advanced_kit_1.createDigitalFingerprint)(documentBuffer, {
                includeMetadata: true,
                algorithm: 'sha256',
            });
        }
         ...
         * ```
         */
        async detectPhiDocumentTampering(documentBuffer, originalHash) {
            return await (0, document_audit_trail_advanced_kit_1.detectTamperingByHash)(documentBuffer, originalHash);
        }
        // ============================================================================
         ...
         * ```
         */
        async initiatePhiChainOfCustody(documentId, custodian) {
            this.logger.log(`Initiating chain of custody for ${documentId}`);
            return await (0, document_audit_trail_advanced_kit_1.initiateChainOfCustody)(documentId, custodian, 'PHI Document');
        }
        /**
         ...
         * ```
         */
        async transferPhiCustody(custodyId, fromCustodian, toCustodian, reason) {
            await (0, document_audit_trail_advanced_kit_1.transferCustody)(custodyId, fromCustodian, toCustodian, reason);
        }
        /**
         ...
         * ```
         */
        async verifyPhiChainOfCustody(custodyId) {
            return await (0, document_audit_trail_advanced_kit_1.verifyChainOfCustody)(custodyId);
        }
        /**
         ...
         * ```
         */
        async createLegalHoldPreservationPackage(documentId, legalHoldId) {
            this.logger.log('Creating legal hold preservation package');
            return await (0, document_audit_trail_advanced_kit_1.createPreservationPackage)({
                documentId,
                legalHoldId,
                method: 'blockchain',
                timestamp: new Date(),
         ...
         * ```
         */
        async generateHipaaForensicReport(startDate, endDate) {
            this.logger.log('Generating HIPAA forensic report');
            return await (0, document_audit_trail_advanced_kit_1.generateForensicAuditReport)({ startDate, endDate });
        }
        // ============================================================================
         ...
         * @private
         */
        checkForPhiTags(study) {
            const phiTags = ['PatientName', 'PatientID', 'PatientBirthDate'];
            return phiTags.some((tag) => study[tag] !== undefined);
        }
    };
    __setFunctionName(_classThis, "HealthcareHipaaCompositeService");
    (() => {
        const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(null) : void 0;
        __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
        HealthcareHipaaCompositeService = _classThis = _classDescriptor.value;
        if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
        __runInitializers(_classThis, _classExtraInitializers);
    })();
    return HealthcareHipaaCompositeService = _classThis;
})();
exports.HealthcareHipaaCompositeService = HealthcareHipaaCompositeService;
// ============================================================================
// EXPORTS
// ============================================================================
exports.default = HealthcareHipaaCompositeService;
//# sourceMappingURL=document-healthcare-hipaa-composite.js.map