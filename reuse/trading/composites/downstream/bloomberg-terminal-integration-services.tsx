/**
 * LOC: WC-DOWN-TRADING-BBGINTSVC-014
 * File: /reuse/trading/composites/downstream/bloomberg-terminal-integration-services.tsx
 *
 * UPSTREAM (imports from):
 *   - ../order-flow-analytics-composite
 *   - @nestjs/common (v10.x)
 *   - @nestjs/swagger (v7.x)
 *   - sequelize (v6.x)
 *
...
class IntegrationServiceModel extends Model<any, any> {
  declare id: string;
  declare name: string;
...
export function initIntegrationServiceModel(sequelize: Sequelize): typeof IntegrationServiceModel {
  IntegrationServiceModel.init(
    {
      id: {
        type: DataTypes.UUID,
    ...
    {
      sequelize,
      tableName: 'integrationservice_records',
      modelName: 'IntegrationService',
      timestamps: true,
      underscored: true,
      indexes: [
        { fields: ['status'] },
  ...
  return IntegrationServiceModel;
}

class ServiceEndpointModel extends Model<any, any> {
  declare id: string;
  declare name: string;
  declare status: Status;
  declare priority: Priority;
...
export function initServiceEndpointModel(sequelize: Sequelize): typeof ServiceEndpointModel {
  ServiceEndpointModel.init(
    {
      id: {
        type: DataTypes.UUID,
        defaultValue: DataTypes.UUIDV4,
    ...
    {
      sequelize,
      tableName: 'serviceendpoint_records',
      modelName: 'ServiceEndpoint',
      timestamps: true,
      underscored: true,
      indexes: [
        { fields: ['status'] },
...
class ServiceMetricsModel extends Model<any, any> {
  declare id: string;
  declare name: string;
  declare status: Status;
  declare priority: Priority;
...
export function initServiceMetricsModel(sequelize: Sequelize): typeof ServiceMetricsModel {
  ServiceMetricsModel.init(
    {
      id: {
        type: DataTypes.UUID,
        defaultValue: DataTypes.UUIDV4,
    ...
    {
      sequelize,
      tableName: 'servicemetrics_records',
...
@Injectable()
export class BloombergIntegrationServiceService {
  private readonly logger = new Logger(BloombergIntegrationServiceService.name);
  ...
  async create(
    ...
    try {
      ...
      return await IntegrationServiceModel.create(
        {
          name,
          status,
          priority,
  ...
  async findById(
    ...
    try {
      const record = await IntegrationServiceModel.findByPk(id, { transaction });
  ...
  async findAll(
    ...
    try {
      ...
      return await IntegrationServiceModel.findAll({
        where,
        limit,
        offset,
...
@ApiTags('Bloomberg Terminal Integration Services')
@Controller('api/v1/bloomberg-integration-services')
export class BloombergIntegrationServiceController {
  private readonly logger = new Logger(BloombergIntegrationServiceController.name);

  constructor(private readonly service: BloombergIntegrationServiceService) {}
...
export {
  BloombergIntegrationServiceService,
  BloombergIntegrationServiceController,
  IntegrationServiceModel,
  initIntegrationServiceModel,
  ServiceEndpointModel,
  initServiceEndpointModel,
  ServiceMetricsModel,
  initServiceMetricsModel,
};
