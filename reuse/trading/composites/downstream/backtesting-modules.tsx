/**
 * LOC: WC-DOWN-TRADING-BACKMOD-008
 * File: /reuse/trading/composites/downstream/backtesting-modules.tsx
 *
 * UPSTREAM (imports from):
 *   - ../technical-analysis-charting-composite
 *   - @nestjs/common (v10.x)
 *   - @nestjs/swagger (v7.x)
 *   - sequelize (v6.x)
 *
...
class BacktestModuleModel extends Model<any, any> {
  declare id: string;
  declare name: string;
...
export function initBacktestModuleModel(sequelize: Sequelize): typeof BacktestModuleModel {
  BacktestModuleModel.init(
    {
      id: {
        type: DataTypes.UUID,
    ...
    {
      sequelize,
      tableName: 'backtestmodule_records',
      modelName: 'BacktestModule',
      timestamps: true,
      underscored: true,
      indexes: [
        { fields: ['status'] },
  ...
  return BacktestModuleModel;
}

class BacktestScenarioModel extends Model<any, any> {
  declare id: string;
  declare name: string;
  declare status: Status;
  declare priority: Priority;
...
export function initBacktestScenarioModel(sequelize: Sequelize): typeof BacktestScenarioModel {
  BacktestScenarioModel.init(
    {
      id: {
        type: DataTypes.UUID,
        defaultValue: DataTypes.UUIDV4,
    ...
    {
      sequelize,
      tableName: 'backtestscenario_records',
      modelName: 'BacktestScenario',
      timestamps: true,
      underscored: true,
      indexes: [
        { fields: ['status'] },
  ...
  return BacktestScenarioModel;
}

class BacktestParameterModel extends Model<any, any> {
  declare id: string;
  declare name: string;
  declare status: Status;
  declare priority: Priority;
...
export function initBacktestParameterModel(sequelize: Sequelize): typeof BacktestParameterModel {
  BacktestParameterModel.init(
    {
      id: {
        type: DataTypes.UUID,
        defaultValue: DataTypes.UUIDV4,
    ...
    {
      sequelize,
      tableName: 'backtestparameter_records',
      modelName: 'BacktestParameter',
      timestamps: true,
      underscored: true,
      indexes: [
        { fields: ['status'] },
...
@Injectable()
export class BacktestingModuleService {
  private readonly logger = new Logger(BacktestingModuleService.name);
  ...
  async create(
    ...
    try {
      ...
      return await BacktestModuleModel.create(
        {
          name,
          status,
          priority,
  ...
  async findById(
    ...
    try {
      const record = await BacktestModuleModel.findByPk(id, { transaction });
  ...
  async findAll(
    ...
    try {
      ...
      return await BacktestModuleModel.findAll({
        where,
        limit,
        offset,
...
@ApiTags('Backtesting Modules')
@Controller('api/v1/backtesting-modules')
export class BacktestingModuleController {
  private readonly logger = new Logger(BacktestingModuleController.name);

  constructor(private readonly service: BacktestingModuleService) {}
...
export {
  BacktestingModuleService,
  BacktestingModuleController,
  BacktestModuleModel,
  initBacktestModuleModel,
  BacktestScenarioModel,
  initBacktestScenarioModel,
  BacktestParameterModel,
  initBacktestParameterModel,
};
