/**
 * LOC: WC-DOWN-TRADING-BBGINTCTRL-012
 * File: /reuse/trading/composites/downstream/bloomberg-terminal-integration-controllers.tsx
 *
 * UPSTREAM (imports from):
 *   - ../fundamental-analysis-screening-composite
 *   - @nestjs/common (v10.x)
 *   - @nestjs/swagger (v7.x)
 *   - sequelize (v6.x)
 *
...
class IntegrationControllerModel extends Model<any, any> {
  declare id: string;
  declare name: string;
...
export function initIntegrationControllerModel(sequelize: Sequelize): typeof IntegrationControllerModel {
  IntegrationControllerModel.init(
    {
      id: {
        type: DataTypes.UUID,
    ...
    {
      sequelize,
      tableName: 'integrationcontroller_records',
      modelName: 'IntegrationController',
      timestamps: true,
      underscored: true,
      indexes: [
        { fields: ['status'] },
  ...
  return IntegrationControllerModel;
}

class ControllerConfigModel extends Model<any, any> {
  declare id: string;
  declare name: string;
  declare status: Status;
  declare priority: Priority;
...
export function initControllerConfigModel(sequelize: Sequelize): typeof ControllerConfigModel {
  ControllerConfigModel.init(
    {
      id: {
        type: DataTypes.UUID,
        defaultValue: DataTypes.UUIDV4,
    ...
    {
      sequelize,
      tableName: 'controllerconfig_records',
...
class ControllerStatusModel extends Model<any, any> {
  declare id: string;
  declare name: string;
  declare status: Status;
  declare priority: Priority;
...
export function initControllerStatusModel(sequelize: Sequelize): typeof ControllerStatusModel {
  ControllerStatusModel.init(
    {
      id: {
        type: DataTypes.UUID,
        defaultValue: DataTypes.UUIDV4,
    ...
    {
      sequelize,
      tableName: 'controllerstatus_records',
      modelName: 'ControllerStatus',
      timestamps: true,
      underscored: true,
      indexes: [
        { fields: ['status'] },
...
@Injectable()
export class BloombergIntegrationControllerService {
  private readonly logger = new Logger(BloombergIntegrationControllerService.name);
  ...
  async create(
    ...
    try {
      ...
      return await IntegrationControllerModel.create(
        {
          name,
          status,
          priority,
  ...
  async findById(
    ...
    try {
      const record = await IntegrationControllerModel.findByPk(id, { transaction });
  ...
  async findAll(
    ...
    try {
      ...
      return await IntegrationControllerModel.findAll({
        where,
        limit,
        offset,
...
@ApiTags('Bloomberg Terminal Integration Controllers')
@Controller('api/v1/bloomberg-integration-controllers')
export class BloombergIntegrationControllerController {
  private readonly logger = new Logger(BloombergIntegrationControllerController.name);

  constructor(private readonly service: BloombergIntegrationControllerService) {}
...
export {
  BloombergIntegrationControllerService,
  BloombergIntegrationControllerController,
  IntegrationControllerModel,
  initIntegrationControllerModel,
  ControllerConfigModel,
  initControllerConfigModel,
  ControllerStatusModel,
  initControllerStatusModel,
};
