/**
 * LOC: WC-DOWN-TRADING-BBGINTS-015
 * File: /reuse/trading/composites/downstream/bloomberg-terminal-integrations.tsx
 *
 * UPSTREAM (imports from):
 *   - ../market-making-liquidity-composite
 *   - @nestjs/common (v10.x)
 *   - @nestjs/swagger (v7.x)
 *   - sequelize (v6.x)
 *
...
class IntegrationModel extends Model<any, any> {
  declare id: string;
  declare name: string;
...
export function initIntegrationModel(sequelize: Sequelize): typeof IntegrationModel {
...
class IntegrationLogModel extends Model<any, any> {
  declare id: string;
  declare name: string;
  declare status: Status;
  declare priority: Priority;
...
export function initIntegrationLogModel(sequelize: Sequelize): typeof IntegrationLogModel {
  IntegrationLogModel.init(
    {
      id: {
        type: DataTypes.UUID,
        defaultValue: DataTypes.UUIDV4,
    ...
    {
      sequelize,
      tableName: 'integrationlog_records',
...
class IntegrationMetricModel extends Model<any, any> {
  declare id: string;
  declare name: string;
  declare status: Status;
  declare priority: Priority;
...
export function initIntegrationMetricModel(sequelize: Sequelize): typeof IntegrationMetricModel {
  IntegrationMetricModel.init(
    {
      id: {
        type: DataTypes.UUID,
        defaultValue: DataTypes.UUIDV4,
    ...
    {
      sequelize,
      tableName: 'integrationmetric_records',
      modelName: 'IntegrationMetric',
      timestamps: true,
      underscored: true,
      indexes: [
        { fields: ['status'] },
...
@Injectable()
export class BloombergIntegrationsService {
  private readonly logger = new Logger(BloombergIntegrationsService.name);
  ...
  async findById(
    ...
    try {
      const record = await IntegrationModel.findByPk(id, { transaction });
  ...
  async findAll(
    ...
    try {
      ...
      return await IntegrationModel.findAll({
        where,
        limit,
        offset,
...
@ApiTags('Bloomberg Terminal Integrations')
@Controller('api/v1/bloomberg-integrations')
export class BloombergIntegrationsController {
  private readonly logger = new Logger(BloombergIntegrationsController.name);

  constructor(private readonly service: BloombergIntegrationsService) {}
...
export {
  BloombergIntegrationsService,
  BloombergIntegrationsController,
  IntegrationModel,
  initIntegrationModel,
  IntegrationLogModel,
  initIntegrationLogModel,
  IntegrationMetricModel,
  initIntegrationMetricModel,
};
