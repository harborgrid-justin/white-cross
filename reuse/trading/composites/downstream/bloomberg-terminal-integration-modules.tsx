/**
 * LOC: WC-DOWN-TRADING-BBGINTMOD-013
 * File: /reuse/trading/composites/downstream/bloomberg-terminal-integration-modules.tsx
 *
 * UPSTREAM (imports from):
 *   - ../trade-settlement-clearing-composite
 *   - @nestjs/common (v10.x)
 *   - @nestjs/swagger (v7.x)
 *   - sequelize (v6.x)
 *
...
class IntegrationModuleModel extends Model<any, any> {
  declare id: string;
  declare name: string;
...
export function initIntegrationModuleModel(sequelize: Sequelize): typeof IntegrationModuleModel {
  IntegrationModuleModel.init(
    {
      id: {
        type: DataTypes.UUID,
    ...
    {
      sequelize,
      tableName: 'integrationmodule_records',
      modelName: 'IntegrationModule',
      timestamps: true,
      underscored: true,
      indexes: [
        { fields: ['status'] },
...
class ModuleConfigModel extends Model<any, any> {
  declare id: string;
  declare name: string;
  declare status: Status;
  declare priority: Priority;
...
export function initModuleConfigModel(sequelize: Sequelize): typeof ModuleConfigModel {
  ModuleConfigModel.init(
    {
      id: {
        type: DataTypes.UUID,
        defaultValue: DataTypes.UUIDV4,
    ...
    {
      sequelize,
      tableName: 'moduleconfig_records',
...
class ModuleHealthModel extends Model<any, any> {
  declare id: string;
  declare name: string;
  declare status: Status;
  declare priority: Priority;
...
export function initModuleHealthModel(sequelize: Sequelize): typeof ModuleHealthModel {
  ModuleHealthModel.init(
    {
      id: {
        type: DataTypes.UUID,
        defaultValue: DataTypes.UUIDV4,
    ...
    {
      sequelize,
      tableName: 'modulehealth_records',
      modelName: 'ModuleHealth',
      timestamps: true,
      underscored: true,
      indexes: [
        { fields: ['status'] },
...
@Injectable()
export class BloombergIntegrationModuleService {
  private readonly logger = new Logger(BloombergIntegrationModuleService.name);
  ...
  async create(
    ...
    try {
      ...
      return await IntegrationModuleModel.create(
        {
          name,
          status,
          priority,
  ...
  async findById(
    ...
    try {
      const record = await IntegrationModuleModel.findByPk(id, { transaction });
  ...
  async findAll(
    ...
    try {
      ...
      return await IntegrationModuleModel.findAll({
        where,
        limit,
        offset,
...
@ApiTags('Bloomberg Terminal Integration Modules')
@Controller('api/v1/bloomberg-integration-modules')
export class BloombergIntegrationModuleController {
  private readonly logger = new Logger(BloombergIntegrationModuleController.name);

  constructor(private readonly service: BloombergIntegrationModuleService) {}
...
export {
  BloombergIntegrationModuleService,
  BloombergIntegrationModuleController,
  IntegrationModuleModel,
  initIntegrationModuleModel,
  ModuleConfigModel,
  initModuleConfigModel,
  ModuleHealthModel,
  initModuleHealthModel,
};
