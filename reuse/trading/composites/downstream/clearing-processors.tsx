/**
 * LOC: WC-DOWN-TRADING-CLEARING-018
 * File: /reuse/trading/composites/downstream/clearing-processors.tsx
 *
...
class ClearingInstructionModel extends Model<any, any> {
  declare id: string;
  declare name: string;
...
export function initClearingInstructionModel(sequelize: Sequelize): typeof ClearingInstructionModel {
  ClearingInstructionModel.init(
    {
      id: {
        type: DataTypes.UUID,
    ...
    {
      sequelize,
      tableName: 'clearinginstruction_records',
      modelName: 'ClearingInstruction',
      timestamps: true,
      underscored: true,
      indexes: [
        { fields: ['status'] },
...
class ClearingStatusModel extends Model<any, any> {
  declare id: string;
  declare name: string;
  declare status: Status;
  declare priority: Priority;
...
export function initClearingStatusModel(sequelize: Sequelize): typeof ClearingStatusModel {
  ClearingStatusModel.init(
    {
      id: {
        type: DataTypes.UUID,
        defaultValue: DataTypes.UUIDV4,
    ...
    {
      sequelize,
      tableName: 'clearingstatus_records',
      modelName: 'ClearingStatus',
      timestamps: true,
      underscored: true,
      indexes: [
        { fields: ['status'] },
  ...
  return ClearingStatusModel;
}

class ClearingReportModel extends Model<any, any> {
  declare id: string;
  declare name: string;
  declare status: Status;
  declare priority: Priority;
...
export function initClearingReportModel(sequelize: Sequelize): typeof ClearingReportModel {
  ClearingReportModel.init(
    {
      id: {
        type: DataTypes.UUID,
        defaultValue: DataTypes.UUIDV4,
    ...
    {
      sequelize,
      tableName: 'clearingreport_records',
      modelName: 'ClearingReport',
      timestamps: true,
      underscored: true,
      indexes: [
        { fields: ['status'] },
...
@Injectable()
export class ClearingProcessorService {
  private readonly logger = new Logger(ClearingProcessorService.name);
  ...
  async create(
    ...
    try {
      ...
      return await ClearingInstructionModel.create(
        {
          name,
          status,
          priority,
  ...
  async findById(
    ...
    try {
      const record = await ClearingInstructionModel.findByPk(id, { transaction });
  ...
  async findAll(
    ...
    try {
      ...
      return await ClearingInstructionModel.findAll({
        where,
        limit,
        offset,
...
@ApiTags('Clearing Processors')
@Controller('api/v1/clearing')
export class ClearingProcessorController {
  private readonly logger = new Logger(ClearingProcessorController.name);

  constructor(private readonly service: ClearingProcessorService) {}
...
export {
  ClearingProcessorService,
  ClearingProcessorController,
  ClearingInstructionModel,
  initClearingInstructionModel,
  ClearingStatusModel,
  initClearingStatusModel,
  ClearingReportModel,
  initClearingReportModel,
};
