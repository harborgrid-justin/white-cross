/**
 * LOC: WC-DOWN-TRADING-BONDPRICE-017
 * File: /reuse/trading/composites/downstream/bond-pricing-services.tsx
 *
 * UPSTREAM (imports from):
 *   - ../fixed-income-analytics-composite
 *   - @nestjs/common (v10.x)
 *   - @nestjs/swagger (v7.x)
 *   - sequelize (v6.x)
 *
...
class BondPriceModel extends Model<any, any> {
  declare id: string;
  declare name: string;
...
export function initBondPriceModel(sequelize: Sequelize): typeof BondPriceModel {
  BondPriceModel.init(
    {
      id: {
        type: DataTypes.UUID,
...
export function initYieldCurveModel(sequelize: Sequelize): typeof YieldCurveModel {
  YieldCurveModel.init(
    {
      id: {
        type: DataTypes.UUID,
        defaultValue: DataTypes.UUIDV4,
    ...
    {
      sequelize,
      tableName: 'yieldcurve_records',
      modelName: 'YieldCurve',
      timestamps: true,
      underscored: true,
      indexes: [
        { fields: ['status'] },
...
class BondAnalyticsModel extends Model<any, any> {
  declare id: string;
  declare name: string;
  declare status: Status;
  declare priority: Priority;
...
export function initBondAnalyticsModel(sequelize: Sequelize): typeof BondAnalyticsModel {
  BondAnalyticsModel.init(
    {
      id: {
        type: DataTypes.UUID,
        defaultValue: DataTypes.UUIDV4,
    ...
    {
      sequelize,
      tableName: 'bondanalytics_records',
      modelName: 'BondAnalytics',
      timestamps: true,
      underscored: true,
      indexes: [
        { fields: ['status'] },
...
@Injectable()
export class BondPricingService {
  private readonly logger = new Logger(BondPricingService.name);
  ...
  async create(
    ...
    try {
      ...
      return await BondPriceModel.create(
        {
          name,
          status,
          priority,
  ...
  async findById(
    ...
    try {
      const record = await BondPriceModel.findByPk(id, { transaction });
  ...
  async findAll(
    ...
    try {
      ...
      return await BondPriceModel.findAll({
        where,
        limit,
        offset,
...
@ApiTags('Bond Pricing Services')
@Controller('api/v1/bond-pricing')
export class BondPricingController {
  private readonly logger = new Logger(BondPricingController.name);

  constructor(private readonly service: BondPricingService) {}
...
export {
  BondPricingService,
  BondPricingController,
  BondPriceModel,
  initBondPriceModel,
  YieldCurveModel,
  initYieldCurveModel,
  BondAnalyticsModel,
  initBondAnalyticsModel,
};
