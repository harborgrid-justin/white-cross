/**
 * LOC: WC-DOWN-TRADING-COMMSURV-020
 * File: /reuse/trading/composites/downstream/communication-surveillance-processors.tsx
 *
...
class CommunicationRecordModel extends Model<any, any> {
  declare id: string;
  declare name: string;
...
export function initCommunicationRecordModel(sequelize: Sequelize): typeof CommunicationRecordModel {
  CommunicationRecordModel.init(
    {
      id: {
        type: DataTypes.UUID,
    ...
    {
      sequelize,
      tableName: 'communicationrecord_records',
      modelName: 'CommunicationRecord',
      timestamps: true,
      underscored: true,
      indexes: [
        { fields: ['status'] },
...
class SurveillanceAlertModel extends Model<any, any> {
  declare id: string;
  declare name: string;
  declare status: Status;
  declare priority: Priority;
...
export function initSurveillanceAlertModel(sequelize: Sequelize): typeof SurveillanceAlertModel {
  SurveillanceAlertModel.init(
    {
      id: {
        type: DataTypes.UUID,
        defaultValue: DataTypes.UUIDV4,
    ...
    {
      sequelize,
      tableName: 'surveillancealert_records',
...
class ComplianceFlagModel extends Model<any, any> {
  declare id: string;
  declare name: string;
  declare status: Status;
  declare priority: Priority;
...
export function initComplianceFlagModel(sequelize: Sequelize): typeof ComplianceFlagModel {
  ComplianceFlagModel.init(
    {
      id: {
        type: DataTypes.UUID,
        defaultValue: DataTypes.UUIDV4,
    ...
    {
      sequelize,
      tableName: 'complianceflag_records',
...
@Injectable()
export class CommunicationSurveillanceProcessorService {
  private readonly logger = new Logger(CommunicationSurveillanceProcessorService.name);
  ...
  async create(
    ...
    try {
      ...
      return await CommunicationRecordModel.create(
        {
          name,
          status,
          priority,
  ...
  async findById(
    ...
    try {
      const record = await CommunicationRecordModel.findByPk(id, { transaction });
  ...
  async findAll(
    ...
    try {
      ...
      return await CommunicationRecordModel.findAll({
        where,
        limit,
        offset,
...
@ApiTags('Communication Surveillance Processors')
@Controller('api/v1/communication-surveillance')
export class CommunicationSurveillanceProcessorController {
  private readonly logger = new Logger(CommunicationSurveillanceProcessorController.name);

  constructor(private readonly service: CommunicationSurveillanceProcessorService) {}
...
export {
  CommunicationSurveillanceProcessorService,
  CommunicationSurveillanceProcessorController,
  CommunicationRecordModel,
  initCommunicationRecordModel,
  SurveillanceAlertModel,
  initSurveillanceAlertModel,
  ComplianceFlagModel,
  initComplianceFlagModel,
};
