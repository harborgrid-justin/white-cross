/**
 * LOC: WC-DOWN-TRADING-COMMODITY-019
 * File: /reuse/trading/composites/downstream/commodity-trading-controllers.tsx
 *
 * UPSTREAM (imports from):
 *   - ../commodities-trading-analytics-composite
 *   - @nestjs/common (v10.x)
 *   - @nestjs/swagger (v7.x)
 *   - sequelize (v6.x)
 *
...
class CommodityTradeModel extends Model<any, any> {
  declare id: string;
  declare name: string;
...
export function initCommodityTradeModel(sequelize: Sequelize): typeof CommodityTradeModel {
  CommodityTradeModel.init(
    {
      id: {
        type: DataTypes.UUID,
    ...
    {
      sequelize,
      tableName: 'commoditytrade_records',
      modelName: 'CommodityTrade',
      timestamps: true,
      underscored: true,
      indexes: [
        { fields: ['status'] },
  ...
  return CommodityTradeModel;
}

class CommodityPositionModel extends Model<any, any> {
  declare id: string;
  declare name: string;
  declare status: Status;
  declare priority: Priority;
...
export function initCommodityPositionModel(sequelize: Sequelize): typeof CommodityPositionModel {
  CommodityPositionModel.init(
    {
      id: {
        type: DataTypes.UUID,
        defaultValue: DataTypes.UUIDV4,
    ...
    {
      sequelize,
      tableName: 'commodityposition_records',
      modelName: 'CommodityPosition',
      timestamps: true,
      underscored: true,
      indexes: [
        { fields: ['status'] },
  ...
  return CommodityPositionModel;
}

class CommodityRiskModel extends Model<any, any> {
  declare id: string;
  declare name: string;
  declare status: Status;
  declare priority: Priority;
...
export function initCommodityRiskModel(sequelize: Sequelize): typeof CommodityRiskModel {
  CommodityRiskModel.init(
    {
      id: {
        type: DataTypes.UUID,
        defaultValue: DataTypes.UUIDV4,
    ...
    {
      sequelize,
      tableName: 'commodityrisk_records',
      modelName: 'CommodityRisk',
      timestamps: true,
      underscored: true,
      indexes: [
        { fields: ['status'] },
...
@Injectable()
export class CommodityTradingControllerService {
  private readonly logger = new Logger(CommodityTradingControllerService.name);
  ...
  async create(
    ...
    try {
      ...
      return await CommodityTradeModel.create(
        {
          name,
          status,
          priority,
  ...
  async findById(
    ...
    try {
      const record = await CommodityTradeModel.findByPk(id, { transaction });
  ...
  async findAll(
    ...
    try {
      ...
      return await CommodityTradeModel.findAll({
        where,
        limit,
        offset,
...
@ApiTags('Commodity Trading Controllers')
@Controller('api/v1/commodity-trading')
export class CommodityTradingController {
  private readonly logger = new Logger(CommodityTradingController.name);

  constructor(private readonly service: CommodityTradingControllerService) {}
...
export {
  CommodityTradingControllerService,
  CommodityTradingController,
  CommodityTradeModel,
  initCommodityTradeModel,
  CommodityPositionModel,
  initCommodityPositionModel,
  CommodityRiskModel,
  initCommodityRiskModel,
};
