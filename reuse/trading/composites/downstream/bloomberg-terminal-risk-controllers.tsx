/**
 * LOC: WC-DOWN-TRADING-BBGRISK-016
 * File: /reuse/trading/composites/downstream/bloomberg-terminal-risk-controllers.tsx
 *
...
class RiskControllerModel extends Model<any, any> {
  declare id: string;
  declare name: string;
...
export function initRiskControllerModel(sequelize: Sequelize): typeof RiskControllerModel {
  RiskControllerModel.init(
    {
      id: {
        type: DataTypes.UUID,
    ...
    {
      sequelize,
      tableName: 'riskcontroller_records',
      modelName: 'RiskController',
      timestamps: true,
      underscored: true,
      indexes: [
        { fields: ['status'] },
  ...
  return RiskControllerModel;
}

class RiskLimitModel extends Model<any, any> {
  declare id: string;
  declare name: string;
  declare status: Status;
  declare priority: Priority;
...
export function initRiskLimitModel(sequelize: Sequelize): typeof RiskLimitModel {
  RiskLimitModel.init(
    {
      id: {
        type: DataTypes.UUID,
...
class RiskAlertModel extends Model<any, any> {
  declare id: string;
  declare name: string;
  declare status: Status;
  declare priority: Priority;
...
export function initRiskAlertModel(sequelize: Sequelize): typeof RiskAlertModel {
  RiskAlertModel.init(
    {
      id: {
        type: DataTypes.UUID,
...
@Injectable()
export class BloombergRiskControllerService {
  private readonly logger = new Logger(BloombergRiskControllerService.name);
  ...
  async create(
    ...
    try {
      ...
      return await RiskControllerModel.create(
        {
          name,
          status,
          priority,
  ...
  async findById(
    ...
    try {
      const record = await RiskControllerModel.findByPk(id, { transaction });
  ...
  async findAll(
    ...
    try {
      ...
      return await RiskControllerModel.findAll({
        where,
        limit,
        offset,
...
@ApiTags('Bloomberg Terminal Risk Controllers')
@Controller('api/v1/bloomberg-risk')
export class BloombergRiskController {
  private readonly logger = new Logger(BloombergRiskController.name);

  constructor(private readonly service: BloombergRiskControllerService) {}
...
export {
  BloombergRiskControllerService,
  BloombergRiskController,
  RiskControllerModel,
  initRiskControllerModel,
  RiskLimitModel,
  initRiskLimitModel,
  RiskAlertModel,
  initRiskAlertModel,
