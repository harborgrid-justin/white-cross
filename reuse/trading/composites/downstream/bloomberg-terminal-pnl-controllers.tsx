/**
 * LOC: WC-DOWN-TRADING-BBGPNL-010
 * File: /reuse/trading/composites/downstream/bloomberg-terminal-pnl-controllers.tsx
 *
 * UPSTREAM (imports from):
 *   - ../pnl-calculation-attribution-composite
 *   - @nestjs/common (v10.x)
 *   - @nestjs/swagger (v7.x)
 *   - sequelize (v6.x)
 *
...
class BloombergPnLModel extends Model<any, any> {
  declare id: string;
  declare name: string;
...
export function initBloombergPnLModel(sequelize: Sequelize): typeof BloombergPnLModel {
  BloombergPnLModel.init(
    {
      id: {
        type: DataTypes.UUID,
    ...
    {
      sequelize,
      tableName: 'bloombergpnl_records',
      modelName: 'BloombergPnL',
      timestamps: true,
      underscored: true,
      indexes: [
        { fields: ['status'] },
...
export function initPnLAttributionModel(sequelize: Sequelize): typeof PnLAttributionModel {
  PnLAttributionModel.init(
    {
      id: {
        type: DataTypes.UUID,
        defaultValue: DataTypes.UUIDV4,
    ...
    {
      sequelize,
      tableName: 'pnlattribution_records',
      modelName: 'PnLAttribution',
      timestamps: true,
      underscored: true,
      indexes: [
        { fields: ['status'] },
...
@Injectable()
export class BloombergPnLControllerService {
  private readonly logger = new Logger(BloombergPnLControllerService.name);
  ...
  async create(
    ...
    try {
      ...
      return await BloombergPnLModel.create(
        {
          name,
          status,
          priority,
  ...
  async findById(
    ...
    try {
      const record = await BloombergPnLModel.findByPk(id, { transaction });
  ...
  async findAll(
    ...
    try {
      ...
      return await BloombergPnLModel.findAll({
        where,
        limit,
        offset,
...
@ApiTags('Bloomberg Terminal P&L Controllers')
@Controller('api/v1/bloomberg-pnl')
export class BloombergPnLController {
  private readonly logger = new Logger(BloombergPnLController.name);

  constructor(private readonly service: BloombergPnLControllerService) {}
...
export {
  BloombergPnLControllerService,
  BloombergPnLController,
  BloombergPnLModel,
  initBloombergPnLModel,
  PnLSnapshotModel,
  initPnLSnapshotModel,
  PnLAttributionModel,
  initPnLAttributionModel,
};
