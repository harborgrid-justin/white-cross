/**
 * LOC: WC-DOWN-TRADING-BBGINT-011
 * File: /reuse/trading/composites/downstream/bloomberg-terminal-integration.tsx
 *
 * UPSTREAM (imports from):
 *   - ../multi-asset-portfolio-composite
 *   - @nestjs/common (v10.x)
 *   - @nestjs/swagger (v7.x)
 *   - sequelize (v6.x)
 *
...
class BloombergConnectionModel extends Model<any, any> {
  declare id: string;
  declare name: string;
...
export function initBloombergConnectionModel(sequelize: Sequelize): typeof BloombergConnectionModel {
  BloombergConnectionModel.init(
    {
      id: {
        type: DataTypes.UUID,
    ...
    {
      sequelize,
      tableName: 'bloombergconnection_records',
      modelName: 'BloombergConnection',
      timestamps: true,
      underscored: true,
      indexes: [
        { fields: ['status'] },
  ...
  return BloombergConnectionModel;
}

class BloombergDataFeedModel extends Model<any, any> {
  declare id: string;
  declare name: string;
  declare status: Status;
  declare priority: Priority;
...
export function initBloombergDataFeedModel(sequelize: Sequelize): typeof BloombergDataFeedModel {
  BloombergDataFeedModel.init(
    {
      id: {
        type: DataTypes.UUID,
        defaultValue: DataTypes.UUIDV4,
    ...
    {
      sequelize,
      tableName: 'bloombergdatafeed_records',
      modelName: 'BloombergDataFeed',
      timestamps: true,
      underscored: true,
      indexes: [
        { fields: ['status'] },
  ...
  return BloombergDataFeedModel;
}

class BloombergSessionModel extends Model<any, any> {
  declare id: string;
  declare name: string;
  declare status: Status;
  declare priority: Priority;
...
export function initBloombergSessionModel(sequelize: Sequelize): typeof BloombergSessionModel {
  BloombergSessionModel.init(
    {
      id: {
        type: DataTypes.UUID,
        defaultValue: DataTypes.UUIDV4,
    ...
    {
      sequelize,
      tableName: 'bloombergsession_records',
      modelName: 'BloombergSession',
      timestamps: true,
      underscored: true,
      indexes: [
        { fields: ['status'] },
...
@Injectable()
export class BloombergIntegrationService {
  private readonly logger = new Logger(BloombergIntegrationService.name);
  ...
  async create(
    ...
    try {
      ...
      return await BloombergConnectionModel.create(
        {
          name,
          status,
          priority,
  ...
  async findById(
    ...
    try {
      const record = await BloombergConnectionModel.findByPk(id, { transaction });
  ...
  async findAll(
    ...
    try {
      ...
      return await BloombergConnectionModel.findAll({
        where,
        limit,
        offset,
...
@ApiTags('Bloomberg Terminal Integration')
@Controller('api/v1/bloomberg-integration')
export class BloombergIntegrationController {
  private readonly logger = new Logger(BloombergIntegrationController.name);

  constructor(private readonly service: BloombergIntegrationService) {}
...
export {
  BloombergIntegrationService,
  BloombergIntegrationController,
  BloombergConnectionModel,
  initBloombergConnectionModel,
  BloombergDataFeedModel,
  initBloombergDataFeedModel,
  BloombergSessionModel,
  initBloombergSessionModel,
};
