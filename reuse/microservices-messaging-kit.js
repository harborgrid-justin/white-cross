...
exports.wrapMessageWithMetadata = wrapMessageWithMetadata;
exports.unwrapMessageEnvelope = unwrapMessageEnvelope;
exports.generateCorrelationId = generateCorrelationId;
exports.createRabbitMQConfig = createRabbitMQConfig;
exports.createDeadLetterQueue = createDeadLetterQueue;
exports.acknowledgeRabbitMQMessage = acknowledgeRabbitMQMessage;
exports.rejectRabbitMQMessage = rejectRabbitMQMessage;
...
exports.createKafkaConsumerGroup = createKafkaConsumerGroup;
...
exports.publishToNats = publishToNats;
...
exports.subscribeToRedisPattern = subscribeToRedisPattern;
exports.publishWithTTL = publishWithTTL;
...
exports.executeWithRetryObservable = executeWithRetryObservable;
exports.createRetryPolicyForErrors = createRetryPolicyForErrors;
...
exports.addComponentHealthCheck = addComponentHealthCheck;
...
exports.checkMessageIdempotency = checkMessageIdempotency;
exports.markMessageAsProcessed = markMessageAsProcessed;
exports.createSagaContext = createSagaContext;
exports.executeSaga = executeSaga;
exports.compensateSaga = compensateSaga;
exports.createMessageRoutingRule = createMessageRoutingRule;
exports.routeMessage = routeMessage;
exports.createPriorityQueueMessage = createPriorityQueueMessage;
exports.createDistributedTransaction = createDistributedTransaction;
exports.prepareDistributedTransaction = prepareDistributedTransaction;
exports.commitDistributedTransaction = commitDistributedTransaction;
/**
 * File: /reuse/microservices-messaging-kit.ts
 ...
 * ```
 */
function createMessagePattern(cmd, version, priority, metadata) {
    return {
        cmd,
        ...(version && { version }),
 ...
 * ```
 */
function createEventPattern(event, aggregateId, aggregateType, version, metadata) {
    return {
        event,
 ...
 * ```
 */
function wrapMessageWithMetadata(payload, metadata) {
 ...
 * ```
 */
function createRabbitMQConfig(queue, urls, options) {
    return {
        urls,
        queue,
 ...
 * ```
 */
function createDeadLetterQueue(originalQueue, maxRetries = 3, retryDelay = 5000, errorHandler) {
    return {
 ...
 * ```
 */
function acknowledgeRabbitMQMessage(channel, message, multiple = false) {
    try {
        if (!channel || !message) {
 ...
 * ```
 */
function rejectRabbitMQMessage(channel, message, requeue = false, reason) {
    try {
        if (!channel || !message) {
 ...
 * ```
 */
function publishToRabbitMQ(channel, exchange, routingKey, message, options) {
    try {
 ...
 * ```
 */
function createKafkaConfig(clientId, brokers, groupId, options) {
    return {
        clientId,
        brokers,
        groupId,
 ...
 * ```
 */
function createKafkaTopic(topic, numPartitions = 3, replicationFactor = 2, options) {
    return {
        topic,
        numPartitions,
        replicationFactor,
 ...
 * ```
 */
function serializeKafkaMessage(data, key, headers) {
    return {
        ...(key && { key }),
...
function deserializeKafkaMessage(message) {
    try {
        ...
        return {
            ...
            key: null,
            headers: null,
            error: `Failed to deserialize Kafka message: ${error.message}`,
        };
    }
}
/**
 ...
 * ```
 */
function createKafkaConsumerGroup(groupId, topics, options) {
    return {
        groupId,
        topics,
 ...
 * ```
 */
function createNatsConfig(servers, options) {
    return {
        servers,
        user: options?.user,
        pass: options?.pass,
 ...
 * ```
 */
function createNatsSubject(domain, entity, action, environment = 'prod') {
 ...
 * ```
 */
function createNatsQueueGroup(serviceName, version = 'v1', environment = 'prod') {
 ...
 * ```
 */
async function natsRequestReply(client, subject, data, timeoutMs = 5000) {
        ...
        try {
            client.request(subject, JSON.stringify(data), (response) => {
                ...
                try {
                    ...
                    reject(new Error(`Failed to parse NATS response: ${parseError.message}`));
                }
            });
        }
        catch (error) {
 ...
 * ```
 */
function publishToNats(client, subject, data, replyTo) {
    try {
 ...
 * ```
 */
function createRedisConfig(host, port, options) {
    return {
        host,
        port,
        ...
        retryStrategy: options?.retryStrategy || ((times) => Math.min(times * 100, 3000)),
 ...
 * ```
 */
function createRedisChannel(domain, event, namespace = 'default') {
 ...
 * ```
 */
async function publishToRedis(client, channel, message) {
    try {
 ...
 * ```
 */
async function subscribeToRedisPattern(client, pattern, handler) {
    try {
        await client.psubscribe(pattern);
        client.on('pmessage', (subscribedPattern, channel, message) => {
 ...
 * ```
 */
async function publishWithTTL(client, key, message, ttlSeconds) {
    try {
        const messageString = JSON.stringify(message);
 ...
 * ```
 */
async function validateMessagePayload(payload, rules) {
    const errors = [];
 ...
 * ```
 */
function sanitizeMessagePayload(payload, sensitiveFields, mask = false) {
 ...
 * ```
 */
function validateMessageSize(payload, maxSizeBytes) {
 ...
 * ```
 */
function createHealthcareMessageSchema(entityType) {
    const baseRules = [
 ...
 * ```
 */
function createRetryPolicy(maxAttempts, backoffType, initialDelay, options) {
    return {
        maxAttempts,
        backoffType,
        initialDelay,
 ...
 * ```
 */
function executeWithRetryObservable(operation, policy) {
 ...
 * ```
 */
function createRetryPolicyForErrors(retryableErrorCodes, maxAttempts = 3, initialDelay = 1000) {
 ...
 * ```
 */
function updateCircuitBreakerState(state, config, success) {
    const now = Date.now();
    if (success) {
 ...
 * ```
 */
function canExecuteOperation(state, config) {
    const now = Date.now();
 ...
 * ```
 */
async function executeWithCircuitBreaker(operation, state, config, onStateChange) {
    const { allowed, reason, nextState } = canExecuteOperation(state, config);
    if (!allowed) {
 ...
 * ```
 */
function createCircuitBreakerConfig(failureThreshold = 5, successThreshold = 2, timeout = 10000, resetTimeout = 60000) {
    return {
        failureThreshold,
        successThreshold,
        timeout,
 ...
 * ```
 */
async function registerServiceInstance(registration) {
 ...
 * ```
 */
function createHealthCheckStatus(serviceId, version, startTime = Date.now()) {
    return {
        status: 'healthy',
        timestamp: new Date(),
 ...
 * ```
 */
function addComponentHealthCheck(status, componentName, componentHealth) {
    const updatedChecks = {
        ...status.checks,
 ...
 * ```
 */
async function checkDatabaseHealth(connection, componentId) {
 ...
 * ```
 */
async function checkMessageBrokerHealth(client, brokerType, componentId) {
    const startTime = Date.now();
    try {
        const isConnected = client && (typeof client.isConnected === 'function' ? client.isConnected() :
        ...
        return {
            ...
            componentId,
            time: new Date().toISOString(),
            output: `${brokerType} connection failed: ${error.message}`,
        };
    }
}
// ============================================================================
 ...
 * ```
 */
function createIdempotencyRecord(messageId, ttlSeconds = 3600, result) {
    const now = new Date();
    return {
        messageId,
        processedAt: now,
 ...
 * ```
 */
async function checkMessageIdempotency(messageId, store) {
    try {
 ...
 * ```
 */
async function markMessageAsProcessed(messageId, store, result, ttlSeconds = 3600) {
 ...
 * ```
 */
function createSagaContext(sagaId, initialData) {
    return {
 ...
 * ```
 */
async function executeSaga(steps, context) {
    ...
    try {
        for (let i = 0; i < steps.length; i++) {
            ...
            try {
                ...
                const result = await Promise.race([
                    step.action(context),
                    new Promise((_, reject) => setTimeout(() => reject(new Error(`Step timeout: ${step.stepName}`)), stepTimeout)),
                ]);
 ...
 * ```
 */
async function compensateSaga(steps, context, failedStepIndex) {
 ...
 * ```
 */
function createMessageRoutingRule(pattern, destination, priority = 0, condition) {
    return {
        pattern,
        destination,
        priority,
 ...
 * ```
 */
function routeMessage(message, rules) {
 ...
 * ```
 */
function createPriorityQueueMessage(payload, priority) {
    return {
        priority,
        payload,
        timestamp: new Date(),
 ...
 * ```
 */
function createDistributedTransaction(participants, timeout = 30000) {
    return {
 ...
 * ```
 */
async function prepareDistributedTransaction(context, prepareFunc) {
    const preparePromises = context.participants.map(async (participant) => {
        try {
            const canPrepare = await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error('Prepare timeout')), context.timeout)),
            ]);
            if (canPrepare) {
 ...
 * ```
 */
async function commitDistributedTransaction(context, commit, commitFunc) {
...
//# sourceMappingURL=microservices-messaging-kit.js.map