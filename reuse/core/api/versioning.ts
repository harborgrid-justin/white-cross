...
export function extractApiVersion(
  ...
  options: Pick<VersioningOptions, 'headerName' | 'queryParam'> = {}
): string | null {
...
export function createVersionRouter(
  versions: Record<string, RequestHandler | any>,
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    if (config.deprecated) {
      ...
      if (config.sunsetDate) {
        res.setHeader(
          'Sunset',
          config.sunsetDate.toUTCString()
        );
      }

      // Log deprecation warning
      console.warn(
        `[API Deprecation] Version ${config.version} accessed`,
        {
          path: req.path,
          method: req.method,
...
export function createVersionNegotiator(
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    res.setHeader(
      ...
      getLatestVersion(availableVersions) || ''
    );
...
export function createVersionRegistry() {
  const versions = new Map<string, VersionMetadata>();
