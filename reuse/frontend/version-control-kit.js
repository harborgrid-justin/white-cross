...
exports.calculateContentSize = calculateContentSize;
...
exports.setVersionPermissions = setVersionPermissions;
exports.checkVersionPermission = checkVersionPermission;
exports.lockVersion = lockVersion;
exports.unlockVersion = unlockVersion;
exports.isVersionLocked = isVersionLocked;
exports.exportVersion = exportVersion;
exports.importVersion = importVersion;
exports.exportVersionHistory = exportVersionHistory;
exports.purgeOldVersions = purgeOldVersions;
exports.cleanupOrphanedVersions = cleanupOrphanedVersions;
exports.optimizeVersionStorage = optimizeVersionStorage;
exports.addVersionComment = addVersionComment;
exports.getVersionComments = getVersionComments;
exports.updateVersionMetadata = updateVersionMetadata;
exports.addVersionTags = addVersionTags;
exports.removeVersionTags = removeVersionTags;
exports.searchVersionsByTags = searchVersionsByTags;
exports.useVersionHistory = useVersionHistory;
exports.useVersionCompare = useVersionCompare;
exports.useAutoVersioning = useAutoVersioning;
exports.useVersionRestore = useVersionRestore;
exports.useVersionLock = useVersionLock;
exports.VersionTimeline = VersionTimeline;
exports.VersionList = VersionList;
exports.VersionViewer = VersionViewer;
exports.DiffViewer = DiffViewer;
exports.SideBySideCompare = SideBySideCompare;
exports.ChangesHighlighter = ChangesHighlighter;
...
async function createVersion(options) {
    const { contentId, content, message, createdBy, createdByName, branch = 'main', parentId = null, tags = [], metadata = {}, status = 'draft' } = options;
 ...
 * ```
 */
async function deleteVersion(versionId, hard = false) {
    // In real implementation: API call
 ...
 * ```
 */
async function getVersionHistory(options) {
    const { contentId, branch, limit = 50, includeDeleted = false, sortOrder = 'desc' } = options;
 ...
 * ```
 */
async function getLatestVersion(contentId, branch = 'main') {
    const versions = await getVersionHistory({
        contentId,
        branch,
        limit: 1,
 ...
 * ```
 */
async function getVersionByNumber(contentId, versionNumber, branch = 'main') {
 ...
 * ```
 */
async function compareVersions(sourceVersion, targetVersion) {
...
function generateDiff(source, target) {
    ...
    function diffObjects(src, tgt, path = '') {
 ...
 * ```
 */
function calculateChanges(oldVersion, newVersion) {
 ...
 * ```
 */
function detectConflicts(baseVersion, targetVersion) {
    const conflicts = [];
...
async function mergeBranch(options) {
    const { sourceBranch, targetBranch, strategy = 'auto', message, createdBy, createdByName } = options;
 ...
 * ```
 */
function resolveMergeConflicts(conflicts, resolutions) {
 ...
 * ```
 */
async function getBranches(contentId) {
    // In real implementation: API call
    return [];
}
/**
 ...
 * ```
 */
async function switchBranch(contentId, branchName) {
 ...
 * ```
 */
async function restoreVersion(versionId, options = {}) {
    const { createNew = true, message = 'Restored version', notify = false, onSuccess, onError } = options;
    try {
 ...
 * ```
 */
async function rollbackToVersion(versionId, deleteSubsequent = false) {
 ...
 * ```
 */
async function revertChanges(versionId, changePaths) {
    const version = await loadVersion(versionId);
    if (!version.parentId) {
 ...
 * ```
 */
async function setVersionPermissions(versionId, userId, permissions, grantedBy) {
 ...
 * ```
 */
async function checkVersionPermission(versionId, userId, permission) {
 ...
 * ```
 */
async function lockVersion(versionId, userId, userName, duration = 3600000, // 1 hour default
reason) {
    const lock = {
        versionId,
        lockedBy: userId,
 ...
 * ```
 */
async function unlockVersion(versionId, userId) {
 ...
 * ```
 */
async function isVersionLocked(versionId) {
 ...
 * ```
 */
async function exportVersion(versionId, format = 'json', options) {
    const version = await loadVersion(versionId);
 ...
 * ```
 */
async function importVersion(exportData, options) {
 ...
 * ```
 */
async function exportVersionHistory(contentId, format = 'json') {
    ...
    switch (format) {
        ...
        case 'xml':
            // Convert to XML
            return `<?xml version="1.0"?>\n<versions>\n${versions.map(v => `  <version id="${v.id}" number="${v.versionNumber}"/>`).join('\n')}\n</versions>`;
        default:
 ...
 * ```
 */
async function purgeOldVersions(contentId, policy) {
    const { keepLast = 10, olderThanDays = 90, keepTagged = true, keepPublished = true } = policy;
    ...
    const toPurge = versions
        ...
        if (v.createdAt > cutoffDate)
            return false;
        // Keep tagged versions if policy says so
        if (keepTagged && v.tags.length > 0)
            return false;
        // Keep published versions if policy says so
        if (keepPublished && v.status === 'published')
            return false;
        return true;
    })
        .map(v => v.id);
 ...
 * ```
 */
async function cleanupOrphanedVersions(contentId) {
    const versions = await getVersionHistory({ contentId });
 ...
 * ```
 */
async function optimizeVersionStorage(contentId) {
    const versions = await getVersionHistory({ contentId });
 ...
 * ```
 */
async function addVersionComment(versionId, comment) {
    const newComment = {
 ...
 * ```
 */
async function getVersionComments(versionId) {
 ...
 * ```
 */
async function updateVersionMetadata(versionId, metadata) {
    const version = await loadVersion(versionId);
 ...
 * ```
 */
async function addVersionTags(versionId, tags) {
    const version = await loadVersion(versionId);
 ...
 * ```
 */
async function removeVersionTags(versionId, tags) {
    const version = await loadVersion(versionId);
 ...
 * ```
 */
async function searchVersionsByTags(contentId, tags, matchAll = false) {
    const versions = await getVersionHistory({ contentId });
 ...
 * ```
 */
function useVersionHistory(contentId, options = {}) {
    const { branch = 'main', limit = 50, autoRefresh = false, refreshInterval = 30000 } = options;
    const [versions, setVersions] = (0, react_1.useState)([]);
    const [loading, setLoading] = (0, react_1.useState)(true);
    const [error, setError] = (0, react_1.useState)(null);
    const loadVersions = (0, react_1.useCallback)(async () => {
        try {
            setLoading(true);
    ...
    const createVersionHandler = (0, react_1.useCallback)(async (data) => {
        const version = await createVersion({
            contentId,
            branch,
            ...data
    ...
    const restoreVersionHandler = (0, react_1.useCallback)(async (versionId, options) => {
        const restored = await restoreVersion(versionId, options);
        await loadVersions();
        return restored;
    }, [loadVersions]);
    const deleteVersionHandler = (0, react_1.useCallback)(async (versionId, hard = false) => {
 ...
 * ```
 */
function useVersionCompare(sourceVersionId, targetVersionId) {
 ...
 * ```
 */
function useAutoVersioning(config) {
    const { enabled, interval, maxVersions, includeMetadata = true, onSave, onError } = config;
    const [isPaused, setIsPaused] = (0, react_1.useState)(!enabled);
    const [lastSaved, setLastSaved] = (0, react_1.useState)(null);
    const contentRef = (0, react_1.useRef)(null);
    const intervalRef = (0, react_1.useRef)(null);
    const savePoint = (0, react_1.useCallback)((content) => {
 ...
 * ```
 */
function useVersionRestore() {
    const [restoring, setRestoring] = (0, react_1.useState)(false);
    const [success, setSuccess] = (0, react_1.useState)(false);
    const [error, setError] = (0, react_1.useState)(null);
    const [restoredVersion, setRestoredVersion] = (0, react_1.useState)(null);
    const restore = (0, react_1.useCallback)(async (versionId, options) => {
        try {
            setRestoring(true);
            setSuccess(false);
            setError(null);
 ...
 * ```
 */
function useVersionLock(versionId) {
    const [locked, setLocked] = (0, react_1.useState)(false);
    const [lockInfo, setLockInfo] = (0, react_1.useState)(null);
    const [loading, setLoading] = (0, react_1.useState)(true);
    const checkLock = (0, react_1.useCallback)(async () => {
        try {
            setLoading(true);
    ...
    const acquireLock = (0, react_1.useCallback)(async (userId, userName, duration, reason) => {
        const lock = await lockVersion(versionId, userId, userName, duration, reason);
        setLocked(true);
        setLockInfo(lock);
        return lock;
    }, [versionId]);
    const releaseLock = (0, react_1.useCallback)(async (userId) => {
 ...
 * ```
 */
function VersionTimeline(props) {
    const { versions, currentVersionId, onVersionClick, onRestore, highlightCurrent = true, className = '' } = props;
    return className = {} `version-timeline ${className}`;
}
 >
...
"version-header" >
    className;
"version-message" > { version, : .message } < /span>
    < span;
...
className = "version-meta" >
    className;
"version-author" > { version, : .createdByName } < /span>;
{
    version.tags.length > 0 && className;
    "version-tags" >
        { version, : .tags.map(tag => key = { tag }, className = "tag" > { tag } < /span>) }
        < /div>;
}
/div>;
{
...
function VersionList(props) {
    const { versions, selectedVersionId, onSelect, onDelete, showActions = true, className = '' } = props;
    return className = {} `version-list ${className}`;
}
 >
...
/span>
    < span;
className = "version-message" > { version, : .message } < /span>
    < /div>
    < div;
className = "version-details" >
    ...
    < span;
className = {} `status status-${version.status}`;
 >
    { version, : .status }
    < /span>
    < /div>
...
function VersionViewer(props) {
    const { version, showMetadata = true, showComments = false, renderContent, className = '' } = props;
    ...
    if (!version) {
        return className = {} `version-viewer empty ${className}`;
    }
     >
        No;
    version;
...
return className = {} `version-viewer ${className}`;
 >
    className;
"version-header" >
    Version;
...
className = "version-date" >
    ...
    < /div>;
{
    showMetadata && className;
    "version-metadata" >
        className;
    "meta-item" >
        Author;
    /strong> {version.createdByName}
        < /div>
        < div;
    className = "meta-item" >
        Message;
    ...
    className = "meta-item" >
        Branch;
    /strong> {version.branch}
        < /div>
        < div;
    className = "meta-item" >
        Status;
    /strong> {version.status}
        < /div>;
    {
        version.tags.length > 0 && className;
        "meta-item" >
...
"version-content" >
    {}(version.content);
({ JSON, : .stringify(version.content, null, 2) } < /pre>);
/div>;
{
    showComments && comments.length > 0 && className;
    "version-comments" >
        Comments({ comments, : .length }) < /h4>;
    {
        comments.map(comment => key = { comment, : .id }, className = "comment" >
...
function DiffViewer(props) {
    const { diff, viewMode = 'unified', highlightChanges = true, className = '' } = props;
    if (!diff) {
        return className = {} `diff-viewer empty ${className}`;
    }
     >
        No;
    diff;
...
return className = {} `diff-viewer mode-${viewMode} ${className}`;
 >
    className;
"diff-stats" >
    className;
"additions" > +{ diff, : .additions } < /span>
    < span;
className = "deletions" > -{ diff, : .deletions } < /span>
    < span;
className = "modifications" > ~{ diff, : .modifications } < /span>
    < span;
className = "similarity" > { diff, : .similarity } % similar < /span>
    < /div>
    < div;
className = "diff-changes" >
    { diff, : .changes.map(change => key = { change, : .id }, className = {} `diff-change type-${change.type} ${highlightChanges ? 'highlighted' : ''}`) }
    >
        className;
"change-path" > { change, : .path } < /div>;
{
    change.type === 'addition' && className;
...
{
    change.type === 'deletion' && className;
    "change-value deletion" >
        className;
    "label" > Deleted;
    /span>
...
{
    change.type === 'modification' && className;
    "change-value old" >
        className;
    "label" > Old;
    /span>
 ...
 * ```
 */
function SideBySideCompare(props) {
    const { leftVersion, rightVersion, highlightDifferences = true, renderContent, className = '' } = props;
    const diff = (0, react_1.useMemo)(() => {
        if (!leftVersion || !rightVersion)
            return null;
        return generateDiff(leftVersion.content, rightVersion.content);
    }, [leftVersion, rightVersion]);
    return className = {} `side-by-side-compare ${className}`;
}
 >
    className;
"compare-header" >
...
/div>
    < div;
className = "version-header right" >
    { rightVersion } && v;
{
...
className = "compare-content" >
    className;
"content-pane left" >
    { leftVersion } && (renderContent ? (renderContent(leftVersion.content)) : ({ JSON, : .stringify(leftVersion.content, null, 2) } < /pre>));
/div>
    < div;
className = "content-pane right" >
    { rightVersion } && (renderContent ? (renderContent(rightVersion.content)) : ({ JSON, : .stringify(rightVersion.content, null, 2) } < /pre>));
/div>
    < /div>;
{
    diff && highlightDifferences && className;
    "compare-diff" >
        diff;
    {
        diff;
 ...
 * ```
 */
function ChangesHighlighter(props) {
    const { content, changes, highlightColor = '#ffeb3b', className = '' } = props;
    // In a real implementation, this would intelligently highlight changes in the rendered content
    return className = {} `changes-highlighter ${className}`;
}
 >
...
//# sourceMappingURL=version-control-kit.js.map