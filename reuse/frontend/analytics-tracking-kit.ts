...
let globalConsent: ConsentConfig = {
  analytics: false,
  marketing: false,
  functional: false,
  necessary: true,
 ...
 * ```
 */
export function initializeGTM(config: AnalyticsConfig): void {
  if (typeof window === 'undefined' || !config.containerId) return;
 ...
 * ```
 */
export function initializeSegment(config: AnalyticsConfig): void {
  if (typeof window === 'undefined' || !config.writeKey) return;
 ...
 * ```
 */
export function endSession(trackSessionEnd = true): void {
 ...
 * ```
 */
export function trackPageView(pageData?: Partial<PageViewEvent>): void {
  if (!globalConsent.analytics || typeof window === 'undefined') return;
...
export function trackEvent(eventName: string, parameters?: EventParameters): void {
  if (!globalConsent.analytics) return;
 ...
 * ```
 */
export function setCustomDimension(dimension: CustomDimension): void {
  if (!globalConsent.analytics) return;
 ...
 * ```
 */
export function setCustomMetric(metric: CustomMetric): void {
  if (!globalConsent.analytics) return;
 ...
 * ```
 */
export function trackProductView(item: EcommerceItem): void {
  if (!globalConsent.analytics) return;
 ...
 * ```
 */
export function trackRefund(transactionId: string, value: number, currency = 'USD'): void {
  if (!globalConsent.analytics) return;
 ...
 * ```
 */
export function trackScrollDepth(percentage: number, contentId?: string): void {
  if (!globalConsent.analytics) return;
 ...
 * ```
 */
export function trackReadingTime(seconds: number, contentId: string, contentType?: string): void {
  if (!globalConsent.analytics) return;
 ...
 * ```
 */
export function trackContentEngagement(engagement: ContentEngagement): void {
  if (!globalConsent.analytics) return;
 ...
 * ```
 */
export function trackVideoInteraction(video: VideoAnalytics): void {
  if (!globalConsent.analytics) return;
 ...
 * ```
 */
export function trackMediaDownload(mediaId: string, mediaType: string, fileName: string): void {
  if (!globalConsent.analytics) return;
 ...
 * ```
 */
export function trackFormInteraction(form: FormTracking): void {
  if (!globalConsent.analytics) return;
 ...
 * ```
 */
export function trackFormError(formId: string, fieldName: string, errorMessage: string): void {
  if (!globalConsent.analytics) return;
 ...
 * ```
 */
export function trackError(error: ErrorTracking): void {
  if (!globalConsent.analytics) return;
 ...
 * ```
 */
export function trackExperimentVariant(variant: ExperimentVariant): void {
  if (!globalConsent.analytics) return;
 ...
 * ```
 */
export function trackSearch(search: SearchTracking): void {
  if (!globalConsent.analytics) return;
 ...
 * ```
 */
export function setConsent(consent: ConsentConfig): void {
  globalConsent = {
    ...consent,
    timestamp: Date.now(),
    version: '1.0',
 ...
 * ```
 */
export function getConsent(): ConsentConfig {
 ...
 * ```
 */
export function hasConsent(purpose: keyof ConsentConfig): boolean {
 ...
 * ```
 */
export function initializeHeatmap(config: HeatmapConfig): void {
 ...
 * ```
 */
export function triggerHeatmapEvent(eventName: string, parameters?: EventParameters): void {
  if (typeof window === 'undefined') return;
...
export function useTracking(): UseTrackingReturn {
  const [isInitialized, setIsInitialized] = useState(!!globalConfig);

  useEffect(() => {
    setIsInitialized(!!globalConfig);
  }, []);

  const track = useCallback((eventName: string, parameters?: EventParameters) => {
  ...
  const updateConsent = useCallback((consent: ConsentConfig) => {
    setConsent(consent);
  }, []);
 ...
 * ```
 */
export function usePageView(pageData?: Partial<PageViewEvent>): void {
...
export function useEvent(
  eventName: string,
  baseParameters?: EventParameters
): (parameters?: EventParameters) => void {
  return useCallback(
    (parameters?: EventParameters) => {
...
export function useEcommerceTracking() {
  const trackView = useCallback((item: EcommerceItem) => {
    trackProductView(item);
  }, []);

  const trackAdd = useCallback((item: EcommerceItem) => {
    trackAddToCart(item);
  }, []);

  const trackRemove = useCallback((item: EcommerceItem) => {
    trackRemoveFromCart(item);
  }, []);

  const trackCheckout = useCallback((items: EcommerceItem[], value: number, currency = 'USD') => {
    trackBeginCheckout(items, value, currency);
  }, []);

  const trackTransaction = useCallback((transaction: TransactionData) => {
 ...
 * ```
 */
export function useScrollDepth(contentId?: string, milestones: number[] = [25, 50, 75, 100]): void {
 ...
 * ```
 */
export function useReadingTime(contentId: string, contentType?: string): number {
 ...
 * ```
 */
export function useVideoTracking(videoId: string, videoTitle: string, provider?: string) {
