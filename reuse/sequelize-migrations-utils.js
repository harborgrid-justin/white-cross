...
exports.generateTableCreationMigration = generateTableCreationMigration;
exports.generateAddColumnMigration = generateAddColumnMigration;
exports.generateModifyColumnMigration = generateModifyColumnMigration;
exports.generateDataMigrationTemplate = generateDataMigrationTemplate;
exports.createTableWithTimestamps = createTableWithTimestamps;
exports.renameTableSafely = renameTableSafely;
exports.addColumnWithBackfill = addColumnWithBackfill;
exports.changeColumnTypeSafely = changeColumnTypeSafely;
exports.dropColumnSafely = dropColumnSafely;
exports.createIndexConcurrently = createIndexConcurrently;
exports.dropIndexConcurrently = dropIndexConcurrently;
...
exports.addMultipleForeignKeys = addMultipleForeignKeys;
exports.recreateForeignKey = recreateForeignKey;
exports.batchDataMigration = batchDataMigration;
exports.migrateDataBetweenTables = migrateDataBetweenTables;
exports.cleanupDataMigration = cleanupDataMigration;
exports.createMigrationSnapshot = createMigrationSnapshot;
exports.restoreFromSnapshot = restoreFromSnapshot;
exports.validateRollbackSafety = validateRollbackSafety;
exports.generateInverseMigration = generateInverseMigration;
exports.seedData = seedData;
exports.generateSeedFromTable = generateSeedFromTable;
exports.truncateTable = truncateTable;
exports.validateMigration = validateMigration;
exports.checkMigrationDependencies = checkMigrationDependencies;
exports.estimateMigrationTime = estimateMigrationTime;
exports.zeroDowntimeTableMigration = zeroDowntimeTableMigration;
exports.testMigrationOnShadowTable = testMigrationOnShadowTable;
 ...
 * ```
 */
async function generateMigrationFile(name, migrationsDir, description = '') {
 ...
 * ```
 */
async function generateTableCreationMigration(config, outputPath) {
 ...
 * ```
 */
function generateAddColumnMigration(config) {
 ...
 * ```
 */
function generateModifyColumnMigration(oldConfig, newConfig) {
    return `
  async up(queryInterface: QueryInterface): Promise<void> {
    await queryInterface.changeColumn('${newConfig.tableName}', '${newConfig.columnName}', ${JSON.stringify(newConfig, null, 2)});
  },

  async down(queryInterface: QueryInterface): Promise<void> {
    await queryInterface.changeColumn('${oldConfig.tableName}', '${oldConfig.columnName}', ${JSON.stringify(oldConfig, null, 2)});
  }
  `;
}
/**
 ...
 * ```
 */
function generateDataMigrationTemplate(description, tableName) {
    return `/**
 ...
 * ```
 */
async function createTableWithTimestamps(queryInterface, tableName, attributes, paranoid = true) {
    const columns = {
        ...attributes,
        createdAt: {
 ...
 * ```
 */
async function renameTableSafely(queryInterface, oldName, newName) {
    const transaction = await queryInterface.sequelize.transaction();
    try {
        // Rename the table
 ...
 * ```
 */
async function addColumnWithBackfill(queryInterface, tableName, columnName, columnDef, backfillFn) {
    const transaction = await queryInterface.sequelize.transaction();
    try {
        ...
        for (const record of records) {
            ...
            await queryInterface.sequelize.query(`UPDATE ${tableName} SET ${columnName} = :value WHERE id = :id`, {
                replacements: { value, id: record.id },
                transaction,
            });
        }
 ...
 * ```
 */
async function changeColumnTypeSafely(queryInterface, tableName, columnName, newType, converter) {
    const transaction = await queryInterface.sequelize.transaction();
    try {
 ...
 * ```
 */
async function dropColumnSafely(queryInterface, tableName, columnName, force = false) {
    const transaction = await queryInterface.sequelize.transaction();
    try {
        if (!force) {
 ...
 * ```
 */
async function createIndexConcurrently(queryInterface, config) {
 ...
 * ```
 */
async function dropIndexConcurrently(queryInterface, tableName, indexName, concurrently = true) {
    const dialect = queryInterface.sequelize.getDialect();
    if (dialect === 'postgres' && concurrently) {
 ...
 * ```
 */
async function createPartialIndex(queryInterface, tableName, fields, whereClause, indexName) {
    await queryInterface.addIndex(tableName, {
 ...
 * ```
 */
async function createFullTextIndex(queryInterface, tableName, columns, indexName) {
    const dialect = queryInterface.sequelize.getDialect();
    if (dialect === 'postgres') {
 ...
 * ```
 */
async function addForeignKeyConstraint(queryInterface, config) {
 ...
 * ```
 */
async function removeForeignKeyConstraint(queryInterface, tableName, constraintName) {
    try {
 ...
 * ```
 */
async function addMultipleForeignKeys(queryInterface, configs) {
    const transaction = await queryInterface.sequelize.transaction();
    try {
 ...
 * ```
 */
async function recreateForeignKey(queryInterface, tableName, oldConstraintName, newConfig) {
    const transaction = await queryInterface.sequelize.transaction();
    try {
 ...
 * ```
 */
async function batchDataMigration(queryInterface, config, progressCallback) {
    const { tableName, batchSize = 1000, transform, where = {}, order = [['id', 'ASC']] } = config;
    const transaction = await queryInterface.sequelize.transaction();
    let totalProcessed = 0;
    try {
        // Get total count
        const [countResult] = await queryInterface.sequelize.query(`SELECT COUNT(*) as count FROM ${tableName} WHERE ${Object.keys(where)
            .map((key) => `${key} = :${key}`)
            .join(' AND ')}`, {
            replacements: where,
            transaction,
         ...
         LIMIT :limit OFFSET :offset`, {
            ...
            for (const record of records) {
                ...
                await queryInterface.sequelize.query(`UPDATE ${tableName} SET ${updateFields} WHERE id = :id`, {
                    replacements: { ...transformed, id: record.id },
                    transaction,
                });
            }
 ...
 * ```
 */
async function migrateDataBetweenTables(queryInterface, sourceTable, destTable, transformer, validator) {
    const transaction = await queryInterface.sequelize.transaction();
    let count = 0;
    try {
 ...
 * ```
 */
async function cleanupDataMigration(queryInterface, tableName, cleanupCondition, archive = true, archiveTable) {
    const transaction = await queryInterface.sequelize.transaction();
    let count = 0;
    try {
        if (archive && archiveTable) {
            ...
            const whereClause = Object.keys(cleanupCondition)
                .map((key) => `${key} = :${key}`)
                .join(' AND ');
            await queryInterface.sequelize.query(`INSERT INTO ${archiveTable} SELECT * FROM ${tableName} WHERE ${whereClause}`, {
                replacements: cleanupCondition,
                transaction,
            });
        }
        // Delete records
        const [result] = await queryInterface.sequelize.query(`DELETE FROM ${tableName} WHERE ${Object.keys(cleanupCondition)
            .map((key) => `${key} = :${key}`)
            .join(' AND ')}
    RETURNING id`, {
            replacements: cleanupCondition,
 ...
 * ```
 */
async function createMigrationSnapshot(queryInterface, tableName, snapshotName) {
 ...
 * ```
 */
async function restoreFromSnapshot(queryInterface, tableName, snapshotName, dropSnapshot = false) {
    const transaction = await queryInterface.sequelize.transaction();
    try {
 ...
 * ```
 */
async function validateRollbackSafety(queryInterface, tableName, criticalColumns) {
    try {
 ...
 * ```
 */
function generateInverseMigration(migrationCode) {
    const inversions = {
 ...
 * ```
 */
async function seedData(queryInterface, config) {
 ...
 * ```
 */
async function generateSeedFromTable(queryInterface, tableName, whereClause = {}, outputPath) {
    const records = await queryInterface.sequelize.query(`SELECT * FROM ${tableName} ${Object.keys(whereClause).length > 0
 ...
 * ```
 */
async function truncateTable(queryInterface, tableName, cascade = false, restartIdentity = true) {
    const dialect = queryInterface.sequelize.getDialect();
    if (dialect === 'postgres') {
 ...
 * ```
 */
async function validateMigration(queryInterface, migrationName, upFn) {
    const errors = [];
    const transaction = await queryInterface.sequelize.transaction();
    try {
 ...
 * ```
 */
async function checkMigrationDependencies(queryInterface, requiredMigrations) {
    try {
 ...
 * ```
 */
async function estimateMigrationTime(queryInterface, tableName, operationType) {
    try {
 ...
 * ```
 */
async function zeroDowntimeTableMigration(queryInterface, config) {
 ...
 * ```
 */
async function testMigrationOnShadowTable(queryInterface, tableName, migrationFn) {
    ...
    try {
        ...
        console.log(`Shadow table test completed. Records: ${result[0].count}`);
        // Cleanup
 ...
 * ```
 */
async function generateMigrationDocumentation(config, affectedTables, outputPath) {
 ...
 * ```
 */
async function exportSchemaDocumentation(queryInterface, outputPath) {
    const [tables] = await queryInterface.sequelize.query(`
    SELECT table_name
    FROM information_schema.tables
    WHERE table_schema = 'public'
...
//# sourceMappingURL=sequelize-migrations-utils.js.map