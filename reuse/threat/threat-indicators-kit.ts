...
interface DomainIoc extends IocIndicator {
  ...
  registeredDate?: Date;
  nameservers?: string[];
  isDga?: boolean; // Domain Generation Algorithm detection
}

/**
 * Hash specific IOC for file identification
 */
interface HashIoc extends IocIndicator {
  type: 'hash';
  hashType: HashType;
  fileSize?: number;
  fileName?: string;
...
/**
 * File IOC with binary characteristics
 */
interface FileIoc extends IocIndicator {
  type: 'file';
  fileName: string;
  fileSize?: number;
  fileType?: string;
  hashes?: Partial<Record<HashType, string>>;
  isPacked?: boolean;
...
/**
 * IOC correlation result linking related indicators
 */
interface IocCorrelation {
  primaryIoc: IocIndicator;
  relatedIocs: IocIndicator[];
  correlationType: 'network' | 'temporal' | 'campaign' | 'infrastructure' | 'behavioral';
  confidence: number; // 0-1 correlation confidence
  description?: string;
}

/**
 * IOC confidence scoring factors
 */
interface ConfidenceFactors {
  sourceReliability: number; // 0-1
  ageOfIndicator: number; // 0-1 (newer is better)
  observationCount: number; // Multiple sightings increase confidence
  falsePositiveRate: number; // 0-1 (lower is better)
  contextRelevance: number; // 0-1
}

/**
 * IOC lifecycle event tracking
 */
interface IocLifecycleEvent {
  iocId: string;
  event: 'created' | 'updated' | 'activated' | 'deactivated' | 'expired' | 'marked_false_positive';
  timestamp: Date;
  actor?: string;
  reason?: string;
  metadata?: Record<string, unknown>;
...
interface DeduplicationResult {
  uniqueIocs: IocIndicator[];
  duplicates: Array<{
    original: IocIndicator;
    duplicates: IocIndicator[];
  }>;
  merged: IocIndicator[];
}
...
interface SwaggerIocApiDefinition {
  '/api/v1/iocs': {
    get: {
      ...
      responses: Record<number, { description: string; content?: unknown }>;
    };
    post: {
 ...
 * ```
 */
export const isValidHash = (hash: string, type: HashType): boolean => {
  const hashRegexes: Record<HashType, RegExp> = {
    md5: /^[a-fA-F0-9]{32}$/,
    sha1: /^[a-fA-F0-9]{40}$/,
    sha256: /^[a-fA-F0-9]{64}$/,
 ...
 * ```
 */
export const normalizeHash = (hash: string, type: HashType): string => {
  const trimmed = hash.trim().toLowerCase();

  if (!isValidHash(trimmed, type)) {
 ...
 * ```
 */
export const validateIoc = (value: string, type: IocType): IocValidationResult => {
  const result: IocValidationResult = {
    isValid: false,
    errors: [],
    warnings: [],
    suggestions: [],
    ...
    switch (type) {
      case 'ip':
        if (isValidIpv4(value) || isValidIpv6(value)) {
          result.isValid = true;
          result.normalized = normalizeIp(value);
        } else {
      ...
      case 'domain':
        if (isValidDomain(value)) {
          result.isValid = true;
          result.normalized = normalizeDomain(value);
        } else {
      ...
      case 'url':
        if (isValidUrl(value)) {
          result.isValid = true;
          result.normalized = normalizeUrl(value);
        } else {
      ...
      case 'email':
        if (isValidEmail(value)) {
          result.isValid = true;
          result.normalized = normalizeEmail(value);
        } else {
      ...
      case 'hash':
        ...
        const hashTypes: HashType[] = ['md5', 'sha1', 'sha256', 'sha512', 'ssdeep'];
        const detectedType = hashTypes.find(ht => isValidHash(value, ht));

        if (detectedType) {
          result.isValid = true;
          result.normalized = normalizeHash(value, detectedType);
 ...
 * ```
 */
export const sanitizeIoc = (value: string): string => {
  return value
 ...
 * ```
 */
export const validateIocIndicator = (ioc: IocIndicator): IocValidationResult => {
  const result: IocValidationResult = {
    isValid: true,
    errors: [],
    warnings: [],
  ...
  if (ioc.type && ioc.value) {
    const valueValidation = validateIoc(ioc.value, ioc.type);
 ...
 * ```
 */
export const extractIpMetadata = (ip: string): Partial<IpIoc> => {
  const metadata: Partial<IpIoc> = {
    type: 'ip',
    value: ip,
  };
 ...
 * ```
 */
export const extractDomainMetadata = (domain: string): Partial<DomainIoc> => {
 ...
 * ```
 */
export const extractUrlMetadata = (url: string): Partial<UrlIoc> => {
 ...
 * ```
 */
export const extractEmailMetadata = (email: string): Partial<EmailIoc> => {
 ...
 * ```
 */
export const enrichIoc = (ioc: IocIndicator): IocIndicator => {
  const enriched = { ...ioc };
  ...
  switch (ioc.type) {
    case 'ip':
      const ipMeta = extractIpMetadata(enriched.value);
      Object.assign(enriched, ipMeta);
      break;
    case 'domain':
      const domainMeta = extractDomainMetadata(enriched.value);
      Object.assign(enriched, domainMeta);
      break;
    case 'url':
      const urlMeta = extractUrlMetadata(enriched.value);
      Object.assign(enriched, urlMeta);
      break;
    case 'email':
      const emailMeta = extractEmailMetadata(enriched.value);
...
// ============================================================================
// IOC CORRELATION FUNCTIONS
// ============================================================================

/**
 * Finds related IOCs based on network infrastructure correlation.
 *
 ...
 * @example
 * ```typescript
 * const primary: IocIndicator = { type: 'domain', value: 'malware.com', ... };
 * const pool: IocIndicator[] = [/* ... */];
...
export const correlateByInfrastructure = (
  ...
  return {
    ...
    description: `Found ${relatedIocs.length} IOCs sharing infrastructure`,
  };
};
 ...
 * @example
 * ```typescript
 * const primary: IocIndicator = { type: 'ip', value: '1.2.3.4', firstSeen: new Date(), ... };
...
export const correlateByTime = (
  ...
  return {
    ...
    description: `Found ${relatedIocs.length} IOCs within ${timeWindowMs / 1000}s time window`,
  };
};
 ...
 * @example
 * ```typescript
 * const primary: IocIndicator = {
 *   type: 'hash',
 *   value: 'abc123...',
 *   metadata: { campaigns: ['APT28'] },
 *   ...
 * };
...
export const correlateByCampaign = (
  ...
  return {
    ...
    description: `Found ${relatedIocs.length} IOCs with shared campaign/actor attribution`,
  };
};
 ...
 * @example
 * ```typescript
 * const ioc: IocIndicator = { type: 'ip', value: '1.2.3.4', confidence: 'medium', ... };
...
export const deduplicateIocs = (iocs: IocIndicator[]): DeduplicationResult => {
  ...
  iocs.forEach(ioc => {
    const validation = validateIoc(ioc.value, ioc.type);
    const normalizedKey = `${ioc.type}:${validation.normalized || ioc.value}`;
...
export const extractIocsFromText = (text: string, type: IocType): string[] => {
  const patterns: Record<IocType, RegExp> = {
    ...
    hash: /\b[a-fA-F0-9]{32,128}\b/g,
    file: /[\w\-. ]+\.(?:exe|dll|bat|cmd|ps1|vbs|js|jar|zip|rar|7z|doc|docx|xls|xlsx|pdf)\b/gi,
    registry: /\b(?:HKEY_LOCAL_MACHINE|HKLM|HKEY_CURRENT_USER|HKCU|HKEY_CLASSES_ROOT|HKCR)\\[\\a-zA-Z0-9_\-]+/g,
    mutex: /\b[A-Za-z0-9_\-]{8,}\b/g,
    'user-agent': /.*/g, // Requires manual extraction
    certificate: /[A-F0-9:]{40,}/g,
  };
 ...
 * ```
 */
export const createPatternMatchConfig = (
  type: IocType,
 ...
 * ```
 */
export const convertToStix = (ioc: IocIndicator): StixIndicator => {
  if (!ioc.id) {
  ...
  const stixIndicator: StixIndicator = {
    type: 'indicator',
    spec_version: '2.1',
    id: stixId,
    created: now,
    ...
    labels: ioc.tags || [],
    confidence: confidenceLevelToStixScore(ioc.confidence),
  };
 ...
 * ```
 */
export const confidenceLevelToStixScore = (level: ConfidenceLevel): number => {
 ...
 * ```
 */
export const convertFromStix = (stix: StixIndicator): IocIndicator => {
  ...
  const ioc: IocIndicator = {
    ...
    type,
    value,
    confidence: stixScoreToConfidenceLevel(stix.confidence || 50),
    severity: 'medium',
    status: 'active',
 ...
 * ```
 */
export const stixScoreToConfidenceLevel = (score: number): ConfidenceLevel => {
 ...
 * ```
 */
export const generateSwaggerIocDefinition = (): SwaggerIocApiDefinition => {
  return {
    '/api/v1/iocs': {
      get: {
        summary: 'List IOC indicators',
 ...
 * ```
 */
export const generateSequelizeIocModel = (): IocModelAttributes => {
  return {
    id: {
      type: 'UUID',
      defaultValue: 'UUIDV4',
...
export default {
  ...
  // Pattern Matching
  extractIocsFromText,
  createPatternMatchConfig,

  // STIX Conversion
  convertToStix,
  confidenceLevelToStixScore,
  convertFromStix,
  stixScoreToConfidenceLevel,

  // Swagger & Sequelize
  generateSwaggerIocDefinition,
  generateSequelizeIocModel,
};
