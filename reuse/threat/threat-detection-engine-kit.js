...
exports.mergeDetectionRules = exports.importDetectionRules = exports.exportDetectionRules = exports.versionDetectionRule = exports.optimizeDetectionRule = exports.testDetectionRule = exports.validateDetectionRule = exports.createDetectionRule = exports.createWhitelistEntry = exports.tuneDetectionRule = exports.analyzeFalsePositive = exports.aggregateRiskScores = exports.calculateRiskTrend = exports.assessEntityRisk = exports.normalizeMLFeatures = exports.ensembleMLPrediction = exports.predictThreatWithML = exports.prepareMLFeatures = exports.detectCausalCorrelations = exports.correlateEventsByTime = exports.correlateSecurityEvents = exports.detectAttackChains = exports.detectSequentialPatterns = exports.matchThreatPatterns = exports.updateAnomalyBaseline = exports.detectTemporalAnomalies = exports.detectBehavioralAnomaly = exports.detectStatisticalAnomalies = exports.aggregateThreatScores = exports.calculateDynamicThreatScore = exports.performRealtimeThreatAssessment = exports.evaluateRule = exports.calculateThreatScore = exports.getMLModelAttributes = exports.getAnomalyDetectionModelAttributes = exports.getThreatDetectionModelAttributes = exports.getDetectionRuleModelAttributes = exports.MLModelType = exports.FalsePositiveAction = exports.RiskLevel = exports.CorrelationType = exports.AnomalyType = exports.EntityType = exports.ActionType = exports.AggregationType = exports.ConditionOperator = exports.RuleType = exports.ThreatSeverity = exports.DetectionSensitivity = exports.DetectionType = void 0;
exports.predictThreatTrends = exports.evaluateDetectionModel = exports.updateDetectionModel = exports.trainDetectionModel = exports.performThreatHunting = exports.generateDetectionSummary = exports.calculateDetectionEffectiveness = exports.generateDetectionCoverageReport = void 0;
/**
 * File: /reuse/threat/threat-detection-engine-kit.ts
...
(function (DetectionType) {
    DetectionType["SIGNATURE_BASED"] = "SIGNATURE_BASED";
    DetectionType["ANOMALY_BASED"] = "ANOMALY_BASED";
    DetectionType["BEHAVIOR_BASED"] = "BEHAVIOR_BASED";
    DetectionType["HEURISTIC"] = "HEURISTIC";
    DetectionType["ML_BASED"] = "ML_BASED";
    DetectionType["HYBRID"] = "HYBRID";
})(DetectionType || (exports.DetectionType = DetectionType = {}));
/**
 * Detection sensitivity levels
 */
var DetectionSensitivity;
(function (DetectionSensitivity) {
    DetectionSensitivity["VERY_LOW"] = "VERY_LOW";
    DetectionSensitivity["LOW"] = "LOW";
    DetectionSensitivity["MEDIUM"] = "MEDIUM";
    DetectionSensitivity["HIGH"] = "HIGH";
    DetectionSensitivity["VERY_HIGH"] = "VERY_HIGH";
})(DetectionSensitivity || (exports.DetectionSensitivity = DetectionSensitivity = {}));
/**
...
var RuleType;
(function (RuleType) {
    RuleType["SIGNATURE"] = "SIGNATURE";
    RuleType["THRESHOLD"] = "THRESHOLD";
    RuleType["CORRELATION"] = "CORRELATION";
    RuleType["STATISTICAL"] = "STATISTICAL";
    RuleType["BEHAVIORAL"] = "BEHAVIORAL";
    RuleType["MACHINE_LEARNING"] = "MACHINE_LEARNING";
...
(function (AnomalyType) {
    ...
    AnomalyType["TEMPORAL_ANOMALY"] = "TEMPORAL_ANOMALY";
    AnomalyType["SPATIAL_ANOMALY"] = "SPATIAL_ANOMALY";
...
(function (CorrelationType) {
    ...
    CorrelationType["SEQUENTIAL"] = "SEQUENTIAL";
 ...
 */
var FalsePositiveAction;
(function (FalsePositiveAction) {
    FalsePositiveAction["SUPPRESS"] = "SUPPRESS";
    FalsePositiveAction["TUNE_RULE"] = "TUNE_RULE";
    FalsePositiveAction["WHITELIST"] = "WHITELIST";
    FalsePositiveAction["REQUIRE_REVIEW"] = "REQUIRE_REVIEW";
    FalsePositiveAction["NO_ACTION"] = "NO_ACTION";
})(FalsePositiveAction || (exports.FalsePositiveAction = FalsePositiveAction = {}));
/**
 * ML model types
 */
var MLModelType;
 ...
 * ```
 */
const getDetectionRuleModelAttributes = () => ({
    id: {
        type: 'UUID',
        defaultValue: 'UUIDV4',
        primaryKey: true,
...
exports.getDetectionRuleModelAttributes = getDetectionRuleModelAttributes;
/**
 ...
 * ```
 */
const getThreatDetectionModelAttributes = () => ({
    id: {
        type: 'UUID',
        defaultValue: 'UUIDV4',
        primaryKey: true,
...
exports.getThreatDetectionModelAttributes = getThreatDetectionModelAttributes;
/**
 ...
 * ```
 */
const calculateThreatScore = async (event, rules, options) => {
    ...
    const matchedRules = rules.filter(rule => (0, exports.evaluateRule)(rule, event));
    ...
    // Weighted sum
    const threatScore = (severityScore * weights.severity) +
 ...
 */
const calculateConfidenceScore = (event, matchedRules) => {
    if (matchedRules.length === 0)
        return 0;
 ...
 */
const calculateAnomalyScore = (event, baselineData) => {
 ...
 */
const calculateCorrelationScore = (event) => {
 ...
 * ```
 */
const performRealtimeThreatAssessment = async (events, config) => {
    const results = [];
    for (const event of events) {
        const threatScore = await (0, exports.calculateThreatScore)(event, []);
 ...
 */
const extractAffectedEntities = (event) => {
    const entities = [];
    if (event.userId) {
        entities.push({
 ...
 */
const extractThreatIndicators = (event) => {
    const indicators = [];
    if (event.sourceIp) {
        indicators.push({
            type: 'ip_address',
 ...
 */
const generateRecommendedActions = (score) => {
    const actions = [];
    if (score >= 90) {
 ...
 * ```
 */
const calculateDynamicThreatScore = async (event, historicalEvents) => {
    const baseScore = await (0, exports.calculateThreatScore)(event, []);
    // Adjust based on frequency
    const recentSimilarEvents = historicalEvents.filter(e => e.type === event.type &&
 ...
 * ```
 */
const aggregateThreatScores = (scores, weights) => {
 ...
 * ```
 */
const detectStatisticalAnomalies = (values, options) => {
 ...
 */
const calculateStatisticalBaseline = (values) => {
 ...
 * ```
 */
const detectBehavioralAnomaly = (currentBehavior, historicalBehavior) => {
    let anomalyScore = 0;
    ...
    if (currentBehavior.accessedResources) {
        const unusualAccess = currentBehavior.accessedResources.filter((resource) => {
            return !historicalBehavior.some(b => b.accessedResources?.includes(resource));
        });
 ...
 * ```
 */
const detectTemporalAnomalies = (timeSeries, options) => {
 ...
 * ```
 */
const updateAnomalyBaseline = (baseline, newValues) => {
 ...
 * ```
 */
const matchThreatPatterns = (events, patterns) => {
    const results = [];
    for (const pattern of patterns) {
        ...
        if (matchScore > 50) {
            results.push({
                ...
                matchedEvents: events.filter(e => pattern.sequence?.includes(e.type)),
 ...
 */
const calculatePatternMatchScore = (events, pattern) => {
    ...
    for (const eventType of eventTypes) {
        if (eventType === pattern.sequence[patternIndex]) {
            matchCount++;
            patternIndex++;
            if (patternIndex >= pattern.sequence.length)
                break;
        }
    }
 ...
 */
const calculateTimespan = (events) => {
    if (events.length < 2)
        return 0;
 ...
 * ```
 */
const detectSequentialPatterns = (events, minSupport = 2) => {
 ...
 * ```
 */
const detectAttackChains = (events, options) => {
 ...
 * ```
 */
const correlateSecurityEvents = (events, correlationFields) => {
    const correlations = [];
 ...
 * ```
 */
const correlateEventsByTime = (events, timeWindow = 300000) => {
    const correlations = [];
 ...
 * ```
 */
const detectCausalCorrelations = (events, options) => {
 ...
 * ```
 */
const prepareMLFeatures = (event, featureNames) => {
    const features = {};
 ...
 * ```
 */
const predictThreatWithML = async (event, model) => {
    const features = (0, exports.prepareMLFeatures)(event, model.features);
...
exports.predictThreatWithML = predictThreatWithML;
/**
 ...
 * ```
 */
const ensembleMLPrediction = async (event, models) => {
    const predictions = await Promise.all(models.map(model => (0, exports.predictThreatWithML)(event, model)));
...
exports.ensembleMLPrediction = ensembleMLPrediction;
/**
 ...
 * ```
 */
const normalizeMLFeatures = (features, options) => {
...
exports.normalizeMLFeatures = normalizeMLFeatures;
// ============================================================================
 ...
 * ```
 */
const assessEntityRisk = async (entityId, entityType, events) => {
    const riskFactors = [];
    ...
    const overallRiskScore = riskFactors.reduce((sum, factor) => sum + (factor.score * factor.weight), 0);
...
exports.assessEntityRisk = assessEntityRisk;
/**
 ...
 */
const generateMitigationRecommendations = (riskFactors) => {
 ...
 * ```
 */
const calculateRiskTrend = (entityId, historicalAssessments) => {
    if (historicalAssessments.length < 2) {
        return { direction: 'stable', change: 0, velocity: 0 };
    }
    const sortedAssessments = [...historicalAssessments].sort((a, b) => a.assessmentTimestamp.getTime() - b.assessmentTimestamp.getTime());
 ...
 * ```
 */
const aggregateRiskScores = (assessments) => {
 ...
 * ```
 */
const analyzeFalsePositive = async (detection, options) => {
    const reasons = [];
    let fpScore = 0;
    ...
    const historicalAccuracy = calculateHistoricalAccuracy(detection.triggeredRules, options?.historicalData || []);
...
exports.analyzeFalsePositive = analyzeFalsePositive;
/**
 ...
 */
const calculateHistoricalAccuracy = (rules, historicalData) => {
    if (historicalData.length === 0)
        return 0.7; // Default assumption
    const ruleIds = rules.map(r => r.id);
    const relevantDetections = historicalData.filter(d => d.triggeredRuleIds?.some((id) => ruleIds.includes(id)));
 ...
 */
const matchesKnownFPPattern = (detection) => {
 ...
 */
const determineRecommendedAction = (fpScore, detection) => {
    if (fpScore >= 80) {
 ...
 * ```
 */
const tuneDetectionRule = (rule, fpAnalyses) => {
 ...
 * ```
 */
const createWhitelistEntry = (detection, reason) => {
    return {
        id: crypto.randomUUID(),
...
exports.createWhitelistEntry = createWhitelistEntry;
// ============================================================================
 ...
 * ```
 */
const validateDetectionRule = (rule) => {
    const errors = [];
    const warnings = [];
    // Required fields
 ...
 * ```
 */
const testDetectionRule = (rule, sampleEvents) => {
    const matchedEvents = sampleEvents.filter(event => (0, exports.evaluateRule)(rule, event));
    return {
...
exports.testDetectionRule = testDetectionRule;
/**
 ...
 * ```
 */
const optimizeDetectionRule = (rule) => {
...
exports.optimizeDetectionRule = optimizeDetectionRule;
/**
 ...
 */
const estimateConditionSelectivity = (condition) => {
 ...
 * ```
 */
const versionDetectionRule = (rule, changeType) => {
...
exports.versionDetectionRule = versionDetectionRule;
/**
 ...
 * ```
 */
const exportDetectionRules = (rules, format) => {
    if (format === 'json') {
...
exports.exportDetectionRules = exportDetectionRules;
/**
 ...
 * ```
 */
const importDetectionRules = (data, format) => {
    if (format === 'json') {
...
exports.importDetectionRules = importDetectionRules;
/**
 ...
 * ```
 */
const mergeDetectionRules = (ruleSets, options) => {
...
exports.mergeDetectionRules = mergeDetectionRules;
/**
 ...
 * ```
 */
const generateDetectionCoverageReport = (rules, options) => {
    const report = {
...
exports.generateDetectionCoverageReport = generateDetectionCoverageReport;
/**
 ...
 * ```
 */
const calculateDetectionEffectiveness = (detections, validatedThreats) => {
...
exports.calculateDetectionEffectiveness = calculateDetectionEffectiveness;
/**
 ...
 * ```
 */
const generateDetectionSummary = (detections, timeRange) => {
    let filtered = detections;
    if (timeRange) {
        filtered = detections.filter(d => d.timestamp >= timeRange.start && d.timestamp <= timeRange.end);
    }
    const summary = {
...
exports.generateDetectionSummary = generateDetectionSummary;
/**
 ...
 * ```
 */
const performThreatHunting = (events, hypothesis) => {
    const results = [];
    ...
    for (const event of sortedEvents) {
        ...
        if (eventTime - windowStart <= timeWindow) {
            ...
            const matchingEvents = currentWindow.filter(e => hypothesis.indicators.some(indicator => JSON.stringify(e).toLowerCase().includes(indicator.toLowerCase())));
...
exports.performThreatHunting = performThreatHunting;
/**
 ...
 * ```
 */
const trainDetectionModel = (trainingData, options) => {
...
exports.trainDetectionModel = trainDetectionModel;
/**
 ...
 * ```
 */
const updateDetectionModel = (model, newData) => {
...
exports.updateDetectionModel = updateDetectionModel;
/**
 ...
 * ```
 */
const evaluateDetectionModel = (model, testData) => {
...
exports.evaluateDetectionModel = evaluateDetectionModel;
/**
 ...
 * ```
 */
const predictThreatTrends = (historicalDetections, forecastDays = 30) => {
    // Simple trend analysis and forecasting
    const sortedDetections = [...historicalDetections].sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime());
    // Group by day
...
//# sourceMappingURL=threat-detection-engine-kit.js.map