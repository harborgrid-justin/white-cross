...
export class StatisticsService {
  ...
  async getComplianceStatistics(query: QueryStatisticsDto) {
    ...
    return {
      ...
      reports: {
        ...
        nonCompliant: reports.data.filter(r => r.status === 'NON_COMPLIANT').length,
        pending: reports.data.filter(r => r.status === 'PENDING').length,
      },
      checklists: {
        total: checklists.total,
        completed: checklists.data.filter(c => c.status === 'COMPLETED').length,
      ...
      violations: {
        ...
        critical: violations.data.filter(v => v.severity === 'CRITICAL').length,
  ...
  async getHipaaStatus() {
    ...
    return {
      ...
      openViolations: violations.data.filter(v => v.status !== 'RESOLVED').length,
