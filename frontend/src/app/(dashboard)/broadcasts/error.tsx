...
export default function Error({ error, reset }: ErrorPageProps) {
  useEffect(() => {
    console.error('Broadcasts page error:', error);
  }, [error]);
  ...
  const getErrorMessage = (error: Error): string => {
    ...
    if (error.message.includes('404')) {
      return 'The requested broadcast data could not be found.';
    }
    if (error.message.includes('403')) {
      return 'You do not have permission to view this broadcast data.';
    }
    if (error.message.includes('500')) {
      return 'A server error occurred. Please try again later.';
    }
    return 'An unexpected error occurred while loading broadcast data.';
  };
