...
export default function Error({ error, reset }: ErrorPageProps) {
  ...
  const getErrorMessage = (error: Error): string => {
    ...
    if (error.message.includes('403') || error.message.includes('unauthorized')) {
      return 'You do not have permission to create new appointments.';
    }
    if (error.message.includes('conflict')) {
      return 'There is a scheduling conflict. The selected time slot may already be booked.';
    }
    if (error.message.includes('500')) {
      return 'A server error occurred. Please try again later.';
    }
    return 'An unexpected error occurred while loading the new appointment form.';
  };
        ...
        <Card className="w-full max-w-2xl border-destructive">
          <CardHeader className="text-center space-y-4">
            ...
            <CardTitle className="text-2xl text-destructive">
              Unable to Load New Appointment Form
            </CardTitle>
          </CardHeader>
