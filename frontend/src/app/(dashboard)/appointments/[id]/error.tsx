...
export default function Error({ error, reset }: ErrorPageProps) {
  ...
  useEffect(() => {
    console.error('Appointment detail page error:', error);
  }, [error]);
  ...
  const getErrorMessage = (error: Error): string => {
    ...
    if (error.message.includes('404') || error.message.includes('not found')) {
      return 'The requested appointment could not be found. It may have been cancelled or deleted.';
    }
    if (error.message.includes('403') || error.message.includes('unauthorized')) {
      return 'You do not have permission to view this appointment.';
    }
    if (error.message.includes('500')) {
      return 'A server error occurred while loading appointment details. Please try again later.';
    }
    return 'An unexpected error occurred while loading the appointment.';
  };
