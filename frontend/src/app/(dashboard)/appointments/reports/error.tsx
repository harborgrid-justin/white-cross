...
export default function Error({ error, reset }: ErrorPageProps) {
  useEffect(() => {
    console.error('Appointment reports page error:', error);
  }, [error]);
  ...
  const getErrorMessage = (error: Error): string => {
    ...
    if (error.message.includes('403') || error.message.includes('unauthorized')) {
      return 'You do not have permission to view appointment reports.';
    }
    if (error.message.includes('timeout')) {
      return 'Report generation is taking longer than expected. Please try with a narrower date range.';
    }
    if (error.message.includes('500')) {
      return 'A server error occurred while generating the report. Please try again later.';
    }
    return 'An unexpected error occurred while loading appointment reports.';
  };
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="text-sm text-blue-800 dark:text-blue-200">
                  ...
                  <ul className="list-disc list-inside space-y-1 mt-2">
                    ...
                    <li>Use filters to focus on specific appointment types</li>
                  </ul>
                </div>
              </div>
            </div>
