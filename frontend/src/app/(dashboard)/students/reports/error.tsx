...
export default function Error({ error, reset }: ErrorPageProps) {
  useEffect(() => {
    ...
    console.error('Student reports page error:', error);
  }, [error]);
  ...
  const getErrorMessage = (error: Error): string => {
    ...
    if (error.message.includes('403') || error.message.includes('unauthorized')) {
      return 'You do not have permission to view student reports.';
    }
    if (error.message.includes('timeout')) {
      return 'The report generation is taking longer than expected. Please try again.';
    }
    if (error.message.includes('500')) {
      return 'A server error occurred while generating the report. Please try again later.';
    }
    return 'An unexpected error occurred while loading student reports.';
  };
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="text-sm text-blue-800 dark:text-blue-200">
                  ...
                  <ul className="list-disc list-inside space-y-1 mt-2">
                    ...
                    <li>Try narrowing your date range or filters</li>
                    <li>Check that you have permission to access all selected data</li>
                  </ul>
                </div>
              </div>
            </div>
