...
export default function Error({ error, reset }: ErrorPageProps) {
  useEffect(() => {
    console.error('Incident reports page error:', error);
  }, [error]);
  ...
  const getErrorMessage = (error: Error): string => {
    ...
    if (error.message.includes('403') || error.message.includes('unauthorized')) {
      return 'You do not have permission to view incident reports.';
    }
    if (error.message.includes('timeout')) {
      return 'Report generation is taking longer than expected. Try narrowing your filters.';
    }
    if (error.message.includes('500')) {
      return 'A server error occurred while generating the report. Please try again later.';
    }
    return 'An unexpected error occurred while loading incident reports.';
  };
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="text-sm text-blue-800 dark:text-blue-200">
                  ...
                  <ul className="list-disc list-inside space-y-1 mt-2">
                    ...
                    <li>Try narrowing your date range for faster results</li>
                    <li>Use filters to focus on specific incident types</li>
                    <li>Check the analytics page for visual insights</li>
                  </ul>
                </div>
              </div>
            </div>
