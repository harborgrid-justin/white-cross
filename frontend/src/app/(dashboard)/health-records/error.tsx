...
export default function Error({ error, reset }: ErrorPageProps) {
  useEffect(() => {
    ...
    console.error('Health Records page error:', error);
  }, [error]);
  ...
  const getErrorMessage = (error: Error): string => {
    ...
    if (error.message.includes('404')) {
      return 'The requested health record data could not be found.';
    }
    if (error.message.includes('403')) {
      return 'You do not have permission to view this health record data.';
    }
    if (error.message.includes('500')) {
      return 'A server error occurred. Please try again later.';
    }
    return 'An unexpected error occurred while loading health record data.';
  };
