...
export class BroadcastsApi {
  ...
  async getAll(filters?: BroadcastFilters): Promise<PaginatedResponse<Broadcast>> {
    ...
    const response = await apiInstance.get<PaginatedResponse<Broadcast>>(
  ...
  async getById(broadcastId: string): Promise<Broadcast> {
    const response = await apiInstance.get<ApiResponse<Broadcast>>(
  ...
  async create(broadcastData: CreateBroadcastRequest): Promise<Broadcast> {
    const response = await apiInstance.post<ApiResponse<Broadcast>>(
      '/api/v1/communications/broadcasts',
      broadcastData
    );
    return response.data.data!;
  ...
  async update(broadcastId: string, broadcastData: UpdateBroadcastRequest): Promise<Broadcast> {
    const response = await apiInstance.put<ApiResponse<Broadcast>>(
      `/api/v1/communications/broadcasts/${broadcastId}`,
      broadcastData
    );
    return response.data.data!;
  ...
  async getDeliveryDetails(broadcastId: string, params?: {
    ...
    const response = await apiInstance.get<PaginatedResponse<BroadcastDelivery>>(
  ...
  async getStatistics(params?: {
    ...
    const response = await apiInstance.get<ApiResponse<BroadcastStatistics>>(
...
export const broadcastsApi = new BroadcastsApi();
