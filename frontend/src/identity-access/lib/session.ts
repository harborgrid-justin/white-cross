 ...
 * @deprecated Use centralized cookie config from ./config/cookies.ts
 *
 * This configuration is maintained for backward compatibility only.
 * New code should import from ./config/cookies.ts for consistent,
 * secure cookie settings with __Host- prefix.
 */
const SESSION_CONFIG = {
  tokenName: COOKIE_NAMES.ACCESS_TOKEN,
  refreshName: COOKIE_NAMES.REFRESH_TOKEN,
...
export async function getServerSession(): Promise<Session | null> {
  try {
    ...
    // Use centralized cookie getter with legacy fallback support
    let token = getAccessTokenFromCookies(cookieStore);
 ...
 * @deprecated Import from ./config/roles instead
 *
 * This is maintained for backward compatibility only.
 * New code should use the centralized role configuration from ./config/roles.
 */
import { ROLE_HIERARCHY as CENTRALIZED_ROLE_HIERARCHY, hasMinimumRole as hasMinimumRoleLevel } from './config/roles';
...
export async function requireMinimumRole(minimumRole: string): Promise<SessionUser> {
  ...
  // Use centralized role hierarchy for consistent role checking
...
export async function createSession(
  ...
  // Use centralized cookie configuration with option overrides
  cookieStore.set(
    COOKIE_NAMES.ACCESS_TOKEN,
    token,
    getAccessTokenCookieOptions(options)
  );
}
...
export async function createSessionWithRefresh(
  ...
  // Use centralized cookie configuration for both tokens
  cookieStore.set(
    COOKIE_NAMES.ACCESS_TOKEN,
    accessToken,
    getAccessTokenCookieOptions(options)
  ...
  cookieStore.set(
    ...
    refreshToken,
    getRefreshTokenCookieOptions(options)
  );
}
...
export async function destroySession(): Promise<void> {
  ...
  // Use centralized clear function that handles both new and legacy cookies
...
export async function hasSession(): Promise<boolean> {
  const cookieStore = await cookies();
  // Check new secure cookie name or legacy names
  return !!getAccessTokenFromCookies(cookieStore);
}
