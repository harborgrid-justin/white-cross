...
import { IncidentSeverity, IncidentStatus } from '../../types/incidents';
...
function FilteredIncidentReports() {
  ...
  return (
    <div>
      ...
      {sortedReports.map((report: IncidentReport) => (
...
function CreateIncidentReportForm() {
  ...
  return (
    <form onSubmit={(e: React.FormEvent) => { e.preventDefault(); /* handle submit */ }}>
      {/* Form fields */}
    ...
    <div>
      ...
      <div>
        ...
        {isLoadingWitnesses ? (
          ...
          witnesses.map((witness: WitnessStatement) => (
            <div key={witness.id}>
      ...
      <div>
        ...
        {isLoadingActions ? (
          ...
          actions.map((action: FollowUpAction) => (
            <div key={action.id}>
...
function QuickIncidentUpdate({ incidentId }: { incidentId: string }) {
  ...
  const handleQuickStatusUpdate = async (newStatus: IncidentStatus) => {
    ...
    // Note: status is UI-specific, use appropriate backend fields in production
    const result = await dispatch(updateIncidentReport({
      id: incidentId,
      data: { followUpRequired: newStatus !== IncidentStatus.CLOSED }
    }));
...
function IncidentSearch() {
  ...
  return (
    <div>
      ...
      <div>
        {searchResults.map((report: IncidentReport) => (
          <div key={report.id}>
            <h3>{report.type}</h3>
            <p>{report.description}</p>
          </div>
...
function ErrorHandlingExample() {
  ...
  const handleClearError = (errorKey: string) => {
    dispatch(clearError(errorKey as any));
  };
 ...
 *    const isLoading = useAppSelector(selectIsLoading('list'));
 *    if (isLoading) return null; // or show loading component
 *
