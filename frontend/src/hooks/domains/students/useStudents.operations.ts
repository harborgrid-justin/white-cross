/**
 * Student Composite Hook - Core Operations
 *
 * Main hook implementation with all operations
 *
 * @module hooks/domains/students/useStudents.operations
...
import type { StudentOperations, UseStudentsOptions } from './useStudents.types';
import { useCacheManager } from '@/hooks/core/useCacheManager';
import { useHealthcareCompliance } from '@/hooks/compliance/useHealthcareCompliance';
import { useStudentDetails } from './queries/useStudentDetails';
...
export function useStudents(options: UseStudentsOptions = {}): StudentOperations {
  ...
  const createStudent = useCallback(async (data: CreateStudentData): Promise<Student> => {
    ...
    return student;
  }, [mutations.createStudent, prefetchRelated, prefetchStudent]);
