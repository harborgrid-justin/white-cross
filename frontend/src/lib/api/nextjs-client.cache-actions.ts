/**
 * @fileoverview Server Actions for Cache Invalidation
 * @module lib/api/nextjs-client.cache-actions
 * @category API Client
 *
 * Server actions for cache invalidation that can be called from client components
...
function safeRevalidateTag(tag: string): void {
  try {
    ...
    try {
      ...
      (revalidateTag as (tag: string, type?: string) => void)(tag, 'page');
    } catch (fallbackError) {
