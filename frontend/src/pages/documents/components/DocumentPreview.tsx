/**
 * DocumentPreview Component
 * 
 * Document Preview for documents module.
 */

import React from 'react';
import { useAppSelector } from '../../../hooks/shared/store-hooks-index';

interface DocumentPreviewProps {
  className?: string;
}

/**
 * DocumentPreview component - Document Preview
 */
const DocumentPreview: React.FC<DocumentPreviewProps> = ({ className = '' }) => {
  return (
    <div className={`document-preview ${className}`}>
      <div className="card p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Document Preview</h3>
        <div className="text-center text-gray-500 py-8">
          <p>Document Preview functionality</p>
          <p className="text-sm mt-2">Connected to documents Redux slice</p>
        </div>
      </div>
    </div>
  );
};

export default DocumentPreview;
