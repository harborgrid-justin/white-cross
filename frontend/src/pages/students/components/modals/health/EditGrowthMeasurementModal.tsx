/**
 * Edit Growth Measurement Modal Component
 * Modal for editing existing growth measurements with auto-calculated BMI
 */
...
import { useUpdateGrowthMeasurement } from '@/hooks/domains/health-records/useHealthRecords';
import type { GrowthMeasurement } from '@/types/healthRecords';

interface EditGrowthMeasurementModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const EditGrowthMeasurementModal: React.FC<EditGrowthMeasurementModalProps> = ({
  isOpen,
  onClose,
  measurement,
}) => {
  ...
  const updateMutation = useUpdateGrowthMeasurement();
  ...
  useEffect(() => {
    if (measurement) {
      reset({
        measurementDate: measurement.measurementDate?.split('T')[0] || new Date().toISOString().split('T')[0],
        measuredBy: measurement.measuredBy,
        height: measurement.height || undefined,
        heightUnit: (measurement.heightUnit as 'cm' | 'in') || 'cm',
  ...
  return (
    <Modal open={isOpen} onClose={onClose} size="lg">
      <form onSubmit={handleSubmit(onSubmit)}>
        <ModalHeader>
          <ModalTitle>Edit Growth Measurement</ModalTitle>
        </ModalHeader>
        ...
        <ModalFooter>
          ...
          <Button
            ...
            disabled={updateMutation.isPending}
          >
            {updateMutation.isPending ? 'Updating...' : 'Update Measurement'}
          </Button>
        </ModalFooter>
      </form>
    </Modal>
