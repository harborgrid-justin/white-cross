...
import type { BillingDetailProps, BillingDetailTab } from './types';
...
const BillingDetail: React.FC<BillingDetailProps> = ({
  ...
  // Tab configuration
  const tabs: Array<{ id: BillingDetailTab; label: string; icon: typeof Eye }> = [
  ...
  return (
    <div className={`bg-white rounded-lg shadow ${className}`}>
      ...
      <div className="p-6">
        ...
        {activeTab === 'history' && <InvoiceHistory invoice={invoice} />}

        {activeTab === 'documents' && <InvoiceDocuments invoice={invoice} />}
      </div>
