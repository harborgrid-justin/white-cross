...
async function main() {
    ...
    const testNursePassword = await bcryptjs_1.default.hash('NursePassword123!', 10);
    const testAdminPassword = await bcryptjs_1.default.hash('AdminPassword123!', 10);
    ...
    const testReadOnlyPassword = await bcryptjs_1.default.hash('ReadOnlyPassword123!', 10);
    const testCounselorPassword = await bcryptjs_1.default.hash('CounselorPassword123!', 10);
    ...
    await Promise.all(medications.map((medication) => prisma.medicationInventory.create({
        data: {
    ...
    await Promise.all(daysOfWeek.map((day) => prisma.nurseAvailability.create({
        data: {
            nurseId: nurseUser.id,
            dayOfWeek: day,
            startTime: '08:00',
    ...
    await Promise.all(configs.map((config) => prisma.systemConfiguration.upsert({
    ...
    const nursePermissions = createdPermissions.filter((p) => !p.resource.includes('administration') && p.action !== 'delete');
    await Promise.all(nursePermissions.map((permission) => prisma.rolePermission.upsert({
    ...
    await Promise.all(readOnlyPermissions.map((permission) => prisma.rolePermission.upsert({
    ...
    const counselorPermissions = createdPermissions.filter((p) => (p.resource === 'students' || p.resource === 'health_records') && p.action !== 'delete');
    await Promise.all(counselorPermissions.map((permission) => prisma.rolePermission.upsert({
