...
**CRITICAL REQUIREMENT**: Every security control addition/change (guard, header, rate limit) triggers batch updates. Reference `_standard-orchestration.md`.
Security posture changes require strict tracking and cross-agent consistency.

### Files
- `task-status-{id}.json` – security hardening streams (auth, RBAC, audit, input validation)
- `plan-{id}.md` – phased security improvement plan
- `checklist-{id}.md` – tasks (add guard, rotate secrets, enforce CSP, implement rate limiter)
- `progress-{id}.md` – vulnerability remediation status
- `architecture-notes-{id}.md` – threat model, security control matrix

### Sync Triggers
New vulnerability logged/resolved; guard or middleware added; policy change (headers / rate limits); blocker (library limitation); phase shift.

### Completion
All high/critical findings remediated or justified; summary archived.
