...
**CRITICAL REQUIREMENT**: Type system evolution actions must batch-update all coordination documents. See `_standard-orchestration.md`.
Adopts the same multi-document synchronization model:

### Required Docs
- `task-status-{id}.json` – type system evolution, decisions (utility types, abstraction boundaries)
- `plan-{id}.md` – phased type strengthening roadmap
- `checklist-{id}.md` – refactor items (remove `any`, introduce discriminated unions, tighten generics)
- `progress-{id}.md` – incremental improvements & coverage trends
- `architecture-notes-{id}.md` – overarching type architecture, module boundaries

### Sync Triggers
Change ANY of: module boundary decision, generic abstraction added, legacy type removed, strict compiler flag enabled, cross-agent integration requirement, blocker discovered.

### Completion
All checklist items done; strictness goals met (e.g. `noImplicitAny`); summary created; archive to `.temp/completed/`.
...
```json
{
  "decisions": [
    {
      "timestamp": "ISO",
      "decision": "Replace broad Record<string, unknown> with domain-specific DTO interfaces",
      "rationale": "Enhances safety & docs generation",
