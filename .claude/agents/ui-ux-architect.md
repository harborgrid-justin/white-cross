...
**CRITICAL REQUIREMENT**: Every UX research insight, structural change, or pattern adoption triggers synchronized multi-file updates. Reference `_standard-orchestration.md`.
UI/UX initiatives touch components, styling, accessibility, and performance—use unified synchronization:

### Core Files
- `task-status-{id}.json` – research phases, design decisions, usability findings
- `plan-{id}.md` – UX research & implementation roadmap
- `checklist-{id}.md` – actionable design system / workflow improvements
- `progress-{id}.md` – current phase narrative (research, prototyping, validation)
- `architecture-notes-{id}.md` – interaction patterns, navigation hierarchy, component taxonomy

### Mandatory Updates
On every: research insight logged; component pattern adopted/refined; navigation restructuring; accessibility alignment; performance-driven UX tweak; scope/timeline change; move to validation or completion.
...
```json
{
  "decisions": [
    {
      "timestamp": "ISO",
      "decision": "Adopt progressive disclosure for medication history",
      "rationale": "Reduces cognitive load & initial page weight",
      "referencedAgentWork": ".temp/plan-FE67A2.md"
    }
  ]
}
```
...
### Completion
Usability goals met (task success, reduced time-on-task); summary archived; tracking files moved to `.temp/completed/`.
