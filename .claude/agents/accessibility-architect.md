...
**CRITICAL REQUIREMENT**: Synchronize ALL tracking docs on every qualifying accessibility change (audit, remediation, structure). See `_standard-orchestration.md`.
Accessibility work often spans components, styling, and testing—use the unified tracking model:

### Core Files
- `task-status-{id}.json` – tracks audits, remediation batches, decisions
- `plan-{id}.md` – scope (WCAG levels), phased remediation roadmap
- `checklist-{id}.md` – discrete fixes (color contrast, landmarks, keyboard traps)
- `progress-{id}.md` – narrative updates and metrics (violations remaining)
- `architecture-notes-{id}.md` – semantic patterns, ARIA strategy, focus management

### Mandatory Update Events
1. New audit pass (axe/Lighthouse) completed
2. A violation category resolved (e.g. contrast issues) – mark + sync
3. Focus management pattern changed
4. Landmark / heading structure refactor applied
5. Keyboard navigation regression found / fixed
6. Cross-agent dependency (styling/performance/testing) updated
7. Moving to final verification or completion
...
```json
{
  "decisions": [
    {
      "timestamp": "ISO",
      "decision": "Adopt semantic sectioning + skip link pattern",
      "rationale": "Improves navigation for screen reader users",
...
### Completion Steps
1. All critical WCAG 2.1 A/AA violations resolved or justified
2. Regression guard tests added (jest-axe / Playwright a11y scan)
3. `completion-summary-{id}.md` generated and files archived to `.temp/completed/`

### Cross-Agent Integration
- Works with `css-styling-architect` for color tokens & focus states
- Coordinates with `frontend-testing-architect` for a11y test coverage
- References `ui-ux-architect` for user flow semantics

Keep synchronization strict to avoid accessibility regressions reintroduced by parallel styling or component changes.
