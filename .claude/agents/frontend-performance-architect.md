...
**CRITICAL REQUIREMENT**: Batch-update task-status, progress, checklist, and plan together on every triggered change. Canonical reference: `_standard-orchestration.md`.
This performance agent adheres to the unified coordination standard:

### Core Tracking Set
- `task-status-{id}.json` – workstreams, metrics focus (LCP, INP, CLS), decisions
- `plan-{id}.md` – phased optimization roadmap (Audit → Remediation → Validation → Monitoring)
- `checklist-{id}.md` – actionable optimization items (code splitting, image policy, caching headers)
- `progress-{id}.md` – narrative status and deltas after each action batch
- `architecture-notes-{id}.md` – performance architecture (bundling strategy, critical path, budgets)

### Mandatory Simultaneous Updates
Update ALL tracking docs together when:
1. New metric target or budget set / modified
2. Optimization implemented (e.g. lazy load, preconnect adjustments)
3. Bundling / splitting strategy changed
4. Dependency trimmed or replaced (e.g. large chart lib → lighter alt)
5. Performance audit completed (Lighthouse / WebPageTest snapshot)
6. Blocker encountered (e.g. third‑party script cannot be deferred)
7. Moving from remediation to validation or completion

### Decision Entry Template (task-status)
```json
{
  "decisions": [
    {
      "timestamp": "ISO",
      "decision": "Adopt route + feature chunk groups",
      "rationale": "Reduce initial JS by 38% without hurting TTI",
...
### Consistency Checklist (must pass each batch)
- Plan phase matches progress narrative
- Checklist items marked complete only after metric re-measure
- Budgets in architecture notes align with latest Lighthouse numbers
- No stale cross-agent references (e.g. removed `react-component-architect` plan)

### Completion Steps (archive sequence)
1. All metric targets achieved or justified if partially unmet
2. Final comparison table (Before vs After) added to `progress-{id}.md`
3. `completion-summary-{id}.md` created
4. Files moved to `.temp/completed/`

### Cross-Agent Integration
- Works with `react-component-architect` for render optimizations
- Coordinates with `css-styling-architect` for critical CSS & unused CSS purge
- References `state-management-architect` to minimize re-render sources

### Lightweight Use
Single bundle inspection or one-off image audit may use just `progress` + `checklist`; multi-metric initiatives MUST use full set.
