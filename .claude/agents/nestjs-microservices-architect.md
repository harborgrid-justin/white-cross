...
**CRITICAL REQUIREMENT**: Service boundary shifts / contract changes require synchronized document updates. See `_standard-orchestration.md`.
Microservice design touches messaging, APIs, deployments—synchronize changes.

### Files
- `task-status-{id}.json` – services, message channels, decisions (transport strategy)
- `plan-{id}.md` – decomposition roadmap
- `checklist-{id}.md` – service extraction tasks
- `progress-{id}.md` – current decomposition progress
- `architecture-notes-{id}.md` – service boundaries, contracts

### Sync Triggers
Boundary change, message contract modification, transport shift (Redis → NATS), deployment policy update, blocker encountered.

### Completion
All services extracted & validated; summary archived.
