...
**CRITICAL REQUIREMENT**: Schema & migration changes require simultaneous updates of all tracking docs. Reference `_standard-orchestration.md`.
Ensure schema evolution, migration sequencing, and performance changes stay aligned:

### Tracking Set
- `task-status-{id}.json` – workstreams (schema design, indexing, migration, optimization)
- `plan-{id}.md` – phased approach (Modeling → Migrations → Optimization → Validation)
- `checklist-{id}.md` – actionable migration/index creation tasks
- `progress-{id}.md` – current phase, completed DDL, pending performance tests
- `architecture-notes-{id}.md` – ER diagrams, normalization decisions, partitioning strategy

### Mandatory Simultaneous Updates
Triggered by: new migration added; indexing strategy altered; performance benchmark completed; rollback decision; cross-agent dependency (ORM/model layer) updated; blocker (locking, deadlock) found; transition to validation/completion.
...
```json
{
  "decisions": [
    {
      "timestamp": "ISO",
      "decision": "Introduce composite index (student_id, administered_at)",
      "rationale": "Accelerates medication audit queries by ~45%",
      "referencedAgentWork": ".temp/plan-DE45F6.md"
    }
  ]
}
```
...
### Completion
1. All migrations applied & tested in staging
2. Performance baselines documented pre/post optimization
3. Archival: create `completion-summary-{id}.md` then move tracking files to `.temp/completed/`

### Cross-Agent Integration
- References Sequelize model decisions
- Coordinates with API architect for query surface impacts
- Works with performance agent for slow query profiling
