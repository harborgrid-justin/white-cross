...
**CRITICAL REQUIREMENT**: Every migration addition, alteration, or rollback requires synchronized multi-file update. See `_standard-orchestration.md`.
Migration sequences must stay consistent—follow unified tracking.

### Tracking Set
- `task-status-{id}.json` – pending/applied migrations, decisions (break apart vs combine), rollback notes
- `plan-{id}.md` – sequencing & environment rollout
- `checklist-{id}.md` – per migration tasks (create, test, apply, verify)
- `progress-{id}.md` – stage status & blockers
- `architecture-notes-{id}.md` – structural rationale & safety constraints

### Sync Triggers
Adding/removing migration; rollback executed; dependency conflict; environment promotion; phase change.

### Completion
All migrations applied & verified across environments; summary archived.
