/**
 * Edit Chronic Condition Modal Component
 * Modal for editing existing chronic condition information
 */
...
import { useUpdateCondition } from '@/hooks/domains/health-records/useHealthRecords';
import { SEVERITY_LEVELS, CONDITION_STATUS_OPTIONS } from '@/constants/healthRecords';
import type { ChronicCondition, ConditionStatus, ConditionSeverity } from '@/types/healthRecords';
  ...
  useEffect(() => {
    if (condition) {
      reset({
        ...
        diagnosisDate: condition.diagnosisDate?.split('T')[0] || new Date().toISOString().split('T')[0],
        severity: condition.severity,
        status: condition.status,
        treatments: condition.treatments || '',
        accommodationsRequired: condition.accommodationsRequired || false,
        accommodationDetails: condition.accommodationDetails || '',
        emergencyProtocol: condition.emergencyProtocol || '',
        actionPlan: condition.actionPlan || '',
        triggers: Array.isArray(condition.triggers) ? condition.triggers.join(', ') : '',
  ...
  return (
    <Modal open={isOpen} onClose={onClose} size="xl">
      <form onSubmit={handleSubmit(onSubmit)}>
        <ModalHeader>
          <ModalTitle>Edit Chronic Condition</ModalTitle>
        </ModalHeader>
        ...
        <ModalFooter>
          ...
          <Button
            ...
            disabled={updateMutation.isPending}
          >
            {updateMutation.isPending ? 'Updating...' : 'Update Condition'}
          </Button>
        </ModalFooter>
      </form>
    </Modal>
