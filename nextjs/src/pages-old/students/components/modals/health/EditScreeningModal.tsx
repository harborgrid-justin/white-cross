 ...
 * Modal for editing existing health screening records
 */
...
import { useUpdateScreening } from '@/hooks/domains/health-records/useHealthRecords';
import type { Screening, ScreeningType, ScreeningOutcome } from '@/types/healthRecords';
...
export const EditScreeningModal: React.FC<EditScreeningModalProps> = ({
  isOpen,
  onClose,
  screening,
}) => {
  ...
  useEffect(() => {
    if (screening) {
      reset({
        ...
        screeningDate: screening.screeningDate?.split('T')[0] || new Date().toISOString().split('T')[0],
        screenedBy: screening.screenedBy,
        outcome: screening.outcome,
        referralRequired: screening.referralRequired || false,
        referralTo: screening.referralTo || '',
        rightEye: screening.rightEye || '',
        leftEye: screening.leftEye || '',
        rightEar: screening.rightEar || '',
        leftEar: screening.leftEar || '',
  ...
  return (
    <Modal open={isOpen} onClose={onClose} size="lg">
      <form onSubmit={handleSubmit(onSubmit)}>
        <ModalHeader>
          <ModalTitle>Edit Health Screening</ModalTitle>
        </ModalHeader>
        ...
        <ModalFooter>
          ...
          <Button
            ...
            disabled={updateMutation.isPending}
          >
            {updateMutation.isPending ? 'Updating...' : 'Update Screening'}
          </Button>
        </ModalFooter>
      </form>
    </Modal>
