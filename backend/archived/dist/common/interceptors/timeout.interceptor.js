"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.TimeoutInterceptor = void 0;
const common_1 = require("@nestjs/common");
const rxjs_1 = require("rxjs");
const operators_1 = require("rxjs/operators");
const base_interceptor_1 = require("./base.interceptor");
let TimeoutInterceptor = class TimeoutInterceptor extends base_interceptor_1.BaseInterceptor {
    defaultTimeout = parseInt(process.env.REQUEST_TIMEOUT || '30000', 10);
    intercept(context, next) {
        const request = context.switchToHttp().getRequest();
        const { handler, controller } = this.getHandlerInfo(context);
        const customTimeout = this.getTimeout(context);
        const timeoutMs = customTimeout || this.defaultTimeout;
        return next.handle().pipe((0, operators_1.timeout)(timeoutMs), (0, operators_1.catchError)((err) => {
            if (err instanceof rxjs_1.TimeoutError) {
                this.logError(`Request timeout in ${controller}.${handler}`, new Error(`Request exceeded timeout of ${timeoutMs}ms`), {
                    timeoutMs,
                    controller,
                    handler,
                });
                return (0, rxjs_1.throwError)(() => new common_1.RequestTimeoutException(`Request exceeded timeout of ${timeoutMs}ms`));
            }
            return (0, rxjs_1.throwError)(() => err);
        }));
    }
    getTimeout(context) {
        const handler = context.getHandler();
        return Reflect.getMetadata('timeout', handler);
    }
};
exports.TimeoutInterceptor = TimeoutInterceptor;
exports.TimeoutInterceptor = TimeoutInterceptor = __decorate([
    (0, common_1.Injectable)()
], TimeoutInterceptor);
//# sourceMappingURL=timeout.interceptor.js.map