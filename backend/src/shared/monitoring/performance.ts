...
export async function monitorDatabaseQuery<T>(
  ...
  if (result.duration > 1000) {
    console.warn(
      `Slow database query detected: ${queryName} took ${result.duration}ms`,
    );
  }
...
export async function monitorAPIEndpoint<T>(
  ...
  if (result.duration > 2000) {
    console.warn(
      `Slow API endpoint detected: ${endpoint} took ${result.duration}ms`,
    );
  }
...
export function checkMemoryThreshold(threshold: number = 500): boolean {
  ...
  if (heapUsedMB > threshold) {
    console.warn(
      `High memory usage detected: ${Math.round(heapUsedMB)}MB (threshold: ${threshold}MB)`,
    );
    return true;
  }
  ...
  return async (request: any, h: any) => {
    ...
    h.response = function (result: any) {
      ...
      if (logSlowRequests && duration > slowRequestThreshold) {
        ...
        console.warn(
          `Slow request: ${method} ${path} took ${Math.round(duration)}ms`,
        );
