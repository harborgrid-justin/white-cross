...
export class ClinicalProtocolController {
  ...
  @ApiOperation({
    ...
    description: 'Creates a new clinical protocol for standardized healthcare procedures. Protocols define step-by-step medical procedures, treatment guidelines, and emergency response procedures. Requires appropriate clinical privileges.',
  })
  @ApiBody({
    ...
    schema: {
      ...
      properties: {
        name: { type: 'string', example: 'Emergency Cardiac Response Protocol', description: 'Protocol name' },
        category: { type: 'string', example: 'emergency', description: 'Protocol category (emergency, routine, diagnostic, etc.)' },
        description: { type: 'string', example: 'Standard protocol for cardiac emergency response in school settings' },
        steps: {
          type: 'array',
          items: {
            type: 'object',
            properties: {
              ...
              instruction: { type: 'string', example: 'Check patient responsiveness' },
              timeLimit: { type: 'number', example: 30, description: 'Time limit in seconds' },
        ...
        createdBy: { type: 'string', example: 'dr_smith_123', description: 'Creating healthcare provider ID' }
      }
    }
  })
  @ApiResponse({
    ...
    schema: {
      type: 'object',
      properties: {
        id: { type: 'string', example: 'protocol_456', description: 'Unique protocol identifier' },
        name: { type: 'string', example: 'Emergency Cardiac Response Protocol' },
        category: { type: 'string', example: 'emergency' },
        status: { type: 'string', enum: ['draft', 'active', 'archived'], example: 'draft' },
        version: { type: 'number', example: 1, description: 'Protocol version number' },
  ...
  @ApiResponse({
    status: 400,
    description: 'Bad Request - invalid protocol data or missing required fields',
  })
  @ApiResponse({
    status: 401,
  ...
  @ApiResponse({
    status: 403,
    description: 'Forbidden - insufficient clinical privileges to create protocols',
  })
  ...
  @ApiOperation({
    ...
    description: 'Retrieves clinical protocols with filtering and pagination. Supports searching by category, status, creator, and text search across protocol content. Essential for protocol management and quick reference during clinical scenarios.',
  })
  @ApiQuery({
    name: 'category',
    required: false,
        ...
        protocols: {
          ...
          items: {
            ...
            properties: {
              ...
              name: { type: 'string', example: 'Emergency Cardiac Response Protocol' },
              category: { type: 'string', example: 'emergency' },
              description: { type: 'string', example: 'Standard protocol for cardiac emergencies' },
              status: { type: 'string', enum: ['draft', 'active', 'archived'], example: 'active' },
              version: { type: 'number', example: 2 },
              lastUpdated: { type: 'string', format: 'date-time' },
              createdBy: { type: 'string', example: 'dr_smith_123' },
              usageCount: { type: 'number', example: 15, description: 'Number of times protocol has been used' },
              avgExecutionTime: { type: 'number', example: 8.5, description: 'Average execution time in minutes' }
            }
          }
        },
        pagination: {
  ...
  @ApiOperation({
    ...
    description: 'Retrieves all currently active clinical protocols ready for immediate use. Excludes draft and archived protocols. Critical for emergency situations where quick protocol access is essential.',
  })
  @ApiResponse({
    status: 200,
    ...
    schema: {
      ...
      items: {
        ...
        properties: {
          ...
          estimatedTime: { type: 'number', example: 10, description: 'Estimated execution time in minutes' },
          requiredStaff: {
            type: 'array',
            items: { type: 'string', example: 'nurse' }
          },
  ...
  @ApiOperation({
    ...
    description: 'Retrieves detailed information for a specific clinical protocol including all steps, requirements, and execution history. Used for protocol review, execution, and compliance documentation.',
  })
  @ApiParam({
    name: 'id',
    type: 'string',
  ...
  @ApiResponse({
    ...
    schema: {
      ...
      properties: {
        ...
        name: { type: 'string', example: 'Emergency Cardiac Response Protocol' },
        category: { type: 'string', example: 'emergency' },
        description: { type: 'string', example: 'Comprehensive cardiac emergency response procedure' },
        status: { type: 'string', enum: ['draft', 'active', 'archived'], example: 'active' },
        version: { type: 'number', example: 2 },
        steps: {
          type: 'array',
          items: {
            ...
            properties: {
              ...
              timeLimit: { type: 'number', example: 30, description: 'Time limit in seconds' },
              critical: { type: 'boolean', example: true },
              equipment: { type: 'array', items: { type: 'string', example: 'AED' } },
              notes: { type: 'string', example: 'Ensure clear airway before proceeding' }
            }
          }
        },
        metadata: {
        ...
        statistics: {
          ...
          properties: {
            ...
            successRate: { type: 'number', example: 95.2, description: 'Success rate percentage' },
  ...
  @ApiOperation({
    ...
    description: 'Activates a clinical protocol for immediate use. Requires proper authorization and validates protocol completeness. Creates audit trail for compliance tracking.',
  })
  @ApiParam({
    name: 'id',
    type: 'string',
  ...
  @ApiBody({
    ...
    schema: {
      ...
      properties: {
        activatedBy: { type: 'string', example: 'chief_nurse_789', description: 'ID of authorizing staff member' },
        reason: { type: 'string', example: 'Protocol review complete, ready for deployment' },
        effectiveDate: { type: 'string', format: 'date-time', example: '2024-01-15T10:00:00Z' },
        notifications: {
          type: 'array',
  ...
  @ApiResponse({
    ...
    schema: {
      ...
      properties: {
        ...
        notificationsSent: { type: 'number', example: 25, description: 'Number of staff notified' }
      }
    }
  })
  @ApiResponse({
    status: 400,
    description: 'Bad Request - protocol cannot be activated (incomplete, already active, etc.)',
  })
  @ApiResponse({
    status: 401,
    description: 'Unauthorized - authentication required',
  ...
  @ApiOperation({
    ...
    description: 'Deactivates a clinical protocol, removing it from active use. Protocol remains in system for audit purposes but cannot be executed. Creates compliance audit trail.',
  })
  @ApiParam({
    name: 'id',
    type: 'string',
  ...
  @ApiResponse({
    status: 400,
    description: 'Bad Request - protocol cannot be deactivated (already inactive, in use, etc.)',
  })
  @ApiResponse({
    status: 401,
    description: 'Unauthorized - authentication required',
