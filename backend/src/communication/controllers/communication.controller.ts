...
export class CommunicationController {
  ...
  @ApiResponse({ status: 400, description: 'Invalid template data or validation errors' })
  @ApiResponse({ status: 401, description: 'Unauthorized - Authentication required' })
  @ApiResponse({ status: 403, description: 'Forbidden - Insufficient permissions to update templates' })
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized - Authentication required' })
  @ApiResponse({ status: 403, description: 'Forbidden - Insufficient permissions to delete templates' })
  @ApiResponse({ status: 404, description: 'Template not found' })
  @ApiResponse({ status: 409, description: 'Cannot delete template - currently in use by active messages' })
  @ApiResponse({ status: 500, description: 'Internal server error' })
        ...
        recipients: {
          ...
          items: {
            ...
            properties: {
              ...
              recipientType: { type: 'string', enum: ['parent', 'student', 'staff'] },
  ...
  @ApiResponse({ status: 400, description: 'Invalid message data or validation errors' })
  @ApiResponse({ status: 401, description: 'Unauthorized - Authentication required' })
  @ApiResponse({ status: 403, description: 'Forbidden - Insufficient permissions or recipient access denied' })
  @ApiResponse({ status: 422, description: 'Message content violates healthcare communication policies' })
  @ApiResponse({ status: 500, description: 'Internal server error or delivery service failure' })
  ...
  @ApiResponse({ status: 400, description: 'Invalid broadcast data or validation errors' })
  @ApiResponse({ status: 401, description: 'Unauthorized - Authentication required' })
  @ApiResponse({ status: 403, description: 'Forbidden - Admin or nurse role required for broadcasts' })
  @ApiResponse({ status: 422, description: 'Broadcast criteria results in no recipients' })
  @ApiResponse({ status: 500, description: 'Internal server error or delivery service failure' })
  ...
  @ApiResponse({
    ...
    schema: {
      ...
      properties: {
        ...
        alertType: { type: 'string', enum: ['medical_emergency', 'lockdown', 'evacuation', 'weather', 'security'] },
        ...
        deliveryChannels: { type: 'array', items: { type: 'string', enum: ['sms', 'email', 'push', 'voice_call', 'pa_system'] } },
        sentAt: { type: 'string', format: 'date-time' },
  ...
  @ApiResponse({ status: 400, description: 'Invalid alert data or missing critical information' })
  @ApiResponse({ status: 401, description: 'Unauthorized - Authentication required' })
  @ApiResponse({ status: 403, description: 'Forbidden - Admin or authorized emergency personnel only' })
  @ApiResponse({ status: 500, description: 'CRITICAL ERROR - Emergency alert delivery failed' })
  ...
  @ApiQuery({ name: 'type', required: false, enum: ['email', 'sms', 'push', 'in_app', 'emergency'], description: 'Filter by message type' })
  @ApiQuery({ name: 'status', required: false, enum: ['queued', 'sent', 'delivered', 'failed', 'cancelled'], description: 'Filter by delivery status' })
  @ApiQuery({ name: 'recipientType', required: false, enum: ['parent', 'student', 'staff', 'emergency_contact'], description: 'Filter by recipient type' })
  @ApiQuery({ name: 'dateFrom', required: false, format: 'date', description: 'Filter messages from this date' })
  @ApiQuery({ name: 'dateTo', required: false, format: 'date', description: 'Filter messages up to this date' })
  @ApiQuery({ name: 'search', required: false, description: 'Search in message content and subjects' })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized - Authentication required' })
  @ApiResponse({ status: 403, description: 'Forbidden - Insufficient permissions to view message history' })
  @ApiResponse({ status: 500, description: 'Internal server error' })
