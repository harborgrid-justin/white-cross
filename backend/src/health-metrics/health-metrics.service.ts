...
export class HealthMetricsService extends BaseService {
  async getMetricsOverview(
    ...
    try {
      ...
      bedOccupancy.percentage = Math.round(
  ...
  async getPatientTrends(
    ...
    try {
      ...
      const trends: any[] = [];
      const numPoints =
        granularity === 'hour' ? 24 : granularity === 'day' ? 7 : 4;
  ...
  async recordVitals(data: CreateVitalsDto) {
    try {
  ...
  async getHealthAlerts(
    ...
    try {
      ...
      for (let i = 0; i < numAlerts; i++) {
        const alertSeverity = severities[
  ...
  async updateAlertStatus(alertId: number, updateData: UpdateAlertDto) {
    try {
      ...
      if (vitalRecord.systolic_bp && vitalRecord.diastolic_bp) {
        if (vitalRecord.systolic_bp > 140 || vitalRecord.diastolic_bp > 90) {
          alerts.push({
            type: 'hypertension',
            severity:
              vitalRecord.systolic_bp > 160 || vitalRecord.diastolic_bp > 100
                ? 'critical'
                : 'high',
  ...
  private generateAlertMessage(alertType: string, severity: string): string {
    ...
    return (
      messages[alertType as keyof typeof messages] || 'Health alert detected'
    );
  }
