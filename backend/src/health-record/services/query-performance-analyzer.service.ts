...
import { MetricType, PerformanceMetric } from '../../database/models/performance-metric.model';
...
export class QueryPerformanceAnalyzer implements OnModuleDestroy {
  ...
  constructor(
    ...
    @InjectModel(PerformanceMetric)
    private readonly performanceMetricModel: typeof PerformanceMetric,
  ) {
  ...
  private initializeAnalyzer(): void {
    this.logger.log(
      'Initializing Query Performance Analyzer with database persistence',
    );
  ...
  async recordQuery(
    ...
    try {
      ...
      this.logger.debug(
        `Recorded query performance: ${queryId}, time: ${executionTime}ms, complexity: ${queryMetrics.complexity}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to record query metrics for ${queryId}:`,
        error,
      );
    }
  ...
  async getQueryPerformanceStats(
    ...
    try {
      ...
      const slowQueriesCount = metrics.filter(
        (m) => m.value > this.slowQueryThreshold,
      ).length;
      ...
      const topSlowQueries = await this.getTopSlowQueries(
        10,
        startDate,
        endDate,
      );
  ...
  async storePerformanceAnalysisSnapshot(): Promise<void> {
    try {
      ...
      this.logger.log(
        `Stored performance analysis snapshot: ${stats.totalQueries} queries analyzed`,
      );
    } catch (error) {
      this.logger.error(
        'Failed to store performance analysis snapshot:',
        error,
      );
    }
  }
  ...
  private async recordSlowQuery(queryMetrics: QueryMetrics): Promise<void> {
    ...
    try {
      ...
      if (existing) {
        ...
        existing.maxTime = Math.max(
          existing.maxTime,
          queryMetrics.executionTime,
        );
        existing.minTime = Math.min(
          existing.minTime,
          queryMetrics.executionTime,
        );
      ...
      this.logger.warn(
        `Slow query detected: ${queryMetrics.queryId}, time: ${queryMetrics.executionTime}ms`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to record slow query ${queryMetrics.queryId}:`,
        error,
      );
    }
  }
  ...
  private calculateComplexity(sql: string): QueryComplexity {
    ...
    const hasCTE =
      ...
      lowerSQL.includes('(');
    const hasAggregation = /\b(count|sum|avg|min|max|group by)\b/.test(
      lowerSQL,
    );
