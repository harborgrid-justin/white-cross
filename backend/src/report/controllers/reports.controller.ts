...
export class ReportsController {
  ...
  @ApiQuery({ name: 'startDate', required: false, description: 'Start date for analysis period', example: '2024-01-01' })
  @ApiQuery({ name: 'endDate', required: false, description: 'End date for analysis period', example: '2024-12-31' })
  @ApiQuery({ name: 'schoolId', required: false, description: 'Filter by specific school' })
  @ApiQuery({ name: 'gradeLevel', required: false, description: 'Filter by grade level' })
  @ApiQuery({ name: 'includeGraphs', required: false, description: 'Include trend visualizations', example: true })
  @ApiResponse({ 
    status: 200, 
  ...
  @ApiQuery({ name: 'startDate', required: false, description: 'Start date for usage analysis' })
  @ApiQuery({ name: 'endDate', required: false, description: 'End date for usage analysis' })
  @ApiQuery({ name: 'medicationType', required: false, description: 'Filter by medication type (controlled, OTC, prescription)' })
  @ApiQuery({ name: 'includeAdherence', required: false, description: 'Include adherence calculations', example: true })
  @ApiQuery({ name: 'groupBy', required: false, description: 'Group results by (medication, student, condition)', example: 'medication' })
  @ApiResponse({ 
    status: 200, 
  ...
  @ApiQuery({ name: 'startDate', required: false, description: 'Start date for incident analysis' })
  @ApiQuery({ name: 'endDate', required: false, description: 'End date for incident analysis' })
  @ApiQuery({ name: 'incidentType', required: false, description: 'Filter by incident type (injury, illness, emergency)' })
  @ApiQuery({ name: 'location', required: false, description: 'Filter by location (classroom, playground, gymnasium)' })
  ...
  @ApiQuery({ name: 'startDate', required: false, description: 'Start date for correlation analysis' })
  @ApiQuery({ name: 'endDate', required: false, description: 'End date for correlation analysis' })
  @ApiQuery({ name: 'minimumVisits', required: false, description: 'Minimum health visits threshold', example: '3' })
  @ApiQuery({ name: 'includeChronicConditions', required: false, description: 'Include chronic condition analysis', example: true })
  @ApiResponse({
    status: 200,
  ...
  @ApiQuery({ name: 'auditPeriod', required: false, description: 'Audit period (monthly, quarterly, annual)', example: 'quarterly' })
  @ApiQuery({ name: 'includeRecommendations', required: false, description: 'Include compliance recommendations', example: true })
  @ApiQuery({ name: 'complianceType', required: false, description: 'Filter by compliance type (HIPAA, FERPA, both)', example: 'HIPAA' })
  @ApiResponse({ 
    status: 200, 
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 403, description: 'Insufficient permissions for compliance reporting' })
  ...
  @ApiQuery({ name: 'schoolId', required: false, description: 'Filter by specific school for district users' })
  @ApiQuery({ name: 'refreshRate', required: false, description: 'Data refresh rate in minutes', example: '5' })
  @ApiResponse({ 
    status: 200, 
  ...
  @ApiBody({
    ...
    schema: {
      ...
      properties: {
        reportType: {
          type: 'string',
          enum: ['health-trends', 'medication-usage', 'incident-statistics', 'attendance-correlation', 'compliance'],
        ...
        options: {
          type: 'object',
          properties: {
            password: { type: 'string', description: 'Password protection for sensitive reports' },
            includeCharts: { type: 'boolean', example: true, description: 'Include visualizations in export' },
            watermark: { type: 'string', description: 'Custom watermark text' },
            branding: { type: 'boolean', example: true, description: 'Include organizational branding' }
          }
        }
      }
    }
  ...
  @ApiResponse({
    ...
    schema: {
      type: 'object',
      properties: {
        exportId: { type: 'string', example: 'export_12345', description: 'Unique export identifier' },
        downloadUrl: { type: 'string', example: '/api/downloads/report_12345.pdf', description: 'Secure download URL' },
        ...
        expiresAt: { type: 'string', format: 'date-time', example: '2024-01-15T12:00:00Z', description: 'Download link expiration' },
  ...
  @ApiBody({
    ...
    schema: {
      ...
      properties: {
        reportType: {
          type: 'string',
          enum: ['health-trends', 'medication-usage', 'incident-statistics', 'attendance-correlation', 'compliance', 'custom-analytics'],
        ...
        outputOptions: {
          type: 'object',
          properties: {
            format: { type: 'string', enum: ['json', 'excel', 'pdf'], example: 'json' },
  ...
  @ApiResponse({
    ...
    schema: {
      type: 'object',
      properties: {
        reportId: { type: 'string', example: 'report_67890', description: 'Unique report identifier' },
        generatedAt: { type: 'string', format: 'date-time', example: '2024-01-15T10:30:00Z' },
        ...
        recommendations: {
          ...
          items: {
            ...
            properties: {
              ...
              recommendation: { type: 'string', example: 'Increase playground supervision during peak injury hours' },
              evidence: { type: 'string', example: 'Based on 65% of playground injuries occurring between 12-1 PM' }
            }
          }
        }
      }
