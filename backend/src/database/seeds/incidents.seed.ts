...
export function generateIncidents(
  ...
  for (const studentId of studentIds) {
    ...
    const numIncidents = Math.random() < 0.8 ? 1 : Math.floor(Math.random() * incidentsPerStudent) + 1;
    ...
    for (let i = 0; i < numIncidents; i++) {
      ...
      const parentNotified = severity !== IncidentSeverity.LOW ? true : Math.random() > 0.3;
      ...
      const followUpRequired = severity === IncidentSeverity.MEDIUM || 
                               severity === IncidentSeverity.HIGH || 
                               severity === IncidentSeverity.CRITICAL;
      
      // Insurance claim for high/critical injuries
      const needsInsuranceClaim = (type === IncidentType.INJURY || type === IncidentType.EMERGENCY) && 
                                   (severity === IncidentSeverity.HIGH || severity === IncidentSeverity.CRITICAL);
      
      // Determine status based on severity and type
      const statuses = [IncidentStatus.PENDING_REVIEW, IncidentStatus.UNDER_INVESTIGATION, IncidentStatus.REQUIRES_ACTION, IncidentStatus.RESOLVED];
      const status = severity === IncidentSeverity.CRITICAL || severity === IncidentSeverity.HIGH 
      ...
      const incident: IncidentSeedData = {
        ...
        parentNotified,
        parentNotificationMethod: parentNotified ? (Math.random() > 0.5 ? 'Phone' : 'Email') : undefined,
        parentNotifiedAt: parentNotified ? new Date(occurredAt.getTime() + 30 * 60000) : undefined, // 30 min after incident
        parentNotifiedBy: parentNotified ? reportedById : undefined,
        followUpRequired,
        followUpNotes: followUpRequired ? 'Schedule follow-up appointment to assess recovery progress' : undefined,
        attachments: [],
        evidencePhotos: [],
        evidenceVideos: [],
        insuranceClaimNumber: needsInsuranceClaim ? `CLM-${Date.now()}-${Math.floor(Math.random() * 10000)}` : undefined,
        insuranceClaimStatus: needsInsuranceClaim ? InsuranceClaimStatus.FILED : undefined,
        legalComplianceStatus: severity === IncidentSeverity.CRITICAL ? ComplianceStatus.UNDER_REVIEW : ComplianceStatus.COMPLIANT,
        occurredAt,
