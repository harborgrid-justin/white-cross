...
@Injectable()
export class TreatmentPlanRepository extends BaseRepository<
  any,
  TreatmentPlanAttributes,
...
> {
  ...
  async findByStudent(studentId: string): Promise<TreatmentPlanAttributes[]> {
    try {
      ...
      throw new RepositoryError(
        'Failed to find treatment plans',
  ...
  async findActiveByStudent(
    studentId: string,
  ): Promise<TreatmentPlanAttributes[]> {
    try {
      const plans = await this.model.findAll({
      ...
      throw new RepositoryError(
        'Failed to find active treatment plans',
        'FIND_ACTIVE_ERROR',
        500,
        { studentId, error: (error as Error).message },
      );
  ...
  protected async invalidateCaches(plan: TreatmentPlan): Promise<void> {
    try {
      ...
      await this.cacheManager.delete(
        this.cacheKeyBuilder.entity(this.entityName, planData.id),
      );
      await this.cacheManager.deletePattern(
        `white-cross:treatment-plan:student:${planData.studentId}:*`,
      );
    } catch (error) {
