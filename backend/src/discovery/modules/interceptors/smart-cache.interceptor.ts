...
export class SmartCacheInterceptor implements NestInterceptor {
        ...
        next.handle().pipe(
          tap(result => {
            ...
            this.cacheService.set(
              ...
              this.logger.error(`Failed to cache result for ${cacheKey}:`, error);
            });
          })
        ).subscribe({
  ...
  private generateCacheKey(request: any, controllerName: string, handlerName: string): string {
    const method = request.method;
    const url = request.url;
    const queryString = JSON.stringify(request.query || {});
    const bodyHash = request.body ? JSON.stringify(request.body).slice(0, 100) : '';
