...
export class DiscoveryController {
  ...
  getMonitoredProviders(
    @Query('level') level?: 'basic' | 'detailed',
  ): ProviderInfo[] {
  ...
  getControllersWithMetadata(
    ...
    return this.discoveryService.getControllersWithMetadata(
      metadataKey,
      metadataValue,
    );
  }
  ...
  getDiscoverySummary() {
    ...
    const experimentalProviders =
      this.discoveryService.getProvidersWithFeatureFlag('experimental');
    ...
    const rateLimitedProviders =
      this.discoveryService.getRateLimitedProviders();
    const monitoredProviders = this.discoveryService.getMonitoredProviders();

    // Get unique domains
    const domains = new Set<string>();
    allProviders.forEach((provider) => {
      const providerInfo = this.discoveryService.getProviderDetails(
        provider.token,
      );
  ...
  async getPrometheusMetrics() {
    ...
    if (health.metrics.counters) {
      for (const [name, counter] of Object.entries(health.metrics.counters)) {
        ...
        lines.push(
          `${metricName} ${(counter as any)?.value || 0} ${timestamp}`,
        );
      }
    }
    ...
    if (health.metrics.histograms) {
      for (const [name, histogram] of Object.entries(
        health.metrics.histograms,
      )) {
        const metricName = `discovery_${name}`;
    ...
    if (health.cache) {
      ...
      lines.push(
        `discovery_cache_hits_total ${health.cache.hits || 0} ${timestamp}`,
      );

      lines.push('# HELP discovery_cache_misses_total Total cache misses');
      lines.push('# TYPE discovery_cache_misses_total counter');
      lines.push(
        `discovery_cache_misses_total ${health.cache.misses || 0} ${timestamp}`,
      );
      ...
      lines.push(
        `discovery_cache_hit_rate ${health.cache.hitRate || 0} ${timestamp}`,
      );
    }
  ...
  async getCacheDebug() {
    ...
    return {
      ...
      warning:
        'This endpoint provides internal cache state for debugging purposes',
    };
  }
  ...
  getSystemDebug() {
    ...
    return {
      ...
      warning:
        'This endpoint provides internal system state for debugging purposes',
    };
  }
}
